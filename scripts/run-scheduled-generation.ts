...
async function main() {
  ...
  // audit #17 (2026-06-14): HOISTED out of the SEMrush try above. Clarity is
  // an INDEPENDENT connector, but as a nested block it silently never ran
  // whenever a SEMrush import/call threw before reaching it (the throw
  // jumped straight to the SEMrush catch). Its own top-level try makes it a
  // true sibling of the GSC / SEMrush / Profound / GA4 steps — one
  // connector's failure can't suppress another's nightly harvest.
  try {
