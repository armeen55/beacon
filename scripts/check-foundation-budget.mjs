...
//
// This guard holds STRUCTURAL ceilings only. Scanning source for phrases was tried here (section 13, removed
// 2026-08-14) and is not protection: a grep pins the spelling of a past incident, not the behaviour, so it
// fails on a paraphrase and passes on a rewrite that reintroduces the bug. Behavioural promises are owed a
// behavioural test. Deleting it gives up two greps: Today's research narration and connector copy naming a
// server setting. Both are copy rules with no test standing behind them today.
