...
describe("buildCompetitorPageBlueprints — snapshot integration", () => {
  it("byte-identical to pre-patch when competitorPageSnapshotsByUrl is empty Map", () => {
  ...
  it("snapshot title overrides citation-evidence pageTitle", () => {
    // No competitorPages here; the only title source is the snapshot.
    const packet = buildSpecificEditEvidencePacket(
      buildArgs({
        competitorPageSnapshotsByUrl: new Map([[COMP_URL, buildSnap()]]),
      }),
    ...
    expect(packet.competitorPageBlueprints[0].pageTitle).toBe(
      "Whole-Home Renovation | Element Homes",
    );
  });
