...
export function nextObligation(p: ChangeProposal): Obligation | null {
  ...
  const settlement = p.obligation?.kind === "terminal" && (NO_GAP_NAMED.test(p.obligation.reason) || hold.defects.length > 0) ? p.obligation : null, /* the walk's own "no gap named" settlement is a statement about research, not about these words: it keeps carrying the reading the row owes */ owedRead = p.obligation?.kind === "evidence" && p.obligation.need.reasonCode === "no_winner_to_read" ? p.obligation : null, onFile = p.winnersOnFile ?? null, asked = (p.primaryQuery ?? "").trim();
  ...
  const owedReview = unreviewed(p), faults = (p.faults ?? []).filter((f) => hold.defects.includes(f) && f !== owedReview
