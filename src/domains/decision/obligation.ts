...
export function nextObligation(p: ChangeProposal): Obligation | null {
  ...
  const faults = (p.faults ?? []).filter((f) => hold.defects.includes(f) && !COPY_RULES.reviewHold(f) && f !== owedReview && !(owedReview != null && (f.endsWith(`: ${owedReview}`) || COPY_RULES.supersededEditorFinding(f))) /* raw or composed by the sweep, a review-hold sentence is a reading, never a paid rewrite, whatever sentence the live verdict composes today (audit, 2026-09-14): a stale composed fault survived while owedReview was null or a different sentence and minted a paid redraft */
