...
const MAX_ATTEMPTS = 4; const SETTLEMENT_DAYS = 7; /* NOTHING IS PERMANENT (operator, 2026-09-10): a settlement is a rest, not a grave; after a week the row is ordinary work again, re-judged by whatever rules stand then. */ const settlementExpired = (p: { previousCopy?: { at?: string } | null; createdAt?: string }): boolean => Date.now() - (Date.parse(p.previousCopy?.at ?? "") || Date.parse(p.createdAt ?? "") || Date.now()) > SETTLEMENT_DAYS * 86_400_000; // raised with ship mode (operator, 2026-09-10): taste no longer burns attempts, so the remaining refusals are real and worth more corrective rounds
...
export function nextObligation(p: ChangeProposal): Obligation | null {
  ...
  const hold = openHold(p), owedReview = unreviewed(p);
