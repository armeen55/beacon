...
export function nextObligation(p: ChangeProposal): Obligation | null {
  ...
  if ((p.faults ?? []).some((w) => /category, tag or media index page/.test(w))) return { kind: "terminal", reason: "its evidence was a category, tag or media index page, which no redraft can fix; the pages winning this search are read again without it" }; // never a paid redraft on navigation copied from a rival (operator audit, 2026-09-15)
