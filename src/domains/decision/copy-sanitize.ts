...
export const COPY_RULES = { codeSuffix: CODE_SUFFIX, proportion: SPELLED_PROPORTION_RE,
  pageEvidence: (passages: readonly string[], selected: string, budget: number, faq?: { query: string; pairs: OwnedPageBody["faqs"] }): { evidence: Record<string, string>; truncated: boolean } => {
    const evidence: Record<string, string> = {}, normalize = (t: string): string => t.replace(/\s+/g, " ").trim(); let room = budget, n = 0, truncated = false;
    const question = (t: string): string => normalize(t).toLowerCase().replace(/[?？]+$/, ""), pair = faq?.pairs?.find((f) => f.answerComplete === true && question(f.question) === question(faq.query));
    const answer = pair ? `${pair.question}\n${pair.answer}` : ""; if (answer.length > budget || passages.join(" ").length > budget && selected.length > budget) truncated = true;
    const held = answer && answer.length <= budget ? answer : passages.join(" ").length > budget && selected.length <= budget ? selected : "";
    if (held) { evidence[`page-copy-${++n}`] = held; room -= held.length; truncated ||= held !== answer; }
