/**
 * decision/diagnose (evidence-qualified changes, 2026-07-27): WHY this page loses clicks, READ OFF THE EXACT RESULTS PAGE rather than counted off its stored title.
 *
 * What it replaced: token containment. The old rule asked whether the searcher's words appeared in the stored title, and recommended a title rewrite when they did
 * not. On live data that was wrong in the worst possible way: the stored title read "Top 20 Famous Persian Actresses and Actors", so the check fired, while Google was
 * already DISPLAYING that page as "Famous Iranian & Persian Actors, Actresses & Celebrities". The words a searcher actually reads carried the search. The stored
 * title was never the explanation, and a rewrite of it would have changed nothing a human sees.
 *
 * So the diagnosis reads three things off the results page collected for that EXACT search, and nothing else:
 ...
 * Anything else is `inconclusive`, which keeps the page under investigation and spends nothing on drafting copy nobody can justify. Zero ready is a real answer.
 *
 * A meta or content action is never concluded here: naming one honestly needs the
 * description Google displays or the page's own body words, and neither is on file, so this module refuses rather than guesses the field.
 *
 * PURE + deterministic: the same evidence in any order produces a byte-identical diagnosis. No I/O, no LLM, no tenant or vertical vocabulary.
 */
...
export const RECEIPT = {
  ...
  body: "copy-body",
  // The cause ladder (decision/diagnosis) reads five more things this account already bought. The three `winners*` ids are the coverage verdict's OWN receipt lines, cited by the exact ids it writes them
...
export function recurringPattern(input: DiagnosisInput, owned: DisplayedResult | null): string[] {
  ...
  // words ("explode", "mine", "naval") read as what the whole page agrees on, and that list is handed to the drafter as guidance.
  const sites = new Map<string, Set<string>>();
  for (const r of input.organic ?? []) {
    if (canonicalUrlKey(r.url) === ownedKey) continue;
    // ONE PUBLISHER IS ONE VOTE, the same rollup the packet uses: bare rootDomain kept en / simple / de of one encyclopedia as three sites, so one publisher agreeing with
...
/**
 * THE reasoning step between "this page underperforms" and "change this". Returns a DIAGNOSED title action only when the results page itself proves the lever.
 */
export function diagnoseCandidate(input: DiagnosisInput): ActionDiagnosis {
  if (!input.organic || input.organic.length === 0) {
    return open("unknown", "The results page for that search has not been read yet, so what to change cannot be named. It is first in line on the next research pass.", [RECEIPT.gsc], []);
  }
  const owned = ownedResultOf(input);
  if (!owned) {
    // GSC counts every country and device over 90 days; the results page I collect is ONE check. Saying "you rank third" and "this page was not found" in the same breath
    // is how a product loses a customer, so a held position means the CHECK missed the page, and only a page with no position at all may be called not ranking.
    const ranks = typeof input.gscPosition === "number" && Number.isFinite(input.gscPosition);
    return ranks
      ? open("serp_market_mismatch",
        `Google reports this page at about position ${input.gscPosition!.toFixed(1)} for that search, and the results page collected did not list it, so this is a different slice of Google than your visitors see, and it gets checked again before a fix is named.`,
        [RECEIPT.gsc, RECEIPT.serp],
        [{ alternative: "Rewrite the page title", reason: "The line a searcher actually reads for this page is not visible yet, so a rewrite would be a guess.", evidenceKeys: [RECEIPT.serp] }])
      : open("wrong_page_ranking",
        "The results page was checked and this page is not on it, so which of your pages Google shows instead comes before touching its wording.",
        [RECEIPT.serp],
  ...
  // THE RULE THAT DEMOTES A REWRITE NOBODY WOULD SEE: Google rewrites the line it displays, and when that line already carries the search, the stored title is not the explanation however poorly it reads.
  if (displayed && wanted.length > 0 && missing.length === 0) {
    return open("google_rewrite_already_matches",
      `The results page was checked, and Google already shows this page as ${quote(displayed)}, which carries the words people are searching for, so rewriting the title would not change what a searcher reads. What the page itself offers is next.`,
  ...
  if (!displayed || shared.length === 0) {
    return open("ambiguous_search_intent",
      `The ${input.organic.length} ${input.organic.length === 1 ? "result" : "results"} that come up for that search share no wording this page is missing, so the title is not the provable problem. The pages that beat it are next.`,
      [RECEIPT.serp, RECEIPT.ownedResult],
      [{ alternative: "Rewrite the page title", reason: "Nothing recurs across the pages that beat this one here, so nothing proves sharper wording is what wins the click.", evidenceKeys: [RECEIPT.serp, RECEIPT.pattern] }]);
  }
  return {
    status: "diagnosed",
    ...
    explanation: `The results page was checked: Google shows this page as ${quote(displayed)}, and the other sites that come up share wording that line does not carry: ${words(shared)}.`,
  };
}
