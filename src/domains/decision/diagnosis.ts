...
/** A PASSAGE OF DATED EVENTS CARRIES "HISTORY" WITHOUT THE WORD (review, 2026-09-14): a bare "persian flag history" wants dated events, and a page telling them under "Timeline" wording read as unanswered. Two four-digit years, or a century or era word, carry the label. */ const dated = (t: string): boolean => (t.match(/\b(?:1\d{3}|20\d{2})\b/g) ?? []).length >= 2 || /\b(?:centur(?:y|ies)|eras?|dynast(?:y|ies))\b/i.test(t) || /\b(?:BCE?|CE|AD)\b/.test(t); /** AND THE PAGE'S OWN DEMAND AS A TYPED GAP (operator, 2026-09-02): the strongest Stage 2 opportunities on a live account are searches the page already earns and does not answer, and no cause payload names one. Ownership comes first: while two of the account's own pages divide one search, the survivor owns it and the other writes nothing. PURE. */ function demandGap(card: { id?: string; pageUrl?: string | null; pagePath?: string | null; causeFinding?: CauseFinding; primaryQuery?: string }, d: PageDemand, taken?: string): SubstantiveGap | null {
  ...
  /* The ratio discovers candidates; winner comparisons can still name a deeper gap after this page answers the broad topic. */ const held = (g: { q: string; ask: string[] }): boolean => taken != null && (d.written ?? []).some((w) => !topicTokens.sameQuery(w.query, g.q) && readerOf(g)(w.copy)), collection = (g: { ask: string[]; bare: boolean }) => collectionCoverage(g.bare, readerOf(g)(d.promise), d.h2 ?? [], d.units ?? []), unanswered = d.complete ? ranked.find((g) => g.ask.length >= 2 && !d.passages.some(readerOf(g)) && !rosterAnswers(g) && collection(g)?.missing !== null && !held(g)) : taken == null ? ranked[0] : undefined, top = unanswered ?? ranked[0]!, cmp = d.research ? jobComparison(d.research, top.qs, { url: d.url, text: `${d.promise} ${d.passages.join(" ")}`, headings: [], passages: d.passages, complete: d.complete }) : null, seen = cmp ? comparisonObservations(cmp) : [], short = seen.find((o) => o.kind === "answers") ?? seen[0] ?? null;
...
/** DOES THIS TEXT ANSWER THIS SEARCH: the ONE rule, hoisted out of the demand reading so the page's own passages, the copy a live change would publish and an AI case landing on the same page are all read by it and never by three spellings of it. */ const readerOf = (g: { ask: string[]; bare?: boolean }): ((t: string) => boolean) => { const subjects = g.bare ? g.ask : g.ask.filter((w) => !LABELLED.test(w)), asked = subjects.length >= 2 ? subjects : g.ask, need = Math.max(2, Math.ceil(asked.length * COVERAGE)), relates = asked.filter((w) => RELATIONAL.test(w) || (g.bare && LABELLED.test(w))); /* a bare "persian flag history" wants dated events, so its label word must be carried like a relation, or the flag's own page answers it with no history at all */ /* ONE WORD IS NOT A SUBJECT HERE AND A RELATION THERE (reviewer, 2026-09-05). `relates` was taken from the whole ask while `asked` had the shape words removed, so "meaning" was struck out of the denominator and left in the numerator: the same word was "not a subject" and "a relational word whose literal token the answering text must carry". A page whose own first passage says a flag symbolizes the rule of an empire was refused for saying "symbolizing" and not "meaning", and its research was funded. Taken from the same list `asked` is, measured over all 227 owned pages: the one false missing answer closes and every other verdict on the account is byte-identical. */ return (t: string): boolean => { const read = g.bare && dated(t) ? `${t} history` : t; return covered(asked, read) >= need && covered(relates, read) === relates.length; }; }; /** THE SAME RULE, ASKED OF ONE SEARCH AND ONE PIECE OF TEXT, for a caller that holds no demand row (decision/producers/ai-cases). A search naming fewer than two words answers nothing here, exactly as the grouping above refuses it. */ export const answeredIn = (question: string, text: string): boolean => { const ask = topicTokens.semanticAtoms(question); return ask.length >= 2 && readerOf({ ask })(text); };
