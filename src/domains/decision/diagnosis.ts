...
/** A PASSAGE OF DATED EVENTS CARRIES "HISTORY" WITHOUT THE WORD (review, 2026-09-14): a bare "persian flag history" wants dated events, and a page telling them under "Timeline" wording read as unanswered. Two four-digit years, or a century or era word, carry the label. */ const dated = (t: string): boolean => (t.match(/\b(?:1\d{3}|20\d{2})\b/g) ?? []).length >= 2 || /\b(?:centur(?:y|ies)|eras?|dynast(?:y|ies))\b/i.test(t) || /\b(?:BCE?|CE|AD)\b/.test(t); /** AND THE PAGE'S OWN DEMAND AS A TYPED GAP (operator, 2026-09-02): the strongest Stage 2 opportunities on a live account are searches the page already earns and does not answer, and no cause payload names one. Ownership comes first: while two of the account's own pages divide one search, the survivor owns it and the other writes nothing. PURE. */ function demandGap(card: { causeFinding?: CauseFinding; primaryQuery?: string }, d: PageDemand, taken?: string): SubstantiveGap | null {
  ...
  /* ONE THING A READER WANTS TO KNOW, HOWEVER MANY WAYS IT IS ASKED, GROUPED BEFORE DEMAND IS JUDGED and keyed on what the search NAMES rather than on what the page happens to lack. The floor is asked of the GROUP, so no single phrasing carries a universal impression floor of its own: three ways of asking one question count once, together, and one three-impression question is still noise. */ for (const r of [...d.rows].sort((a, b) => b.impressions - a.impressions)) {
    const ask = topicTokens(r.query), at = groups.get(sign(r.query)), asks = ASKS.test(r.query) || RELATIONAL.test(r.query); if (ask.length === 0) continue; if (at) { at.n += r.impressions; at.asks = at.asks || asks; at.bare = at.bare && !asks; if (!at.qs.includes(r.query)) at.qs.push(r.query); } else groups.set(sign(r.query), { q: r.query, n: r.impressions, ask, qs: [r.query], asks, bare: !asks }); } // A SEARCH NAMING NOTHING AT ALL IS FILLER AND JOINS NO GROUP, and a one-word ask is the page's own subject said back. A BARE SUBJECT OF TWO OR MORE WORDS ASKS "WHAT OR WHO IS THIS" (audit, 2026-09-14): "iranian actors" and "persian flag history" could never be unanswered because the reader demanded a question word, so a 676-impression, zero-click search minted no section anywhere; it is read by the same coverage test, and a label word it carries ("history") is the whole intent and stays in the ask
