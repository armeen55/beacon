...
/** A WORD THAT NAMES A SHAPE OF ANSWER RATHER THAN AN ANSWER. When every word a page lacks is one of these, the search wants the page's own subject under a different label, which is vocabulary and not a missing answer: "iran kit history" against a page that already tells that history is a wording gap, and no section closes it. MATCHED ON THE STEM, NEVER THE WHOLE WORD (reviewer, 2026-09-02): a list of exact spellings let the same label through under its other spelling, because `topicTokens` keeps "meaning" whole and cuts "means" to "mean", so "what does bahar mean" bought a source for a page whose own words already give the meaning while "bahar meaning" correctly bought nothing. Over-inclusion here refuses a card; under-inclusion spends money. */
const VOCABULARY_ONLY = ["histor", "mean", "tip", "idea", "exampl", "fact", "info", "overview", "detail", "summar"], label = (w: string): boolean => VOCABULARY_ONLY.some((l) => w.startsWith(l)), LABELLED = new RegExp(`\\b(?:${VOCABULARY_ONLY.join("|")})\\w*\\b`);
...
function demandGap(card: { causeFinding?: CauseFinding; primaryQuery?: string }, d: PageDemand): SubstantiveGap | null {
  ...
  for (const r of [...d.rows].sort((a, b) => b.impressions - a.impressions)) { if (!ASKS.test(r.query) && !RELATES.test(r.query)) continue;
    const absent = topicTokens(r.query).filter((w) => !said.has(w) && !label(w)); if (absent.length === 0) continue;
  ...
  // OWNERSHIP IS ASKED OF THE SEARCH BEING MINTED ON, AND OFF THE LADDER'S OWN SPLIT (reviewer, 2026-09-02). Asked of `card.primaryQuery` it settled a different search from the one the gap names, so a hold could block a search nobody contested; asked only of the card's own payload it never fired on the loser card that carried none, and both pages of one split bought the same source and then both answered it.
  ...
  const split = (d.splits ?? []).find((s) => canonicalQueryKey(s.query) === canonicalQueryKey(top.q)) ?? (p && p.competingPaths.some(mine) ? { competingPaths: p.competingPaths, survivor: p.survivor } : null);
