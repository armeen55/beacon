...
type SubstantiveGap = { kind: "missing_answer" | "incomplete_answer" | "scattered_answer" | "stale_fact" | "weak_extractability" | "missing_comparison" | "missing_procedure" | "missing_evidence" | "intent_mismatch" | "false_page_promise" | "no_substantive_gap"; propositions: string[]; owed?: Obligation; why?: string; /** THE SEARCH THIS GAP IS ABOUT AND THE DEMAND BEHIND IT, TYPED, on a demand-derived gap alone: a producer minting the card reads them here rather than back out of the proposition sentence, so the number on the card and the number in the words are one value. */ query?: string; impressions?: number };
...
type PageDemand = { url: string; urlKey: string; rows: readonly { query: string; impressions: number }[]; stored: string; complete: boolean; facts: readonly { id: string; fact: string }[]; splits?: readonly { query: string; competingPaths: readonly string[]; survivor: string | null }[] };
/** A SEARCH THAT ASKS SOMETHING, read off the searcher's own words: it opens with a question word, or it carries a relational word that names an attribute rather than a subject ("iran flag BEFORE 1979", "iran NATIONAL animal"). A bare subject phrase is the page's own topic said back and asks nothing. AND IT IS ASKED OFTEN ENOUGH TO BE WORTH A SECTION: the floor is a ROW floor, not a page gate (reviewer, 2026-09-02). Asked only of the page, one 5,000-impression row let a three-impression question mint a funded answer block; asked of the search, a handful of impressions stays what it is, which is noise. */
const ASKS = /^\s*(?:what|which|who|where|when|why|how|is|are|does|do|can)\b/i, RELATES = /\b(?:before|after|meanings?|capital|largest|national|differences?|versus|vs\.?)\b/i, MIN_ASKED = 30;
/** A WORD THAT NAMES A SHAPE OF ANSWER RATHER THAN AN ANSWER. When every word a page lacks is one of these, the search wants the page's own subject under a different label, which is vocabulary and not a missing answer: "iran kit history" against a page that already tells that history is a wording gap, and no section closes it. A LABEL GOVERNS A WORD ONLY THROUGH AN INFLECTION (reviewer, 2026-09-02): matched on the bare prefix, "fact" swallowed "factory" and "idea" swallowed "ideal", so the largest carpet factory in Iran read as a vocabulary ask and no card could ever be minted for it. The remainder after the label must be nothing, s, es, ed or ing; "meaning" still counts because `topicTokens` keeps it whole and cuts "means" to "mean". Over-inclusion here refuses a card; under-inclusion spends money. THE FIVE THE INFLECTION RULE DROPPED ARE BACK BY NAME (reviewer, 2026-09-02): historical, historic, informative, factual and exemplary were governed by the old bare-prefix list and no inflection reaches them, so each one standing as the only absent word bought a card for a wording gap. */
const VOCABULARY_ONLY = ["history", "historical", "historic", "mean", "tip", "idea", "example", "exemplary", "fact", "factual", "info", "information", "informative", "overview", "detail", "summary"], INFLECTION = /^(?:|s|es|ed|ing)$/, label = (w: string): boolean => VOCABULARY_ONLY.some((l) => w.startsWith(l) && INFLECTION.test(w.slice(l.length))), LABELLED = new RegExp(`\\b(?:${VOCABULARY_ONLY.join("|")})(?:s|es|ed|ing)?\\b`);
...
function demandGap(card: { causeFinding?: CauseFinding; primaryQuery?: string }, d: PageDemand): SubstantiveGap | null {
  const said = new Set(topicTokens(d.stored)), asked = (q: string): string => topicTokens(q).filter((w) => !said.has(w) && !label(w)).sort().join(" "), groups = new Map<string, { q: string; n: number; key: string }>();
  for (const r of [...d.rows].sort((a, b) => b.impressions - a.impressions)) { if (r.impressions < MIN_ASKED || (!ASKS.test(r.query) && !RELATES.test(r.query))) continue;
    const key = asked(r.query); if (key === "") continue;
    const at = groups.get(key); if (at) at.n += r.impressions; else groups.set(key, { q: r.query, n: r.impressions, key }); } // ONE MISSING ANSWER, however many ways it is asked: the three searches for Iran's national animal are one thing a reader wants to know, and their demand is counted together
  const top = [...groups.values()].sort((a, b) => b.n - a.n || a.q.localeCompare(b.q))[0]; if (!top) return null;
  // OWNERSHIP IS ASKED OF THE WHOLE GROUP, AND OFF THE LADDER'S OWN SPLIT (reviewer, 2026-09-02). Asked of `card.primaryQuery` it settled a different search from the one the gap names; asked of the group's one representative phrasing it missed the ladder's own wording, so two pages leading with "iran capital" and "capital of iran" both minted, both bought the same source and both answered one search. The split is read through the same words-this-page-lacks key the group is built on, so every phrasing of the group is asked at once.
  const mine = (u: string | null): boolean => !!u && (canonicalUrlKey(u) === d.urlKey || namesPage(u, d.urlKey)), p = card.causeFinding?.payload?.cause === "cannibalization" ? card.causeFinding.payload : null;
  const split = (d.splits ?? []).find((s) => asked(s.query) === top.key || canonicalQueryKey(s.query) === canonicalQueryKey(top.q)) ?? (p && p.competingPaths.some(mine) ? { competingPaths: p.competingPaths, survivor: p.survivor } : null);
  ...
  const q = topicTokens(top.q), of = { propositions: [`${top.q} (${num(top.n)} searches in 90 days)`], query: top.q, impressions: top.n };
  if (!d.complete) return { kind: "missing_answer", ...of, owed: { kind: "evidence", need: { kind: "page_source", query: top.q, url: d.url, reasonCode: "acquire_page_source" } }, why: `Not all of this page's own words are on file, so the capture of ${d.url} is read before anything is written for "${top.q}".` };
  ...
  return q.length > 1 && d.facts.some((f) => { const w = new Set(topicTokens(f.fact)); return q.every((x) => w.has(x)); })
    ? { kind: shapeOf(top.q) ?? "missing_answer", ...of, why: `"${top.q}" put this page in front of ${num(top.n)} searches in 90 days and the page does not answer it; a checked source on file does.` }
    : { kind: "missing_answer", ...of, owed: { kind: "evidence", need: { kind: "factual_source", query: top.q, url: d.url, missingTopic: top.q, reasonCode: "acquire_factual_source" } }, why: `"${top.q}" put this page in front of ${num(top.n)} searches in 90 days and nothing checked on file answers it, so the source comes before the copy.` };
}
