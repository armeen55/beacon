...
import { canonicalQueryKey, FURNITURE_LABEL, RELATIONAL, topicTokens } from "@/domains/evidence/relevance-gate";
...
import { collectionCoverage } from "./demand-collection";
...
type CausePayload =
  ...
  | { cause: "retrieved_not_cited"; engine: string; promptText: string; missing?: string; aeoKind?: string; ownedIds?: string[]; packet?: string; contentHash?: string; completeness?: string }
  | { cause: "ai_citation_gap"; engine: string; promptText: string; missing?: string; aeoKind?: string; ownedIds?: string[]; packet?: string; contentHash?: string; completeness?: string } /* THE PROPOSITION AND ITS EXACT OWNED-PAGE PROOF, TYPED. A structural diagnosis may authorize only the passages it cited on the content revision it read; dropping these fields at conversion turned an exact verdict into permission to rewrite from every passage on the page. */
...
const RULES: Rule[] = [
  ...
  { // HOW THE PAGE OPENS, against how every winner opens. Not taste: they all answer the search in their first words, and this page's first words never say what it is about.
    cause: "weak_opening",
    held: (c) => (!winnerOpening(c) ? "No source-linked reading of how the pages winning this subject open is on file."
...
/** A semantic opening pattern is actionable only when its comparison brief names the exact publishers behind it. */
const winnerOpening = (c: Ctx): string | null => {
  const pattern = c.pattern, opening = pattern?.openingPattern?.trim();
  return opening && pattern?.brief?.deltas.some((delta) => delta.dimension === "opening" && delta.need === opening && delta.sources.length > 0) ? opening : null;
};
...
const COVERAGE = 2 / 3, SAME_LEMMA = 6, MAX_READINGS = 2, INFLECTION = /^(?:s|es|n|an|ian|ic|al|ish)$/, skeleton = (w: string): string => w.replace(/[aeiouy]/g, ""); /* two transliterations of one name share their consonants ("balochistan" and "baluchistan"), and a six-letter word with the same consonants in the same order is the same word (operator walk, 2026-09-16: an answer was bought to re-explain the Baluchistan bear to the Baluchistan bear page) */ /* the remainder must be an ending, or "flags" covers "flagship" (reviewer, 2026-09-15) */ /* COVERAGE: what share of the words a search names one passage must carry to be the answer to it, where two thirds lets one function word or one synonym differ without turning a real answer into a missing one and still refuses a passage sharing only the page's subject. SAME_LEMMA: how many leading characters two tokens share to be one word wearing a different ending ("history" and "historical" share six and are one word, "fact" and "factory" share four and are two), which is the general form of the sixteen-word label list this replaces, needs no language, and only ever makes a page look MORE answering, refusing a card rather than spending money. */ const covered = (ask: readonly string[], text: string): number => { const said = topicTokens.semanticAtoms(text); return ask.filter((w) => said.some((t) => topicTokens.sameAtom(t, w) || !t.includes("~") && !w.includes("~") && ((t.length >= SAME_LEMMA && w.length >= SAME_LEMMA && t.slice(0, SAME_LEMMA) === w.slice(0, SAME_LEMMA)) || (Math.min(t.length, w.length) >= 5 && (t === `${w}s` || w === `${t}s` || (t.startsWith(w) && INFLECTION.test(t.slice(w.length))) || (w.startsWith(t) && INFLECTION.test(w.slice(t.length))))) || (Math.min(t.length, w.length) >= 6 && skeleton(t) === skeleton(w))))).length; }; /* AND A FIVE-LETTER SPELLING OF THE SUBJECT IS THE SUBJECT (operator walk, 2026-09-15): "irani singer" was diagnosed missing on the Famous Iranian Singers page because "irani" carries five letters and no rule joined it to "iranian", and the writer was paid to define an Iranian singer as an Iranian singer; a search token that is a prefix of a page word, or the page word a prefix of it, at five letters or more, is one word */ /* AND A FIVE-LETTER STEM MATCHES ITS PLURAL (review, 2026-09-14): "actor" and "actors" share five and were two words */
const NUMBER_WORDS = "zero one two three four five six seven eight nine ten eleven twelve thirteen fourteen fifteen sixteen seventeen eighteen nineteen twenty".split(" "); export const tableAnswers = (query: string, rows: PageDemand["tableRows"], proposed?: string): boolean => { const ask = topicTokens.semanticAtoms(query), numbers = ask.filter((w) => NUMBER_WORDS.includes(w)); if (numbers.length !== 1) return false; const n = NUMBER_WORDS.indexOf(numbers[0]!), subject = ask.filter((w) => w !== numbers[0]), pron = subject.includes("pronunciation"); if (!subject.some((w) => w === "farsi" || w === "persian") || subject.some((w) => !["farsi", "persian", "pronunciation"].includes(w))) return false; return (rows ?? []).some(({ heading, headers, cells }) => { const context = topicTokens.semanticAtoms(heading), numberAt = headers.findIndex((h) => /\bnumber\b/i.test(h)), answerAt = headers.findIndex((h) => pron ? /\bpronunciation\b/i.test(h) : /\b(?:persian|farsi)\s+word\b/i.test(h)); return context.some((w) => w === "farsi" || w === "persian") && numberAt >= 0 && answerAt >= 0 && cells[numberAt] === String(n) && !!cells[answerAt]?.trim() && (!proposed || proposed.includes(cells[answerAt]!.trim())); }); };
/** A PASSAGE OF DATED EVENTS CARRIES "HISTORY" WITHOUT THE WORD (review, 2026-09-14): a bare "persian flag history" wants dated events, and a page telling them under "Timeline" wording read as unanswered. Two four-digit years, or a century or era word, carry the label. */ const dated = (t: string): boolean => (t.match(/\b(?:1\d{3}|20\d{2})\b/g) ?? []).length >= 2 || /\b(?:centur(?:y|ies)|eras?|dynast(?:y|ies))\b/i.test(t) || /\b(?:BCE?|CE|AD)\b/.test(t); /** AND THE PAGE'S OWN DEMAND AS A TYPED GAP (operator, 2026-09-02): the strongest Stage 2 opportunities on a live account are searches the page already earns and does not answer, and no cause payload names one. Ownership comes first: while two of the account's own pages divide one search, the survivor owns it and the other writes nothing. PURE. */ function demandGap(card: { id?: string; pageUrl?: string | null; pagePath?: string | null; causeFinding?: CauseFinding; primaryQuery?: string }, d: PageDemand, taken?: string): SubstantiveGap | null {
  const groups: { q: string; n: number; ask: string[]; qs: string[]; asks: boolean; bare: boolean }[] = [];
  /* ONE THING A READER WANTS TO KNOW, HOWEVER MANY WAYS IT IS ASKED, GROUPED BEFORE DEMAND IS JUDGED and keyed on what the search NAMES rather than on what the page happens to lack. The floor is asked of the GROUP, so no single phrasing carries a universal impression floor of its own: three ways of asking one question count once, together, and one three-impression question is still noise. */ for (const r of [...d.rows].sort((a, b) => b.impressions - a.impressions)) {
    const ask = topicTokens.semanticAtoms(r.query), hits = groups.filter((g) => g.qs.some((q) => topicTokens.sameQuery(q, r.query))), asks = ASKS.test(r.query) || RELATIONAL.test(r.query); if (ask.length === 0 || /[^\x00-\x7F]/.test(r.query) || /(?:^|\s)(?:site|inurl|intitle|filetype):/i.test(r.query)) continue; /* A SEARCH IN ANOTHER ALPHABET OR A SEARCH-ENGINE OPERATOR IS NOT A READER TASK THIS PAGE CAN ANSWER. */ if (hits.length) { const at = hits[0]!; at.n += r.impressions + hits.slice(1).reduce((n, g) => n + g.n, 0); at.asks = asks || hits.some((g) => g.asks); at.bare = !asks && hits.every((g) => g.bare); at.qs = [...new Set([...hits.flatMap((g) => g.qs), r.query])]; at.ask = [ask, ...hits.map((g) => g.ask)].find((xs) => xs.some((x) => x.endsWith("~e"))) ?? at.ask; for (const g of hits.slice(1)) groups.splice(groups.indexOf(g), 1); } else groups.push({ q: r.query, n: r.impressions, ask, qs: [r.query], asks, bare: !asks }); } // A SEARCH NAMING NOTHING AT ALL IS FILLER AND JOINS NO GROUP, and a one-word ask is the page's own subject said back.
  /* ONE CARD A PAGE, ABOUT THE BIGGEST THING THE PAGE DOES NOT ANSWER (falsifier, 2026-09-05): the highest-earning group was taken and, when the page answered THAT one, the page minted nothing at all, so every smaller unanswered question behind it was invisible. Measured on the acceptance account: 10 of the 59 qualifying groups read as unanswered and only 4 pages could ever mint, because the other unanswered groups sat behind an answered one on the same page. A PAGE IS A CONTAINER OF ATOMIC OPPORTUNITIES (Product Truth, one page is never one opportunity), so this reading is asked TWICE: first with no `taken`, which picks the largest group this page's own passages do not answer, and then with that group's own search key in `taken`, which picks the next largest unanswered one. The second is a separate section answering a separate question, never a second name for the first, and it exists only where the whole page is on file: an absence claimed off a partial capture is still not an absence, and the capture a page owes is one step however many questions wait behind it. */ const ranked = groups.filter((g) => g.n >= MIN_ASKED && !(taken ?? "").split(",").includes(canonicalQueryKey(g.q))).sort((a, b) => Number(b.asks) - Number(a.asks) || b.n - a.n || a.q.localeCompare(b.q)); if (ranked.length === 0) return null; // THE ASKS KEEP THEIR PRIORITY: a question-shaped or relational group outranks every phrasing that names nothing, so a page carrying one is read exactly as it was and the rest are reached only behind it
  /* A collection's heading and its opening roster are one reader unit. The live /traditions page names four holidays in its H1 section and explains each below; reading each paragraph without its heading called that complete overview "scattered" and hired a writer for one holiday's paragraph. Every named section must give a sentence about its subject; an empty or teaser section leaves the task open. */ const rosterAnswers = (g: { ask: string[]; bare: boolean }): boolean => { if (!g.bare || !readerOf(g)(d.promise)) return false; const units = d.units ?? [], h2 = d.h2 ?? [], labels = new Set(h2.map((h) => h.trim().toLowerCase())), opening = units.find((u) => u.heading && !labels.has(u.heading.trim().toLowerCase()) && !u.id.endsWith("#0"))?.text.toLowerCase() ?? ""; if (!opening) return false; const sections = h2.filter((h) => !FURNITURE_LABEL.test(h) && !/^frequently asked questions$/i.test(h.trim())), named = sections.filter((h) => { const name = h.split("(")[0]!.trim().toLowerCase(); return name.length > 3 && opening.includes(name); }); return named.length >= 2 && sections.every((h) => { const name = h.split("(")[0]!.trim().toLowerCase(), prose = units.filter((u) => u.heading?.trim().toLowerCase() === h.trim().toLowerCase() && !u.id.endsWith("#0")).map((u) => u.text.toLowerCase()).join(" "); return prose.includes(name) && /\b(?:is|are|means|marks|celebrat\w*|takes place|observed)\b/.test(prose) && /[.!?]/.test(prose); }); };
  /* Coherent table rows and heading/value pairs answer through their structure, not a flattened teaser. */ const held = (g: { q: string; ask: string[] }): boolean => taken != null && (d.written ?? []).some((w) => !topicTokens.sameQuery(w.query, g.q) && readerOf(g)(w.copy)), collection = (g: { ask: string[]; bare: boolean }) => collectionCoverage(g.bare, readerOf(g)(d.promise), d.h2 ?? [], d.units ?? []), unanswered = d.complete ? ranked.find((g) => g.ask.length >= 2 && !d.passages.some(readerOf(g)) && !tableAnswers(g.q, d.tableRows) && !phraseAnswer(g, d) && !rosterAnswers(g) && collection(g)?.missing !== null && !held(g)) : taken == null ? ranked[0] : undefined, top = unanswered ?? ranked[0]!, cmp = d.research ? jobComparison(d.research, top.qs, { url: d.url, text: `${d.promise} ${d.passages.join(" ")}`, headings: [], passages: d.passages, complete: d.complete }) : null, seen = cmp ? comparisonObservations(cmp) : [], short = seen.find((o) => o.kind === "answers") ?? seen[0] ?? null;
  if (!unanswered && (taken != null || !short)) return taken != null || phraseAnswer(top, d) ? null : settleOn(d, top); // Exact published phrase values settle their own task; other answered searches still owe the winner reading that could name a distinct gap.
  ...
  const split = (d.splits ?? []).find((s) => topicTokens.sameQuery(s.query, top.q) || canonicalQueryKey(s.query) === canonicalQueryKey(top.q)) ?? (p && p.competingPaths.some(mine) ? { competingPaths: p.competingPaths, survivor: p.survivor } : null);
  ...
  const of = { propositions: [`${top.q} (${num(top.n)} searches in 90 days)`], query: top.q, impressions: top.n, queries: top.qs.slice(0, 6) }; if (!d.complete) return { kind: "unknown_capture", ...of, owed: { kind: "evidence", need: { kind: "page_source", query: top.q, url: d.url, reasonCode: "acquire_page_source" } }, why: `Not all of this page's own words are on file, so the capture of ${d.url} is read before anything is written for "${top.q}".` }; const answers = readerOf(top), checkedCarries = (t: string): boolean => { if (/^why\b/i.test(top.q)) return d.causalQueries?.includes(top.q) === true; const ask = topicTokens.semanticAtoms(t); const has = (words: string[], x: string): boolean => words.some((y) => topicTokens.sameAtom(y, x) || !x.includes("~") && !y.includes("~") && x.length >= 4 && y.length >= 4 && y.slice(0, 4) === x.slice(0, 4)); return ask.length > 0 && d.facts.some((f) => { const words = topicTokens.semanticAtoms(f.fact), hit = ask.filter((x) => has(words, x)).length; return hit >= (ask.length >= 3 ? ask.length - 1 : ask.length); }); }; /* SHIP MODE: a checked answer with a joined transliteration carries the detached or hyphenated query through the same identity used for grouping; one miss is allowed on a three-word ask. */
  const missingEntry = collection(top)?.missing; if (missingEntry) { const need = `what ${missingEntry} means`, supported = checkedCarries(need); return { kind: "incomplete_answer", ...of, propositions: [need], ...(supported ? {} : { owed: { kind: "evidence", need: { kind: "factual_source", query: top.q, url: card.pageUrl ?? card.pagePath ?? d.url, missingTopic: need, proposalId: card.id ?? d.urlKey, reasonCode: "source_support_unconfirmed" } } as Obligation }), why: `The complete saved page names ${missingEntry} as an entry but gives no meaning for it. ${supported ? "A checked source can support its answer." : "A source must establish its meaning before copy is written."}` }; }
  /* A RIVAL NAMES AN INFORMATION GAP ONLY AFTER THIS PAGE'S OWN COMPLETE ANSWER HAS BEEN ACCOUNTED FOR; IT NEVER BECOMES THE PUBLISHER FOR OUR CLAIM. The earlier write-first branch deliberately hired a writer from rival prose and hoped the fact rotation would validate it later. That is how briefing leaked into customer copy and how an unverified section could look finished. A winner observation can deepen an answer the page already carries; it cannot erase the more specific finding that the page's own answer is scattered. Once the owned answer is not itself the defect, the observation emits the exact factual-source dependency before any drafting call, unless a checked publisher fact already carries that exact topic. */ if (short && !unanswered) { const topics = cmp ? comparisonTopics(cmp) : [], about = (short.topic ? topics.find((t) => t.topic === short.topic) : undefined) ?? topics.find((t) => short.quote.startsWith(t.topic)) ?? topics[0] ?? null, missingTopic = about?.topic ?? short.text; if (!checkedCarries(missingTopic)) return { kind: "incomplete_answer", ...of, propositions: [missingTopic], owed: { kind: "evidence", need: { kind: "factual_source", query: top.q, url: card.pageUrl ?? card.pagePath ?? d.url, missingTopic, proposalId: card.id ?? d.urlKey, reasonCode: "source_support_unconfirmed" } }, why: `"${top.q}" put this page in front of ${num(top.n)} searches in 90 days and a page winning that search carries something this one does not: ${short.text} That rival observation identifies what to research, but a publisher source must carry it before copy is written.` }; return { kind: "incomplete_answer", ...of, propositions: [missingTopic], why: `"${top.q}" put this page in front of ${num(top.n)} searches in 90 days and a page winning that search carries something this one does not: ${short.text} A checked publisher source on file carries that topic.` }; }
  ...
  if (d.passages.length === 0 && !!d.h2?.length && !d.facts.some((f) => top.ask.every((atom) => topicTokens.semanticAtoms(f.fact).some((word) => topicTokens.sameAtom(word, atom))))) { const on = d.research ? winnersRead(d.research, top.q) : "unseen"; return { kind: "false_page_promise", ...of, owed: { kind: "evidence", need: { kind: on === "read" ? "factual_source" : on === "unread" ? "competitor_page" : "serp", query: top.q, ...(on === "read" ? { url: card.pageUrl ?? card.pagePath ?? d.url, missingTopic: top.q, proposalId: card.id ?? d.urlKey } : {}), reasonCode: "thin_page_answer_unconfirmed" } }, why: `The complete page has no substantive answer to "${top.q}" beyond its title and headings. A qualified source for the reader's task is needed before any whole-page or section copy is written.` }; }
  ...
  const on = d.research ? winnersRead(d.research, top.q) : "read"; if (on !== "read") return { kind: "missing_answer", ...of, owed: { kind: "evidence", need: { kind: on === "unread" ? "competitor_page" : "serp", query: top.q, reasonCode: "no_winner_to_read" } }, why: `"${top.q}" put this page in front of ${num(top.n)} searches in 90 days and the page does not answer it, and ${on === "unread" ? "none of the pages winning it has been read" : "no results page for it is on file"}, so the pages winning it are read before anything is written here.` }; /* WRITE FIRST NEEDS SOMETHING TO WRITE FROM (operator, 2026-09-15, the first funded answer restated the page to itself): no fact check is owed before a draft, but with no checked fact and no winner's words on file the only ground is the page's own passages, and an answer drawn from those hands the reader what the page already says. The winners are read first, cheaply, and the writer is hired once with their words in the packet. */
  return { kind: "missing_answer", ...of, owed: { kind: "evidence", need: { kind: "factual_source", query: top.q, url: card.pageUrl ?? card.pagePath ?? d.url, missingTopic: top.q, proposalId: card.id ?? d.urlKey, reasonCode: "source_support_unconfirmed" } }, why: `"${top.q}" put this page in front of ${num(top.n)} searches in 90 days and the page does not answer it. The winning pages establish the opportunity, but a publisher source must carry the answer before copy is written.` };
}
/** One answer check for prose in the complete passage supplied. */ const readerOf = (g: { ask: string[]; bare?: boolean }): ((t: string) => boolean) => { const subjects = g.bare ? g.ask : g.ask.filter((w) => !LABELLED.test(w)), asked = subjects.length >= 2 ? subjects : g.ask, need = Math.max(2, Math.ceil(asked.length * COVERAGE)), relates = asked.filter((w) => RELATIONAL.test(w) || (g.bare && LABELLED.test(w))); return (t: string): boolean => { const read = g.bare && dated(t) ? `${t} history` : t; return covered(asked, read) >= need && covered(relates, read) === relates.length; }; }; export const answeredIn = (question: string, text: string): boolean => { const ask = topicTokens.semanticAtoms(question); return ask.length >= 2 && readerOf({ ask })(text); };
const phraseAnswer = (g: { q: string; ask: string[]; bare: boolean }, d: PageDemand): boolean => {
  if (!d.complete || !/\b(?:farsi|persian)\b/i.test(g.q)) return false;
  const language = (s: string) => s.replace(/\bfarsi\b/gi, "persian"), ask = topicTokens.semanticAtoms(language(g.q));
  const namesTask = (s: string) => ask.every((word) => topicTokens.semanticAtoms(language(s)).some((held) => topicTokens.sameAtom(word, held)));
  const units = d.units ?? [], pairs = units.filter((u) => /#0$/.test(u.id) && /^how to say\b/i.test(u.text)
    && units.some((value) => value.id.startsWith(u.id.slice(0, -1)) && value.id !== u.id && /[\u0600-\u06ff]/u.test(value.text)));
  return g.ask.includes("greeting") ? namesTask(d.promise) && pairs.some((p) => /\b(?:hi|hello)\b/i.test(p.text)) && pairs.some((p) => /\b(?:good morning|good afternoon|good evening|how are you|welcome)\b/i.test(p.text))
    : pairs.some((p) => namesTask(p.text));
};
