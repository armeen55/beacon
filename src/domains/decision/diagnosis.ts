...
/** A PASSAGE OF DATED EVENTS CARRIES "HISTORY" WITHOUT THE WORD (review, 2026-09-14): a bare "persian flag history" wants dated events, and a page telling them under "Timeline" wording read as unanswered. Two four-digit years, or a century or era word, carry the label. */ const dated = (t: string): boolean => (t.match(/\b(?:1\d{3}|20\d{2})\b/g) ?? []).length >= 2 || /\b(?:centur(?:y|ies)|eras?|dynast(?:y|ies))\b/i.test(t) || /\b(?:BCE?|CE|AD)\b/.test(t); /** AND THE PAGE'S OWN DEMAND AS A TYPED GAP (operator, 2026-09-02): the strongest Stage 2 opportunities on a live account are searches the page already earns and does not answer, and no cause payload names one. Ownership comes first: while two of the account's own pages divide one search, the survivor owns it and the other writes nothing. PURE. */ function demandGap(card: { id?: string; pageUrl?: string | null; pagePath?: string | null; causeFinding?: CauseFinding; primaryQuery?: string }, d: PageDemand, taken?: string): SubstantiveGap | null {
  ...
  /* ONE THING A READER WANTS TO KNOW, HOWEVER MANY WAYS IT IS ASKED, GROUPED BEFORE DEMAND IS JUDGED and keyed on what the search NAMES rather than on what the page happens to lack. The floor is asked of the GROUP, so no single phrasing carries a universal impression floor of its own: three ways of asking one question count once, together, and one three-impression question is still noise. */ for (const r of [...d.rows].sort((a, b) => b.impressions - a.impressions)) {
    const ask = topicTokens.semanticAtoms(r.query), hits = groups.filter((g) => g.qs.some((q) => topicTokens.sameQuery(q, r.query))), asks = ASKS.test(r.query) || RELATIONAL.test(r.query); if (ask.length === 0 || /[^\x00-\x7F]/.test(r.query)) continue; /* A SEARCH IN ANOTHER ALPHABET IS NOT A GAP AN ENGLISH PAGE CAN FILL (2026-09-17): "iranlı yönetmenler", "farsca sayılar" and "begyndere for persisk" owed winner readings whose winners are Turkish and Danish pages; a query carrying a letter outside ASCII is left to the language it was asked in */ if (hits.length) { const at = hits[0]!; at.n += r.impressions + hits.slice(1).reduce((n, g) => n + g.n, 0); at.asks = asks || hits.some((g) => g.asks); at.bare = !asks && hits.every((g) => g.bare); at.qs = [...new Set([...hits.flatMap((g) => g.qs), r.query])]; at.ask = [ask, ...hits.map((g) => g.ask)].find((xs) => xs.some((x) => x.endsWith("~e"))) ?? at.ask; for (const g of hits.slice(1)) groups.splice(groups.indexOf(g), 1); } else groups.push({ q: r.query, n: r.impressions, ask, qs: [r.query], asks, bare: !asks }); } // A SEARCH NAMING NOTHING AT ALL IS FILLER AND JOINS NO GROUP, and a one-word ask is the page's own subject said back. A BARE SUBJECT OF TWO OR MORE WORDS ASKS "WHAT OR WHO IS THIS" (audit, 2026-09-14): "iranian actors" and "persian flag history" could never be unanswered because the reader demanded a question word, so a 676-impression, zero-click search minted no section anywhere; it is read by the same coverage test, and a label word it carries ("history") is the whole intent and stays in the ask
  ...
  /* Coherent table rows answer through their headers and cells; flat text alone cannot prove which value names which word. */ const held = (g: { q: string; ask: string[] }): boolean => taken != null && (d.written ?? []).some((w) => !topicTokens.sameQuery(w.query, g.q) && readerOf(g)(w.copy)), collection = (g: { ask: string[]; bare: boolean }) => collectionCoverage(g.bare, readerOf(g)(d.promise), d.h2 ?? [], d.units ?? []), unanswered = d.complete ? ranked.find((g) => g.ask.length >= 2 && !d.passages.some(readerOf(g)) && !tableAnswers(g.q, d.tableRows) && !rosterAnswers(g) && collection(g)?.missing !== null && !held(g)) : taken == null ? ranked[0] : undefined, top = unanswered ?? ranked[0]!, cmp = d.research ? jobComparison(d.research, top.qs, { url: d.url, text: `${d.promise} ${d.passages.join(" ")}`, headings: [], passages: d.passages, complete: d.complete }) : null, seen = cmp ? comparisonObservations(cmp) : [], short = seen.find((o) => o.kind === "answers") ?? seen[0] ?? null;
