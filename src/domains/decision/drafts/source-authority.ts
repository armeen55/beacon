...
/**
 * Classify ONE source's authority. This is the single source of truth for `SourceRef.authority`, callers should overwrite whatever value the LLM
 * proposed with this function's result rather than trusting the draft's own claim (see `stampSourceAuthority` below for the whole-array helper).
 */
...
/**
 * Re-stamp a whole sources array with the DETERMINISTIC authority for each entry, discarding whatever the LLM proposed. Pure, returns a new array,
 * never mutates the input. Callers (structured-drafter.ts) run this on every
 * validated draft before it is persisted or handed to the quality gate, so `authority` on a stored draft is always this module's verdict, never the model's guess.
 */
 ...
 * Extended Arabic-Indic / Persian (۰-۹, U+06F0-06F9) digits to their ASCII value - the SAME numeral ranges draft-quality.ts's GENERIC_NUMBER regex
 * already treats as "this is a number". Everything else passes through unchanged (including ASCII digits, which map to themselves).
 */
...
export function findSupportingSpan(
  ...
  // `nowYear` keeps the number semantics aligned with groundedNumberSet / the factual-entailment gate (year-adjacent + proof-window numbers), even though
...
/**
 * NEGATION-PARITY (trust-230, Codex P1). Parity (0 = affirmative, 1 = negated) of the negation cues in a fragment. A protected claim and the span that
 * supposedly backs it must AGREE here: an affirmative source can never cover a negated claim (and vice versa), even when every entity, number, and content
 * token lines up. Even counts fold back to affirmative (a double negative), so "it is not uncommon" reads affirmative, matching how the words scan.
 */
...
/**
 * A sentence is PROTECTED (it makes a checkable claim that needs a source) when it carries a non-structural number, a named entity, or a superlative. A
 * framing sentence with none of those asserts nothing verifiable and needs no source. `structural` is groundedNumberSet("", nowYear) - the year window and
 * the 7/14/28 proof-window constants, which are methodology, not claims. The
 * number check uses `sentenceNumbers` (re-audit P2 fix), so a Persian/Arabic- Indic numeral protects a sentence exactly like an ASCII digit does.
 */
...
export function draftFactsCoveredBySources(
  ...
  for (const sentence of protectedSentences) {
    ...
    for (const s of qualifying) {
      // G6 (2026-07-10): prefer the FULL fetched page text when a caller supplied it (generation time), so one qualifying source page can back MANY
      // sentences of a roundup - a 10-name list is structurally impossible to
      // cover from a single ~400-char stamped excerpt. Falls back to the persisted excerpt (render/eval path), then the bare claim (pre-F2 draft).
...
/**
 * Boolean wrapper over `draftFactsCoveredBySources` (kept as the name the draft-quality gate already calls). True only when EVERY protected claim in
 * `draftText` is backed by an authoritative + generation-time-verified source
 * whose excerpt actually entails it. Zero sources, unverified/weak sources, or a source that backs only some claims all return false, the caller
 * (`draft-quality.ts`) turns that into a "missing_source" verdict for factual drafts (and a persisted pre-P0-1 draft, whose sources default to
...
/**
 * Drafter last-mile G4 (2026-07-10): the MARKETING-superlative net (bare promotional ranking words) the structured drafter's content firewall uses,
 * lifted here so the drafter and the coverage check agree on what counts as a superlative. Disjoint-by-design from factual-entailment's `findSuperlatives`
 * (which catches "the largest/first/only ..." and "most \w+"): this one adds the bare puffery ("leading", "best-known", "premier", "world-class") that has
 * no leading article. The union of the two is the full set of claims that need a superlative-asserting source.
 */
...
/**
 * G4: every superlative phrase in `text` - the union of the ranking net (factual-entailment.findSuperlatives: "the largest", "most famous", ...) and
 * the marketing net (MARKETING_SUPERLATIVE: "leading", "best-known", ...). Pure, deterministic, lowercased + deduped. Empty = the text asserts no superlative.
 */
...
export function ungroundedSuperlatives(
  ...
  for (const sentence of sentences) {
    ...
    for (const s of qualifying) {
      ...
      const span = findSupportingSpan(sentence, evidence, nowYear);
      if (!span.supported || span.excerpt == null) continue;
      // Superlative-parity: the backing span must ITSELF carry a superlative, not merely ground the sentence's entities/facts.
 ...
 * fields - query, brief, outline entries, FAQs, evidence hints - each passed SEPARATELY, never pre-joined) name THREE OR MORE distinct entities - a
 * "most famous X" / "top N" roundup, where each entity's own per-claim facts need that entity's OWN reference page, never a shared bare list/index page
 * (see ENTITY_REFERENCE_INSTRUCTION in structured-drafter.ts, the caller). Reuses the SAME entity extraction the entailment/coverage gates already use
 * (extractCapitalizedSpans) so "entity" here means exactly what those gates already treat as a named-entity claim, never a separate heuristic that
 * could disagree with them. A single-fact topic (0-2 named entities, e.g. "the national animal of Iran") returns false, so the per-entity citation guidance stays scoped to genuine roundups. PURE, no I/O.
 */
...
/**
 * Pilot loop 4 (2026-07-10): true when `url` looks like a bare list/index page rather than one entity's own reference page - the exact proven gap
 * from the pilot re-run (the model cited Wikipedia's List_of_Iranian_singers,
 * which names every singer but discusses none of their honors/songs, so it cannot entail a per-singer claim; that singer's OWN Wikipedia article
 * could). Used to filter "sources you may cite" candidates BEFORE they ever reach the prompt - never to police what the model actually cites (the
 * existing generation-time source verification + per-claim coverage gate still decide that, unchanged). A malformed/empty URL is not a list/index page (nothing to flag). PURE, no I/O.
 */
