...
type EvaluateTitleInput = {
   ...
   *  reaches this at all. */
  sources?: readonly ClassifiableSource[];
  authoritativeSourceDomains?: readonly string[]; /** THE ROW'S OWN CHECKED SUPPORT, as the consumer that admitted it banked it: one entry per fact id a claim of this proposal actually cites. Absent means the caller holds no record, and the rule below then asks exactly what it asked before this field existed. */ citedSupport?: readonly { id: string; fact: string }[];
};
...
export function evaluateTitleMetaQuality(input: EvaluateTitleInput): DraftQualityResult {
  ...
  /* AUTHORITY IS DECIDED ONCE, AT THE CONSUMER THAT ADMITTED THE READING (live 07:01Z, /iran-flags/pahlavi-iran-flag). The first substantive body answer the ordinary paid walk ever drafted under the proportional bar was refused here for "no cited authoritative source" while its own claim cited fact-1, a checked publisher sentence banked on the row and admitted by `authorizedCorrections`, because this rule read the account's trusted-domain list and the proposal's source pack and never the row's own checked support. A claim whose supportedBy names a fact id present in the row's supportFacts IS a cited authoritative source here, and an admitted reading is evidence about the world in the way a page asserting a thing about the world never is, so it answers a symbol or officialness claim too. */ const admitted = (input.citedSupport ?? []).map((f) => f.fact).join(" ").replace(/\s+/g, " ").toLowerCase(), pageWords = (input.pageBodyText ?? "").replace(/\s+/g, " ").toLowerCase();
  const owedFacts = (t: string): string[] => (t.match(new RegExp(SPECIFIC_FACT.source, "gi")) ?? []).filter((f) => { const needle = f.replace(/\s+/g, " ").toLowerCase(); /* AND THE RULE FIRES FOR THE SPECIFIC FACT NO ADMITTED SUPPORT CARRIES, never for the whole line: asked of the sentence, one unbacked figure beside five the page itself states refused all six and briefed a redraft to go and source words it had lifted off the page. */
    return !admitted.includes(needle) && (pageWords.length === 0 || !pageWords.includes(needle) || /\bnational (?:animal|flag|symbol|language|bird)\b|\bofficial\b/i.test(f)); });
