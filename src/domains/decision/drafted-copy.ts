...
async function draftBlock(card: ChangeProposal, page: OwnedPageEvidence, body: OwnedPageBody | null,
  ...
  const hints = [...Object.entries(packet.evidence).map(([id, text]) => `${id}: ${text.slice(0, 400)}`),
    ...
    ...(kind === "title" || kind === "h1" ? ["Keep every word of the current line that the demand ids show this page earns clicks on; drop a word only when a demand id you cite proves the replacement earns more. Use only vocabulary the evidence above already contains."] : []),
    "Every claim you make must name the ids above that carry it. Write only what those words already show about this page. DECLARE A CLAIM FOR EVERY ASSERTION YOUR COPY MAKES: anything the copy says that no claim of yours covers is refused.",
    "This copy stands on the page's own stored words, so return sources as an EMPTY array. Never invent a source entry, and never send one with a blank url, title, domain or date."];
