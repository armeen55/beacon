...
async function draftBlock(card: ChangeProposal, page: OwnedPageEvidence, body: OwnedPageBody | null, opts: DraftedCopyOptions, kind: DraftKind, siblings: ReadonlyMap<string, OwnedPageBody>, checked: readonly FactCheck[]): Promise<{ d: EditorDeliverable; ready: boolean } | null> {
  ...
  const hints = [...Object.entries(packet.evidence).map(([id, text]) => `${id}: ${text.slice(0, id.startsWith("page-copy") ? 700 : 400)}`),
    ...
    ...(kind === "answer" ? [
      ...
      "`rival-*` ids are the pages that already win this search. They tell you WHAT IS MISSING and HOW THE WINNING ANSWER IS SHAPED, and they are the one class NO claim may ever name: their words are not checked and are not yours. Use them to choose the subjects, the questions and the ordering this answer owes, then say those things from `page-*` and `fact-*` evidence. Where a rival asserts a fact this page cannot support, leave it out and say so in your limitations; never copy a rival's wording.",
...
export async function applyDraftedCopy(cards: readonly ChangeProposal[], opts0: DraftedCopyOptions): Promise<ChangeProposal[]> {
  ...
  // AND THE PAGES THAT HOLD WHAT THOSE PAGES DO NOT SAY, INSIDE THE READ'S OWN BOUND. The reader refuses the WHOLE request when it is asked for more pages than its bound, and returns an empty map rather than a short one, so widening this list past that bound did not add siblings: it silently removed every target page's body. Every draft since then was written from titles and headings alone, which is why /funny-farsi-phrases listed eight phrases and could not give one meaning while every gloss sat in the body it never received. Targets first, siblings only into the room that is left.
