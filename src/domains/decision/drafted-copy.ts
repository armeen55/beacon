...
function packetFor(card: ChangeProposal, page: OwnedPageEvidence, body: OwnedPageBody | null, owned: readonly OwnedPageEvidence[], bannedTerms: readonly string[], siblings: ReadonlyMap<string, OwnedPageBody> = new Map(), facts: readonly FactCheck[] = [], basis: string | null = null, research: EvidenceSnapshot["research"] | null = null): SourcePacket {
  ...
  (research ? jobComparison(research, card.primaryQuery, `${page.content?.title ?? ""} ${(body?.passages ?? []).join(" ")}`, page.content?.outline ?? []) : []).forEach((line, i) => { evidence[`rival-${i + 1}`] = line; });
...
async function draftBlock(card: ChangeProposal, page: OwnedPageEvidence, body: OwnedPageBody | null, opts: DraftedCopyOptions, kind: DraftKind, siblings: ReadonlyMap<string, OwnedPageBody>, checked: readonly FactCheck[]): Promise<{ d: EditorDeliverable; ready: boolean } | null> {
  const verifiedFacts = authorized(checked, body?.contentHash ?? null, opts.basis ?? null); const packet = packetFor(card, page, body, opts.snapshot.ownedPages, opts.bannedTerms ?? [], siblings, checked, opts.basis ?? null, opts.snapshot.research), outline = (page.content?.outline ?? []).slice(0, 8);
  ...
  if (!deliverable) {
    ...
    if (gainSeen && !opts.unsettled?.has(key)) {
      const step = GAIN.resolution(judgeStep, opts.snapshot, card, page, body, verifiedFacts.length);
...
export async function applyDraftedCopy(cards: readonly ChangeProposal[], opts0: DraftedCopyOptions): Promise<ChangeProposal[]> {
  ...
  for (const card of cards) {
    ...
    if (drafted) {
      ...
      out.push(finished);
      // AND THE STORE DECIDES WHETHER THIS COUNTED. The caller persists it and answers whether a Ready row durably landed; only that moves the pass's shortfall, so generated, held, refused and lost work all count zero and the walk simply continues to the next candidate.
...
export function staleCopyReasons(p: ChangeProposal, bodies: ReadonlyMap<string, OwnedPageBody>, bannedTerms: readonly string[], page?: HeldPage | null, strict = false,
  ...
  // WHAT WOULD STILL STAND UNDER THIS EDIT, off the stored body the caller already holds. Empty when the row replaces nothing or the body is not on file, which asks the structural rule nothing rather than guessing.
