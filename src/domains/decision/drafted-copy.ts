...
type DraftedCopyOptions = { tenantId: string; snapshot: EvidenceSnapshot; now: Date; basis?: string | null; complete?: CompleteFn; bypassCache?: boolean; judge?: JudgeFn;
  /** HOW MANY FINISHED CHANGES THIS PASS IS SHORT. Reached, it stops drawing: the ranked list may be walked past a refusal, and nothing is bought once the shortfall is filled. Absent = walk the whole funded list, which is what a caller with no stock target wants. */ readyTarget?: number;
...
export function deliverableFailures(d: EditorDeliverable, p: SourcePacket): string[] {
  ...
  const dupe = GAIN.absorption(d.finalCopy, p.remains ?? ""), below = (p.remains ?? "").toLowerCase(), verbatim = below.length > 0 && d.finalCopy.split("\n").map((l) => l.trim()).filter((l) => l.length > 25)
...
async function runEditor(packet: SourcePacket, field: EditorField, pageLabel: string, hints: string[],
  ...
  const carried = rewrite ? GAIN.absorption(deliverable.finalCopy, rewrite.remains) : null;
...
export async function applyDraftedCopy(cards: readonly ChangeProposal[], opts0: DraftedCopyOptions): Promise<ChangeProposal[]> {
  ...
  // THE DEFICIT IS FINISHED CHANGES OWED, NEVER CANDIDATES ALLOWED. Funding `min(deficit, 5)` candidates meant a queue one row short attempted exactly ONE page and stopped, whatever that page turned out to be: two dispatches spent their whole allowance on the two candidates that happened to rank highest and never reached the work that had just gained the evidence it was waiting for. The pass now walks the ranked list while the target is unmet, and stops spending the moment it is met; everything past that point is left untried rather than bought.
  let landed = 0;
  for (const card of cards) {
    ...
    const slice = (opts.stopBy != null && Date.now() >= opts.stopBy) || (opts.readyTarget != null && landed >= opts.readyTarget) ? null : budget.draw(DRAFT_BUDGET.keyOf(card), DRAFT_BUDGET.DELIVERABLE_CALLS); if (!slice) log.info("[drafted-copy] paid work stopped for this card: the pass's plan funded no allowance for it", { tenantId: opts.tenantId, path: card.pagePath, owed: wants });
...
function bankedCopyReasons(p: ChangeProposal, bannedTerms: readonly string[], held: HeldPage | null, preserve: readonly string[] = [], remains = ""): string[] {
  ...
  const absorbed = GAIN.absorption(c.after, remains).repeats;
