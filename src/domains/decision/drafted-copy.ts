...
type EditorDeliverable = {
  ...
  /** THE ENTRIES BELOW THIS COPY THAT IT NOW CARRIES ITSELF, named exactly. Present only on a COMPLETE consolidation: the removal is part of the change, so the card names every section that goes and the operator makes the page say it once in one pass. */ absorbs?: readonly string[];
...
/** THE PAGE VERSION A FACT IS AUTHORIZED AGAINST IS THE VERSION THE FACT BANK STAMPS: pageHashOf over the stored
 *  body's own join (title, h1, headings, passages), the exact preimage fact-check-run banks and factual-defects
 *  already computes. This compared the crawler's `contentHash` (a hash of a DIFFERENT string) instead, so two
 *  sha256s of two preimages never matched, `verifiedFacts` was empty on every production draft, the fact-* channel
 *  was dead end to end, and the missing-information loop could mint, spend, bank, and never close: the researched
 *  fact existed and the writer could never see it (audit, 2026-08-26). FAIL CLOSED WITHOUT A BODY: no stored body,
...
function rereadableRefusals(copy: string, p: SourcePacket, heading: string | null = null): string[] {
  ...
  // A BANNED WORD THE SEARCHERS THEMSELVES USE IS DIFFERENT SPEECH (operator ruling, 2026-08-16): "Persian, never Farsi" holds for Beacon's own voice, but when a real search for this page carries the word, using it beside the preferred one is the page speaking its readers' language. THE WORDS THIS PAGE EARNS CLICKS ON COUNT TOO, not only its vocabulary list: /farsi-numbers could not have its counting heading widened from 1-10 to 0-10, because keeping "Farsi Numbers" tripped this rule while dropping it tripped the rule that a change may not lose a term the page earns clicks on. One word, two rules, opposite answers, and no heading could satisfy both.
  ...
  const banned = p.bannedTerms.filter((t) => t.trim() && new RegExp(`\\b${t.trim().replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`, "i").test(`${copy} ${heading ?? ""}`)
    && ![...p.demand.vocabulary, ...p.demand.preserve].some((v) => new RegExp(`\\b${t.trim().replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`, "i").test(v)));
...
function packetFor(card: ChangeProposal, page: OwnedPageEvidence, body: OwnedPageBody | null, owned: readonly OwnedPageEvidence[], bannedTerms: readonly string[], siblings: ReadonlyMap<string, OwnedPageBody> = new Map(), facts: readonly FactCheck[] = [], basis: string | null = null, research: EvidenceSnapshot["research"] | null = null): SourcePacket {
  ...
  // A BODY EDIT GETS THE PAGE IN ORDER, NOT THE SIX PASSAGES THAT REPEAT THE QUESTION. Scoring by query overlap hands over the passages that talk ABOUT the topic and drops the ones that CONTAIN the answers: on /funny-farsi-phrases it kept the intro and the FAQ and dropped "Khar means donkey" and every other gloss, so the writer produced a list of phrases with no meanings. For copy that lands in the body the page IS the evidence base, so it arrives whole in document order under a character budget; a field edit still gets six.
  ...
  authorized(facts, body, basis).slice(0, FACTS_IN_PACKET).forEach((f, i) => {
...
async function runEditor(packet: SourcePacket, field: EditorField, pageLabel: string, hints: string[],
  ...
  if (field === "answer_block" && rewrite) {
    ...
    const spot = [packet.h1, packet.title, ...packet.headings].find((x): x is string => !!x && !blankish(x) && x.trim().length <= ANCHOR_MAX && !CHROME_AT.test(x) && !/[a-z][A-Z]/.test(x) && !/\S\s{2,}\S/.test(x.trim()));
  ...
  const carried = rewrite ? GAIN.absorption(deliverable.finalCopy, rewrite.remains, packet.headings) : null;
...
async function draftBlock(card: ChangeProposal, page: OwnedPageEvidence, body: OwnedPageBody | null, opts: DraftedCopyOptions, kind: DraftKind, siblings: ReadonlyMap<string, OwnedPageBody>, checked: readonly FactCheck[]): Promise<{ d: EditorDeliverable; ready: boolean } | null> {
  const verifiedFacts = authorized(checked, body, opts.basis ?? null); const packet = packetFor(card, page, body, opts.snapshot.ownedPages, opts.bannedTerms ?? [], siblings, checked, opts.basis ?? null, opts.snapshot.research), outline = (page.content?.outline ?? []).slice(0, 8);
  ...
  let rewrite: { heading: string | null; replaces: string; remains: string } | null = null;
            ...
            : link
              ...
              : shape === "replace"
                ? [`Open the site editor on ${card.pagePath}`, `Find the passage beginning "${drafted.beforeText!.slice(0, 80)}" under "${drafted.placementAnchor}"`,
                  ...
                  ...(consolidate ? [absorbs.length > 0 ? `Delete the sections below for ${absorbs.join(", ")}: the passage above now carries all of them, so the page says it once` : "Remove each entry below that this section now covers, so the page says it once"] : []),
...
export function staleCopyReasons(p: ChangeProposal, bodies: ReadonlyMap<string, OwnedPageBody>, bannedTerms: readonly string[], page?: HeldPage | null, strict = false,
  ...
  const parts = p.bundle?.components ?? []; if (parts.length === 0) return strict && (!p.claims?.length || !p.supportFacts?.length) ? ["this copy carries no record of what it stands on, so it is held rather than promoted"] : bankedCopyReasons(p, bannedTerms, page ?? null, preserve, survives);
