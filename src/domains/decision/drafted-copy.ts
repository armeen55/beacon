...
async function draftBlock(card: ChangeProposal, page: OwnedPageEvidence, body: OwnedPageBody | null, opts: DraftedCopyOptions, kind: DraftKind, siblings: ReadonlyMap<string, OwnedPageBody>, checked: readonly FactCheck[]): Promise<{ d: EditorDeliverable; ready: boolean } | null> {
  ...
  // AND THE SYNTHESIS QUESTION IS ASKED AFTER THE SECTION IS FOUND, NOT BEFORE IT. This sat ABOVE the block that computes `rewrite`, reading a variable still initialised to null, so the condition was false on every card ever drafted and the structural-synthesis instruction reached the evaluator exactly ZERO times. A replacement was then judged by the standard written for a brand-new section, which is why a correctly targeted /funny-farsi-phrases rewrite came back "adds no information the page does not already say". Its gain is the SHAPE the page lacks, which is a reason to replace weaker presentation IN PLACE.
...
export function staleCopyReasons(p: ChangeProposal, bodies: ReadonlyMap<string, OwnedPageBody>, bannedTerms: readonly string[], page?: HeldPage | null, strict = false,
  ...
  // TWO RULES THAT REACH EVERY STORED ROW, bundle or not: the re-read below walks bundle COMPONENTS, so a plain field edit was never re-read at all and kept whatever a past generation decided. (1) A CLAIM ABOUT THE WORLD NEEDS A SOURCE and this page is not one: "Iran's national animal is the Asiatic cheetah" is a claim about a COUNTRY, and authoritative sources confirm the cheetah is critically endangered and survives only in Iran while NOT establishing the national-animal claim; a `fact-` id is the only support from outside the page. (2) A SPLIT MAY NOT PROMISE CLICKS: a modelled CTR gap says what a page's positions usually earn, never what this wording recovers, and "about 176 clicks short" beside a title change is a promise the evidence never made, so new cards no longer write it and a stored one is held until the ownership work it belongs to is finished.
  const first: string[] = [], copy = p.recommendedChange?.kind === "existing_edit" ? p.recommendedChange.after : "";
  if (/\bnational (?:animal|flag|symbol|language|bird)\b|\bofficial\b/i.test(copy) && !(p.claims ?? []).some((c) => c.supportedBy.some((id) => id.startsWith("fact-")))) first.push("it states what a country's national symbol is and stands only on this page saying so, which is not a source: that claim is held until one is on file");
  if ((p.causeFinding?.cause ?? p.diagnosisCause) === "cannibalization" && /\d[\d,.]*\s*clicks short/i.test(p.whyItMatters ?? "")) first.push("its reason promises clicks a wording change has never been shown to recover, so it is held until the ownership work it belongs to is finished");
  const parts = p.bundle?.components ?? []; if (first.length > 0) return first; if (parts.length === 0) return strict && (!p.claims?.length || !p.supportFacts?.length) ? ["this copy carries no record of what it stands on, so it is held rather than promoted"] : bankedCopyReasons(p, bannedTerms, page ?? null, preserve);
