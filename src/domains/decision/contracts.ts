...
export type ChangeProposal = {
  ...
  supportFacts?: readonly { id: string; fact: string; /** THE ADDRESSES THIS FACT WAS READ FROM THAT ACTUALLY QUOTED SOMETHING, banked with it so publishers are counted off typed provenance instead of hostnames pulled out of the fact's own prose, which reads an address the reading quoted nothing from as a source. Absent on a row banked before this existed, which is read from its text instead. */ sources?: readonly string[] }[];
...
const ChangeProposalSchema: z.ZodType<ChangeProposal> = z.object({
  ...
  supportFacts: z.array(z.object({ id: z.string().min(1), fact: z.string().min(1), sources: z.array(z.string().min(1)).optional() })).optional(),
