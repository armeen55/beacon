...
export type ChangeProposal = {
  ...
  semanticReview?: { editor?: z.infer<typeof EditorAcceptanceSchema>; aeoPacket?: z.infer<typeof AEO_BAR.schema>; of: string; version: number; claims: readonly { i: number; by: readonly string[]; entailed: boolean }[]; materialChange?: boolean };
...
const ChangeProposalSchema: z.ZodType<ChangeProposal> = z.object({
  ...
  semanticReview: z.object({ editor: EditorAcceptanceSchema.optional(), aeoPacket: AEO_BAR.schema.optional(), of: z.string().min(1), version: z.number().int(), claims: z.array(z.object({ i: z.number().int().min(0), by: z.array(z.string()), entailed: z.boolean() })), materialChange: z.boolean().optional() }).optional(), informationGain: z.object({ adds: z.string().min(1), by: z.array(z.string()), pageWhole: z.boolean(), bodyHash: z.string().min(1).optional(), targetHash: z.string().min(1).optional() }).optional(),
