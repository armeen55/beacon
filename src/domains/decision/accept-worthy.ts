...
const applies = (field: string, standard?: string, unpublished = false, shape?: string) => enabled() && !unpublished && /^(answer_block|section)$/.test(field)
  && ["section", "direct_answer", "restructure"].includes(shape ?? "")
...
const failures = (field: string, copy: string, limitations: readonly string[] = [], standard?: string, unpublished = false, shape?: string): string[] => {
  if (!applies(field, standard, unpublished, shape)) return [];
...
export const AEO_BAR = { applies, policy, failures, schema, passed,
  ...
  forRow: (p: ChangeProposal): boolean => p.recommendedChange.kind === "existing_edit" && !p.recommendedChange.linkTo
    && applies(p.recommendedChange.field, p.changeFamily === "factual_correction" ? "correction" : p.assignment?.standard, false, p.assignment?.treatment === "restructure" ? "restructure" : p.assignment?.shape),
