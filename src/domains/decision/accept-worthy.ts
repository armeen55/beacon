...
const phraseIncludes = (heading: string, entity: string): boolean => {
  const h = phraseTokens(heading), e = phraseTokens(entity);
  if (h.length === 0 || e.length === 0) return false;
  const hs = ` ${h.join(" ")} `, es = ` ${e.join(" ")} `;
  return hs.includes(es) || e.every((token) => h.includes(token));
};
...
const failures = (field: string, copy: string, limitations: readonly string[] = [], standard?: string, unpublished = false, shape?: string, row: Parameters<typeof signal>[0] = {}): string[] => {
  ...
  if (tableStyled || boldLabeled || markdownLinks.length > 1)
    out.push(holds.criteria);
  for (let i = 0; i < groups.length; i += 2) {
    const heading = (groups[i] ?? "").trim(), prose = (groups[i + 1] ?? "").split("\n").filter((l) => !/^\s*(?:[-*•|#]|\d+[.)])/.test(l)).join(" ").trim();
    if (entities.some((entity) => phraseIncludes(heading, entity)) || !/[.!?]/.test(prose)) {
