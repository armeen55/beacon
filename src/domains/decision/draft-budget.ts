...
const MAX_PAID_CALLS = 600; /* 60 under the two-drives-a-day era; raised for the constant ten-minute cycle (operator, 2026-09-10, unlock all caps and constant over cycle): up to ~144 drives a day now share one day purse, and 60 calls starved every drive after the morning. The money brakes stand unchanged: the $50 daily and $250 monthly ledgers refuse work this count would never reach. */
/** HOW MANY TIMES ONE JOB MAY TAKE REAL CALLS IN A DAY AND FINISH NOTHING (the same number the copy contract settles on, applied to the money). THE SECOND NUMBER BESIDE IT IS DELETED (measured on production receipts, 2026-09-05): a job funded and never begun three drives running used to be demoted to the back exactly as an attempted one is, and because a demoted job is never begun the count only ever grew, so the demotion was a ratchet nothing could release. On run tenant-iranopedia:2026-09-05 it put a sourced missing answer of 1,361 searches twentieth on the manifest behind SIXTEEN summary lines worth 0.59 recoverable clicks and less, every one of them ranked below it. What the demotion guarded against is guarded by the order itself now: being unreached last walk is a TIE-BREAK under worth rather than a band above it, so unreached work holds the head only where it is also the strongest work, which is the answer that was wanted all along. */
...
/** LOGICAL OPERATIONS, NEVER PROVIDER CALLS (Codex, 2026-08-23: a seven-unit price met a sixteen-call dispatch, because one structured operation retries internally and can be two real calls). One round is a draft and its ONE evaluation; the last evaluation IS the promotion decision, so no final-review unit exists any more. Real calls and real dollars are metered off the gateway's own receipts, per page, and reported as themselves. AND THE PRICE IS WHAT ONE DELIVERABLE ACTUALLY RUNS (audit 3.7, 2026-09-14): drafted-copy reads the winners' comparison once, then writes the body companion and the description off the SAME slice, each with its own judge and its own retries, so six calls (one piece's rounds) was spent by a zero-retry body plus meta at five and any retry hit SPENT mid-deliverable, filed the card retryable_blocked and re-funded it next cycle. One comparison, then two pieces at their full rounds. */
const PER_DELIVERABLE_CALLS = 1 + 2 * (1 + EDITOR_RETRIES) * CALLS_PER_ROUND;
...
/** NOTHING LEFT THE PROCESS FOR THIS ANSWER, and it is the ONE predicate both halves of the meter read (reviewer, 2026-09-06). Four answers say so: the model is off, the day's cap refused the call before it was made, the call cache served it, and the receipt itself counts zero requests. THE DOLLARS ARE ASKED FIRST, AND OF ALL FOUR (reviewer, 2026-09-06, third pass): the cost guard sat on the receipt clause alone, so an answer stamped `off`, `blocked_budget` or `cached` beside real money was handed its attempt back by the refund AND dropped from the dollars by `recordOn` below, the same money leaving both halves at once and nothing anywhere naming it. A receipt that names money is a call that left the process whatever it calls itself, so the status is asked only where the cost is zero, and the gateway stamps zero on every refusal it makes before the wire (a missing account, a credit hold, a paused account, a schema nothing can convert) so nothing honest changes. What a page spent and what it has left stay one arithmetic. Three doors read it: the refund below, the dollars beside it, and the page reading whose pool is given its unit back the same way (producers/page-job.ts). PURE. */
const noCallMade = (r: unknown): boolean => { const x = r as { status?: string; cached?: boolean; attempts?: number; costUsd?: number } | null; return !!x && (x.costUsd ?? 0) <= 0 && (x.status === "off" || x.status === "blocked_budget" || x.cached === true || x.attempts === 0); };
...
const HARD_REFUSAL = /not on the stored page|is not the one this page carries|page this evidence is not about|blank or still carries a placeholder|not attached to it|no such thing|mentions them|names evidence that is not on file|no claim anybody could check|names no evidence at all|cannot be identified|rewritten under the entity-first brief/;
