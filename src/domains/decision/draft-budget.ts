...
/** NOTHING LEFT THE PROCESS FOR THIS ANSWER, and it is the ONE predicate both halves of the meter read (reviewer, 2026-09-06). Three answers say so in their status: the model is off, the day's cap refused the call before it was made, and the call cache served it. AND THE RECEIPT ITSELF SAYS SO WHENEVER IT COUNTS ZERO REQUESTS AND ZERO DOLLARS (reviewer, 2026-09-06, second pass): the gateway stamps `httpAttempts` 0 for every refusal it makes before the wire and 1 for anything that touched it, so a call refused for a missing account, a credit hold, a paused account or a schema nothing can convert comes back `attempts: 0, costUsd: 0` and `recordOn` below already records no provider call for it, while the refund charged the page an attempt for a call nobody made. Asking the receipt what it counted, instead of listing statuses, is what stops a fifth shape reopening this: what a page spent and what it has left are one arithmetic. PURE. */
const noCallMade = (r: unknown): boolean => { const x = r as { status?: string; cached?: boolean; attempts?: number; costUsd?: number } | null; return !!x && (x.status === "off" || x.status === "blocked_budget" || x.cached === true || (x.attempts === 0 && (x.costUsd ?? 0) <= 0)); };
