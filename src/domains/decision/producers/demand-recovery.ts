...
export async function demandRecoveryCards(input: { tenantId: string; snapshot: EvidenceSnapshot; now: Date;
  ...
  try {
    ...
    const owned = new Set(snapshot.ownedPages.map((p) => canonicalUrlKey(p.url))); /** HAS ANYBODY READ WHAT WINS THIS ROW'S OWN SEARCH, asked ONCE here and stamped on the card, because the obligation ladder is pure and holds no snapshot (production 09:03:46Z, 2026-09-06). `read` is a winner of this search read whole; `unread` is a results page on file FOR THIS SEARCH with nothing off it read; `none` is no results page on file for it, which is one unbought reading and never a reason to settle. Refreshed on every re-mint, so the day a reading lands the stamp moves with it. */ const winnersFor = (u: { label: string }): "none" | "unread" | "read" => jobWinners(snapshot.research, [u.label]).some((w) => (w.extract?.wordCount ?? 0) > 0) ? "read" : snapshot.research.serpEvidence.some((e) => canonicalQueryKey(e.query) === canonicalQueryKey(u.label)) || jobWinners(snapshot.research, [u.label]).length > 0 ? "unread" : "none"; /* ALL THREE DOORS ASK THE SEARCH THIS ROW IS WORKED FOR, never the union (reviewer three, 2026-09-06). `read` always did, deliberately (measured against evidence/comparison's own call, which passes this one phrasing wherever no gap names others): a winner that ranks under a SIBLING phrasing joins no comparison this row will ever see, so counting it as read is how three rows were settled on winners nobody had read for them. `unread` asked the whole GROUP, so a group whose only results page belongs to a sibling phrasing was stamped unread, owed the winners of a search no results page exists for, could never reach `read`, and bought that unbuyable reading on every drive for ever. Asked of this search alone, `unread` means the results page for THIS search is on file so its winners can be read, `none` means buy that results page first, and the stamp, the need the ladder names and the discharge agree by construction. */
