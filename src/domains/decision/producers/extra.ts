...
async function linkCards(tenantId: string, pages: OwnedPageEvidence[], weak: ReadonlySet<string>, u: Understanding, expectedCtrAt: (position: number) => number, bodies: ReadonlyMap<string, OwnedPageBody>): Promise<{ drafts: Draft[]; complete: boolean }> {
  ...
  for (const from of strongest) {
    ...
    out.push({
      ...
      hints: [`${labelOf(from)} links to ${count(links.size, "page")} of this site and none of them is ${dest}`, `${support}, counted across every page of this site read so far`, `${dest} ranks at position ${position} on Google for "${target.query.query}", shown ${count(target.query.impressions, "time")} in Search Console`, `${labelOf(from)} earns ${count(clicksOf(from), "click")} in the last 90 days`],
...
function schemaCards(tenantId: string, pages: OwnedPageEvidence[], bodies: ReadonlyMap<string, OwnedPageBody>, now: Date): ChangeProposal[] {
  ...
  for (const p of pages) {
    ...
    const card = mint(tenantId, { page: p, slug: "faq_schema", field: "schema", query: topQueryOf(p), before: existing?.raw ?? null, after, minutes: 3, confidence: "high", refs: pairs.length,
      headline: existing ? `FAQ structured data that matches the ${n} this page answers` : `FAQ structured data for the ${n} this page answers`,
      why: existing ? `This page has FAQ structured data whose pairs differ from the ${n} it answers, so the markup describes answers the page does not show.` : `This page answers ${n} and has no FAQ structured data, so search engines are not told which questions it answers.`, /* the card already names the page; its full title read as a sentence subject ("Popular Persian Female(Girl) First Names and their Meanings answers 3 questions") */
...
export async function extraQueuePass(input: { tenantId: string; snapshot: EvidenceSnapshot; now: Date;
  curve?: Parameters<typeof extraQueueCards>[0]["curve"]; reads?: { left: number }; persist?: boolean; aeoDiagnoses?: number; basis?: string | null; checked?: readonly FactCheck[] | null }): Promise<{
