...
import { mutationFootprint } from "../mutation-footprint"; import { AEO_BAR } from "../accept-worthy";
...
function unansweredCards(snapshot: EvidenceSnapshot, pages: OwnedPageEvidence[], expectedCtrAt: (position: number) => number, read: { bodies: ReadonlyMap<string, OwnedPageBody>; misses: ReadonlyMap<string, "no_capture" | "read_failed">; facts: ReadonlyMap<string, FactCheck[]>; /** THE COPY THE CHANGES ALREADY ON FILE FOR EACH PAGE WOULD PUBLISH, keyed by the page's own path in lower case, so a question one of them already answers is not offered again. */ written: ReadonlyMap<string, { query: string; copy: string }[]>; basis: string | null; tenantId: string }): Draft[] {
  const out: Draft[] = []; for (const p of pages) { const path = pathOf(p.url); if (path === "/" || STOREFRONT.test(path)) continue; // an essay never goes on the home page or a shop rail, the same rule every other body card here obeys
    ...
    const checked = authorizedCorrections(read.facts.get(path) ?? [], { pageContentHash: body ? pageHashOf([body.title, body.h1, ...body.headings, ...body.passages].filter(Boolean).join("\n")) : "", evidenceBasis: read.basis }, read.tenantId), groupingOwed = AEO_BAR.applies("answer_block", undefined, false, undefined, { pageUrl: p.url }) && AEO_BAR.hasGrouping(checked.flatMap((f) => f.sources)).length < 2;
    out.push({ page: p, factIdentity: JSON.stringify(checked.map((f) => [f.statementKey, f.sourceReadAt ?? f.checkedAt]).sort((a, b) => JSON.stringify(a).localeCompare(JSON.stringify(b)))), slug: nth === 0 ? "missing_answer" : `missing_answer@${canonicalQueryKey(q)}`, field: spread ? "section" : "answer_block", query: q, asked: q, treatment: spread ? "rewrite_existing_section" : "add_answer_section", obligation: groupingOwed ? { kind: "evidence", need: { kind: "factual_source", query: q, url: p.url, reasonCode: "facts_owed", missingTopic: AEO_BAR.groupingQuestion } } : gap.owed ?? { kind: "draft" },
      ...
      cause: { cause: "incomplete_coverage", action: "opening_answer", evidenceKeys: [RECEIPT.gsc], competingExplanations: [], notConsidered: [], explanation: packet && nth === 0 ? first!.why! : spread ? `Google shows ${path} to ${seen} in 90 days for "${q}" and this page's own words answer it in pieces rather than in one place.` : unknown ? `Google shows ${path} to ${seen} in 90 days for "${q}" and this page's own words are not all on file yet.` : `Google shows ${path} to ${seen} in 90 days for "${q}" and the page's own stored words never answer it.`,
        falsifier: packet && nth === 0 ? "If the stored collection answer passes the packet structure checks, no restructuring is owed." : spread ? `If one passage of this page turns out to answer "${q}" on its own, there is nothing to bring together here.` : `If this page's stored copy turns out to answer "${q}" once the whole page is read, there is no missing answer here.` } }); } }
