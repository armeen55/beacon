...
function composedReplacement(before: string, proposed: string): string {
  ...
  const prefix = lv ? `${lv.label}:${lv.latin ? " " : lv.gap}` : "", bare = lv ? proposed.trim().replace(new RegExp(`^${lv.label.trim().replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*:\\s*`, "i"), "") : proposed; // THE LABEL IS ADDED ONCE (live 2026-09-02): a judge that handed back "Meaning: distinguished, ..." under a "Meaning:" entry shipped "Meaning: Meaning: distinguished" to Ready
