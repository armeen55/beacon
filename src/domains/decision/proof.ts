...
export const REVIEW_CONTRACT = 5; // THE PERSISTED AUTHORIZATION CONTRACT, one number for every family that banks a reading. It read `draft.factual_review`, which is a prompt CACHE version and would have governed editor receipts by accident; bumped to 5 because a reading banked before the role ruling existed never asked whether the source was about the same thing in the same relationship, so it cannot inherit that promise and is read again.
