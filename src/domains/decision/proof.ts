...
import { FURNITURE_LABEL } from "@/domains/evidence/relevance-gate"; /** THE NAVIGATION LABEL IS ONE DEFINITION AND IT LIVES IN THE EVIDENCE LAYER (campaign review, 2026-09-05): `FURNITURE_LABEL` in evidence/relevance-gate is read by the writer's door, the replacement door, the assignment's anchor rule and the content comparison, which is the door that had no copy of it and let a winner's chrome mint a paid observation. The copy that stood here, and the prefix-matching copy in drafted-copy, are deleted. */
...
export const REVIEW_CONTRACT = 8; // Publisher-role and material-form authorization; independent of prompt-cache versions.
...
export function unreviewed(p: ChangeProposal): string | null {
  ...
  const needsEditor = p.assignment != null || p.kind === "new_page" || p.changeFamily !== "factual_correction" && p.recommendedChange.kind === "existing_edit" && !p.recommendedChange.linkTo && /^(answer_block|section)$/.test(p.recommendedChange.field)
  ...
  if (needsEditor && (claims.length === 0 || !COPY_RULES.accepted(r?.editor) || r?.of !== copyKey(p) || p.kind === "new_page" && r?.scope !== "whole_page")) return "the exact copy, assignment and sources have no complete editor acceptance on file, so the preserved words owe a review";
  if (needsEditor && r?.version !== REVIEW_CONTRACT) return "the reading on file was made under an older review contract, so it is read again before these words are offered";
  if (AEO_BAR.forRow(p) && (!AEO_BAR.passed(r?.aeoPacket) || r?.of !== copyKey(p))) return AEO_BAR.holds.unreviewed;
  if (!(needsEditor || AEO_BAR.forRow(p) || p.changeFamily === "factual_correction" || p.informationGain || claims.some((c) => c.supportedBy.some((id) => id.startsWith("fact-"))))) return null;
...
export function evidenceShortfall(p: ChangeProposal): string | null { // ONE AUTHORIZATION VOCABULARY: a bundle's `plan.removes` and a component's `preserves.losses` are the customer-facing SUMMARY of a change and were pooled in as though they were the same verified record, so they are display only now; a ledger entry answers for ONE unit, because one entry quoting the whole passage claimed every unit had been considered while naming none (Codex, 2026-08-28)
  ...
  if (before) {
    ...
    if (c.field === "section" || c.field === "answer_block") {
      /* A DOOR MAY ONLY DEMAND WHAT THE WRITER CAN GIVE, AND A REORGANIZATION ANSWERS FOR THE PASSAGE IT REORGANIZES (measured, 2026-09-05). This arm asks every sentence of a replaced passage to survive letter for letter or be carried in the row's `preservation` ledger, and the drafter's schema has no field for that ledger at all: it is written at exactly two places in this codebase, by the correction producer and by a consolidation's absorbed entries, so a writer told to restructure a passage cannot answer the arm and a rewrite is refused for being a rewrite. `/cuisine`, the account's largest body case, spent two funded corrective attempts against an instruction nothing it could return would satisfy. THE EXEMPTION IS ONE TREATMENT AND IS TYPED, NEVER LEXICAL: where the gain receipt names ONLY the page's own material (no checked source, no other owned page, no rival or results-page briefing) and was judged against the whole page, the change adds nothing from outside and its entire gain IS the page's own units in a better order, so the units of the passage were moved rather than destroyed. `unreviewed` has already run above, so reaching here also means a reading bound to these exact words, at the current contract, ruled on every claim and entailed each one. WHAT IT DOES NOT TOUCH, and the fixtures below this line pin each: a replacement carrying an outside `fact-` or another owned page still answers for every unit, so a lowercase call to action, a qualifier, a list member, a worked example, a reversed polarity and a faithful paraphrase all still vanish only over a typed disposition; a correction owes no gain receipt at all, so all eight of this account's corrections keep the arm exactly as it stood, which is the case "Meaning: Light." turns on; and a link, a figure or a name that goes missing is refused below by `materialLosses`, which reads the whole passage on every replacement. Measured over the store: 29 replacements carry a `before`, 1 clears this arm by the gate, and it is the row the item exists for. */
      const ownMaterial = (p.informationGain?.by ?? []).length > 0 && (p.informationGain?.by ?? []).every((id) => !QUALIFIED.test(id) && !BRIEFING.test(id));
      const lost = ownMaterial && p.informationGain?.pageWhole === true ? undefined : units.filter((u) => !carriesUnit(u, c.after) && losable(u)).map((u) => ({ u, why: unaccounted(u) })).find((x) => x.why != null);
    ...
    const loss = materialLosses(p).filter(losable).map((l) => ({ l, why: unaccounted(l.replace(/^the (?:link|figure) /, "")) })).find((x) => x.why != null);
    if (loss) return `it replaces a passage that carries ${loss.l} and ${loss.why}`;
  }
  return null;
}

/* PAGE FURNITURE IS NOT MATERIAL, AND REPLACING IT IS NOT A LOSS (measured, 2026-09-05). Nineteen link rewrites, eighteen of them already applied by the operator, replace a card label reading exactly "Learn More" with the name of the page it opens, and every one was refused for not accounting for the words "Learn More": the door asked a change to justify destroying the one thing it was written to destroy. A navigation label carries no fact, no figure and no route, and the route itself survives untouched, so nothing is owed for its going. THE SCOPE IS THE LABEL AND NOTHING WIDER: a call to action that sells, a qualifier, a list member and a worked example are all still material and still vanish in silence if nothing accounts for them. */ /** A LINE WHOSE ONLY CONTENT IS AN INSTRUCTION TO THE READER: the verb, one article or adverb, and nothing after it but the end of the sentence. ONE DEFINITION, TWO DOORS (2026-09-05): the writer's door refuses a summary line that is an invitation and nothing else, and the replacement door owes nothing for destroying one, so both read this. */ const losable = (u: string): boolean => !FURNITURE_LABEL.test(u.replace(/^["\u201c\u2018]+|["\u201d\u2019]+$/g, "").trim());
