...
export const FURNITURE_AT = /^\s*(?:explore more|related(?: (?:articles?|posts?|pages?|topics?))?|you may also like|more(?: (?:from|like|articles?))?|read more|learn more|see also|share this|categories|tags|menu|navigation|footer|header|sidebar|newsletter|subscribe|follow us|shop( now)?|iranopedia|table of contents|contents|on this page|in this article|jump to|quick links)\s*$/i; /** A NAVIGATION LABEL A PAGE REPEATS AROUND ITS CONTENT, matched whole: the words a reader clicks past rather than reads. ONE DEFINITION, TWO DOORS (2026-09-05): the writer's door refuses to anchor a link on furniture, and the replacement door owes nothing for replacing a piece of it, so both read this one. "learn more" joins the list it was always a member of: "read more" and "explore more" stood here while the label 19 of this account's own cards actually carry did not. THE ONE ACCOUNT NAME IN IT IS NOT THIS ROUND'S TO RETIRE: "iranopedia" stands here from an earlier round, two shipped promises assert it, and the universality probe already counts the line; it is named in the report as the debt it is. */
/* PAGE FURNITURE IS NOT MATERIAL, AND REPLACING IT IS NOT A LOSS (measured, 2026-09-05). Nineteen link rewrites, eighteen of them already applied by the operator, replace a card label reading exactly "Learn More" with the name of the page it opens, and every one was refused for not accounting for the words "Learn More": the door asked a change to justify destroying the one thing it was written to destroy. A navigation label carries no fact, no figure and no route, and the route itself survives untouched, so nothing is owed for its going. THE SCOPE IS THE LABEL AND NOTHING WIDER: a call to action that sells, a qualifier, a list member and a worked example are all still material and still vanish in silence if nothing accounts for them. */ /** A LINE WHOSE ONLY CONTENT IS AN INSTRUCTION TO THE READER: the verb, one article or adverb, and nothing after it but the end of the sentence. ONE DEFINITION, TWO DOORS (2026-09-05): the writer's door refuses a summary line that is an invitation and nothing else, and the replacement door owes nothing for destroying one, so both read this. */ const losable = (u: string): boolean => !FURNITURE_AT.test(u.replace(/^["\u201c\u2018]+|["\u201d\u2019]+$/g, "").trim());
