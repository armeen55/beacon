...
import { readFactChecks } from "@/domains/evidence/pages/fact-checks"; import { REVIEW_CONTRACT, reviewFits, unreviewed } from "./proof"; import { staleCopyReasons } from "./drafted-copy"; import { loadOwnedPageBodies, type OwnedPageBody } from "@/domains/evidence/pages/owned-context"; import { canonicalUrlKey } from "@/domains/evidence/snapshot"; import { footprintCovers, footprintKey, footprintsOverlap } from "./mutation-footprint"; import { COPY_RULES } from "./copy-sanitize";
...
export async function answerReviewedProposal(tenantId: string, id: string, version: string, basis: string | null,
  answer: { kind: "promote" | "redraft"; by?: string; at: string }, reviewed?: ChangeProposal): Promise<{ status: "promoted" | "stale" | "refused" | "failed"; refusal?: string }> {
  ...
  try {
    ...
    return { status: data === "answered" ? "promoted" : "stale" };
