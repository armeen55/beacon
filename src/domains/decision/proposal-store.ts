...
async function preflightReviewedProposal(tenantId: string, p: ChangeProposal, at: Date): Promise<string | null> { try { const urls = publicationUrls(p), { body, canon } = await publicationCanon(tenantId, p, at, urls); if (p.recommendedChange.kind === "existing_edit" && p.pageUrl && !body) return "the current page capture is unavailable, so this copy cannot be checked before review"; return canon.need ? canon.reasons[0] ?? "current evidence is still owed" : canonFindings(canon)[0] ?? null; } catch { return "the current publication context is ambiguous or unreadable, so this copy cannot be checked before review"; } } export { preflightReviewedProposal };
...
export async function answerReviewedProposal(tenantId: string, id: string, version: string, basis: string | null, answer: { kind: "promote" | "redraft"; by?: string; at: string }, reviewed?: ChangeProposal): Promise<{ status: "promoted" | "stale" | "refused" | "failed"; refusal?: string }> {
  ...
  try {
    ...
    let urls: string[]; try { urls = answer.kind !== "promote" ? [] : publicationUrls(promoted); } catch { return { status: "refused", refusal: "it lacks an unambiguous publication component claim or page owner, so current source support is still owed; all banked copy is preserved" }; }
    const checked = answer.kind === "promote" ? await readFactChecks(tenantId).catch(() => null) : null, current = answer.kind === "promote" ? await publicationCanon(tenantId, promoted, new Date(answer.at), urls) : null, bodies = current?.bodies ?? new Map<string, OwnedPageBody>(), body = current?.body ?? null, canon = current?.canon ?? null,
      refusal = answer.kind !== "promote" ? undefined : openHold(promoted, { found: [...staleCopyReasons(promoted, bodies, [], body && { title: body.title, h1: body.h1, metaDescription: body.metaDescription, outline: body.headings }, true, [], [], undefined, { checked, basis }), ...actionableProposalFailures(promoted, { tenantId, currentBasis: basis, now: new Date(answer.at) }), ...canonFindings(canon!)] }).defects[0]; /** ONE VERDICT AT THE ONE DOOR THAT WRITES `ready` (owner's editorial policy, 2026-09-06). This was six refusals chained with `??`: the deliverable gaps, the open hold, the unsettled cause, the banked-copy re-read, the actionable failures and the canon, each with its own idea of what disqualifies work, and adding a seventh anywhere else in the kernel meant a screen and this door could disagree about the same row. The findings the chain used to hold are handed to the ONE readiness verdict as what this door found, and the door promotes exactly when its DEFECTS are empty: the advisories ride on the row and reach the operator as caveats. The SAFETY hold is not a defect here, because this door IS the deliberate confirmation it asks for. */
