...
const FAMILY_BY_KIND: Record<BundleComponentKind, ActionFamily> = { title: "title-family", meta: "title-family", h1: "title-family",
  ...
  factual_correction: "accuracy-family", source_update: "accuracy-family", entity_expansion: "section-family", // REPLACING UNTRUE WORDS IS ITS OWN HYPOTHESIS ABOUT A PAGE, never the same one as adding a section. Filed under section-family, the accuracy card and the ranking-loss card for one page superseded each other, so the page could hold only one of two true findings at a time (operator, 2026-08-17: they are separate).
...
export function proposalFingerprint(p: ChangeProposal): string {
  ...
  const material = {
    ...
    components: (p.bundle?.components ?? []).map((c) => [c.kind, c.page ?? null, c.where ?? null, c.before, c.after, // EVERY MATERIAL FIELD OF A PIECE, so a piece cannot change what it MEANS without a new identity. Kind, words, evidence and risk alone left the page it lands on, the place on that page, what it is for and why it works out of the hash: a four-address differentiation could drop an address, move a component from one page to another, or re-aim the whole change, and compute "unchanged" against the row it replaced.
      c.evidenceKeys, c.risk, c.objective ?? null, c.mechanism ?? null, c.anchorAfter ?? null, c.redirectTo ?? null, ...(c.units ? [c.units] : []), ...(c.target ? [c.target] : []), ...(c.preserves ? [c.preserves] : [])]),
    dispositions: p.bundle?.dispositions ?? null, ...(draft ? { newPageDraft: draft } : {}), ...(p.preservationNotes ? { preservationNotes: p.preservationNotes } : {}), ...(p.preservation?.length ? { preservation: p.preservation } : {}),
...
export async function saveChangeProposal(proposal: ChangeProposal, transition?: symbol, keep?: (row: ChangeProposal) => void): Promise<SaveResult> {
  ...
  try {
    ...
    const ident = identityOf(proposal); // recomputed AFTER the merge: what a row writes is derived from the copy that stands, and the merge can hand back the stored copy
    ...
    if (stored && proposalFingerprint(stored) === proposalFingerprint(proposal)) {
      if (JSON.stringify(stored.rankingReceipt ?? null) !== JSON.stringify(proposal.rankingReceipt ?? null)) await sb.from(TABLE).update({ ranking_receipt: proposal.rankingReceipt ?? null }).eq("tenant_id", proposal.tenantId).eq("id", proposal.id);
    ...
    if (handover) {
      ...
      if (error || data !== "saved") {
        log.error("[proposal-store] atomic supersession did not land, the stored change is unchanged", { tenantId: proposal.tenantId, id: proposal.id, answer: data ?? null, error: error?.message ?? null });
      ...
      keep?.(proposal); await reopenDependentSchema(proposal, stored?.status ?? null); return "saved";
    }
    try { await dualWriteUpsertScoped(TABLE, [rowFor(proposal, ident, version)], "id", proposal.tenantId); }
    catch (e) {
      log.error("[proposal-store] save failed, the stored change is unchanged", { tenantId: proposal.tenantId, id: proposal.id, error: e instanceof Error ? e.message : String(e) });
      return "failed"; }
    keep?.(proposal); await reopenDependentSchema(proposal, stored?.status ?? null); return "saved";
...
export async function transitionProposalToImplemented(tenantId: string, id: string, shipmentId: string, liveUrl?: string): Promise<boolean> {
  ...
  const { data, error } = await getSupabaseAdmin().from(TABLE)
    .update({ status: "implemented_pending_verification", payload: JSON.parse(serializeChangeProposal(flipped)) as unknown, terminal_disposition: null, superseded_by: null, withdrawn_reason: null, updated_at: new Date().toISOString() }) // the press outvotes a reconciliation withdrawal of this row, so the rescue flip walks it back to life exactly as the full save did
    .eq("tenant_id", tenantId).eq("id", id).select("id");
  if (error || (data ?? []).length === 0) { log.error("[proposal-store] the implemented flip did not land", { tenantId, id, error: error?.message ?? "no row matched" }); return false; }
  await reopenDependentSchema(flipped, proposal.status); return true; }
/** CONTENT CHANGES REOPEN THE SCHEMA THAT DESCRIBES THEM (audit, 2026-09-14): a section, an answer or a factual correction landing on a page (Ready, or marked done) puts that page's stored schema row back into review with the reason on it, and the next $0 pass re-derives the block from the current page under the same replacement identity. One rule, run after every landing, never on another page. ONLY A LANDING, NEVER A RE-SAVE (review, 2026-09-15): `from` is the status the row held on file before this write, so a ranking or evidence re-save of a Ready row, or a second implemented flip of the same row, is not a landing and reopens nothing; the sentence below is the one-pass guard, and the next $0 pass settles a reopened row either way, re-deriving the block from the page or taking the row back when the page mints nothing. */ const SCHEMA_REOPENED = "the page content this block describes changed; the block is re-derived from the current page";
async function reopenDependentSchema(landed: ChangeProposal, from: ChangeProposal["status"] | null): Promise<void> { const c = landed.recommendedChange; if (from === landed.status || c.kind !== "existing_edit" || !(c.field === "section" || c.field === "answer_block" || landed.changeFamily === "factual_correction") || (landed.status !== "ready" && landed.status !== "implemented_pending_verification")) return;
  const { data } = await getSupabaseAdmin().from(TABLE).select(CANON_COLUMNS).eq("tenant_id", landed.tenantId).eq("page_key", identityOf(landed).page_key).is("terminal_disposition", null);
  for (const r of (data ?? []) as CanonRow[]) { const s = r.id === landed.id ? null : decode(r.payload); if (!s || s.recommendedChange.kind !== "existing_edit" || s.recommendedChange.field !== "schema" || (s.status !== "ready" && s.status !== "needs_review") || s.limitations.includes(SCHEMA_REOPENED)) continue; const reopened: ChangeProposal = { ...s, status: "needs_review", obligation: { kind: "review" }, limitations: [...s.limitations, SCHEMA_REOPENED] };
    await dualWriteUpsertScoped(TABLE, [rowFor(reopened, identityOf(reopened), (r.proposal_version ?? 0) + 1)], "id", s.tenantId).catch((e: unknown) => log.error("[proposal-store] the dependent schema row was not reopened", { tenantId: s.tenantId, id: s.id, error: e instanceof Error ? e.message : String(e) })); } }
...
export async function answerReviewedProposal(tenantId: string, id: string, version: string, basis: string | null,
  ...
  try {
    ...
    let urls: string[]; try { urls = answer.kind !== "promote" || promoted.recommendedChange.kind === "new_page" ? [] : [...new Set([promoted.pageUrl ?? "", ...(COPY_RULES.reviewParts(promoted).length ? COPY_RULES.reviewSubjects(promoted, false).subjects.map(({ one }) => one.pageUrl!) : [])].filter(Boolean))]; } catch { return { status: "refused", refusal: "it lacks an unambiguous publication component claim or page owner, so current source support is still owed; all banked copy is preserved" }; }
    const checked = answer.kind === "promote" ? await readFactChecks(tenantId).catch(() => null) : null, bodies = answer.kind !== "promote" ? new Map<string, OwnedPageBody>() : await loadOwnedPageBodies(tenantId, urls).catch(() => new Map<string, OwnedPageBody>()), body = promoted.pageUrl ? bodies.get(canonicalUrlKey(promoted.pageUrl)) ?? null : null, canon = answer.kind !== "promote" ? null : validateProposal(promoted, { /* THE SAME READER AS THE WRITER'S DOOR (reviewer, 2026-09-02): this door judged finished copy against four stored fields and no page body at all */ now: new Date(answer.at), /* AND ON THE SAME GROUNDING (campaign, 2026-09-05): the diagnosis lines and the cause sentence are what the row was ASKED to fix, not words anything checked, so a figure the brief printed grounded the claim it was meant to judge. Dropped at both doors together, or copy the writer's door refuses is promoted by this one. */ ...canonTextOf(null, body, [(promoted.bundle?.receipt.items ?? []).map((i) => i.fact).join(" "), (promoted.supportFacts ?? []).map((f) => f.fact).join(" ")].filter(Boolean)) }),
      refusal = answer.kind !== "promote" ? undefined : openHold(promoted, { found: [...staleCopyReasons(promoted, bodies, [], body && { title: body.title, h1: body.h1, metaDescription: body.metaDescription, outline: body.headings }, true, [], [], undefined, { checked, basis }), ...actionableProposalFailures(promoted, { tenantId, currentBasis: basis, now: new Date(answer.at) }), ...(canon!.verdict === "rejected" ? canon!.reasons : canon!.qualityStatus !== "ready" ? [canon!.qualityStatus === "missing_source" ? "This change states a fact with no source behind it. It is held until a source is added." : canon!.qualityStatus === "useful_but_needs_review" ? "This change introduces a number that has not been confirmed yet. It is held until that number is checked." : (canon!.reasons[0] ?? "This change is not finished enough to promote yet.")] : [])] }).defects[0]; /** ONE VERDICT AT THE ONE DOOR THAT WRITES `ready` (owner's editorial policy, 2026-09-06). This was six refusals chained with `??`: the deliverable gaps, the open hold, the unsettled cause, the banked-copy re-read, the actionable failures and the canon, each with its own idea of what disqualifies work, and adding a seventh anywhere else in the kernel meant a screen and this door could disagree about the same row. The findings the chain used to hold are handed to the ONE readiness verdict as what this door found, and the door promotes exactly when its DEFECTS are empty: the advisories ride on the row and reach the operator as caveats. The SAFETY hold is not a defect here, because this door IS the deliberate confirmation it asks for. */
    if (canon?.need) return { status: "refused", refusal: canon.reasons[0] };
    if (refusal) return { status: "refused", refusal };
    const { data, error } = await getSupabaseAdmin().from(TABLE)
      .update({ status: promoted.status, payload: JSON.parse(serializeChangeProposal(promoted.status === "ready" ? finished(promoted) : promoted)) as unknown, proposal_version: row.proposal_version + 1, // the operator's yes writes a FINISHED row, and it is normalized after the refusal chain above has judged the row exactly as it stands, never before
...
export async function loadChangeProposals(tenantId: string): Promise<Map<string, ChangeProposal>> {
  ...
  try {
    ...
    while (out.size < QUEUE_CEILING) {
      ...
      if (error) { log.error("[proposal-store] canonical read failed, showing history only", { tenantId, error: error.message }); break; }
      canonical = true;
  ...
  } catch (e) { log.error("[proposal-store] canonical read threw, showing history only", { tenantId, error: e instanceof Error ? e.message : String(e) }); }
