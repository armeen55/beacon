...
import { confirmedVersion, GATE_WORDS, openHold, preferFinished } from "./completeness";
...
import { readFactChecks } from "@/domains/evidence/pages/fact-checks"; import { REVIEW_CONTRACT, reviewFits, unreviewed } from "./proof"; import { EDITOR_SHARED, staleCopyReasons } from "./drafted-copy"; import { loadOwnedPageBodies, type OwnedPageBody } from "@/domains/evidence/pages/owned-context"; import { canonicalUrlKey } from "@/domains/evidence/snapshot"; import { isCurrent } from "@/domains/evidence/freshness"; import { footprintCovers, footprintKey, footprintsOverlap } from "./mutation-footprint"; import { COPY_RULES } from "./copy-sanitize";
...
export async function saveChangeProposal(proposal: ChangeProposal, transition?: symbol, keep?: (row: ChangeProposal) => void, exactExpected?: ChangeProposal, expectAbsent = false): Promise<SaveResult> {
  ...
  if (proposal.status === "implemented_pending_verification") {
  ...
  try {
    ...
    if (mine?.status === "implemented_pending_verification") { log.info("[proposal-store] you already marked this change done, so a new draft is not written over it", { tenantId: proposal.tenantId, id: proposal.id }); return "blocked"; }
    ...
    if (proposal.changeFamily === "factual_correction" && proposal.status === "needs_review" && proposal.researchOnly !== true && proposal.limitations.includes(COPY_RULES.reviewHolds.correction) && unreviewed(proposal) == null) {
      const current = { ...proposal, status: "ready" as const, limitations: proposal.limitations.filter((l) => l !== COPY_RULES.reviewHolds.correction) }, canon = validateProposal(current);
      if (canon.verdict === "ready" && canon.qualityStatus === "ready" && openHold(current).defects.length === 0) proposal = current;
    }
...
const promotionRejection = async (tenantId: string, p: ChangeProposal, basis: string | null, at: Date, ignoreReviewDebt = false): Promise<PromotionCheck> => { const urls = publicationUrls(p), [checked, current] = await Promise.all([readFactChecks(tenantId).catch(() => null), publicationCanon(tenantId, p, at, urls)]), { bodies, body, canon } = current, page = body && { title: body.title, h1: body.h1, metaDescription: body.metaDescription, outline: body.headings }, canRefresh = ignoreReviewDebt && p.bundle == null, normal = staleCopyReasons(p, bodies, [], page, true, [], [], undefined, { checked, basis }), refreshed = staleCopyReasons(p, bodies, [], page, true, [], [], undefined, { checked, basis }, canRefresh), reviewRefresh = canRefresh && normal.includes(COPY_RULES.pageState.whole) && !refreshed.includes(COPY_RULES.pageState.capture), ignorable = (reason: string): boolean => ignoreReviewDebt && (COPY_RULES.reviewHold(reason) || canRefresh && reason === COPY_RULES.pageState.whole), found = [...refreshed, ...actionableProposalFailures(p, { tenantId, currentBasis: basis, now: at }), ...canonFindings(canon)].filter((reason) => !ignorable(reason)), refusal = openHold(p, { found }).defects.find((reason) => !ignorable(reason)), units = body ? [body.title ?? "", body.h1 ?? "", ...body.headings, ...body.passages].filter(Boolean) : [], held = COPY_RULES.flat(units.join(" ")), evidence = Object.fromEntries((p.supportFacts ?? []).filter((f) => EDITOR_SHARED.OWN_PAGE_ID.test(f.id) && held.includes(COPY_RULES.flat(f.fact))).map((f) => [f.id, f.fact])), diagnosedNoGain = ignoreReviewDebt && body?.version === "current" && body.completeness === "complete" && EDITOR_SHARED.diagnosedMissingAnswerPresent(p, units) ? EDITOR_SHARED.NO_CHANGE_SAYS : null, noGain = ignoreReviewDebt && body && EDITOR_SHARED.ownPageOnlyMissing(p, evidence) ? EDITOR_SHARED.NO_CHANGE_SAYS : diagnosedNoGain, unread = ignoreReviewDebt && p.recommendedChange.kind === "existing_edit" && p.pageUrl && !body ? "the current page capture is unavailable, so this copy cannot be checked before review" : null, linkTarget = p.recommendedChange.kind === "existing_edit" && p.recommendedChange.linkTo ? bodies.get(canonicalUrlKey(new URL(p.recommendedChange.linkTo, /^https?:\/\//.test(p.pageUrl ?? "") ? p.pageUrl! : `https://${p.pageUrl ?? ""}`).toString())) : null, linkOwed = p.recommendedChange.kind === "existing_edit" && p.recommendedChange.linkTo && ([body, linkTarget].some((b) => !b || b.version !== "current" || b.completeness !== "complete" || !isCurrent("owned_page", b.fetchedAt, at.getTime()) || !b.pageId || !b.latestCaptureId) || !samePublicationAddress(p.pageUrl!, urls[1]!, true) || !samePublicationAddress(p.pageUrl!, body?.url) || !samePublicationAddress(p.pageUrl!, (body as OwnedPageBody & { finalUrl?: string | null } | null)?.finalUrl) || !samePublicationAddress(urls[1]!, linkTarget?.url) || !samePublicationAddress(urls[1]!, (linkTarget as OwnedPageBody & { finalUrl?: string | null } | null)?.finalUrl)) ? COPY_RULES.pageState.capture : null, captures = [...bodies.values()].flatMap((b): PromotionCapture[] => {
  ...
  }); return { reason: canon.need ? canon.reasons[0] ?? "current evidence is still owed" : linkOwed ?? refusal ?? unread ?? noGain, reviewRefresh, captures }; };
...
/** Raw CAS token for one owner-visible version; the Shipment RPC compares it under the proposal row lock. */
