...
const promotionRejection = async (tenantId: string, p: ChangeProposal, basis: string | null, at: Date, ignoreReviewDebt = false): Promise<string | null> => { const urls = publicationUrls(p), [checked, current] = await Promise.all([readFactChecks(tenantId).catch(() => null), publicationCanon(tenantId, p, at, urls)]), { bodies, body, canon } = current, found = [...staleCopyReasons(p, bodies, [], body && { title: body.title, h1: body.h1, metaDescription: body.metaDescription, outline: body.headings }, true, [], [], undefined, { checked, basis }), ...actionableProposalFailures(p, { tenantId, currentBasis: basis, now: at }), ...canonFindings(canon)].filter((reason) => !ignoreReviewDebt || !COPY_RULES.reviewHold(reason)), refusal = openHold(p, { found }).defects[0], unread = ignoreReviewDebt && p.recommendedChange.kind === "existing_edit" && p.pageUrl && !body ? "the current page capture is unavailable, so this copy cannot be checked before review" : null; return canon.need ? canon.reasons[0] ?? "current evidence is still owed" : refusal ?? unread; };
async function preflightReviewedProposal(tenantId: string, p: ChangeProposal, basis: string | null, at: Date): Promise<string | null> { try { return await promotionRejection(tenantId, { ...p, status: "ready", obligation: undefined, faults: p.faults?.filter((f) => !COPY_RULES.reviewHold(f)), limitations: p.limitations.filter((f) => !COPY_RULES.reviewHold(f)) }, basis, at, true); } catch { return "the current publication context is ambiguous or unreadable, so this copy cannot be checked before review"; } } export { preflightReviewedProposal };
