...
export async function answerReviewedProposal(tenantId: string, id: string, version: string, basis: string | null,
  ...
  try {
    ...
    const redraftWorkKey = answer.kind === "redraft" ? `operator-redraft:${createHash("sha256").update(JSON.stringify([stored.workKey ?? id, version, answer.at])).digest("hex").slice(0, 24)}` : null;
    const promoted: ChangeProposal = answer.kind === "redraft" ? { ...stored, redraftRequested: answer.at, workKey: redraftWorkKey! }
