...
export async function buildNewPageProposal(decided: DecidedTopic, tenantId: string, opts: ProposeOptions = {}): Promise<NewPageOutcome> {
  ...
  const write = (heading: string, covers: string): Promise<NonNullable<Awaited<ReturnType<typeof draftFieldForPage>>> | null> =>
    draftFieldForPage({ field: "answer_block", body: soFar(), query: inv.label, ownedPaths: owned.map((c) => c.path), minutes: 15, evidenceHints: facts, facts: qualified, unpublished: true, /* THE OPENING THIS MODEL JUST WROTE IS NOT A READING OF A PAGE (probe, 2026-09-05): `soFar` is the outline model's own opening plus this page's earlier drafted sections, and it entered the next section's packet as `page-copy-N`, which every gate reads as words observed on a live page. It rides as generated draft context now, so it can still be seen and can never be cited as the page's own. */
