...
export async function buildNewPageProposal(decided: DecidedTopic, tenantId: string, opts: ProposeOptions & { /** THE PAGES THAT WIN THESE SEARCHES, as the pass already holds them, so the writer of a section this account has no page for is handed the same reading the writer of an existing page's section gets. Absent means nobody has read them, and unknown is never absence. */ research?: Parameters<typeof jobComparison>[0]; /** THE HALF WRITTEN PAGE THIS TOPIC ALREADY HAS ON FILE, when the last pass could not finish it: its brief and every piece already written, so this pass buys only what is still owed. */ held?: ChangeProposal | null } = {}): Promise<NewPageOutcome> {
  ...
  }); if (call) opts.attempts?.record?.(call); /* real requests and real dollars, onto this page's own allowance */
  ...
  if (body.length < outline.length) { const owed = outline.filter((h) => !made.has(norm(h))); log.warn("[new-page] partial draft, nothing proposed", { tenantId, topicKey: inv.key, owed: owed.length, unsourced: owedSource.length });
    /* THE SECTIONS NOBODY CAN SOURCE ARE FILED AS THE RESEARCH THEY ARE, on the row and therefore on the pass's own owed list, exactly as an existing page's missing answer is: a `factual_source` need whose proposition is the section's own heading. The runtime buys the source, and the next pass finds the brief and every finished piece on this same row and writes that section with a `fact-` id it may cite. ONE need at a time is what the buy loops execute, so the first owed heading is the obligation and every one of them is named in the limitations. */ if (owedSource.length === 0 || !home) return { status: "none", reason: `${num(body.length)} of the ${num(outline.length)} sections this page needs are written and ${num(owed.length)} ${owed.length === 1 ? "is" : "are"} still owed: ${owed.join(", ")}. Part of a page is not worth handing over. Ask again and it picks up where it stopped: what is already written costs nothing a second time.` }; const need: EvidenceRequirement = { kind: "factual_source", query: `${owedSource[0]} ${inv.label}`.slice(0, 120), url: home.url, reasonCode: "new_page_section_unsourced", missingTopic: owedSource[0]! };
    return { status: "built", proposal: { id: `${tenantId}::${inv.key}::new_page::bundle`, tenantId, kind: "new_page", changeFamily: "new_page", publish: "manual", pagePath: null, pageUrl: null, pageLabel: v.proposedTitle, primaryQuery: inv.label, opportunityType: "Cover a subject you have no page for", status: "needs_review", researchOnly: true, treatment: "new_page", obligation: { kind: "evidence", need }, research: { missing: `Nothing checked stands behind ${num(owedSource.length)} of the ${num(outline.length)} sections this page needs: ${owedSource.join(", ")}.`, next: `A source is being read for "${owedSource[0]}", and that section gets written once it lands.` }, recommendedChange: { kind: "new_page", proposedTitle: v.proposedTitle, metaDescription: v.metaDescription, openingAnswer: v.openingAnswer, outline, faqQuestions: v.faqQuestions, schemaTypes: [] }, whyItMatters: `${decision.explanation} ${num(body.length)} of its ${num(outline.length)} sections are written and kept, and the rest wait on a source.`, estimatedEffortMinutes: effortForFamily("new_page"), operatorSteps: ["Nothing to do yet: a source is being read for the sections that still owe one"], riskLevel: "low", confidence: "medium", limitations: [...new Set([...missing, ...owedSource.map((h) => `Nothing checked stands behind "${h}" yet, so it is being sourced before it is written.`)])], evidence: { query: inv.label, hints: [...facts.slice(0, 5), ...[...questions.values()].slice(0, 8)], evidenceRefCount: items.length }, impactScore: null, upsidePerMonth: null, createdAt: now.toISOString(), newPageDraft: { brief: v as unknown as Record<string, unknown>, pieces } } }; }
