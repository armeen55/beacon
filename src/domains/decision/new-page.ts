...
import { copyKey, evidenceShortfall, REVIEW_CONTRACT, reviewFits } from "./proof"; import { nextObligation } from "./obligation";
...
export async function buildNewPageProposal(decided: DecidedTopic, tenantId: string, opts: ProposeOptions & { basis?: string | null; site?: string; /** THE PAGES THAT WIN THESE SEARCHES, as the pass already holds them, so the writer of a section this account has no page for is handed the same reading the writer of an existing page's section gets. Absent means nobody has read them, and unknown is never absence. */ research?: Parameters<typeof jobComparison>[0]; /** THE HALF WRITTEN PAGE THIS TOPIC ALREADY HAS ON FILE, when the last pass could not finish it: its brief and every piece already written, so this pass buys only what is still owed. */ held?: ChangeProposal | null } = {}): Promise<NewPageOutcome> {
  ...
  const site = opts.site ?? (owned[0] ? publisherHost(owned[0].url) : (await import("@/domains/account").then((m) => m.getTenant(tenantId)))?.domain?.trim());
  if (!site) return { status: "none", reason: "The website identity is unavailable, so no prospective page is drafted." };
  ...
  if (stored && opts.held!.tenantId !== tenantId) return { status: "none", reason: "The saved page belongs to a different account; no work was bought." };
  const restored = stored ? SCHEMA_BY_KIND.new_page_brief.safeParse(stored.brief) : null; if (restored && !restored.success) return { status: "built", proposal: { ...opts.held!, status: "needs_review", researchOnly: true, obligation: { kind: "terminal", reason: "The saved page has an invalid brief; preserve its copy and reconcile the plan before any paid work." } } };
  ...
  const v = { ...(restored?.success ? restored.data : call!.value) } as NewPageBrief;
  ...
  const qualified = (owned.map((c) => ({ c, text: bankedBodies.get(canonicalUrlKey(c.url))?.passages.join("\n\n") || c.openingSample || "" })).filter(({ text }) => text.trim()).slice(0, 4).map(({ c, text }, i) => ({ id: `owned-page-${i + 1}`, fact: `${c.path}: ${text}`, sources: [{ url: c.url.startsWith("http") ? c.url : `https://${c.url}`, kind: "owner" }] })));
  let outline = v.sections.map((s) => s.heading), openingCopy = v.openingAnswer;
  ...
  const restoredPieces = COPY_RULES.newPagePieces({ brief: v as unknown as Record<string, unknown>, pieces: stored?.pieces ?? [] }), metadata = [...(stored?.metadata ?? [])];
  const planIdentity = COPY_RULES.recordKey([tenantId, publisherHost(site), inv.key, opts.basis ?? opts.held?.basis ?? null, v.sections]); if (stored && (!restoredPieces || stored.brief.identity != null && stored.brief.identity !== planIdentity || ![inv.key, ...inv.aliasKeys].some(k => opts.held!.id.includes(`::${k}::`)) || opts.basis != null && opts.held!.basis != null && opts.basis !== opts.held!.basis)) return { status: "built", proposal: { ...opts.held!, status: "needs_review", researchOnly: true, obligation: { kind: "terminal", reason: "The saved page needs its plan and piece ownership reconciled; all copy is preserved and no work was bought." } } };
  const made = restoredPieces!, kept = [...made.entries()].sort(([a], [b]) => a - b).map(([, piece]) => piece) as Piece[], written = (): string[] => v.sections.map((_, i) => made.get(i + 1)).filter((a): a is Piece => !!a).map(a => `${(a.heading ?? "").trim()}\n\n${a.after}`.replace(/[–—]/g, " ")); (v as unknown as Record<string, unknown>).identity = planIdentity;
  if (made.get(0)) openingCopy = made.get(0)!.after;
  const soFar = (exclude?: Piece): OwnedPageBody => ({ url: `https://${site}`, title: v.proposedTitle, h1: v.proposedTitle, metaDescription: v.metaDescription,
    headings: [v.proposedTitle, ...v.sections.map((_, i) => made.get(i + 1)).filter((p): p is Piece => !!p && p !== exclude).map(p => p.heading ?? "")], passages: [openingCopy, ...written()].filter((p) => p !== exclude?.after && p !== `${exclude?.heading}\n\n${exclude?.after}`), openingSample: openingCopy === exclude?.after ? "" : openingCopy, vocabulary: "", cardTexts: [], faqs: [],
  ...
  const seen = (body = soFar(), focus: readonly string[] = []): JobComparison | null => opts.research ? jobComparison(opts.research, inv.queries, { url: body.url, text: [body.title ?? "", ...body.passages].join(" "), headings: body.headings, passages: body.passages }, undefined, inv.demandBasis === "ai" ? "aeo" : "seo", focus) : null;
  const topic = { key: `topic:${inv.key}`, label: inv.label }, basis = opts.basis !== undefined ? opts.basis : await import("./load-proposals").then((m) => m.resolveCurrentBasis(tenantId));
  const readings = basis ? authorizedCorrections((await readFactChecks(tenantId, topic.key).catch(() => [])).filter((f) => f.page === topic.key && f.current.trim() === "" && f.pageContentHash === null && f.evidenceBasis === basis), undefined, tenantId) : [];
  const factsFor = (heading: string) => readings.filter((f) => norm(f.subject) === norm(`${inv.label} ${heading}`));
  ...
  const write = async (slot: number, heading: string, covers: string, checked: typeof readings, c: JobComparison | null, saved?: Piece): Promise<Piece | null> => {
    const done = await draftFieldForPage({ field: "answer_block", body: soFar(saved), query: inv.label, saved, ownedPaths: owned.map((c) => c.path), minutes: 15, evidenceHints: facts, banked: saved?.supportFacts ?? qualified, checked, basis, topic: topic.key, unpublished: true, comparison: c, refusalKey: heading,
    ...
    { tenantId, now, complete: opts.complete, bypassCache: opts.bypassCache, refusals: whyRefused, ...(opts.attempts ? { attempts: opts.attempts } : {}) }); return done ? { ...done, slot } : null; };
  ...
  if (repair) {
    ...
    for (const target of [...remaining]) {
      ...
      const done = await draftFieldForPage({ field: n === 0 ? "title" : n === 1 ? "meta" : "answer_block", body, query: inv.label, saved: n < 2 ? metadata.find((m) => m.component === n && !COPY_RULES.accepted(m.piece.editor))?.piece : old && !COPY_RULES.accepted(old.editor) ? old : undefined, ownedPaths: owned.map((c) => c.path), minutes: 15, evidenceHints: facts, banked: banked.length ? banked : qualified, checked: readings, basis, topic: topic.key, unpublished: true, comparison: seen(body, [heading, target.instruction]), refusalKey: String(n), brief: `${n >= 2 ? `Write the section headed "${heading}". ` : ""}Revise only this ${part.kind}, not the rest of the page. Previous copy: ${part.after}. Required repair: ${target.instruction}. Use the banked source evidence; never publish these instructions or add unsupported claims.` }, { tenantId, now, complete: opts.complete, bypassCache: opts.bypassCache, refusals: whyRefused, attempts: opts.attempts });
      repairAttempted ||= !!done || (before != null && opts.attempts!.left < before); if (!done) break;
      const piece: Piece = { ...done, ...(n >= 2 ? { slot: n - 2 } : {}) };
      ...
      else { kept[n - 2] = piece; made.set(n - 2, piece); if (n === 2) openingCopy = done.after; }
      if (!COPY_RULES.accepted(done.editor)) break; remaining.splice(remaining.findIndex((t) => t.component === n), 1);
    }
  }
  const opening = made.get(0), openingUnsourced = !opening && qualified.length === 0 && readings.length === 0, owedSource: string[] = openingUnsourced ? [v.proposedTitle] : [];
  const first = opening && COPY_RULES.accepted(opening.editor) ? opening : (openingUnsourced ? null : await write(0, v.proposedTitle, `Answer "${inv.label}" outright in the first lines a reader sees. The planning opening is context, never a source: "${v.openingAnswer}". Verify it against the banked readings and refine it to fit.`, readings, seen(), opening)) ?? opening ?? null; if (first) openingCopy = first.after;
  for (const [i, s] of v.sections.entries()) { if (!first || !COPY_RULES.accepted(first.editor)) break; const saved = made.get(i + 1); if (saved && COPY_RULES.accepted(saved.editor)) continue;
    const c = seen(soFar(saved), [s.heading, s.covers]), fx = factsFor(s.heading), obs = c ? comparisonObservations(c) : []; /* Source acquisition precedes new writing, never review of source-bound saved copy. */ if (!saved && obs.length === 0 && fx.length === 0 && (c?.winners ?? []).some((w) => w.read)) { owedSource.push(s.heading); continue; }
    const done = await write(i + 1, s.heading, s.covers, fx, c, saved); if (!done) break; made.set(i + 1, done); if (!COPY_RULES.accepted(done.editor)) break; }
  outline = v.sections.map((s, i) => made.get(i + 1)?.heading ?? s.heading); const pieces: Piece[] = first ? [first, ...v.sections.map((_, i) => made.get(i + 1)).filter((a): a is Piece => !!a)] : [], body = written(), reviewPending = pieces.some((piece) => !COPY_RULES.accepted(piece.editor));
  if (body.length < outline.length || reviewPending) { const owed = outline.filter((_, i) => !made.has(i + 1)); log.warn("[new-page] partial draft, nothing proposed", { tenantId, topicKey: inv.key, owed: owed.length, unsourced: owedSource.length, reviewPending });
    const need: EvidenceRequirement | null = owedSource.length > 0 ? { kind: "factual_source", query: inv.label, topic, reasonCode: "new_page_section_unsourced", missingTopic: `${inv.label} ${owedSource[0]}` } : null;
    return { status: "built", proposal: { id: `${tenantId}::${inv.key}::new_page::bundle`, tenantId, kind: "new_page", changeFamily: "new_page", publish: "manual", pagePath: null, pageUrl: null, pageLabel: v.proposedTitle, primaryQuery: inv.label, opportunityType: "Cover a subject you have no page for", status: "needs_review", researchOnly: true, treatment: "new_page", obligation: reviewPending ? { kind: "review" } : need ? { kind: "evidence", need } : { kind: "sections", owed: owed.length }, research: reviewPending ? { missing: `Written copy for "${pieces.find((piece) => !COPY_RULES.accepted(piece.editor))?.heading ?? v.proposedTitle}" still awaits review; ${num(owed.length)} sections remain unwritten.`, next: "The saved piece is reviewed before more writing; approved copy is preserved." } : need ? { missing: `Nothing checked stands behind ${num(owedSource.length)} of the ${num(outline.length)} sections this page needs: ${owedSource.join(", ")}.`, next: `A source is being read for "${owedSource[0]}", and that section gets written once it lands.` } : { missing: `${num(owed.length)} of the ${num(outline.length)} sections this page needs are not written yet: ${owed.join(", ")}.`, next: `The next pass writes "${owed[0]}", and what is already written costs nothing a second time.` }, recommendedChange: { kind: "new_page", proposedTitle: v.proposedTitle, metaDescription: v.metaDescription, openingAnswer: openingCopy, outline, faqQuestions: v.faqQuestions, schemaTypes: [] }, whyItMatters: `${decision.explanation} ${num(body.length)} of its ${num(outline.length)} sections are written and kept, ${reviewPending ? "and written copy must pass review before more sections are written." : need ? "and the rest wait on a source." : "and the rest are written on the next pass."}`, estimatedEffortMinutes: effortForFamily("new_page"), operatorSteps: [reviewPending ? "Nothing to do yet: saved copy is awaiting review before the page can be completed" : need ? "Nothing to do yet: a source is being read for the sections that still owe one" : "Nothing to do yet: the remaining sections are written on the next pass"], riskLevel: "low", confidence: "medium", limitations: [...new Set([...missing, ...(need ? owedSource.map((h) => `Nothing checked stands behind "${h}" yet, so it is being sourced before it is written.`) : owed.map((h) => `"${h}" is not written yet, and it is written on the next pass.`))])], evidence: { query: inv.label, hints: [...facts.slice(0, 5), ...[...questions.values()].slice(0, 8)], evidenceRefCount: items.length }, impactScore: null, upsidePerMonth: null, createdAt: now.toISOString(), newPageDraft: { brief: v as unknown as Record<string, unknown>, pieces } } }; }
  ...
  const components: BundleComponent[] = [
    ...
    { kind: "opening_answer", label: "Opening answer", before: null, ...COPY_RULES.publication({ after: openingCopy, units: pieces[0]?.units, target: COPY_RULES.target("", [], "opening") }), evidenceKeys: core, risk: "review" },
    ...pieces.slice(1).map((piece, i): BundleComponent => {
      const previous = pieces[i]!, last = previous.units?.at(-1), anchor = i === 0 ? last ? last.kind === "heading" || last.kind === "paragraph" ? last.text : last.items.join(" ") : previous.after : previous.heading!;
      return { kind: "section", label: piece.heading!, before: null, ...COPY_RULES.publication({ ...piece, target: COPY_RULES.target(anchor, i === 0 && last?.kind !== "heading" ? [] : [anchor], "add") }, piece.heading), evidenceKeys: sectionKeys, risk: "review" };
    }),
  ];
  ...
  const { claims, review: ruled, supportFacts, gain, editor, draftNotes } = assembleCopy(components, [...metadata.map((m) => ({ copy: m.piece, index: m.component })), ...pieces.map((copy, n) => ({ copy, index: n + 2 }))]);
  ...
  const proposal: ChangeProposal = {
    ...
    riskLevel: "low",
    confidence: "medium",
    limitations: [...new Set([...missing, ...draftNotes])],
  ...
  const reviewed = await reviewFinishedCopy({ ...row, ...(opts.held?.semanticReview?.scope === "whole_page" && reviewFits(row, opts.held.semanticReview.of) && opts.held.semanticReview.version === REVIEW_CONTRACT && COPY_RULES.accepted(opts.held.semanticReview.editor) ? { semanticReview: opts.held.semanticReview } : {}) }, { tenantId, now, basis, checked: readings, attempts: opts.attempts, complete: opts.complete, bypassCache: opts.bypassCache });
