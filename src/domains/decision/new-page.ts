...
export async function buildNewPageProposal(decided: DecidedTopic, tenantId: string, opts: ProposeOptions & { basis?: string | null; site?: string; /** THE PAGES THAT WIN THESE SEARCHES, as the pass already holds them, so the writer of a section this account has no page for is handed the same reading the writer of an existing page's section gets. Absent means nobody has read them, and unknown is never absence. */ research?: Parameters<typeof jobComparison>[0]; /** THE HALF WRITTEN PAGE THIS TOPIC ALREADY HAS ON FILE, when the last pass could not finish it: its brief and every piece already written, so this pass buys only what is still owed. */ held?: ChangeProposal | null } = {}): Promise<NewPageOutcome> {
  ...
  const soFar = (): OwnedPageBody => ({ url: `https://${site}`, title: v.proposedTitle, h1: v.proposedTitle, metaDescription: v.metaDescription,
  ...
  if (repair) {
    ...
    for (const target of [...remaining]) {
      ...
      const done = await draftFieldForPage({ field: n === 0 ? "title" : n === 1 ? "meta" : "answer_block", body, query: inv.label, ownedPaths: owned.map((c) => c.path), minutes: 15, evidenceHints: facts, facts: banked.length ? {} : qualified, banked, unpublished: true, comparison: seen(body, [heading, target.instruction]), refusalKey: String(n), brief: `${n >= 2 ? `Write the section headed "${heading}". ` : ""}Revise only this ${part.kind}, not the rest of the page. Previous copy: ${part.after}. Required repair: ${target.instruction}. Use the banked source evidence; never publish these instructions or add unsupported claims.` }, { tenantId, now, complete: opts.complete, bypassCache: opts.bypassCache, refusals: whyRefused, attempts: opts.attempts });
  ...
  const openingUnsourced = !kept[0] && Object.keys(qualified).length === 0 && readings.length === 0, owedSource: string[] = openingUnsourced ? [v.proposedTitle] : [];
  const first = kept[0] ?? (openingUnsourced ? null : await write(v.proposedTitle, `Answer "${inv.label}" outright in the first lines a reader sees. The planning opening is context, never a source: "${v.openingAnswer}". Verify it against the banked readings and refine it to fit.`, readings, seen())); if (first) openingCopy = first.after;
