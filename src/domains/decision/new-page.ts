...
export async function produceNewPage(input: Input): Promise<Result> {
  ...
  const idPart = caseId(topic.key), id = `${tenantId}::${idPart}::new_page`, owner = sourceKey(idPart), prior = input.prior?.id === id ? input.prior : null, bank = prior?.newPageDraft, stored = bank?.brief.kind === "new_page" ? bank.brief : null;
  const holdSource = async (detail: string, source = true): Promise<Result> => { if (!prior) return { row: null, ...(source ? { need: topic.label } : {}), detail }; const held: ChangeProposal = { ...prior, status: "needs_review", researchOnly: true, semanticReview: undefined, obligation: source ? { kind: "evidence", need: { kind: "factual_source", topic: { key: owner, label: topic.label }, query: topic.label, missingTopic: topic.label, reasonCode: "new_page_source_owed" } } : { kind: "terminal", reason: detail } }; return await input.save(held) ? { row: held, ...(source ? { need: topic.label } : {}), detail } : { row: null, ...(source ? { need: topic.label } : {}), detail: `${detail} The safety hold could not be persisted.` }; };
