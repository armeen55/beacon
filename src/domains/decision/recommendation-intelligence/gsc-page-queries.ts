 ...
 * scoreboard chart draws. Reads the small, accurate `gsc_daily_totals` table
 * (the ungrouped totals Google reports directly, so nothing is truncated),
 * ascending by date, bounded to a trailing window. Fail-soft to an empty series:
 * a Search Console outage narrows the chart, it never breaks the surface.
 *
 * Everything else this file once held (per-page query scans, tenant-wide query
 * loaders, striking/declining/rising page finders) had no caller left after the
