...
export function preferFinished(incoming: ChangeProposal, prior: ChangeProposal | null | undefined): ChangeProposal {
  ...
  for (const f of COPY_OWNED) if (prior[f] != null) (kept as Record<string, unknown>)[f] = prior[f]; else if (f !== "preservation" && f !== "informationGain") delete (kept as Record<string, unknown>)[f]; // A RECEIPT ABOUT THE BANKED WORDS MAY ARRIVE LATE: deleting every copy-owned field the prior lacks is right for `faults`, which describe the INCOMING draft, and wrong for the authorization receipts, because identity already proves the copy is byte for byte the same. Without this the factual producer stamped its ledger and preservation threw it away, holding three sound corrections for ever (Codex, 2026-08-28)
  return kept;
}
