...
export function openHold(p: ChangeProposal, also: { found?: readonly string[] } = {}): { lane: "review" | "research"; why: string[]; caveats: string[]; blocking: string | null; faulted: boolean; safetyHold: boolean; defects: string[]; advisories: Advisory[] } {
  ...
  const bodyDefects = AEO_BAR.forRow(p) && c.kind === "existing_edit" ? AEO_BAR.failures(c.field, c.after, p.limitations, p.assignment?.standard) : [];
  ...
  return { lane: p.researchOnly === true || gaps.some((g) => NOT_WRITTEN.test(g)) ? "research" : "review",
    ...
    why: heldBy.length > 0 ? heldBy : [UNJUDGED], caveats: [...advisories.map((a) => a.say), ...p.limitations.filter((l) => !GATE_WORDS.test(l) && !said.includes(l))], blocking, faulted: defects.length > 0, safetyHold: blocking === DANGER, defects, advisories };
}
