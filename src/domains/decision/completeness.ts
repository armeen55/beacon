...
export function openHold(p: ChangeProposal, also: { found?: readonly string[] } = {}): { lane: "review" | "research"; why: string[]; caveats: string[]; blocking: string | null; faulted: boolean; safetyHold: boolean; defects: string[]; advisories: Advisory[] } {
  ...
  const TASTE_FAULT = /^(it points at the page instead of answering|most of what it says is already on the page|its heading is the tracked question|it is not useful or does not read naturally|no serious editor would hand this to a customer|it does not resolve the diagnosed problem|the figure's own sentence says |its "[^"]+" is a ranking or register judgement|the new line carries only the current line's own words|it says [^ ]+ more than once each)/;
  const tasteSaid: string[] = [];
  const every0 = [...new Set([...hard, ...faults.filter(current), ...also.found ?? []])];
  const every = additive ? every0.filter((x) => { if (TASTE_FAULT.test(x) || AEO_BAR.copyRefusals.owns(x)) { tasteSaid.push(x); return false; } return true; }) : every0;
