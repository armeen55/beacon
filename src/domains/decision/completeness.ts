/** decision/completeness: THE ONE CHECK THAT ASKS WHETHER BEACON HAS FINISHED THE WORK. Beacon's analysis is for Beacon. A customer-facing Change states exactly what to add, replace, delete, move, link, redirect or create, exactly where, and the FINAL COPY wherever copy is involved. Anything short of that is an opportunity still being developed: it stays stored with all its evidence, out of the ranked queue, out of Today, out of measurement, and the server refuses to record it done. PURE and derived from the deliverable ITSELF, never from the prose around it, so the queue, the card (a client component) and the server mutation all ask one question and a voice edit moves none of them. It sits beside the contract rather than inside the validator because a client bundle may reach this and may not reach that. */
...
/** A STORED CAVEAT THAT IS A FACT ABOUT THE WORK rather than a note beside it: the cause its own evidence names is unsettled, the exact words are still owed, a source or a fact check is outstanding, or the row says out loud it is not pasteable. Deliberately the phrasings this kernel actually writes, so an honest "what could not be checked" line never blocks anything. */
