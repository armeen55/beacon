...
function copyIdentity(p: ChangeProposal): string {
  const parts = p.bundle?.components ?? [];
  return JSON.stringify([p.copyStamp ?? null, p.factIdentity ?? null, p.changeFamily, p.diagnosisCause ?? null, normIntent(p.primaryQuery),
...
function unretire(p: ChangeProposal | null | undefined): ChangeProposal | null | undefined {
  ...
  if (AEO_BAR.forRow(p) || AEO_BAR.collection(p.primaryQuery ?? "") || !/This draft names "[^"]+"/i.test(was.retiredBecause) || !/\b(?:entity|distinguishing(?: attribute)?|group)\b/i.test(was.retiredBecause) || !/^\s*\|.*\|\s*\r?\n\s*\|?(?:\s*:?-{3,}:?\s*\|)+\s*:?-{3,}:?\s*\|?\s*$/m.test(words)) return p;
...
function decideFinished(incoming0: ChangeProposal, prior00: ChangeProposal | null | undefined): ChangeProposal {
  ...
  const factMoved = !!prior && (prior.factIdentity ?? null) !== (incoming.factIdentity ?? null);
...
const COPY_OWNED = ["claims", "supportFacts", "operatorSteps", "bundle", "newPageDraft", "confirmedVersion", "approval", "faults", "modeledOn", "informationGain", "preservation", "draftNotes", "obligation", "assignment"] as const satisfies readonly (keyof ChangeProposal)[]; // `obligation` rides the banked words so a SETTLED verdict survives every later pass that reaches this row under the same identity; a pass that genuinely moves the identity wins whole and the ladder decides again from nothing
