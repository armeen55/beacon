...
export function openHold(p: ChangeProposal): { lane: "review" | "research"; why: string[]; blocking: string | null; faulted: boolean } {
  const gaps = deliverableGaps(p), c = p.recommendedChange, faults = p.limitations.filter((l) => GATE_WORDS.test(l));
  ...
  return { lane: p.researchOnly === true || gaps.some((g) => NOT_WRITTEN.test(g)) ? "research" : "review",
    ...
    why: said.length > 0 ? said : [UNJUDGED], blocking: hard[0] ?? null, faulted: faults.length > 0 };
}
