...
export const PROMPT_REGISTRY = {
  ...
  "draft.atomic_edit": 8, // v8: research candidates never become absence or factual authority through the operator brief. v7: reader-task-sized additions choose contextual or headed delivery; exact replacements keep their heading, and actual reader questions are allowed. v6 (2026-09-06): a gap read off the pages winning a search now says in the brief that it is a hypothesis about what this page could carry and never a fact, so a cached v5 answer was written under wording that named the same observation as the thing to state. v5 (2026-09-06): OPENING_ANSWER_CLAUSE no longer refuses a definition, a question, a list or a colon and refuses narration and unsupported claims instead, and what the page's own sections are to a body answer is settled by the assignment rather than by a universal clause. A v4 answer was written under wording that forbade shapes this one allows. v4 (2026-08-24): a meta carries META_SUBJECT_CLAUSE, which forbids the page's FAQ rail as its subject and requires it to open by naming the thing.
