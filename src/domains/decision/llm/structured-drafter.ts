...
function intentDirective(intent?: string): string {
  switch (intent) {
    ...
    case "examples": return "a self-contained direct answer, then supported examples with the detail, selection criteria and scope the reader needs";
    case "definition": return "the definition first, then the supported explanation, examples, distinctions and boundaries necessary to understand it";
    case "comparison": return "what each option is, then the supported differences, criteria and tradeoffs necessary for the reader to compare them";
    ...
    case "question": return "the direct answer first, then the supported specifics necessary to complete the reader task";
...
const ADAPTIVE_ANSWER_CLAUSE = " THIS EDIT ADDS COPY AND DELETES NOTHING. Choose an inline insertion with no outer heading only when the adjacent passage already establishes the question, subject and scope. Otherwise give this treatment its own descriptive heading or a question a reader genuinely asks. Use the winning-page observations to choose useful structure and depth, not their wording or unsupported claims. Supply the supported explanation, distinctions, examples, qualifications or ordered steps necessary to complete the diagnosed task; neither a sentence quota nor extra padding makes it complete.";
...
export async function draftAtomicEditStructured(
  ...
  const result = await callStructuredLLM({
    ...
    system: (ATOMIC_HEAD[input.field] ?? ATOMIC_HEAD.default!) + ATOMIC_EDIT_SYSTEM + (input.field === "answer_block" ? input.answerShape === "packet" ? AEO_BAR.policy : OPENING_ANSWER_CLAUSE + (input.answerShape === "inline" ? INLINE_ANSWER_CLAUSE : input.answerShape === "adaptive" ? ADAPTIVE_ANSWER_CLAUSE : SECTION_ANSWER_CLAUSE) : input.field === "meta" ? META_SUBJECT_CLAUSE : input.field === "title" || input.field === "h1" ? TITLE_SHAPE_CLAUSE : "") + fewShots,
    user,
    grounded,
...
export async function readComparison(comparison: JobComparison, owned: { url: string; passages: readonly string[] },
  ...
  const read = withObservations(comparison.owned ? { ...comparison, owned: { ...comparison.owned, heldWhole: comparison.owned.heldWhole && sanitizeNullableEvidence(comparison.owned.held) === comparison.owned.held } } : comparison, by);
