...
export async function callStructuredLLM<K extends StructuredDraftKind>(
  ...
  const promptId = req.promptId ?? `draft.${req.kind}` as PromptId;
...
export async function draftAtomicEditStructured(
  ...
  const protectedWords = currentValue && (input.field === "meta" || input.field === "title" || input.field === "h1") ? COPY_RULES.materialLosses({ recommendedChange: { kind: "existing_edit", field: input.field, before: currentValue, after: "" }, pageUrl: input.packet?.targetUrl ?? "", pagePath: "" }) : [];
  ...
  const user = [
    ...
    protectedWords.length ? `PROTECTED OLD WORDING: ${JSON.stringify(protectedWords)}. For EACH item absent from after, give its own preservation entry with text exactly as printed. An entry for the entire old field does not cover these items. Items still present in after need no removal entry. A replacement in this same field is not a separate body destination; use corrected with a source-supported delivered phrase, or removed with a supported reason. Never infer that an old claim is false merely because it is absent from the new copy.` : "",
