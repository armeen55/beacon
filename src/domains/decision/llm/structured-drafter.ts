...
const ATOMIC_EDIT_SYSTEM = COPY_RULES.publisher + " " +
  ...
  + 'Keep references concise. Each claim.text states one material assertion actually made in after; grounding IDs belong ONLY in supportedBy, not in claim.text or after. If a changed title, description or heading drops a named entity, figure or link from before, account for its exact original words in preservation with a supported correction or removal; do not invent a basis for dropping it. Put instructions, reasoning and omissions in their metadata fields, never in after. Research observations and draft context cannot support factual claims.';
