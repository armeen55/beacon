...
import { EditorAcceptanceSchema } from "../contracts";
...
const PageJobSchema = z.object({
  ...
  /** The title's promise and explicitly observed conversion actions. Neither proves whole-page coverage or absence. */
  promise: z.string().min(3).max(200), sells: z.array(z.string().min(2).max(60)).max(8),
});
