...
export type ProduceProposalsResult = {
  ...
  paid: { declared: readonly string[]; funded: readonly string[]; attemptUnitsSpent: number; declined?: readonly { key: string; family: string; reason: string }[];
    ...
    receipts: readonly { key: string; funded: boolean; family: string; treatment: string | null; impact: number; allowance: number; fallbacks: readonly string[];
      ...
      /** The STORE's own word for what happened to the row, or null where nothing was written. */ persistence: string | null; /** What settled or blocked it, in its own words, WHOLE. */ why?: string }[] }; }; /** Bounded drafting: the strongest few, never a queue. */ export const DEFAULT_MAX_DRAFTS = 5; const MAX_INVENTORY = 200; /** THE RULES TAKEN OUT ON 2026-08-23, in the words they wrote onto the rows they held. BOTH of them: the list member checked against the writer's own declared claims (which refused "with mammals of Iran" and "symbolizing royal authority" as things a page offers), and the member checked against the page's word order (which refused "wool rugs" on a page that prints it, because the token list it compared against was de-duplicated). Withdrawing a rule and not naming its receipt here strands every row it held, with an objection nothing stands behind and no way back but a full redraft. Matched whole, prefix included, so a note that merely ENDS this way is not mistaken for the gate's own line. A row carrying only these is held by nothing that still exists; every other hold outlives the sweep. */
const WITHDRAWN_HOLD = /^it tells a reader this page offers .*(?:and no claim on this card carries it|and this page never puts those words together)$/;
/** THE WRITER/EVIDENCE CONTRACT THIS ROW WAS DRAFTED UNDER. Part of work identity, so when the meaning of the evidence changes the rows drafted under the old meaning stop counting as the same work and are drafted again instead of standing for ever. Bump it whenever the packet's roles or the information-gain rule change. */
...
export async function produceProposalsForTenant(tenantId: string, opts: ProduceProposalsOptions = {}): Promise<ProduceProposalsResult> {
  ...
  const existing = await loadChangeProposals(tenantId).catch(() => new Map<string, ChangeProposal>()); let released = 0; // READ, so a dry run computes against the same priors a real save would
  ...
  const decline = new Map<string, NonNullable<ReturnType<Windows["get"]>>>(); for (const [url, w] of windows) for (const k of pageKeys(url)) decline.set(k, w); const compile = () => compileCandidates(snapshot, { coverage, ...measuring, decline, curve }), bound = Math.min(DEFAULT_MAX_DRAFTS, maxDrafts);
  ...
  const workKeyOf = (c: { pagePath?: string | null; pageUrl?: string | null; changeFamily?: string; treatment?: string | null; causeFinding?: { cause: string } | null; diagnosisCause?: string | null; primaryQuery?: string; kind?: string }): string =>
    [DRAFT_BUDGET.keyOf(c), WRITER_CONTRACT, basis ?? "no-basis", snapshot.evidenceHash ?? "", c.kind ?? "", c.changeFamily ?? "", c.treatment ?? "", (c.causeFinding?.cause ?? c.diagnosisCause) ?? "", canonicalQueryKey(c.primaryQuery ?? "")].join("::");
  ...
  const persisted_ = new Map<string, string>(), finalRow = new Map<string, ChangeProposal>(); // what the STORE answered for this page, as its own word, and the exact row it was answered about
  ...
  const persistIfChanged = async (input0: ChangeProposal): Promise<"saved" | "unchanged" | "refused" | "blocked" | "failed" | "not_persisted"> => {
    ...
    // THE EXACT ROW THE STORE WOULD RECEIVE, KEPT WHETHER OR NOT IT IS WRITTEN. Everything above this line is the same computation in both modes, so what a dry run hands back to be inspected is byte for byte what persistence would keep. The guard used to sit at the TOP of this function, so a dry run returned the PRE-persistence shape and the copy an operator approved was not the copy that later landed.
    finalRow.set(p.id, p); if (!persist) return "not_persisted"; const result = await saveChangeProposal(p);
  ...
  const rankAndStamp = async (rows: readonly ChangeProposal[]): Promise<ChangeProposal[]> => {
    const ranked = rankProposals(rows.map((p) => finalRow.get(p.id) ?? existing.get(p.id) ?? p).map(recovered).map(sized), { ...measuring, familyHistory }); // WHAT WAS PERSISTED IS WHAT GETS RANKED, or a card whose banked copy was kept above would rank as a brief the store no longer holds.
  ...
  if (persist) for (const row of [...existing.values()]) {
    ...
    if (contaminated && row.researchOnly !== true) {
    ...
    const gateLines = row.limitations.filter((l) => /^[a-z]/.test(l)), held = row.status === "needs_review" && gateLines.length > 0; if ((!held && row.status !== "ready") || row.researchOnly === true || row.bundle || row.recommendedChange.kind !== "existing_edit" || !row.recommendedChange.after.trim()) continue;
    ...
    const moved: ChangeProposal | null = why2.length > 0
      ...
      : held && gateLines.every((l) => WITHDRAWN_HOLD.test(l)) && !(unsettledCause(row) ?? openHold(row).blocking)
        ? { ...row, status: "ready", limitations: row.limitations.filter((l) => !WITHDRAWN_HOLD.test(l)) } : null;
