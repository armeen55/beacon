...
export type ProduceProposalsResult = {
  ...
  paid: { declared: readonly string[]; funded: readonly string[]; attemptUnitsSpent: number; declined?: readonly { key: string; family: string; reason: string }[];
    evidenceOwed?: readonly { key: string; kind: string; query: string; reason: string; resume: string }[]; // the exact readings funded candidates were refused for, typed
...
export async function produceProposalsForTenant(tenantId: string, opts: ProduceProposalsOptions = {}): Promise<ProduceProposalsResult> {
  ...
  const evidenceOwed = new Map<string, { kind: string; query: string; reason: string; resume: string }>(); // the exact reading a funded candidate was refused for, as data
  ...
  const stamp = (p: ChangeProposal): ChangeProposal => { const key = (p.pageUrl ?? "").trim().toLowerCase(), pathKey = (p.pagePath ?? "").trim().toLowerCase();
    ...
    return { ...p, ...(basis ? { basis } : {}), workKey: workKeyOf(p),
  ...
  for (const d of deep) {
    ...
    const wantKey = workKeyOf({ pageUrl: d.pageUrl, kind: "existing_edit", changeFamily: "bundle", diagnosisCause: keys.map((k) => judged.get(k)?.cause.cause).find(Boolean) ?? null, primaryQuery: d.evidence?.query ?? "" }), has = currentBundleFor((p) => p.kind === "existing_edit" && keys.includes((p.pagePath ?? "").trim().toLowerCase()) && p.workKey === wantKey && p.status === "ready" && p.researchOnly !== true);
  ...
  for (const d of deep) {
    ...
    // A BUNDLE THAT DID NOT LAND WRITES NOTHING OFF: its refusals are not typed apart yet, so the safe reading is that nobody could answer for that page today, and it is offered again rather than declared impossible. A MISSING READING IS A TYPED REQUIREMENT, NOT PROSE (Codex, 2026-08-23): "No results page for X is on file" was an English sentence the runtime tried to recognise with a regex, so the one thing that would finish this page was never fetched. It is DATA now (what evidence, for which search, which treatment resumes), and the page is closed to fallback drafting because a generic editor cannot treat a loss the deep door could not read.
    if (bundled.status !== "bundled" && /results page/i.test(bundled.reason ?? "")) {
      evidenceOwed.set(DRAFT_BUDGET.keyOf({ pageUrl: d.pageUrl }), { kind: "serp", query: /for "([^"]+)"/.exec(bundled.reason ?? "")?.[1] ?? d.evidence?.query ?? "", reason: bundled.reason, resume: "deep_bundle" });
      file(DRAFT_BUDGET.keyOf({ pageUrl: d.pageUrl }), "evidence_required", true, bundled.reason); }
