...
export async function produceProposalsForTenant(tenantId: string, opts: ProduceProposalsOptions = {}): Promise<ProduceProposalsResult> {
  ...
  const persistIfChanged = async (input0: ChangeProposal): Promise<"saved" | "unchanged" | "refused" | "blocked" | "failed" | "not_persisted"> => { if (!persist) return "not_persisted";
    ...
    const p: ChangeProposal = unfit ? { ...ranked, status: "needs_review", limitations: [...new Set([...ranked.limitations, unfit])] } : ranked; const result = await saveChangeProposal(p);
  ...
  // A CARD THAT ONLY SAYS THE PAGE BACK DOES NOT HOLD THAT PAGE'S SLOT. Finished work covers a page so the pass does not pay twice for it, but a body edit whose every claim rests on the page's own ids adds nothing a reader standing on that page could not already read, and letting it count as coverage is how "saved" became "valuable": the account's five Ready rows blocked every better draft of the same five pages. Descriptions and titles are unaffected, because summarising the page is what they are for.
