...
export async function produceProposalsForTenant(tenantId: string, opts: ProduceProposalsOptions = {}): Promise<ProduceProposalsResult> {
  ...
  const jobs: { key: string; family: string; impact: number; calls: number; fallbacks?: string[]; blocked?: string; treatment?: string; workKey?: string }[] = [], editorCards: ChangeProposal[] = [];
