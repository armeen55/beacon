/**
 * investigation/rank-causes (2026-07-02, master plan item 53) - PURE diagnosis
 * ranker. Given the four evidence findings collect-evidence.ts gathers for a
 * page-family collapse (indexability, cached SERP rank history, recent shipped
 * changes, sitewide algorithm weather), produce a ranked list of plain-English
 * causes with a confidence tier, and a headline sentence for the diagnosis card.
 *
 ...
 *
 * Ranking precedence (operator-locked, mirrors the indexability decision tree's
 * "structural break beats everything" posture):
 ...
 *   4. a sitewide algorithm-weather shock window overlapping the collapse
 *      date - "medium" confidence (site-wide, not page-specific, so it never
 *      outranks a page-specific structural break, but it outranks a stale or
 *      distant recent change).
 *   5. a SERP rank drop on the family's own top queries within the same
 *      window - "low" confidence alone (a lost ranking is as much a symptom
 *      of the collapse as a cause of it), but it corroborates causes 1-4 when
 *      they're also present.
 *   6. nothing found - "none" confidence, an honest single sentence, never a
...
export type RankedCause = {
  kind: "noindex" | "bad_status" | "robots_blocked" | "canonical_elsewhere" | "recent_change" | "algorithm_weather" | "serp_drop";
  confidence: "high" | "medium" | "low";
  /** Plain first-person sentence naming the cause. No dashes. */
  sentence: string;
  /** The one action this cause implies, if any (null for weather - nothing to
   *  fix on our own site). */
...
export type InvestigationDiagnosis = {
  familyLabel: string;
  collapseDate: string;
  /** The single top-line sentence for the card headline. */
  headline: string;
  /** Ranked causes, most likely first. Empty when nothing was found. */
  causes: RankedCause[];
  /** True when at least one cause was found; false means "no clear cause
   *  found", the honest default. */
  hasCause: boolean;
};
...
/** Rank the best algorithm-weather cause, or null when no shock window
 *  overlaps the collapse date. PURE. Confirmed updates rank ahead of
 *  suspected ones when both are present (handled by the caller passing the
 *  already-prioritized overlap). */
function causeForWeather(weather: ReadonlyArray<WeatherFinding>): RankedCause | null {
  if (weather.length === 0) return null;
  const confirmed = weather.find((w) => w.kind === "confirmed");
  const w = confirmed ?? weather[0]!;
  const when = formatDate(w.start);
  const label = w.kind === "confirmed" ? w.label : "a site-wide Google shift I detected the same week";
  return {
    kind: "algorithm_weather",
    confidence: "medium",
    sentence: `Around ${when}, ${label} moved the whole site, so this may not be specific to this page.`,
    actionSentence: null,
  };
}
...
export function rankCauses(findings: InvestigationFindings): InvestigationDiagnosis {
  ...
  // Recent changes and weather are family-level (one entry each, the single
  // best explanation of each type), ranked below any structural break found.
  const changeCause = causeForRecentChanges(findings.recentChanges);
  if (changeCause) causes.push(changeCause);
  const weatherCause = causeForWeather(findings.weather);
  if (weatherCause) causes.push(weatherCause);
  ...
  if (ranked.length === 0) {
    return {
      ...
      headline: `The ${findings.familyLabel} family ${dropPhrase} starting ${when}. I checked the page, the crawlers, recent changes, and site-wide shifts, and found no clear cause. I would look at this one by hand.`,
      causes: [],
      hasCause: false,
    };
  }
  ...
  const top = ranked[0]!;
  const headline = `The ${findings.familyLabel} family ${dropPhrase} starting ${when}. Most likely cause: ${lowerFirst(top.sentence)}`;
