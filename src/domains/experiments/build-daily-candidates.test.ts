...
describe("buildDailyCandidates — item 29 family win propagation (wiring pin)", () => {
  const wonMeta = (path: string): ShippedChangeRecord => ({
    id: `${path}::2026-06-01`,
    ...
    controlPages: [],
    windows: [
      { day: 7, ran: true, controlsUsed: 3 } as ShippedChangeRecord["windows"][number],
      { day: 14, ran: true, controlsUsed: 3 } as ShippedChangeRecord["windows"][number],
      { day: 28, ran: true, controlsUsed: 3 } as ShippedChangeRecord["windows"][number],
    ],
    verdict: "won",
    confidence: "high",
  ...
  // Missing/weak meta -> the deterministic meta proposer fires for every sibling (same lever
  // family as the win). Each page carries a real opening paragraph so proposeSafeMeta succeeds.
  const missingMetaFacts = (query: string): PageFacts => ({
    title: `${query} guide`, meta: null, h1: query,
    openingParagraph: `${query} is a real, documented subject with distinct, well known traits worth reading about.`,
  });

  it("proposes the SAME lever (meta) on untreated same-family siblings, tags provenance, and boosts", () => {
    const win = wonMeta("/iran-animals/persian-cheetah");
    const pages = [
    ...
    for (const c of out) {
      expect(c.leverField).toBe("meta");
      expect(c.familyWin).toBeDefined();
      expect(c.familyWin?.sourceWinPage).toBe("/iran-animals/persian-cheetah");
      expect(c.familyWin?.lever).toBe("meta");
      expect(c.whyNow).toContain("already won on the persian cheetah page");
  ...
  it("does NOT tag a sibling whose OWN lever differs from the family win (never a mismatched claim)", () => {
    const win = wonMeta("/iran-animals/persian-cheetah");
    // Meta already present + query-first title + H1 carries the query -> no meta lever fires here,
    // this page should get NO candidate at all (nothing materially better), not a mistagged one.
    const pages = [gsc({ url: "/iran-animals/red-fox", topQuery: "red fox" })];
    const f = facts({
      "/iran-animals/red-fox": {
        title: "Red Fox (Vulpes vulpes) - Persian Wildlife Guide",
        meta: "The red fox is a common wild canid found across Iran's diverse habitats and terrain types.",
        h1: "Red Fox",
      },
    });
    const out = buildDailyCandidates({ tenantId: "t", pages, facts: f, proofLedger: [win], now: NOW });
  ...
  it("respects the eligibility gate: a sibling mid-measurement on the SAME family never gets propagated", () => {
    const win = wonMeta("/iran-animals/persian-cheetah");
    const measuring: ShippedChangeRecord = {
      ...win,
      id: "/iran-animals/persian-leopard::2026-06-28",
      page: "https://iranopedia.com/iran-animals/persian-leopard",
      path: "/iran-animals/persian-leopard",
      shippedAt: "2026-06-28T00:00:00.000Z", // recent -> still measuring at NOW
      verdict: "measuring",
      windows: [],
    };
    const pages = [gsc({ url: "/iran-animals/persian-leopard", topQuery: "persian leopard", impressions: 9000 })];
  ...
  it("is BOUNDED to at most 3 family-win tags per batch, best demand first", () => {
    const win = wonMeta("/iran-animals/persian-cheetah");
    const pages = Array.from({ length: 6 }, (_, i) =>
      gsc({ url: `/iran-animals/animal-${i}`, topQuery: `animal ${i}`, impressions: 100 * (i + 1) }),
    );
    const f = facts(Object.fromEntries(pages.map((p) => [p.url, missingMetaFacts(p.topQuery)])));
    const out = buildDailyCandidates({ tenantId: "t", pages, facts: f, proofLedger: [win], now: NOW });

    expect(out).toHaveLength(6); // every sibling still gets its own honest meta candidate
    const tagged = out.filter((c) => c.familyWin);
    expect(tagged).toHaveLength(3); // but only the top 3 by demand carry the propagation tag
    // buildDailyCandidates preserves the caller's page order (not the propagation ranking), so
    // assert the SET of tagged urls is the top 3 by impressions, regardless of output order.
    expect(new Set(tagged.map((c) => c.url))).toEqual(new Set([
      "/iran-animals/animal-5",
      "/iran-animals/animal-4",
      "/iran-animals/animal-3",
    ]));
  });

  it("no mature win in the ledger -> no candidate is ever tagged (honest silence)", () => {
    const measuring: ShippedChangeRecord = { ...wonMeta("/iran-animals/persian-cheetah"), verdict: "measuring", windows: [] };
    const pages = [gsc({ url: "/iran-animals/persian-leopard", topQuery: "persian leopard" })];
    const f = facts({ "/iran-animals/persian-leopard": missingMetaFacts("persian leopard") });
    const out = buildDailyCandidates({ tenantId: "t", pages, facts: f, proofLedger: [measuring], now: NOW });
    expect(out[0].familyWin).toBeUndefined();
  });
});
