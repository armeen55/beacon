...
describe("Page Surgeon trust gate — create_new_page safety (/persian-male-names class)", () => {
  // The unserved-cluster signal was sourced only from SEMrush keyword rows
  // (removed Phase F.1). With no first-party equivalent, highValueUnservedCluster
  // is always false, so create_new_page is never eligible.
...
describe("Page Surgeon trust gate — P2 evidence-citation sanitizer", () => {
  it("rejects a change citing question keywords when no keyword source is wired (/farsi-numbers class)", () => {
    const p = packet({
      gsc: gsc({ ctrGap: 0.03 }),
      sourcesPresent: ["gsc", "crawl"], sourcesConnectedButEmpty: ["ga4", "clarity", "profound"],
    });
    const faq = change("faq", { evidence: "Question keywords show users ask how to count in Farsi." });
  ...
  it("does NOT reject a change that HONESTLY acknowledges the absent field", () => {
    ...
    const ib = change("intro_answer_block", { evidence: "No question keywords were returned, so this is grounded in the GSC ctrGap of 0.03." });
...
describe("Page Surgeon trust gate — WL2 per-claim absent-evidence", () => {
  it("one honest absent-source caveat does NOT whitewash a different fabricated citation", () => {
    // GA4 honestly absent in one clause; question keywords fabricated in another.
    ...
    const faq = change("faq", {
      evidence: "No GA4 rows were returned for this page. Question keywords show users ask how to count in Farsi.",
    });
    const out = applyDeterministicGate(decision(change("title"), [faq]), p);
    expect(out.supporting_atomic_changes.some((c) => c.action === "faq")).toBe(false);
    expect(out.rejected_changes.some((r) => r.action === "faq" && /question keyword/i.test(r.reason))).toBe(true);
