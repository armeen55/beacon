 ...
 * is attached to a recommendation on the render path (load-queue.ts decorates
 * it). Microsoft Clarity friction rates and AI-assistant answer counts still
 * live only in the candidate `evidence` array at generation time and are NOT
 * carried onto the rendered row — enriching those remains a follow-up that
 * first needs those signals threaded onto `LiveRecQueueItem`.
 */
...
//
// HONESTY RAILS (same as the GSC helper): we quote ONLY the
