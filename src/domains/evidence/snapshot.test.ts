 ...
 * downstream Decisions consume - not the implementation. They prove: all six
 * mandatory sources normalize in with an honest freshness slot; existing-page
 * and new-page evidence are both present; the assembler is deterministic; native
 * AI can fail soft; and the evidenceHash tracks MATERIAL evidence (including the
 * research funnel's keywords / AI observations / SERP / winning pages) while
 * ignoring every timestamp and spend/cache counter.
 */
...
function fullInput(): EvidenceSnapshotInput {
  return {
    ...
    gsc: { status: "fresh", lastSyncedAt: "2026-07-21T00:00:00.000Z", payload: [
      { url: "https://fixture-content.example/flag", clicks90d: 40, impressions90d: 4000, ctr90d: 0.01, position90d: 8.2, topQueries: [
      ...
      { url: "https://fixture-content.example/flag-history", clicks90d: 5, impressions90d: 900, ctr90d: 0.005, position90d: 14, topQueries: [
        { query: "iran flag meaning", impressions: 900, clicks: 5, position: 14 }] }] },
    ...
    dataforseo: { status: "fresh", lastSyncedAt: "2026-07-19T00:00:00.000Z", payload: [
    ...
    nativeAi: { status: "fresh", lastSyncedAt: "2026-07-21T00:00:00.000Z", payload: {
...
describe("buildEvidenceSnapshot - six-source normalization", () => {
  it("normalizes all six sources and joins owned evidence by canonical URL", () => {
    ...
    expect(meaning.searchVolume).toBe(5400); // DataForSEO volume
...
describe("evidenceHash - research material truth, not the clock", () => {
  function researchFixture(): FunnelResearchEvidence {
    return {
      retainedKeywords: [{ query: "koobideh recipe", searchVolume: 1200, competition: 0.3, competitionLevel: "low", intent: "informational" }],
      aiObservations: [{ promptId: "p1", promptText: "best koobideh recipe", engine: "chatgpt", modelRequested: "gpt-4o", modelServed: "gpt-4o-2026", webSearchReported: true, citationsObserved: true, citations: [{ url: "https://persianfood.example/kebab", domain: "persianfood.example", title: "Kebab" }], fanOutQueries: ["koobideh", "kebab koobideh"], observedAt: "2026-07-22T00:00:00.000Z" }],
  ...
  it("is stable when only clocks and spend/cache counters change", () => {
    ...
    const clockOnly = researchFixture();
  ...
  it("changes for a citation URL, served model, fan-out list, keyword volume/intent, or extract structure", () => {
    const base = hashOf(researchFixture());
