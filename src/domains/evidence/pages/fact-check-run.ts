...
function sectionExcerpts(sections: readonly SourceSection[], groups: readonly string[]): { heading: string; says: string }[] {
  ...
  const ranked = named.map((x, i) => ({ x, i, r: rank(x) })).sort((a, b) => b.r - a.r || a.i - b.i), carrying = ranked.filter((y) => y.r > 0); return (carrying.length >= 2 ? carrying : ranked).slice(0, 5).map(({ x }) => { const opening = x.text.replace(/\s+/g, " ").trim().slice(0, 360), end = Math.max(opening.lastIndexOf(". "), opening.lastIndexOf("! "), opening.lastIndexOf("? "));
    return { heading: x.heading.trim(), says: end > 120 ? opening.slice(0, end + 1) : opening }; }); } function sectionDigest(sections: readonly SourceSection[], max = 6_000): string { return sections.filter((x) => !!x.heading?.trim() && !FURNITURE_LABEL.test(x.heading) && x.text.trim()).map((x) => `${x.heading!.trim()}\n${x.text.replace(/\s+/g, " ").trim().slice(0, 420)}`).join("\n\n").slice(0, max); } // PURE. THE SOURCE LAID OUT AS ITS OWN SECTIONS: each heading on a line above the opening of its words, the apparatus left out, inside the same six thousand characters a window gets
