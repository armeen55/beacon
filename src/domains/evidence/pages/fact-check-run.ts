...
const JUDGE_SYSTEM = 'You compare ONE statement a web page makes against PASSAGES QUOTED FROM SOURCES THAT WERE ACTUALLY FETCHED. '
  ...
  + '`groups` on each supporting source lists only named groups of the page subject that this source distinguishes with qualifying criteria. Use [] for a mere roster or fauna/flora lede on an animal page. Copy names verbatim; quote a consecutive window containing BOTH the proposal and all group names, including adjacent sentences. When a passage is the source laid out as its own sections, each heading on a line above its words, a heading is a group when the words under it say what qualifies for it: name it exactly as the heading reads, quote one sentence from under it, and name the headings that qualify, never a heading that covers history or geography or a subject other than the page subject, and never a count for its own sake. '
...
function sectionExcerpts(sections: readonly SourceSection[], groups: readonly string[]): { heading: string; says: string }[] {
  ...
  const rank = (x: SourceSection & { heading: string }): number => { const h = words(x.heading), body = x.text.toLowerCase(), b = words(body); return (asked.some((a) => a.w.size > 0 && [...a.w].every((w) => h.has(w) || stem(h, w))) ? 4 : 0) + (asked.some((a) => body.includes(a.g)) ? 2 : 0) + [...new Set(asked.flatMap((a) => [...a.w]))].filter((w) => h.has(w) || stem(b, w)).length; }; // a section whose heading IS the group outranks a lede that merely names it // the count of distinct group words a section carries, whole phrases counting double, so a survey that merely says "species" never outranks the section naming the mammals and the birds
  const ranked = named.map((x, i) => ({ x, i, r: rank(x) })).sort((a, b) => b.r - a.r || a.i - b.i), carrying = ranked.filter((y) => y.r > 0); return (carrying.length >= 2 ? carrying : ranked).slice(0, 5).map(({ x }) => { const opening = x.text.replace(/\s+/g, " ").trim().slice(0, 360), end = Math.max(opening.lastIndexOf(". "), opening.lastIndexOf("! "), opening.lastIndexOf("? "));
    return { heading: x.heading.trim(), says: end > 120 ? opening.slice(0, end + 1) : opening }; }); } function sectionDigest(sections: readonly SourceSection[], max = 6_000): string { const open = (t: string): string => { const cut = t.replace(/\s+/g, " ").trim().slice(0, 420), end = Math.max(cut.lastIndexOf(". "), cut.lastIndexOf("! "), cut.lastIndexOf("? ")); return t.replace(/\s+/g, " ").trim().length <= 420 ? t.replace(/\s+/g, " ").trim() : end > 120 ? cut.slice(0, end + 1) : cut; }; return sections.filter((x) => !!x.heading?.trim() && !FURNITURE_LABEL.test(x.heading) && x.text.trim()).map((x) => `${x.heading!.trim()}\n${open(x.text)}`).join("\n\n").slice(0, max); } // PURE. THE SOURCE LAID OUT AS ITS OWN SECTIONS: each heading on a line above the opening of its words, the apparatus left out, inside the same six thousand characters a window gets
