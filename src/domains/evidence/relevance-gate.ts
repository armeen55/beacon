 ...
 *  FURNITURE TOO (production 03:01Z, 2026-09-06): "Gallery", "Notes" and "References" reached a change as three of the eight things a page was missing, and a gallery, a note list, a reference list, a bibliography and a footnote block are what a reference work prints AROUND its content in every vertical, so none of them is a section another page owes a reader. Whole labels still, so "References to the 1979 flag" is untouched. AND "CONTACT" IS A COMMON NOUN BEFORE IT IS A NAVIGATION WORD (journey review, 2026-09-07): the label rule reads up to four words after it, which suppressed the account's own "Contact <site>" chrome as intended but would take a heading about contact lenses, contact sports or contact tracing with it, so those readings are excepted by name. AND A ROSTER OF OTHER LISTS IS THE SITE'S OWN SHELF (production receipts, 2026-09-10): "Trending Baby Names Lists" and "Related Baby Names Lists" are thebump.com's franchise navigation printed on every names page, and each became the missing subject of a paid source need on a Persian names page; a heading that opens with a browse word and ends in "list(s)" is furniture, while "Trending baby names" without the shelf word stays a real subject. */
export const FURNITURE_LABEL =
  /^\s*(?:explore more|related(?: (?:articles?|posts?|pages?|topics?))?|you may also like|more(?: (?:from|like|articles?))?|read more|learn more|see also|gallery|galleries|notes?|references?|external links?|further reading|bibliograph(?:y|ies)|citations?|sources?|footnotes?|share(?: this)?|(?:sub)?categor(?:y|ies)|(?:media|pages|files) in categor(?:y|ies)|tags?|menu|navigation(?: menu)?|footer|header|sidebar|newsletter|subscribe|follow(?: us)?|contact(?:\s+(?!lens|lenses|sports?|tracing|dermatitis|angles?)\S+){0,4}|get in touch|sign ?up|log ?in|book now|shop(?: now)?|table of contents|contents|on this page|in this article|jump to|quick links|home|search|contact(?: us)?|about(?: us)?|comments?|advertisements?|privacy(?: policy)?|terms(?: of (?:use|service))?|(?:related|trending|popular|browse|more|other|similar|featured)(?:\s+\S+){0,4}\s+lists?|(?:score|get|grab|claim|unlock|shop|save|redeem)\b[^,]{0,60}\b(?:deals?|offers?|discounts?|coupons?|savings|newsletters?|products?|prices?|sales?|registry|gifts?)\b[^,]{0,40})\s*$/i;
