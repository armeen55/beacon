...
/** ONE table, because "current" used to mean seven days for a results page, a winner's body and a page of
 *  the account's own alike, and those three things move at completely different speeds. Evidence writes
 *  against this table and Decision's coverage gates read against it, so a look one side calls current is
 *  never the look the other side calls due. AI ANSWERS ARE NOT IN IT: Runtime's daily planner is the one
 *  freshness authority there, because it reads the stored observations. */
type FreshnessKind =
...
/** How long an observation of this kind stays current. */
