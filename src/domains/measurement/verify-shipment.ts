...
function classify(stored: VerifiableShipment["components"][number], live: LiveRead): ReturnType<typeof judged> {
  ...
  // A structured claim requires observed structure. Its words elsewhere on the page never prove placement.
  if (BODY_KINDS.has(component.kind) || component.units && /^(internal_links|internal_link_add)$/.test(component.kind)) return component.units?.length && component.target
    ? live.blocks.length ? publicationMatch(component, live) : judged("unverifiable", "The saved page read has no observable heading, list or table structure, so this placement cannot be confirmed.", "rendered_content_gap")
    : wordingMatch(component, live, headings);
