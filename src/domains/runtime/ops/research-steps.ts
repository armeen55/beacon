...
export const defaultSteps: ResearchCycleSteps = {
  async replenishReady(tenantId, now, seen, stopBy) {
    ...
    for (const r of out.paid.receipts) { const key = r.workKey; if (!key) continue; // work that declares no identity is remembered by nothing, exactly as the plan reads it
      const settled = r.outcome === "produced" || r.outcome === "evidence_banked" || r.outcome === "deterministic_refusal" || r.outcome === "already_complete"; if (!settled && (r.providerCalls ?? 0) === 0) { const m = jobs[key]; if (!m?.settled && (r.outcome === "not_reached" || r.outcome === "cost_blocked")) jobs[key] = { calls: m?.calls ?? 0, last: r.outcome, settled: false, waited: (m?.waited ?? 0) + 1 }; continue; } // A CAUSAL BLOCK IS NOT A VERDICT (incident, 2026-09-04): money, a provider that would not answer and a job never begun all leave the work owed, and only a finished row or a gate that read it settles it. THE TIMES IT WAS NEVER REACHED ARE COUNTED THOUGH, because `waiting` puts that work at the head of the next walk and nothing else could ever take the head away from it; a settled entry is never walked back by one.
