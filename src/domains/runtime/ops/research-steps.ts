  ...
  async acquireEvidence(tenantId, need, basis, budgetMs, review, deliveryScope = "all_changes", runReceipt) {
    ...
    switch (need.kind) {
      ...
      case "page_source": {
        ...
        const current = body?.version === "current" && sameFinal(body.finalUrl, need.url) && !!body.contentHash && isCurrent("owned_page", body.fetchedAt, Date.now());
        ...
        return { acquired, ...("attempted" in out && out.attempted === false ? { attempted: false as const } : {}), ...(code === "unchanged_incomplete" ? { preflight: code } : {}), detail: `own-page read of ${need.url}: ${acquired ? "current complete capture on file" : `current complete capture still owed (${code ?? unitStatus(out)})`}` };
      }
      case "factual_source": {
        if (need.reasonCode === "causal_answer_source_unconfirmed") { if (!need.proposalId || !need.ownerVersion || !need.url) return deferred("this factual reading has no exact proposal, page or owner version"); const { loadChangeProposal } = await import("@/domains/decision/proposal-store"), { loadOwnedPageBodies } = await import("@/domains/evidence/pages/owned-context"); const row = await loadChangeProposal(tenantId, need.proposalId, { canonicalOnly: true }).catch(() => null), owed = row?.obligation?.kind === "evidence" ? row.obligation.need : null, body = need.url ? (await loadOwnedPageBodies(tenantId, [need.url]).catch(() => null))?.get(canonicalUrlKey(need.url)) : null; if (!row || row.status !== "needs_review" || row.tenantId !== tenantId || row.basis !== basis || row.workKey !== need.workKey || !(row.pageUrl ? canonicalUrlKey(row.pageUrl) === canonicalUrlKey(need.url) : (row.pagePath ?? "") === pathOf(need.url)) || owed?.kind !== "factual_source" || owed.query !== need.query || owed.missingTopic !== need.missingTopic || owed.ownerVersion !== need.ownerVersion || owed.reasonCode !== need.reasonCode || !body || body.version !== "current" || body.completeness !== "complete" || body.sourceCapture?.complete !== true || !body.contentHash || body.contentHash !== need.ownerVersion || !isCurrent("owned_page", body.fetchedAt, Date.now()) || body.finalUrl && canonicalUrlKey(body.finalUrl) !== canonicalUrlKey(need.url ?? "")) return deferred("the exact proposal, source and evidence basis no longer owe this factual reading"); }
