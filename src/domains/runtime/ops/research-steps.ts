...
export const defaultSteps: ResearchCycleSteps = {
  async replenishReady(tenantId, now, seen, stopBy) {
    ...
    if (await creditBreakerHeld(tenantId).catch(() => true)) {
      ...
      return mark("retryable_blocked", before, 0, seen?.fingerprint ?? `${stamp}::held`, held, undefined, undefined, undefined, undefined, seen?.settled ? [...seen.settled] : undefined);
    }
    ...
    if (out == null || out.outcome === "evidence_unreadable" || out.outcome === "persistence_failed") return mark("retryable_blocked", before, 0, seen?.fingerprint ?? `${stamp}::unread`, held, undefined, undefined, undefined, undefined, seen?.settled ? [...seen.settled] : undefined);
    ...
    if (after == null) return mark("retryable_blocked", before, persisted, fingerprint, fresh, undefined, undefined, undefined, undefined, seen?.settled ? [...seen.settled] : undefined);
