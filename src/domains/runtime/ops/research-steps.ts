...
export type ResearchCycleSteps = {
  ...
  replenishReady: (tenantId: string, now: Date, seen?: { fingerprint: string | null; attempted: readonly string[]; tried?: readonly string[]; spent?: Readonly<Record<string, number>>; settled?: readonly string[] },
    ...
    /** JOBS ONE OF BEACON'S OWN GATES SETTLED TODAY (`key::family`), carried across manifests within the day: a deterministic refusal against today's evidence is not bought again on the next drive because a banked reading moved the fingerprint (live 2026-09-02: five refused body keys re-took five of eight funded slots on every cycle). */ settled?: string[];
...
export const defaultSteps: ResearchCycleSteps = {
  async replenishReady(tenantId, now, seen, stopBy) {
    ...
    const held = seen?.fingerprint != null && seen.fingerprint.startsWith(`${stamp}::`) ? [...seen.attempted] : []; /* the day's spend memory survives exactly as long as the manifest it was counted against */ const priorSpent: Record<string, number> = { ...(seen?.spent ?? {}) }; /* SPEND MEMORY IS THE DAY'S, NOT THE MANIFEST'S (operator, 2026-09-02): scoped to the fingerprint it reset on every banked reading, and the san-diego description was bought a third time to fail the same superlative firewall */ const floor = await readyStockFloor(tenantId).catch(() => READY_STOCK_ALARM); // the low-stock alarm level: it colors the receipt and nothing else
    const mark = (reason: "made_progress" | "retryable_blocked" | "candidates_exhausted", ready: number, persisted: number, fingerprint: string, attempted: string[],
      outcomes?: { readySaved: number; evidenceBanked: number; refused: number; blocked: number; unreached: number; stuck: string[] }, tried?: string[], evidenceOwed?: readonly OwedReading[], spent?: Record<string, number>, settled?: string[]) =>
      ({ ready, deficit: Math.max(0, floor - ready), persisted, satisfied: reason === "candidates_exhausted", reason, fingerprint, attempted, ...(tried && tried.length > 0 ? { tried } : {}), ...(settled && settled.length > 0 ? { settled } : {}), ...(evidenceOwed && evidenceOwed.length > 0 ? { evidenceOwed } : {}), ...(outcomes ? { outcomes } : {}), ...(spent && Object.keys(spent).length > 0 ? { spent } : {}) });
    ...
    if (await creditBreakerHeld(tenantId).catch(() => true)) {
      ...
      return mark("retryable_blocked", before, 0, seen?.fingerprint ?? `${stamp}::held`, held, undefined, seen?.tried ? [...seen.tried] : undefined, undefined, seen?.spent ? { ...seen.spent } : undefined, seen?.settled ? [...seen.settled] : undefined);
    }
    ...
    const settledToday = seen?.settled ?? []; // settled carries within the day whatever the fingerprint, exactly as `tried` does
    const out = await d.produceProposalsForTenant(tenantId, { now, produce: true, skipKeys: [...new Set([...held, ...settledToday])], retryKeys: seen?.tried ?? [], spentKeys: priorSpent, bypassCache: true, ...(stopBy != null ? { stopBy } : {}) }).catch(() => null);
    ...
    if (out == null || out.outcome === "evidence_unreadable" || out.outcome === "persistence_failed") return mark("retryable_blocked", before, 0, seen?.fingerprint ?? `${stamp}::unread`, held, undefined, seen?.tried ? [...seen.tried] : undefined, undefined, seen?.spent ? { ...seen.spent } : undefined, seen?.settled ? [...seen.settled] : undefined);
    ...
    if (after == null) return mark("retryable_blocked", before, persisted, fingerprint, fresh, undefined, seen?.tried ? [...seen.tried] : undefined, undefined, seen?.spent ? { ...seen.spent } : undefined, seen?.settled ? [...seen.settled] : undefined);
    ...
    const attempted = [...new Set([...fresh, ...settledRows.map((r) => `${r.key}::${r.family}`)])], settledCarry = [...new Set([...settledToday, ...settledRows.filter((r) => r.outcome === "deterministic_refusal").map((r) => `${r.key}::${r.family}`)])];
    ...
    if (after > before) return mark("made_progress", after, persisted, fingerprint, attempted, tally, tried, out.paid.evidenceOwed ?? [], spent, settledCarry);
    ...
    return mark(exhausted ? "candidates_exhausted" : "retryable_blocked", after, persisted, fingerprint, attempted, tally, tried, out.paid.evidenceOwed ?? [], spent, settledCarry);
  },
...
async function factCheckPass(tenantId: string, budgetMs: number, renew: (() => Promise<boolean>) | undefined, firstPage: string | null): Promise<{ status: "advanced" | "done" | "failed"; banked: number; pagesComplete: number; failure?: string; reason?: string }> {
    ...
    try {
      ...
      const gap = (h: (typeof held)[number]): boolean => h.current.trim() === "" && !!h.proposed?.trim(), stale = (h: (typeof held)[number]): boolean => h.current.trim() === "" && (h.rulesVersion !== facts.rulesVersionFor(h) || (h.confidence !== "confirmed" && !!h.proposed?.trim() && h.sources.length > 0 && h.sources.every((s) => s.says.trim() === ""))); // a row whose every source banked an empty quote lost its answer to the verbatim test and is worth one more unit now that the bank searches the passage the judge read; the backfill asks the fresh row again and its note is what makes it once
