...
export async function dueWork(tenantId: string, now: Date = new Date(), deps: DueWorkDeps = {}): Promise<DueWork> {
  ...
  const readable = run.ok && checks.value != null && sources.ok && basis.ok && basis.value != null && version.ok && surface.ok && debt.ok && pages.ok && unread.ok && analyses.ok && consumed.ok && facts.ok && ready.ok && creditHeld.ok && searchHeld.ok && winners.ok;
