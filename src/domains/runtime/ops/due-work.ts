...
export async function dueWork(tenantId: string, now: Date = new Date(), deps: DueWorkDeps = {}): Promise<DueWork> {
  ...
  const [ready, creditHeld, searchHeld] = await Promise.all([
    ...
    // `probe_due` is still an unverified provider, never permission to spend OpenAI. The runtime's free account
    // preflight turns it into clear or held before this read; if that preflight cannot answer, due-work fails closed.
    settled((deps.searchHeld ?? (async (t: string) => (await (await import("@/lib/cost/credit-breaker")).CREDIT_BREAKER.peek(t, {}, "dataforseo")) !== "clear"))(tenantId), false),
  ]);
  ...
  const readable = run.ok && checks.value != null && sources.ok && basis.ok && basis.value != null && version.ok && surface.ok && debt.ok && pages.ok && unread.ok && analyses.ok && consumed.ok && facts.ok && facts.value != null && ready.ok && creditHeld.ok && searchHeld.ok && winners.ok;
