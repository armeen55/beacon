...
import { confirmedVersion, deliverableGaps } from "@/domains/decision/completeness"; import { DRAFT_BUDGET } from "@/domains/decision/draft-budget"; import { nextObligation } from "@/domains/decision/obligation"; import { answerReviewedProposal, loadChangeProposal } from "@/domains/decision/proposal-store";
...
async function run(input: Input, deps: Deps = DEPS) {
  ...
  const reviewed = await PROOF_SPEND.run(tenantId, maxOpenAiCalls, maxOpenAiUsd,
    () => deps.review(row, { tenantId, now: input.now ?? new Date(), bypassCache: true, attempts, proposalWorkKey: row.workKey }));
  ...
  const promoted = await deps.promote(tenantId, proposalId, deps.version(row), row.basis ?? null, { kind: "promote", at: (input.now ?? new Date()).toISOString() }, proposed), stored = await deps.load(tenantId, proposalId);
