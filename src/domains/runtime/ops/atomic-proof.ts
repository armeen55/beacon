...
const DEPS = { permission: researchPermission, load: loadChangeProposal, review: reviewFinishedCopy, save: saveChangeProposal,
...
async function run(input: Input, deps: Deps = DEPS) {
  ...
  const promoted = { ...proposed, status: "ready" as const }, candidate = deps.acceptable(promoted) ? promoted : proposed;
