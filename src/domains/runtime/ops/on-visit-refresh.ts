...
async function driveRun(run: ResearchRun, ownerToken: string, nowFn: () => Date, deadline: number, steps: ResearchCycleSteps, work: DueWork): Promise<DriveReceipt> {
  ...
  const REPLENISH_MIN_MS = 45_000, REPLENISH_RESERVE_MS = 60_000, REPLENISH_BOX_MS = 240_000, LEASE_REPROVE_AFTER_MS = 1_000, STOP_STARTING_MS = 40_000, OWED_PER_DRIVE = 8, REPLENISH_BEGINS_MS = 110_000;
  ...
  type Outcomes = NonNullable<ResearchRunProgress["replenish"]>["outcomes"]; const calls = (o: Outcomes): number => ((o?.receipts ?? []) as { providerCalls?: number }[]).reduce((n, r) => n + (r.providerCalls ?? 0), 0); let ownOutcomes: Outcomes; const keepOutcomes = (next: Outcomes): Outcomes => { const kept = next && (calls(next) > 0 || !ownOutcomes) ? { ...next, pass: run.cycle_key } : ownOutcomes; ownOutcomes = kept; return kept; };
  ...
  const walkNow = async (asked: number): Promise<{ v: Awaited<ReturnType<ResearchCycleSteps["replenishReady"]>> } | null> => { const from = nowFn().getTime(),
    ...
    return Promise.race([steps.replenishReady(tenantId, nowFn(), { jobs: memory(), callsSpent: spentCalls, waiting: waitingKeys(), shared, noRoom: box <= REPLENISH_BEGINS_MS }, from + box - STOP_STARTING_MS).then((v) => ({ v })).catch(() => ({ v: null })), /* AND A BOX THAT CAN BEGIN NOTHING FUNDS NOTHING, AND SAYS SO: the free half still runs, and the manifest is declared and declined with the drive's own arithmetic instead of being funded and filed thirty-nine times as "the time box ended before this page's turn", which reads as a walk that got part way and is a walk that never started. */
