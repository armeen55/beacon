...
async function driveRun(run: ResearchRun, ownerToken: string, nowFn: () => Date, deadline: number, steps: ResearchCycleSteps, work: DueWork): Promise<DriveReceipt> {
  ...
  const record = async (r: NonNullable<Awaited<ReturnType<ResearchCycleSteps["replenishReady"]>>>): Promise<void> => { spentCalls += calls(r.outcomes); walked += 1; progress = { ...progress, replenish: { day, jobs: r.jobs, ...(r.reason === "candidates_exhausted" ? { closed: r.reason, ...(r.closedUnder ? { closedUnder: r.closedUnder } : {}) } : {}), ...((o) => o ? { outcomes: o } : {})(keepOutcomes(r.outcomes)) } }; if (r.persisted > 0) await import("@/app/(shell)/surface-release").then((m) => m.invalidateCoreSurfaces(tenantId)).catch(() => {}); }; // WHAT A WALK LEARNED, ONTO THE ROW: it always REPLACES what stands there, so a walk that served an awakened list clears it, and a day closes only on the exhaustion a walk actually earned // AND A STORE TRANSITION IS ON THE CUSTOMER'S SCREEN BEFORE THE LAST PHASE (live 06:45Z 2026-09-04): five cards the store had demoted still rendered Ready for seven minutes, because the release is rebuilt only in publish_surface and a run stuck earlier never reaches it. A walk that promoted or demoted anything age-stamps the saved release here, so the next visit republishes stored truth at $0 through the rebuild it already runs, and a walk that wrote nothing costs nothing.
