...
async function driveRun(run: ResearchRun, ownerToken: string, nowFn: () => Date, deadline: number, steps: ResearchCycleSteps, work: DueWork): Promise<DriveReceipt> {
  ...
  while (phase !== "done") {
    ...
    if (FUNNEL_PHASES.has(phase) && !replenished && progress.replenish?.closed !== "candidates_exhausted") {
      ...
      let remaining = [...(r?.evidenceOwed ?? [])]; if (r) progress = { ...progress, evidenceOwed: remaining }; // THE PASS'S OWN LIST REPLACES THE OLD ONE AT ONCE (live 2026-09-02): written only inside the loop below, a boxed drive kept the previous list and re-bought the same eight readings every drive, three of them for rows already promoted. The persisted remainder then shrinks as needs land: recomputing it from the pre-loop list re-listed the first landed need whenever a pass acquired two
      for (const need of (r?.evidenceOwed ?? []).slice(0, OWED_PER_DRIVE)) { if (deadline - nowFn().getTime() < STOP_STARTING_MS + 20_000) break; if (!await renewLease(tenantId, run.id, ownerToken, attemptCursor)) return "lost_lease"; // EACH READING RE-PROVES THE LEASE: a reading runs to ninety seconds and the redraft behind it to the whole box, longer together than the lease // AS MANY OWED READINGS AS THE BOX ALLOWS, not two (operator, 2026-09-02): nineteen rows owed a results page and two per drive left seventeen waiting
