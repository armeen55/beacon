...
async function driveRun(run: ResearchRun, ownerToken: string, nowFn: () => Date, deadline: number, steps: ResearchCycleSteps, work: DueWork): Promise<DriveReceipt> {
  ...
  while (phase !== "done") {
    ...
    const stockFirst = FUNNEL_PHASES.has(phase) || phase === "fact_check", basis = stockFirst || phase === "publish_surface" ? (await steps.currentBasis(tenantId)) || null : ""; // INVENTORY BEFORE ACQUISITION HOLDS FOR FACT ACQUISITION TOO (live 2026-09-02): a pass opened on due work spent its whole drive researching twenty-two claims and reached the stock check with no room, so every other pass walked nothing
    ...
    if (stockFirst && !replenished && progress.replenish?.closed !== "candidates_exhausted") {
      replenished = true;
      ...
      type Owed = NonNullable<ResearchRunProgress["evidenceOwed"]>[number]; const boughtNeeds = new Map<string, string>(), stamp = (l: readonly Owed[]): Owed[] => l.map((n) => boughtNeeds.get(n.key) === `${n.kind}::${n.query}` ? { ...n, boughtOn: day } : n); // A READING BOUGHT TODAY THAT LEFT THE SAME REFUSAL STANDING IS NOT BOUGHT AGAIN TODAY (live 2026-09-02): eight needs already on file were re-acquired every cycle and re-owed by the walk unchanged, while twenty-six other needs never reached a slot
      ...
      if (runway > REPLENISH_MIN_MS) {
        ...
        if (r) {
          ...
          progress = { ...progress, replenish: { day, fingerprint: r.fingerprint, attempted: r.attempted, ...(r.tried && r.tried.length > 0 ? { tried: r.tried } : {}), ...(r.settled && r.settled.length > 0 ? { settled: r.settled } : {}), ...(r.spent && Object.keys(r.spent).length > 0 ? { spent: r.spent } : {}), ...(closed ? { closed } : {}), ...((o) => o ? { outcomes: o } : {})(keepOutcomes(r.outcomes)) } };
      ...
      for (const need of remaining.filter((n) => n.boughtOn !== day).slice(0, OWED_PER_DRIVE)) { if (deadline - nowFn().getTime() < STOP_STARTING_MS + 20_000) break; if (!await renewLease(tenantId, run.id, ownerToken, attemptCursor)) return "lost_lease"; // EACH READING RE-PROVES THE LEASE: a reading runs to ninety seconds and the redraft behind it to the whole box, longer together than the lease // AS MANY OWED READINGS AS THE BOX ALLOWS, not two (operator, 2026-09-02): nineteen rows owed a results page and two per drive left seventeen waiting
        ...
        if (got.acquired) {
          ...
          if (again) { r = again;
            ...
            progress = { ...progress, evidenceOwed: remaining,
              replenish: { day: reportingDay(nowFn().getTime()), fingerprint: again.fingerprint, attempted: again.attempted, ...(again.tried && again.tried.length > 0 ? { tried: again.tried } : {}), ...(again.settled && again.settled.length > 0 ? { settled: again.settled } : {}), ...(again.spent && Object.keys(again.spent).length > 0 ? { spent: again.spent } : {}), ...(closed2 ? { closed: closed2 } : {}), ...((o) => o ? { outcomes: o } : {})(keepOutcomes(again.outcomes)) } };
