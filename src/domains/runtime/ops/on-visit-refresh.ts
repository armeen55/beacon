...
async function driveRun(run: ResearchRun, ownerToken: string, nowFn: () => Date, deadline: number, steps: ResearchCycleSteps, work: DueWork): Promise<DriveReceipt> {
  ...
  const REPLENISH_MIN_MS = 45_000, REPLENISH_RESERVE_MS = 60_000, REPLENISH_BOX_MS = 240_000, LEASE_REPROVE_AFTER_MS = 1_000, STOP_STARTING_MS = 40_000, OWED_PER_DRIVE = 8, SAME_ANSWER_STOPS_AT = 2, /* AND A READING THAT KEEPS COMING BACK WITH THE SAME ANSWER IS NOT BOUGHT A THIRD TIME TODAY (measured on run tenant-iranopedia:p2:2026-09-05): two page checks were bought six and nine times on one day and every one of them returned the identical sentence, because a purchase that fails stamps nothing on the need. One repeat proves the repetition and a posted reading still gets the second drive it lands on, which is how a results page bought as "waiting" comes back "done"; past that the money goes to work a purchase can still move. */ REPLENISH_BEGINS_MS = 110_000, /* AND THE WALK'S OWN FLOOR IS MEASURED, NOT GUESSED (2026-09-05): the constant is one account's preparation plus a margin, and the margin is a cliff at both ends. Padded high, a box the walk could just begin one job in funds nothing; padded low, a bigger account funds a manifest it can never start. The last walk that actually reached its funded work reports how long its free half took, the drive reads that number off its own row, and the constant stands only until a pass has measured one. */ beginsMs = ((p) => p == null ? REPLENISH_BEGINS_MS : Math.min(REPLENISH_BOX_MS, Math.max(REPLENISH_MIN_MS, p) + STOP_STARTING_MS))(progress.replenish?.outcomes?.preparedMs);
  ...
  const walkNow = async (asked: number): Promise<{ v: Awaited<ReturnType<ResearchCycleSteps["replenishReady"]>>; boxed?: true } | null> => { const from = nowFn().getTime(),
    ...
    return Promise.race([steps.replenishReady(tenantId, nowFn(), { jobs: memory(), callsSpent: spentCalls, waiting: waitingKeys(), shared, noRoom: box <= beginsMs, filed: (f) => { ask = f; } }, from + box - STOP_STARTING_MS).then((v) => (answeredFirst = true, { v })).catch(() => (answeredFirst = true, { v: null })), /* AND A BOX THAT CAN BEGIN NOTHING FUNDS NOTHING, AND SAYS SO: the free half still runs, and the manifest is declared and declined with the drive's own arithmetic instead of being funded and filed thirty-nine times as "the time box ended before this page's turn", which reads as a walk that got part way and is a walk that never started. */
      new Promise<{ v: Awaited<ReturnType<ResearchCycleSteps["replenishReady"]>>; boxed?: true } | null>((res) => setTimeout(() => { const kept = ((f) => { try { return f?.() ?? null; } catch { return null; } })(answeredFirst ? null : ask) /* the walk that answered in time is read from its own return, and this timer then asks for nothing */ as Awaited<ReturnType<ResearchCycleSteps["replenishReady"]>>; res(kept == null ? null : { v: kept, boxed: true }); }, box + grace))]); };
  ...
  while (phase !== "done") {
    ...
    /** LEAVE THIS PHASE TO THE REST OF THE DAY, through ONE door. Four branches spelled this out separately (an unreadable lane, a lane that moved nothing, a spending cap and now a provider still waiting a drive later) and every one of them is the same decision: what this phase owes is untouched and re-enters through due-work, and the phases behind it run now instead of on whichever later drive happens to get past it. Answers the phase to carry on at, or `lost_lease` when the lease was gone. */
    const yieldPhase = async (why: string, ctx: Record<string, unknown>): Promise<ResearchPhase | "lost_lease"> => { const past = allowed ? nextPlanned(nextPhase(phase), allowed) : nextPhase(phase); log.info(`[research-run] ${why}`, { tenantId, phase, to: past, ...ctx });
    ...
    if (stockFirst && basis !== "" && !replenished && (progress.replenish?.closed !== "candidates_exhausted" || (progress.replenish.awakened ?? []).length > 0)) { // AND A DAY WITH SOMETHING AWAKE IS NOT A CLOSED DAY (operator, 2026-09-02): a fact banked after the last walk left the writer un-hired, and the exhaustion that closed the day was earned before that evidence existed
      ...
      type Owed = NonNullable<ResearchRunProgress["evidenceOwed"]>[number]; const prior = new Map((progress.evidenceOwed ?? []).map((n) => [`${n.key}::${n.kind}::${n.query}`, n] as const)), priorOf = (n: Owed): Owed | undefined => prior.get(`${n.key}::${n.kind}::${n.query}`), attemptOf = (n: Owed): Owed["tried"] => tried.get(readingOf(n)) ?? priorOf(n)?.tried, /** IS THIS READING SPENT FOR TODAY: it came back with the same answer at least twice, the work it serves still wears the identity those attempts were made for, and the day has not turned. A new capture or a newly banked source moves that identity, which is exactly when the reading is worth buying again. */ spentOn = (n: Owed): boolean => ((a) => a != null && a.on === day && a.work === (n.workKey ?? "") && a.count >= SAME_ANSWER_STOPS_AT)(attemptOf(n)), stamp = (l: readonly Owed[]): Owed[] => l.map((n) => ({ ...n, ...(boughtReadings.get(readingOf(n))?.acquired === true || priorOf(n)?.boughtOn === day ? { boughtOn: day } : {}), ...((a) => a ? { tried: a } : {})(attemptOf(n)), ...(stalled.has(n.key) ? { reason: stalled.get(n.key)! } : {}) })); /* AND THE STALL SENTENCE SURVIVES IT TOO (operator, 2026-09-05): the walk rebuilds this list off the rows, so a reading this drive paid for and could not use was re-listed with its original wording and the operator was told the same thing on every drive with nothing about the money already spent on it */ /* THE STAMP SURVIVES THE WALK'S FRESH LIST (reviewer, 2026-09-02): the walk rebuilds the list without stamps, so a need bought on an earlier drive today was bought again by the post-walk loop */ // A READING BOUGHT TODAY THAT LEFT THE SAME REFUSAL STANDING IS NOT BOUGHT AGAIN TODAY (live 2026-09-02): eight needs already on file were re-acquired every cycle and re-owed by the walk unchanged, while twenty-six other needs never reached a slot
      /** ONE PURCHASE PER READING PER DRIVE, AND EVERY ROW THAT OWES THAT READING IS SERVED BY IT (operator, 2026-09-05). The reading's identity leaves the OWNING ROW out, so two rows owing one reading are one purchase and the second reads the first one's answer for free: measured on eight of nine live run rows, one page's source pass was owed by two rows and bought twice every drive. EVERY PURCHASE THEN NAMES WHAT IT UNLOCKS, durably: the row that owes it, the funding identity behind that row, the requirement code, and a typed outcome, so a reading that landed and did not move its obligation is on the run row as a fact instead of being re-owed in silence for two days. A reading that could not be used leaves its reason on the need itself, so the next receipt for that row carries it. */ const readingOf = (n: Owed): string => `${n.kind}::${(n.url ?? "").toLowerCase()}::${n.kind === "semantic_review" ? n.proposalId ?? "" : n.kind === "factual_source" ? n.missingTopic ?? n.query : n.query}`.toLowerCase();
      const buy = async (need: Owed, owed: readonly Owed[], budgetMs: number): Promise<{ acquired: boolean; detail: string }> => {
        ...
        if (!had) boughtReadings.set(reading, got); const alsoOwedBy = owed.filter((n) => n.key !== need.key && readingOf(n) === reading).map((n) => n.key), outcome = !got.acquired ? "not_read" as const : got.unlocked === false ? "read_not_usable" as const : "unlocked" as const; if (!had && outcome === "not_read") tried.set(reading, ((a, why) => ({ on: day, work: need.workKey ?? "", count: a != null && a.on === day && a.work === (need.workKey ?? "") && a.why === why ? a.count + 1 : 1, why }))(priorOf(need)?.tried, got.detail.slice(0, 200))); /* AN ATTEMPT THAT DID NOT LAND IS A FACT ABOUT THE MONEY, and the answer it came back with is what says whether buying it again could teach this drive anything: the same sentence twice under one work identity is a reading that has told this day everything it can. */
        ...
        progress = { ...progress, acquisitions: [...(progress.acquisitions ?? []), { key: need.key, workKey: need.workKey, kind: need.kind, query: need.query, reasonCode: need.reasonCode, ...(need.unlocks ? { unlocks: need.unlocks } : {}), outcome, ...((a) => a ? { attempts: a.count } : {})(tried.get(reading)), detail: `${had ? "served by the purchase this drive already made: " : ""}${got.detail}`.slice(0, 300), ...(alsoOwedBy.length ? { sharedWith: alsoOwedBy } : {}) }] }; log.info("[research-run] a reading bought for one row's own obligation", { tenantId, key: need.key, workKey: need.workKey, kind: need.kind, query: need.query, reasonCode: need.reasonCode, outcome, alsoOwedBy: alsoOwedBy.length, detail: got.detail }); return { acquired: got.acquired, detail: got.detail }; };
      { let left = [...(progress.evidenceOwed ?? [])]; let bought = 0; /* OWED READINGS BEFORE THE WALK (operator, 2026-09-02): the walk spends the whole box, so the exact readings earlier passes were refused for never got their turn (36 rows owed a results page while two cycles bought none); a bounded slice is bought first, and the walk that follows drafts against what landed. AND A READING BOUGHT BEFORE THE WALK IS ONE THE HEAD OF THE ORDER IS WAITING ON, NEVER THE WHOLE OWED LIST (measured on the 10:30Z drive, 2026-09-05): the slice was the eight best-ranked needs whatever their rank, and on that account one served the top-ranked row and seven served rows ranked 15th and 144th to 157th by the producer's own ranker; the two it actually bought were the fifth failed purchase of the same two readings on that run, and the answer second in the funded order, whose source was already on file, was declined for want of time. A need is taken here only while the ranking above it is served, so a head that needs no reading sends the drive straight to the walk. AND THE WALK'S BOX IS RESERVED BEFORE A PENNY OF IT IS SPENT: the old bound stopped at 85 seconds left, which is half of what the walk needs to begin its first job, and it checked 90 seconds of elapsed time before a purchase that may take 60 more. Each reading is bounded by the time that is genuinely spare, so no reading is ever started that the walk's own floor cannot pay for. */ const spare = (): number => deadline - nowFn().getTime() - beginsMs - (stockOnly || shortStock ? 0 : REPLENISH_RESERVE_MS); /* WHAT IS LEFT OVER THE WALK'S OWN FLOOR AND THE RESERVE THE PHASES BEHIND IT ARE BANKING is the only money a reading before the walk may spend */ const defer = (n: Owed, why: string): void => { progress = { ...progress, acquisitions: [...(progress.acquisitions ?? []), { key: n.key, workKey: n.workKey, kind: n.kind, query: n.query, reasonCode: n.reasonCode, ...(n.unlocks ? { unlocks: n.unlocks } : {}), outcome: "deferred" as const, detail: why }] }; }; /* AND A READING THIS DRIVE DECIDED NOT TO BUY SAYS SO ON THE ROW, once and with its reason: a need that simply stops appearing on the receipt is the silence the typed outcomes exist to end */ for (const [at, need] of [...left]/* IN THE RANK ORDER OF THE ROWS THAT OWE THEM (reviewer, 2026-09-04): the fill stamps the rank and the checker already sorts by it, but both purchase loops sliced the list in store order, so the money went to whichever need was filed first; missing rank sorts last */ .sort((a, b) => (a.rank ?? Number.MAX_SAFE_INTEGER) - (b.rank ?? Number.MAX_SAFE_INTEGER)).entries()) {
          if (bought >= OWED_PER_DRIVE || (need.rank ?? Number.MAX_SAFE_INTEGER) > at + 1) { defer(need, bought >= OWED_PER_DRIVE ? `this drive has already bought the ${OWED_PER_DRIVE} readings one drive may buy, so this is owed at its own rank on the next drive` : "the work ranked above this reading needs no reading at all, so the walk was given the box first and this is owed at its own rank on the next drive"); break; } /* the head of the order, and a gap in it means the work above this reading needs no reading at all */ if (need.boughtOn === day || spentOn(need)) continue; /* served already today, or bought until it answered the same thing twice: either way it holds its place at the head and no money goes to it again */ if (spare() < 20_000) { defer(need, "what is left of this drive is the box the walk needs to begin its first job, so nothing was bought for this and it is owed at its own rank on the next drive"); break; } /* under the floor a reading needs, and the walk's box is not the place to find it */ const got = await buy(need, left, Math.min(60_000, spare())); bought += 1; if (got.acquired) { left = left.filter((n) => n.key !== need.key); forget("evidence", "cards", ...(need.kind === "semantic_review" ? ["proposals"] : [])); } }
      ...
      const room = deadline - nowFn().getTime() - (stockOnly || shortStock ? 0 : REPLENISH_RESERVE_MS), held = phase === "fact_check" ? FACT_UNIT_MIN_MS + STOP_STARTING_MS : 0, runway = room - held > beginsMs ? room - held : room; /* INVARIANT: THE UNITS' OWN MINIMUM IS HELD BACK OUT OF THE WALK'S RUNWAY, AND NEVER AT THE COST OF THE WALK ITSELF (live 2026-09-04). Yesterday's answer to a walk that starved the units was to run the units first, and twelve consecutive ticks then re-entered fact_check, banked about one claim and paused with no walk at all. The hold protects both halves without an order that starves either; it is dropped the moment taking it would put the walk under its own floor, because a hold that skips the walk is the same deadlock with the names swapped. AND THE WALK'S OWN FLOOR IS THE BOX IT NEEDS TO BEGIN A JOB, NOT ONE CALL (measured, 2026-09-05): the floor read one call, so an 85-second hold cut a 200-second room to a 55-second box and the walk funded a whole manifest and began none of it. The walk runs at fact_check, which is the first phase that checks the stock, so this one number decided every unattended drive: it began work only where the drive arrived with 164 to 190 seconds left, and began nothing on the 07:00Z, 07:30Z, 08:00Z and 08:30Z passes. Under the walk's real floor the hold is taken whenever the room can pay for both and dropped whenever it cannot, which is what the invariant always said. */
    ...
    if (FUNNEL_PHASES.has(phase)) {
      ...
      if (unit.status !== "done") {
        ...
        if (unit.status === "failed") progress = { ...progress, state: { ...progress.state, blocker: (unit.detail ?? "").slice(0, 300) || null } }; if (unit.status === "waiting") progress = { ...progress, waited: { phase, drives: (progress.waited?.phase === phase ? progress.waited.drives : 0) + 1 } }; /* counted on the row before the advance below persists it, because a pause writes no progress of its own */ /* ONE WAITING DRIVE PER PHASE PER RUN (round-eight reviewer, 2026-09-05): the count is keyed on the phase and is never cleared, which reads as a run that leaves every later lane behind at once the moment it leaves one. It cannot: the cycle walks its phases in one direction and never returns to one it has left, so the count a phase carries is its own, each phase is handed exactly one drive of the provider's time inside a run, and the next drive that still finds it waiting leaves it. */
        ...
        // A PROVIDER'S WAIT IS THE PROVIDER'S TIME, NOT THE WHOLE DAY'S (measured, 2026-09-05): the day's run answered `waiting` at prompt_observations on every drive from 09:03Z and paused there each time, so the source check, the verification and the publication behind it were unreachable for three hours and the surface was never rebuilt. The FIRST wait still pauses, which is what gives the posted task its time and hands the lease straight back. The SECOND drive that finds the same phase still waiting leaves it: the rest of the day runs now, this lane's debt is untouched and re-enters through due-work, and the run reaches its surface at least every other drive.
        if (unit.status === "waiting" && (progress.waited?.drives ?? 0) > 1) { const past = await yieldPhase("this step is still waiting on its provider a second drive later, so the rest of the day runs now and its own debt keeps its place", { drives: progress.waited?.drives ?? 0 }); if (past === "lost_lease") return past; phase = past; cursor = null; continue; }
