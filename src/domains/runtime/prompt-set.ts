...
/**
 * prompt-set - THE SAVE side of the tracked-question set: the id and normalize rules, and the pure declarative save an ACTIVE
 * account drives from Settings. WHICH questions are tracked and WHICH SERIES each is on now belong to `account/tracked-questions`
 * (one owner, readable by every kernel below Runtime); this module re-exports them so every existing caller is unchanged.
 *
...
// ACCOUNT OWNS THE SET, Runtime owns the SAVE. Activation, the core tag, the limits and the version rule all
// live in one place a lower kernel can read without reaching up into the orchestrator; re-exported here so
// every existing caller of prompt-set keeps working unchanged and no predicate is ever defined twice.
import { LIMITS, PROMPT_TAGS, projectTrackedQuestions, versionOf, type TrackedPromptRow, type TrackedQuestion } from "@/domains/account/tracked-questions";
export { LIMITS, PROMPT_TAGS, projectTrackedQuestions, readActiveTrackedPrompts } from "@/domains/account/tracked-questions";
export type { TrackedPromptRow, TrackedQuestion } from "@/domains/account/tracked-questions";
