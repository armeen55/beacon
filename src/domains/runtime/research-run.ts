...
/** The ordered phases of one Research Run. `done` is terminal. The four evidence phases (Slice 6) sit between the connector work and the surface publish: keyword discovery, AI observation, SERPs, winning pages.
 *  crawl_pages reads ONE bounded batch of the account's own website per pass (a render never crawls). */
export type ResearchPhase =
  | "refresh_sources"
  | "gsc_backfill_chunk"
  | "crawl_pages"
  // CHECKING WHAT THIS ACCOUNT'S OWN PAGES CLAIM against sources outside them, AHEAD OF EVERY PAID PHASE and behind only the free ones that feed it. A money reserve alone could not make it reachable: bulk buying spent
  // the day's dollar before its turn arrived (Codex, 2026-08-18), so the run order gives it the turn.
  | "fact_check"
  | "keyword_discovery"
  | "prompt_observations"
  | "serp_analysis"
...
/** Three honestly-produced states only: a transient phase failure PAUSES with a bounded last_error (recoverable next visit), never a terminal `failed`. Completion means every phase succeeded or no-opped. */
...
export type ResearchRunProgress = {
   ...
   *  dies with the run. `retryAfter` is the earliest moment that topic may legally be read again; it is carried, not recomputed, and it decides both what this run may still search for and what Today may call
   *  "checking". `ownedUrl` is the page of the ACCOUNT'S OWN that topic cannot be judged without: Decision NAMES it and never fetches it, one read per run under this lease. */
  ...
  surfacePublished?: boolean; /** THE PHASE THIS RUN HAS BEEN WAITING AT, AND ON HOW MANY DRIVES. A durable provider wait pauses the whole run, so one lane waiting on a task held every phase behind it: the day's run sat at prompt_observations from 09:03Z to 11:34Z on 2026-09-05 and never reached the publication, while nineteen posted tasks stayed pending. One drive of waiting is the provider's own time; the second drive that finds the same phase still waiting yields it, so the source check, the verification and the surface all run and the lane's debt keeps its place in due-work. */ waited?: { phase: ResearchPhase; drives: number; /** AND WHETHER THAT DRIVE COULD NOT PAY FOR THE STEP AT ALL, which is a different fact from waiting on a provider and buys a different answer: the next drive at this phase holds the step's own floor back out of the walk's box so it gets its turn: on the drive after one that could not pay, that phase's own step runs FIRST, ahead of the walk and inside the fact unit's own box, and the mark is dropped the moment the step starts, so the turn can never become a standing one. */ unpaid?: true; /** AND WHAT BECAME OF THE TURN THAT MARK BOUGHT, in the walk's own vocabulary: `ran` is the step taking this drive's first turn, `stopped` is a drive that could not pay for even that. */ turn?: "ran" | "stopped" };
  ...
  /** PROGRESS AS PERSISTED TRUTH, so every surface reads the same numbers on every request: written by the run itself from the due-work read it already made, never derived at render time. */
  state?: {
  ...
  /** WHAT THE FREE COLLECT FOUND ON THIS RUN: tasks already paid for that are still pending, and how many of them came back ready. Present once a run has looked. */
  ...
  acquisitions?: readonly { key: string; workKey: string; kind: string; query: string; reasonCode: string; outcome: "unlocked" | "read_not_usable" | "not_read" | "deferred"; detail: string; /** WHICH ATTEMPT OF THIS READING THIS WAS, counting only the attempts that came back with the same answer as the one before them, so the receipt says how many times the money bought the same sentence. */ attempts?: number; /* `deferred` is a reading this drive DECIDED not to buy and says why: no money left the process for it, and it is owed at its own rank on the next drive. A reading that is simply absent from this list is a silence, which is the thing the typed outcomes exist to end. */ sharedWith?: readonly string[]; /** The row and the ladder rung this purchase was bought to unlock, straight off the typed need. */ unlocks?: { proposalId: string; step: string } }[];
  evidenceOwed?: readonly { key: string; kind: "serp" | "page_source" | "competitor_page" | "factual_source" | "semantic_review"; query: string; url?: string; reasonCode: string; reason: string; workKey: string; /** The missing proposition a factual_source researches, and the rival that identified it (briefing provenance only): declared here so a field-by-field rebuild can never silently drop the topic and degrade the acquisition to a plain re-check. */ missingTopic?: string; rivalUrl?: string; /** The exact change a `semantic_review` reads, so the runtime loads one row instead of the whole queue. */ proposalId?: string; /** THE ROW'S OWN STAMPED RANK, carried by the fill so the consumer buys in global opportunity order rather than store order; absent on a need whose row carries no rank yet, which sorts last. */ rank?: number; /** The reporting day this reading was last bought while the work still refused: not bought again that day, so the drive's slots go to needs a purchase can still answer. */ boughtOn?: string; /** THE ATTEMPT THAT DID NOT LAND: the reporting day it ran on, the work identity it ran for, how many times in a row it came back with that same answer, and the answer itself. A purchase that fails stamps nothing, so two readings were bought six and nine times on one day for the identical sentence; a reading is owed again when the day turns or when the work it serves wears a new identity, which is what a fresh capture or a newly banked source moves. */ tried?: { on: string; work: string; count: number; why: string }; /** WHAT THIS PURCHASE UNLOCKS, as the owed-list door typed it (decision/producers/contract): the exact row the money is for and the rung that row is blocked at once the reading lands. Absent on a need filed before the list carried it, and absent is never an excuse to guess. */ unlocks?: { proposalId: string; step: string } }[];
  replenish?: { day: string; /** THE DAY'S ONE ATTEMPT LEDGER, keyed on the row's own `workKey`: what each job's attempts cost, how the last one ended, and whether anything is left to do for it under this exact evidence. It replaced four page-keyed lists and the manifest fingerprint that reset them, every one of which asked "the same page again" of work whose evidence, obligation or rules had moved. */ jobs: Record<string, { calls: number; last: string; settled: boolean }>; closed?: "candidates_exhausted"; /** The account and evidence version an exhaustion was earned under: fresh evidence reopens the day, because a manifest settled against yesterday's readings says nothing about today's. */ closedUnder?: string; /** THE WORK A FACT BANKED LATER IN THE SAME DRIVE WOKE UP, and has not been walked yet: the exact next unit. Persisted before the walk, so a drive that runs out of time resumes at this point instead of losing the dependency until tomorrow, and a day may never close while it holds anything. */ awakened?: string[];
    ...
    outcomes?: { /** THE PASS THAT PRODUCED THESE RECEIPTS, by its own cycle key, so a reader of the row can prove whose answer it is holding rather than trusting that nothing carried them forward. */ pass?: string;
    /** HOW THE WALK ENDED, TYPED, because "the drive ran out of time" is two different facts with two different answers: `boxed` means the drive stopped waiting while the walk was still running (its calls are remembered and its work is owed again at its own rank), `stopped` means the walk would not start funded work the drive could no longer pay for (nothing is running and that work is the next drive's first), and `ran` means it reached the end of what it funded. */ ended?: "boxed" | "stopped" | "ran"; readySaved: number; evidenceBanked: number; refused: number; blocked: number; unreached: number; stuck: string[];
      ...
      receipts?: unknown[]; ledger?: { before: number; after: number; delta: number; metered: number; unexplained?: number; reconciled: boolean }; /** HOW MANY TEMPLATE SIBLINGS THE SWEEP'S OWN RULE HELD AND HOW MANY THE FAMILY HOLDS, summed over the rows it judged. It was computed on the producer's receipt and dropped before the row was written (measured on run tenant-iranopedia:2026-09-05, familyRead absent from every stored pass), so a verdict that depends on a page's family could not be checked against the family that was read. */ familyRead?: { asked: number; loaded: number }; /** HOW LONG THIS WALK'S FREE HALF TOOK, from the moment the walk began to the moment its first funded job could be started: the snapshot, every $0 producer, the whole-family body read and the re-read of every stored row. Present only on a pass that actually reached funded work, so a pass that funded nothing can never teach the next drive that preparation is cheap. The next drive's own floor is read from it, instead of a constant that is one account's measurement plus a margin. */ preparedMs?: number } };
  ...
  funnel?: {
    ...
    /** WHAT THE READING PASSES ACTUALLY DID, beside how many landed: how many stored answers they took on and how many came back as a non-reading. A bare zero cannot tell a quiet day from a pass that took forty answers
     *  on and could store none of them, and that is precisely the shape a run sitting in one phase for ten hours wears, so both numbers go on the row rather than into a log line. */
    answersAttempted?: number; answersRefused?: number;
    /** WHY THE ATTEMPTED AND THE READ ARE DIFFERENT NUMBERS, summed across this run's reading passes: one bucket per answer taken on, and the buckets add up to answersAttempted. A reader of the row can tell a reader
     *  that refused from a shape nobody could use, from a write that was lost, from a pass that stopped before it asked anything. Without it "505 taken on, 0 read" carried no explanation anywhere, on the row or off it. */
...
/** Deterministic idempotency key for one phase attempt, SCOPED to the account + run + phase + cursor. Identical across retries of the same attempt; different across phases, cursors and accounts. Stored into
...
export type ResearchRunRepo = {
  /** Atomic claim/resume/create for the account. Resumes the single unfinished run regardless of date; creates today's cycle only when none is open and none completed that day (the database computes the key). Null =
   *  the caller did not win (a foreign unexpired lease, or research already current today). */
  claim(input: { tenantId: string; owner: string; leaseSeconds: number }): Promise<ResearchRun | null>;
  /** THE FLEET DOOR (claim_due_research_work). Enumerates every ACTIVE, not-research-paused account whose current Pacific day still owes work and claims up to `limit` of them THROUGH the same claim above, so the
   *  advisory lock and the one-open-run index stay the single mechanism. An account another dispatcher or a visit already holds is simply absent from the result. */
  claimDue(input: { owner: string; limit: number; leaseSeconds: number }): Promise<ResearchRun[]>;
  /** Open ANOTHER pass on a day that already completed one, for an account with genuinely due work. Null = it must not run: the one-open-run-per-account index refuses the insert while any run is unfinished. `progress`
   *  is what the opener already knows (the plan it opened on), written with the row itself. */
  ...
  /** Guarded advance at DATABASE time (id + tenant + owner + a LIVE lease + status='running'). Extends the lease. False ⇒ our lease was lost or expired. */
  advance(input: { tenantId: string; id: string; owner: string; leaseSeconds: number; patch: AdvancePatch }): Promise<boolean>;
  /** Guarded lease renewal at DATABASE time (same guards as advance) that also persists the pre-phase attempt identity (phase_cursor) WITHOUT changing the phase. False ⇒ abort before the side effect. */
...
const supabaseRepo: ResearchRunRepo = {
  ...
  async startPass({ tenantId, owner, leaseSeconds, day, progress }) {
    ...
    // The pass ordinal keeps (tenant, cycle_key) unique for a second pass on the same day, and the key still ENDS in the day because the reporting day is read off its tail: a pass that spans midnight must keep
    // reporting into the day it opened.
    ...
    // A unique violation is the expected LOSS (another pass is open, or another tab inserted first), never an error worth surfacing: the caller simply does nothing.
  ...
  // The reporting day rides on the TAIL of both cycle-key shapes (the daily "<tenant>:<day>" and an extra pass's "<tenant>:p<n>:<day>"), which is exactly why it lives there.
...
/** Renew our lease AND persist the pre-phase attempt identity (cursor) at DATABASE time, owner-guarded, WITHOUT changing the phase. Called BEFORE each phase side effect: false ⇒ our lease was lost or expired, so the
 *  caller aborts before any side-effecting work. Never throws to the caller. */
...
/** Terminal update for the run (owner-guarded), releasing the lease. Returns whether our lease still held. `spendUsd` stamps the row's own spend accumulator at the close, from the spend this run tracked; a run that
 *  bought nothing stamps 0, which is a fact and not an absence. Never throws to the caller. */
