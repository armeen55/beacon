...
export type ResearchRunProgress = {
  ...
  plan?: { units: DuePhase[]; /** TRUE only when the pass was OPENED carrying this plan. A receipt, never an exemption: the paid-rotation shift gate reads no plan at all (every door opens with the whole due list, so an opened-for exemption ran every rotation on every drive, 2026-09-14). */ openedFor?: boolean };
  ...
  acquisitions?: readonly { key: string; workKey: string; kind: string; query: string; reasonCode: string; outcome: "unlocked" | "read_not_usable" | "not_read" | "deferred"; detail: string; preflight?: "owner_unreadable" | "basis_unavailable" | "basis_mismatch" | "owner_changed" | "unchanged_incomplete"; /** WHICH ATTEMPT OF THIS READING THIS WAS, counting every attempt that did not land under one work identity on one day whatever it came back with, so the receipt says how many times the money has bought this reading. */ attempts?: number; /* `deferred` is a reading this drive DECIDED not to buy and says why: no money left the process for it, and it is owed at its own rank on the next drive. A reading that is simply absent from this list is a silence, which is the thing the typed outcomes exist to end. */ sharedWith?: readonly string[]; /** The row and the ladder rung this purchase was bought to unlock, straight off the typed need. */ unlocks?: { beforeMicros?: true; proposalId: string; step: string } }[];
  evidenceOwed?: readonly (Omit<import("@/domains/decision/producers/contract").EvidenceRequirement, "unlocks"> & { key: string; reason: string; workKey: string; rank?: number; boughtOn?: string; postedOn?: string; priorityHeld?: "owner_unconfirmed" | "unavailable_today" | "unchanged_incomplete"; priorityHeldOn?: string; preflightMisses?: { on: string; count: number }; tried?: { on: string; work: string; count: number; why: string }; unlocks?: { beforeMicros?: true; proposalId: string; step: string } })[];
  replenish?: { day: string; /** THE DAY'S ONE ATTEMPT LEDGER, keyed on the row's own `workKey`: what each job's attempts cost, how the last one ended, and whether anything is left to do for it under this exact evidence. It replaced four page-keyed lists and the manifest fingerprint that reset them, every one of which asked "the same page again" of work whose evidence, obligation or rules had moved. */ jobs: Record<string, { calls: number; last: string; settled: boolean }>; closed?: "candidates_exhausted"; /** The account and evidence version an exhaustion was earned under: fresh evidence reopens the day, because a manifest settled against yesterday's readings says nothing about today's. */ closedUnder?: string; /** THE WORK A FACT BANKED LATER IN THE SAME DRIVE WOKE UP, and has not been walked yet: the exact next unit. Persisted before the walk, so a drive that runs out of time resumes at this point instead of losing the dependency until tomorrow, and a day may never close while it holds anything. */ awakened?: string[];
    ...
    /** HOW THE WALK ENDED, TYPED, because "the drive ran out of time" is two different facts with two different answers: `boxed` means the drive stopped waiting while the walk was still running (its calls are remembered and its work is owed again at its own rank), `stopped` means the walk would not start funded work the drive could no longer pay for (nothing is running and that work is the next drive's first), and `ran` means it reached the end of what it funded. */ ended?: "boxed" | "stopped" | "ran"; readySaved: number; evidenceBanked: number; refused: number; blocked: number; unreached: number; stuck: string[];
      ...
      receipts?: unknown[]; ledger?: { before: number; after: number; delta: number; metered: number; unexplained?: number; reconciled: boolean }; /** HOW MANY TEMPLATE SIBLINGS THE SWEEP'S OWN RULE HELD AND HOW MANY THE FAMILY HOLDS, summed over the rows it judged. It was computed on the producer's receipt and dropped before the row was written (measured on run tenant-iranopedia:2026-09-05, familyRead absent from every stored pass), so a verdict that depends on a page's family could not be checked against the family that was read. */ familyRead?: { asked: number; loaded: number }; /** EVERY KEY THE PLAN DECLARED THIS PASS, funded or not, so a receipt reader can tell a job that died from a job that was never considered: the hardest-languages page vanished from three receipt sets and only a log stream could say whether coverage had stopped offering it (production 01:32Z, 2026-09-11). */ declared?: readonly string[]; /** HOW LONG THIS WALK'S FREE HALF TOOK, from the moment the walk began to the moment its first funded job could be started: the snapshot, every $0 producer, the whole-family body read and the re-read of every stored row. Present only on a pass that actually reached funded work, so a pass that funded nothing can never teach the next drive that preparation is cheap. The next drive's own floor is read from it, instead of a constant that is one account's measurement plus a margin. */ preparedMs?: number; /** Exact source-to-writer wake and its typed retirement, if the current proposal can no longer run. */ preferred?: { proposalId: string; workKey: string; currentWorkKey?: string; retiredReason?: "missing" | "blocked" | "settled" | "out_of_scope" } } };
