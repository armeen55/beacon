/** Account lifecycle: the ONE resolver for "what may this account see right now". Every signed-in surface asks this instead of hand-rolling
 *  status checks, so the four outcomes stay identical everywhere and no customer can land on a dead end. INVARIANT: a failed read must
 *  never masquerade as a lifecycle state. A transient `tenants` read failure resolves to `unavailable`, a bounded retryable error surface,
 *  never to `incomplete` (which would bounce a fully onboarded customer back into onboarding) and never to a redirect (which is how sign-in
 *  loops are born). */
...
/** Resolve one account's lifecycle. Pure decision over a single scoped read; never redirects and never throws. */
...
/** Guard for signed-in product surfaces. unavailable → throw AccountUnavailableError (bounded retry boundary) incomplete →
 *  redirect("/onboard") suspended → returned, so the caller renders the notice ready → returned, ONCE its setup truth is proven present
 *  ACTIVE IS A STATUS, NOT PROOF OF SETUP. An account flipped active without a website, without a confirmed profile or without a single
 *  approved question passed this guard and rendered every surface off nothing at all. It now resumes at the FIRST genuinely incomplete step
 *  instead. AN OUTAGE IS NOT INCOMPLETENESS: a gap I could not read is the bounded retry surface, never a bounce into onboarding, because
 *  sending an established customer back to setup over a transient read is the worse of the two failures. */
export async function requireReadyAccount(
  ...
  if (access.kind === "ready") {
    ...
    const gap = await setupGapOnce(tenantId, a.domain ?? "", a.growth_goal ?? "", a.tos_accepted_at ?? "").catch(() => { throw new AccountUnavailableError(tenantId); });
