    ...
    return (
      <section aria-label="Visitor friction fixes" className={CARD}>
        ...
        <div className="mt-2 space-y-2">
          {rows.map(({ s, d }) => (
            <div key={s.url} className="rounded-xl bg-gray-50 px-3 py-2">
              <div className="flex flex-wrap items-baseline gap-2">
                <span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold ${d.severity === "high" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}>
      ...
      <section aria-label="Demand opportunities" className={CARD}>
        ...
        <div className="mt-2 space-y-1.5">
          {res.opportunities.slice(0, 5).map((o) => (
            <div key={o.id} className="flex flex-wrap items-baseline gap-2 rounded-xl bg-gray-50 px-3 py-2">
              <span className="text-[13px] font-semibold text-gray-800">{o.primaryKeyword}</span>
              {o.estDemand > 0 ? (
          ...
          {res.trends.slice(0, 2).map((t) => (
            <div key={`t-${t.id}`} className="flex flex-wrap items-baseline gap-2 rounded-xl bg-emerald-50/60 px-3 py-2">
              <span className="text-[13px] font-semibold text-gray-800">{t.query}</span>
              <span className="text-[11px] text-emerald-700">{t.seasonal ? "seasonal, peak coming" : `trend ${String(t.trend)}`}</span>
              {typeof t.estDemand === "number" && t.estDemand > 0 ? (
                <span className="text-[11px] text-gray-500">{t.estDemand.toLocaleString()} searches/mo</span>
              ) : null}
            </div>
          ))}
        </div>
