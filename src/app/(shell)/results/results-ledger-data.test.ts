...
const rec = (id: string): ShippedChangeRecord =>
  ({ id, path: `/${id}` }) as unknown as ShippedChangeRecord;
...
describe("loadLedgerWithSwr", () => {
  ...
  it("cold start (no snapshot): re-measures synchronously, persists, and returns fresh", async () => {
    const out = await loadLedgerWithSwr("tenant-test");

    expect(loadProofLedgerMock).toHaveBeenCalledWith("tenant-test");
    expect(out.ledger.map((r) => r.id)).toEqual(["fresh-1", "fresh-2"]);
    expect(Date.parse(out.computedAt)).toBeGreaterThan(0);
    expect(afterMock).not.toHaveBeenCalled();
    expect(writeStoreMock).toHaveBeenCalledOnce();
  });
