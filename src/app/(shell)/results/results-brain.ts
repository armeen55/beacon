...
import { MIN_FINISHED_READINGS } from "@/domains/decision"; // THE FUNDING DOOR OWN NUMBER, IMPORTED (2026-09-05): a copy of it stood here saying when a family record starts moving the queue, and a mirror is a second answer waiting to disagree
...
function thoughtOf(bet: TreatmentGroup, rows: ShipmentPresentation[], now: Date, edges: string[], pooled: { percent: number | null; readings: number; early: boolean } | null = null, funding: { readings: number; netLift: number } | null = null): Thought {
  ...
  const teaches = family == null || learned === 0 ? "Nothing here has changed what gets funded next yet." : learned < MIN_FINISHED_READINGS ? `${closed} here, and the next change of this kind is funded exactly as before: ${MIN_FINISHED_READINGS} closed readings that are down between them is what moves the order.`
