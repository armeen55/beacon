...
function thoughtOf(bet: TreatmentGroup, rows: ShipmentPresentation[], now: Date, edges: string[], pooled: { percent: number | null; readings: number; early: boolean } | null = null, funding: { readings: number; netLift: number; of: "kind" | "family" } | null = null): Thought {
...
export function buildResultsBrain(shipments: ReadonlyArray<ShipmentPresentation>, now: Date, actionable: { ready: number | null } = { ready: null }): BrainModel {
  ...
  const funding = learningFromShipments(shipments.map((p) => learningRowOf(p, true))); // THE EXACT MAP THE QUEUE READS, keyed by this exact kind of work AND by the family, built here from the same rows and the same rule, so what this page says about the next decision is checkable against the decision itself rather than described
  const thoughts = [...bets].map(([key, g]) => thoughtOf(g.bet, g.rows, now, edgesOf(key), pooled.get(key) ?? null, fundingFor(funding, g.rows[0]!))) // THE RECORD IS ASKED OF THIS EXACT KIND OF WORK FIRST and of the whole family only where that holds nothing, in the one order the queue asks it
