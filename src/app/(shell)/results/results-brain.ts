...
export function buildResultsBrain(shipments: ReadonlyArray<ShipmentPresentation>, now: Date, actionable: { ready: number | null } = { ready: null }): BrainModel {
  ...
  const funding = familyHistoryFromShipments(shipments.map((p) => learningRowOf(p, true))); // THE EXACT MAP THE QUEUE READS, keyed by this exact kind of work AND by the family, built here from the same rows and the same rule, so what this page says about the next decision is checkable against the decision itself rather than described
