...
function summarizeConnectorSync(result: unknown): ConnectorSyncNowResult {
  ...
  // the refresh fails on Beacon's side, not the customer's connection, so
  // reconnecting cannot fix it. The old copy handed the customer a server
  // setting to change and a deploy to run: instructions nobody on this screen
  // can follow. The real cause is in the sync path's log, where it is useful.
