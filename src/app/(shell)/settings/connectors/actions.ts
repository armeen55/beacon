...
// On-demand connector syncs (2026-06-15), with all crons/Actions off, these existing per-tenant sync engines (HTTP + Supabase, Vercel-safe) must be triggerable from the product. Each "Sync now" action wraps one.
...
/** Manual-refresh provider -> refresh-ledger source name. The manual paths all pull one of the four read sources; each maps 1:1. */
...
/** Record a manual (operator-triggered) refresh into the ledger. Fail-soft: never throws, so recording can't turn a successful refresh into a failure. */
...
/** Read GA4 connector status. Returns disconnected when no token exists OR when the existing token has `disconnected_at` set (soft-disconnect aware via `getConnectorInfo`'s shared branch). */
export async function getGoogleGa4ConnectorStatus(): Promise<ConnectorInfo> {
  return getConnectorInfo("google_ga4");
}

/** WHAT A FAILURE SAYS TO THE OPERATOR. Every catch below logs the real error and hands back ONE of these: a raw exception ("BEACON_OAUTH_STATE_SECRET is not set", a driver stack, a store message) is a fact about Beacon's insides, and on this screen it read as advice the operator could act on. The log keeps the truth for whoever can use it; the screen gets a sentence that says what happened and what to do next. */
const TROUBLE = {
...
/**
 * Build the Google OAuth URL for the given connector kind (default "gsc"). Each kind requests exactly ONE data scope, plus the basic openid + email identity scopes (per-tenant OAuth, 2026-07-09):
 * • gsc → webmasters.readonly
 * • ga4 → analytics.readonly
 *
 * INTENT (per-tenant OAuth, 2026-07-09): the caller says WHY it wants a flow ("connect" for a first grant, "replace" to swap the Google account behind a live grant, "reauth" to reconnect a dead one), and this action, the ONE store-readable call site, verifies that intent against the stored token before threading it into the auth URL and the signed state:
 * • "replace" is only honored when a LIVE grant actually exists; otherwise it degrades to the derived intent (a forged/stale client call can never widen semantics).
 * • Omitted intent derives from the store: no usable grant → "connect"; dead or soft-disconnected → "reauth"; live → "replace". Every intent shows Google's account chooser AND forces consent (a fresh refresh token for the chosen account). Ordinary syncs/refreshes never run OAuth, so this cannot churn Google's cap of 100 refresh tokens per Google account per OAuth client (only deliberate operator clicks reach here; when the cap IS exceeded, Google revokes the oldest token).
 *
 * State is HMAC-signed with BEACON_OAUTH_STATE_SECRET and carries the connector kind + tenantId + nonce + issued-at + intent. The callback verifies the signature before persisting the token.
 */
export async function getGoogleAuthUrl(
  ...
  try {
    ...
    // The client's requested intent is honored only when it makes sense for the stored state; "replace" without a live grant falls back to derived.
...
export async function disconnectGoogle(): Promise<{
  ...
  try {
    ...
    //
    // GA4 is INTENTIONALLY excluded from this affordance. The Google Analytics card on /settings/connectors owns its own `disconnectGoogleGa4` server action so the operator can manage GA4 independently of Search Console (e.g. rotate the property selection without re-running Search Console OAuth).
    const tenantId = await currentTenantId();
    await softDisconnectGsc({ tenantId });
    // A GRANT NOBODY DRIVES ANY MORE IS STILL A GRANT. Business Profile is no longer a connector this product reads, but an account that connected one before then still holds the stored row, and disconnecting Google has to mean it. Leaving it behind kept a live business.manage grant on file that no screen would ever show.
...
/** List GA4 properties the connected operator can read. Wraps the tenant-scoped `listGa4PropertiesForTenant` helper for the property picker UI on `/settings/connectors`. Returns the structured fail-soft result verbatim; the client component renders per-reason copy. */
...
/** Persist the operator's GA4 property selection onto the `google_ga4` token payload. Defense-in-depth: the picker UI passes a property from a fresh `listGa4Properties()` call, but a stale or forged submission could pass an arbitrary id; this action ALSO re-runs the property listing and verifies the submitted id is in the latest set before persisting. */
...
/** Soft-disconnect the GA4 connector. Mirrors the GSC J5 soft-disconnect contract: sets `disconnected_at` on the existing token payload via `updateConnectorToken`, NO destructive `deleteConnectorToken`. The UI shows the Connect button + "Last refreshed at X days ago" copy until the operator reconnects via OAuth (the callback upserts a fresh payload that omits `disconnected_at`, naturally clearing the field). Also clears `ga4_property_id` (and friends) so reconnect re-runs the property picker. */
...
//
// All GitHub Actions + the Vercel cron are disabled. The per-tenant sync engines (syncGscSearchAnalyticsForTenant / syncGa4UrlTrafficForTenant / syncClarityDailyMetricsForTenant) are all pure HTTP→Supabase (no subprocess, Vercel-safe) and were previously reachable ONLY from the nightly script. These actions surface each as an operator-clickable "Sync now" so connecting a source actually pulls data. Each is fail-soft and returns a normalized {ok, detail?, error?} the card renders. They are idempotent (each sync has its own dedupe/watermark/UTC-day guard).
// ─────────────────────────────────────────────────────────────────────
...
function summarizeConnectorSync(result: unknown): ConnectorSyncNowResult {
  ...
  if (r.synced) {
    ...
    // Only count POSITIVE rows as a "N rows" success bit. A successful sync that returned exactly zero rows is reported separately below, silently printing "Synced 0 rows" hid the real story (e.g. GA4 connected but the property has no traffic / its tag isn't collecting). Wave 0 (2026-06-18).
    ...
    // Explicit empty pull (the API call worked but the source returned zero rows for the window). Wave 0 (2026-06-18): say what that means instead of a bare "Synced 0 rows", the #1 cause is the source isn't collecting yet (e.g. a GA4 property whose tracking tag isn't installed/firing on the site).
    if (rowCount === 0) {
      return {
        ok: true,
        detail:
  ...
  // 2026-06-22, Beacon's OWN Google credentials are wrong (invalid_client): the refresh fails on Beacon's side, not the customer's connection, so reconnecting cannot fix it. The old copy handed the customer a server setting to change and a deploy to run: instructions nobody on this screen can follow. The real cause is in the sync path's log, where it is useful.
  if (r.reason === "gsc_client_misconfig") {
    return {
      ok: false,
      error:
        "Google turned down Beacon's own sign-in, so nothing could be pulled. This one is on Beacon's side and your connection is unchanged. Try again in a few minutes.",
    };
  }
  // 2026-06-22, a mid-sync 401/403 / refresh hiccup where the live-refresh probe proved the grant is STILL ALIVE. NOT a reconnect case, Google just had a momentary blip. Say so honestly instead of the alarming "revoked" copy, so a healthy connection never gets told to reconnect.
  ...
  // #208, known real-failure reason codes get plain-English copy instead of leaking the raw token (supabase_unavailable, upsert_failed, …).
  if (r.reason != null && FAILED_REASON_COPY[r.reason] != null) {
    return { ok: false, error: FAILED_REASON_COPY[r.reason] };
  }
  // Anything still unmapped is a real failure, keep it honest, but never surface the raw reason code to a non-technical owner.
  return {
    ok: false,
...
/** Map a "Sync now" action to the connector token-store provider whose `last_synced_at` should be stamped on a successful pull (#72/#85). Only providers backed by the connector store are listed. */
...
async function writeLastSyncedAt(
  ...
  // Best-effort: a freshness-write failure must never turn a successful sync into a reported failure. updateConnectorToken is a no-op when the row is missing (e.g. soft-disconnected), so this is safe to call unconditionally. Switch routes to the right overload (each provider has a distinct patch type; all five accept last_synced_at).
  try {
    const patch = { last_synced_at: now() };
    switch (provider) {
      case "google_gsc":
      case "google_ga4":
        // A good manual pull also clears any bounded needs-attention marker (BUG 2) so the operator's reconnect+sync heals the banner at once.
...
async function runConnectorSyncNow(
  ...
  /** Connector-store provider whose last_synced_at to stamp on success (#72/#85). Omit for sources without a freshness row. */
  ...
  try {
    ...
    // Record the honest per-source outcome into the refresh ledger so a manual "Sync now" leaves a row (cron used to be the only path that recorded).
    if (freshnessProvider != null) {
      await recordManualRefresh(tenantId, freshnessProvider, startedAt, result);
    }
    // #72/#85, stamp freshness ONLY on a genuinely successful pull so the connector card + freshness label stop saying "never refreshed".
...
/** Pull the latest Google Search Console search-analytics for this tenant. First click on a cold tenant auto-backfills the watermark window; click again to extend history further. Each press also walks the deep 16-month history backfill one month deeper when one is seeded, so the operator can recover the pre-collapse record with clicks instead of waiting on passes. */
...
//
// The per-source "Sync now" actions above live on /settings/connectors. The owner asked for a single control on Today ("everything should be updating, I don't need to refresh"). This action pulls EVERY connected READ source in one click, fail-soft, concurrent, and returns a per-source result list the Today button renders.
// ─────────────────────────────────────────────────────────────────────

/** The READ sources a Today refresh pulls, in display order. Each entry maps a connector-store provider → its sync engine + plain-English customer label. */
...
export type RefreshAllConnectedResult = {
  ...
  results: Array<{
    ...
    label: string;
    ok: boolean;
    /** Short, plain-English line, "Synced …" on success, an actionable reason on failure. Never a raw reason code. */
    detail: string;
  }>;
};
 ...
 *
 * - Resolves the tenant the same way the per-source "Sync now" actions do (`currentTenantId()`).
 * - Reads connector status for the read sources; skips any not 'connected'.
 * - Runs the connected engines concurrently with `Promise.allSettled`, one source failing never blocks the others, and the action NEVER throws.
 * - Stamps `last_synced_at` on each genuinely-successful pull (#72/#85), the same freshness contract the per-source actions use.
...
export async function refreshAllConnectedDataNow(): Promise<RefreshAllConnectedResult> {
  ...
  try {
    ...
    // Read status for all read sources in parallel; fail-soft per provider (a status-read error counts as not-connected so a flaky read never alarms the owner about a source they never set up).
    ...
    // BEACON'S OWN RESEARCH IS NOT A CONNECTOR (2026-08-01). Zero connected sources used to return right here, skipping the warm, the extra AI reading and any honest word about what happened, so an account running on Beacon's own research could press this button forever and never get a second reading of today's answers. Only the third-party syncs are skipped now.
    ...
    // Warm the shared surfaces BEFORE the repaint so the render after this refresh is instant, not a cold ~6s demand-graph rebuild right when the operator is watching (the pulls above just changed the underlying data; build-then-write rebuilds the snapshot from it). Free steps only, fail-soft: a warm failure never turns a successful refresh into an error. Bounded by a safety-valve deadline (well under this route's maxDuration) so a pathological hang can never wedge the action; if the cap wins, the build keeps warming in the background for the next visit. UPDATE DATA MEANS A $0 REFRESH, EVERY TIME. This press used to warm paid competitor context, grant an extra AI reading and hand the browser a research continuation, so a button labelled "refresh" silently spent from the account's allowance (Codex, 2026-08-28). It now refreshes the connected first-party data above and republishes the stored truth, and nothing more: research keeps its own schedule under the one approved runtime, and anything that can spend says so where it is actually asked for.
    ...
    // Defense-in-depth: the action must NEVER throw. Even a tenant-resolution failure returns an empty (honest) result rather than crashing Today.
    const err = e instanceof Error ? e.message : String(e);
