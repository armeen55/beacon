/**
 * /onboard/done render pins (2026-07-03, BEACON_500 R12 / T0e).
 *
 * The first-audit scorecard must read honestly in all three states:
 *   - a real scorecard (numbers, the first win, keep-scanning, GSC offer),
 *   - an unreachable site (a clear sentence + retry, never a blank screen),
 *   - no first look yet (points back to the URL entry).
 * All copy dash-free. Loaders and heavy children mocked; the scorecard
 * composition itself runs REAL (composeFirstAuditScorecard is not mocked).
 */
...
import OnboardDonePage from "./page";
...
describe("/onboard/done", () => {
  ...
  it("a completed crawl celebrates and hides the keep-scanning button", async () => {
    frontierState = scorecardState({
      status: "complete",
      frontier: [],
      pages_crawled: 120,
    ...
    const html = renderToStaticMarkup(await OnboardDonePage());
    expect(html).toContain("I read 120 pages on acme.com.");
    expect(html).not.toContain("Keep scanning now");
  });
