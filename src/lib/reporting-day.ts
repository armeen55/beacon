 ...
 *
 * ONE FIXED ZONE for the whole MVP. A per-account reporting timezone is not in
 * scope: a second definition is exactly the defect this module exists to end.
 *
...
/** The YYYY-MM-DD reporting day for an instant (default: now). */
export function reportingDay(now: number | Date = Date.now()): string {
  const at = new Date(now);
  if (Number.isNaN(at.getTime())) return new Date().toISOString().slice(0, 10);
