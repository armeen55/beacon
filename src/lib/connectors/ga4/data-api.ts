...
/** Same ceiling the GSC client uses; a hung Google response fails one page instead of pinning the sync open. */
 ...
 *
 * Expiry-aware via `evaluateExpiry`: a stale access token is refreshed through the stored refresh token however long it sat idle (only Google's invalid_grant proves a grant dead); a 401 after refresh returns `token_expired`.
 *
...
export async function runGa4UrlTrafficReport(
  ...
  if (evaluateExpiry({ token, now: new Date() }) === "stale") {
    try {
