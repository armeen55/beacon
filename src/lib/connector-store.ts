/**
 * 2026-05-16 — connector-tokens-supabase-and-gsc-scope-split. (Extended 2026-05-18 — Slice 9.A1 — GA4 provider.)
 *
 * Server-only token store for platform connectors, persisted in the Supabase `connector_tokens` table (migration: `migrations/2026-05-16_connector_tokens.sql`).
 *
 ...
 * • Tenant-scoped: every read/write threads `tenantId` through the composite primary key.
 * • Plaintext payload (mirrors the prior on-disk posture). Section 4 F3 lock: encryption-at-rest deferred to a follow-up bundle before broader customer onboarding.
 * • RLS deny-all on the table; service-role bypasses. No customer surface reads this module.
 * • No `writeFileSync` / `renameSync` / `mkdirSync` — pinned by `tests/architecture/connector-store-no-disk-write.test.ts`.
 *
 * Soft-fail rules:
 * • Read paths return `null` when the row is missing OR when the `connector_tokens` table is missing entirely (PostgreSQL `42P01` undefined_table). The undefined-table soft-fail covers the deploy window when code is live but the migration hasn't applied yet.
 * • Write paths throw — OAuth callback persistence MUST fail loud so the operator notices a missing migration / RLS misconfig.
 */
...
export type GoogleConnectorToken = {
  ...
  /** Why the needs-attention marker was stamped (2026-07-12, honest initial state). "streak" = the normal path (>= N failed nightly refresh_runs spanning >= M days). "initial_silence" = the refresh ledger had no (or too little) history yet, so the marker was derived from stored evidence (connected_at / last_synced_at) that this source has been silent far past the freshness window - the copy then says it will start fresh on reconnect, rather than implying a long tracked failure streak. Only meaningful when needs_attention_at is set; absent -> treated as "streak". */
  needs_attention_kind?: "streak" | "initial_silence" | null;
  /** Per-tenant OAuth (2026-07-09), the stable Google account id (`sub` from the id_token) this grant belongs to. Used by the OAuth callback to prove a re-consent is the SAME account before retaining a stored refresh token (never-erase guard). Absent on grants authorized before identity scopes were requested; those simply skip the same-account check until the next connect/replace. Never a secret. */
  google_account_sub?: string;
  /** Per-tenant OAuth (2026-07-09), the Google account email for this grant (from the id_token's `email` claim). Display-only ("Connected as <email>"), never used for API calls. Absent on older grants. */
...
export type ConnectorInfo = {
  ...
  /** Reconnect signal (Google only, 2026-06-15). ISO 8601 timestamp set by the sync path when the last sync ended in an auth failure (dead/revoked refresh token); null/absent when the connection is healthy. Read by `getConnectorHealth` to surface a "Reconnect" state. */
  auth_failed_at?: string | null;
  /** Bounded auth-failure escalation marker (2026-07-11, BUG 2). Set when a Google source failed N nights spanning >= M days without proving the grant dead; carried through so the connectors card can surface a needs-attention line. Null when healthy / on non-Google providers. Distinct from auth_failed_at (proven-dead), which always outranks it. */
  needs_attention_at?: string | null;
  /** The date the failing streak began (ISO 8601); feeds the "since <date>" copy. Only meaningful when needs_attention_at is set. */
  needs_attention_since?: string | null;
  /** Why the marker was stamped: "streak" (tracked failing nights) vs "initial_silence" (derived from stored evidence when the refresh ledger had no history yet). Selects the needs-attention copy. Null when healthy. */
  needs_attention_kind?: "streak" | "initial_silence" | null;
  /** Per-tenant OAuth (2026-07-09), the Google account email for the connected grant (Google connectors only), surfaced as "Connected as <email>" on the connector card. null on older grants (authorized before identity scopes) and on non-Google providers. Never a secret. */
 ...
 *
 * `getConnectorToken` soft-fails to null on TRANSIENT read errors, which is right for status renders but catastrophic for the OAuth callback: a soft-failed read there made "no stored refresh token" indistinguishable from "could not check", and the callback's full-row upsert then OVERWROTE a healthy stored refresh token with an empty string. This read distinguishes the three states so write-path callers can ABORT (no write) on a failed read instead of destroying state:
 * • { ok: true, token }       : the row exists and parsed.
 * • { ok: true, token: null } : provably NO row (including 42P01 table-missing, which genuinely means nothing is stored).
 * • { ok: false }             : the read itself failed; the truth is UNKNOWN. Never treat this as "no token".
 */
...
export async function readConnectorToken(
  ...
  try {
    ...
    // No Supabase env here, so the stored truth is unknowable: report the read as failed and let each caller decide what that honestly means.
  ...
  if (error != null) {
    ...
    // Resilience (2026-06-17): a connector READ must never crash the app, and (2026-08-12) must never be reported as a disconnection either. Log loudly and hand back the failure: status renders show "could not check", write paths (OAuth callback, blank-refresh guard) refuse to write on it.
    console.warn(
...
export async function getConnectorToken(
  ...
  // Soft-fail for consumers that only want a usable token. Anything that SPEAKS to a customer reads getConnectorInfo/getConnectorHealth instead, so an unreadable store is never rendered as a disconnection; write paths use readConnectorToken directly and abort on ok:false.
...
export async function getConnectorInfo(
  ...
  // The tri-state read is the substrate: "provably no row" and "the read itself failed" are DIFFERENT answers, and only the first one is a disconnection.
...
/**
 * Derived per-source health, beyond the binary connected/disconnected the token row stores. The owner-reported trust bug: a source can be `status: "connected"` (a token row exists, no `disconnected_at`) yet deliver ZERO data — because the operator never picked a GA4 property, or never ran a first refresh. A blanket green ✓ for those states is a lie.
 *
 * • "connected"       — connected AND has reliably delivered (or can).
 * • "needs_attention" — connected but provably not yet delivering data, with an actionable plain-English reason.
 * • "not_connected"   — no token / soft-disconnected.
 * • "unknown"         : the check itself failed; nothing is claimed.
 *
 * RELIABILITY NOTE (deliberately conservative — honesty over coverage): `needs_attention` is flagged ONLY from signals provable at render time off the persisted token row (no live HTTP): `auth_failed_at` (a dead grant, the highest priority), GA4 with no property picked, and a genuinely old `last_synced_at`. `expires_at` is never one of them: access tokens expire hourly and the next refresh silently heals it, so only a FAILED refresh (which the syncs persist as `auth_failed_at`) is authoritative.
 */
...
type ConnectorHealthInfo = ConnectorInfo & {
  ...
  /** Plain-English, customer-facing, actionable. null when health is a plain "connected" or "not_connected" with nothing to say. */
  healthReason: string | null;
  /** THE ONE CONNECTED-COUNT RULE, decided here so every counter imports the same answer instead of re-deciding it: a source counts while its data still flows. "needs_attention" COUNTS (a stale or failing grant still serves its cached readings until the token dies, and the card says so on its own); "not_connected" and "unknown" do not (one has no grant, the other has no answer). */
  countsAsConnected: boolean;
};

/** The verdict before that rule is applied (applied in exactly one place, `getConnectorHealth`, so no caller can restate it). */
type ConnectorHealthVerdict = Omit<ConnectorHealthInfo, "countsAsConnected">;

/** Connected-but-no-data is more than this many days stale → soft hint. A SYNC-age liveness threshold ("is this connection still alive"), DISTINCT from the per-source DATA-age SLA (SOURCE_SLA) that decides whether a source's numbers are current. Both live in src/domains/runtime/ops/source-freshness.ts so the old scattered constants can never drift apart again. */
const STALE_DAYS = CONNECTION_LIVENESS_STALE_DAYS;

/** Providers that require a per-source selection before any data can flow. Read off the ONE connector registry (GA4 pulls zero rows until the operator picks a property). */
...
/** Format an ISO timestamp/date as a friendly "June 30" for operator copy. Returns null on an empty/invalid value so callers fall back to date-free wording. UTC so it matches the stored date rather than the server locale. */
...
/** Derive honest health for one provider from its persisted token row. Never throws, never blocks a render: a read failure reports `unknown`, not a disconnection. Tenant-scoping is preserved through `getConnectorInfo`. */
...
async function deriveConnectorHealth(
  ...
  try {
    ...
    // The check threw. Say that, never "not connected": the stored grant is exactly as it was, and the operator's next step is to look again.
  ...
  if (info.status !== "connected") {
    // A disconnected source can still hold FRESH cached data — e.g. a Google OAuth refresh-token expired under testing-mode 7-day expiry, but the last sync landed only days ago and the dashboard is actively rendering that source's recent numbers. A bare "Connect →" then reads as "no data / broken" when data is in fact present. Honest middle state: when there's a genuinely recent last_synced_at (< STALE_DAYS), surface needs_attention ("showing cached data · reconnect to refresh") so the card matches what the page shows. Stale or never-synced → not_connected, as before.
  ...
  // 0. Reconnect (HIGHEST priority — most urgent + most actionable). The sync path stamps `auth_failed_at` when a Google sync TERMINATES in an auth failure (the refresh token is dead/revoked/scope-lost). This is the only authoritative reconnect signal — see the field doc on GoogleConnectorToken. It outranks the GA4-no-property and stale rules: those describe a working connection that just needs a nudge, whereas this is a BROKEN connection that delivers nothing until the operator re-authorizes. Read off info.auth_failed_at (carried through from the token row by getConnectorInfo). Cleared back to null by the next successful sync, which silently returns this to plain "connected".
  ...
  // 0b. Bounded escalation (2026-07-11, BUG 2). The grant is NOT proven dead (auth_failed_at is null, checked above), but this source has failed every sync for days, so its data is silently going stale. Surface a needs-attention line WITHOUT claiming revocation - reconnecting is the usual fix, but we do not assert the login expired because we cannot prove it did.
  if (info.needs_attention_at != null && info.needs_attention_at !== "") {
    const since = formatSinceDate(info.needs_attention_since);
    // Honest initial state (2026-07-12): when the marker was DERIVED from stored evidence because the refresh ledger had no history yet (not a tracked failing streak), be explicit that there is no run history to show and that reconnecting starts a clean record from today - rather than implying we have been watching it fail for a while.
  ...
  // 2. Connected + stale beyond STALE_DAYS → soft, non-alarming hint. Only fires when last_synced_at is a REAL timestamp that is genuinely old. We deliberately do NOT alarm on a null/empty last_synced_at: it is an unreliable "never synced" signal (e.g. GSC can hold 90 days of data with a null marker because the marker predates last-synced stamping), and a false ⚠ on a source that actually has data is worse than staying quiet.
  if (info.last_synced_at != null && info.last_synced_at !== "") {
    const lastSynced = Date.parse(info.last_synced_at);
    if (Number.isFinite(lastSynced)) {
      const days = Math.floor(
...
/**
 * True when the tenant has at least one real data source connected. The single source of truth for "is this a real (non-demo) tenant" used by the shell + the Today gate so a GSC-connected (but never-CSV-imported) tenant sees its real command center, not the connect-prompt.
 *
 * Counts by the ONE rule (`countsAsConnected` on ConnectorHealthInfo), the same one the Today header's source count reads, so the two can never disagree: a stale-but-serving source counts, an unreadable one does not. Fail-soft: a read error counts as not-connected for that provider (never blocks a render). Parallel reads (cached tokens).
 */
...
export async function saveConnectorToken(
  ...
  const isGoogle =
    ...
    token.provider === "google_ga4";
  if (isGoogle) {
    // DB-SIDE never-erase guard (operator guardrail, 2026-07-09): Google token saves go through ONE atomic SQL statement (save_connector_token_guarded_v1, mode 'connect') that preserves a stored non-empty refresh_token inside the upsert itself and refuses (raises, statement rolls back) when neither side has one. An app-side read-then-merge can be raced by two concurrent callbacks or two Vercel instances; the database cannot.
    ...
    // The guarded RPC is not installed on this database (fresh install, or the migration is not applied yet). Fall back to the app-side guarded path below, LOUDLY, so a missing migration can never brick a connect.
    console.warn(
    ...
    // NEVER-ERASE assertion (app-side fallback; also defense in depth behind the OAuth callback's own guard): an EMPTY refresh_token must never replace a stored non-empty one, under ANY code path. Google refresh tokens are only minted at consent; overwriting one with "" bricks the grant silently (the access token dies in about an hour with no self-heal). If the stored row cannot be READ, the truth is unknown, so the write is refused too.
    if (token.refresh_token === "") {
      ...
      // Parity with the RPC's fail-closed rule: a Google row must never be WRITTEN with an empty refresh_token at all (it would die in about an hour with no self-heal). Nothing usable is stored either, so refuse.
      throw new Error(
...
/** PostgREST "function not found" (PGRST202, a stale schema cache PGRST205, or raw Postgres 42883) - the ONLY errors allowed to route a guarded write to the app-side fallback. Every other error must surface to the caller. */
...
// NOTE (2026-07-09, review P1-1): `refresh_token` is DELIBERATELY excluded from this patch type. A rotated refresh token must be persisted through `persistRefreshedGoogleToken` (guarded mode 'refresh', the cross-instance compare-and-swap), never via `updateConnectorToken` — the patch path is UPDATE-only, strips refresh_token in SQL, and throws at runtime if a caller includes one, so no future caller can reintroduce the read-merge-write race that resurrected a stale refresh token / dropped a rotation.
type GoogleConnectorPatch = Partial<
  Pick<
    ...
    // Slice 9.A1 (2026-05-18) — GA4 property selection patches the existing google_ga4 token payload after the operator picks a property in the settings UI.
    | "ga4_property_id"
    | "ga4_property_display_name"
    | "ga4_account_display_name"
    // J5 (2026-05-18) — soft-disconnect / reconnect flow patches the payload's disconnected_at without touching the OAuth tokens.
    | "disconnected_at"
    // 2026-06-15 — auth-failure reconnect signal, set on sync auth failure / cleared on sync success by the GSC + GA4 sync paths.
    | "auth_failed_at"
    // 2026-07-11 (BUG 2), bounded escalation marker, set by the nightly auth-escalation check after N failing nights, cleared on sync success.
    | "needs_attention_at"
...
export async function updateConnectorToken(
  ...
  const isGoogle =
    ...
    provider === "google_ga4";
  if (isGoogle) {
    // DB-SIDE patch (operator guardrail, 2026-07-09; review finding P1-1): a Google field patch (last_synced_at, ga4_property_id, disconnected_at, auth_failed_at, selected_location_*) goes through ONE atomic SQL statement (save_connector_token_guarded_v1, mode 'patch') that merges the patch onto the stored row WITHOUT ever touching refresh_token. The old app-side read-then-merge-then-full-upsert could be raced by two Vercel instances: one reads a row, another rotates the refresh_token, then the first writes the stale row back, resurrecting a dead refresh_token and dropping the rotation. p_payload is the PATCH ONLY (not a merged full row).
    //
    // REFUSE refresh_token here (no future caller reintroduces the race): a rotated refresh_token is the ONE thing a patch must never carry. The SQL strips it defensively, but a patch that thinks it can write one is a bug. Rotations go through persistRefreshedGoogleToken (mode 'refresh', the cross-instance compare-and-swap).
    ...
    // The guarded RPC is not installed on this database (fresh install, or the patch-mode migration is not applied yet). Fall back to the app-side read-merge-write LOUDLY so a missing migration can never brick a patch. This fallback keeps the pre-guard behavior: it merges onto the stored row (which retains its refresh_token) and saves via the connect-guarded path.
    console.warn(
...
/**
 * FIX 3 (OAUTH_ROOT_CAUSE_2026-07-09): persist a refreshed Google token best-effort. Writes ONLY the changed fields via `updateConnectorToken` (merge, not full-row upsert) so a rotated refresh_token — when Google returned one — is never lost, and the rest of the payload (scopes, connected_at, disconnected_at, ga4_property_id, ...) is never clobbered. The refresh_token field is included ONLY when Google actually rotated it, so this never overwrites the stored token with an empty value.
 *
 * Fail-soft: a persist error is logged and swallowed — the in-memory token still serves the current run (mirrors the existing per-caller persist posture). Callers that already write `access_token`/`expires_at` themselves (with additional fields like `last_synced_at`) keep doing so; this helper is for the refresh sites that either persisted nothing or want a one-call rotation-safe write.
 */
export async function persistRefreshedGoogleToken(
  ...
  try {
    ...
    // DB-SIDE compare-and-swap (operator guardrail, 2026-07-09): the RPC's 'refresh' mode UPDATEs only where the incoming expires_at is strictly newer than the stored one. That conditional is the CROSS-INSTANCE race resolution: the in-process single-flight in google-auth.ts only dedupes within one lambda, so two Vercel instances refreshing concurrently must resolve at the database, and the staler write silently no-ops. It also never inserts and only touches refresh_token when Google rotated it.
    const admin = getSupabaseAdmin();
    ...
    // RPC not installed: app-side fallback mirrors the same rules (loudly). Monotonic guard first, so even the fallback cannot clobber a FRESHER stored token with this staler one; rotation (a new refresh_token) always persists because its expiry is by construction the newest.
    console.warn(
    ...
    // Write directly via saveConnectorToken, NOT updateConnectorToken: this is the ONE path allowed to persist a rotated refresh_token, and updateConnectorToken now refuses that field (it routes through the patch-mode RPC, which strips refresh_token in SQL). We already hold the read + the monotonic CAS guard above, so merge and save here — the merged row keeps its stored (or freshly-rotated) refresh_token, so the connect-guarded save accepts it.
