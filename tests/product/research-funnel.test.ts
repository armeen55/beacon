...
import { CONFLICT_DETAIL, type FunnelDeps } from "@/domains/evidence/funnel/shared"; import type { AiObservationRecord, CanonicalPairObservation, DueObservation } from "@/domains/evidence/ai-visibility/ai-observations"; import type { PromptAnswerObservation } from "@/domains/evidence/ai-visibility/prompt-answer-observations";
...
describe("research funnel - basis-scoped discovery + isolation", () => {
  ...
  const prompts = [{ id: "p1", text: "best persian restaurant" }, { id: "p2", text: "where to buy saffron" }]; function deps(store: ReturnType<typeof memStore>, rows: PromptAnswerObservation[], claudeModel: string): FunnelDeps {
    ...
    const gem = st.prompts.pairs.find((p) => p.engine === "gemini" && p.status === "done")!; expect(gem.citations).toBeNull(); expect(gem.citationsObserved).toBe(false); const perpRow = rows.find((r) => r.platform === "perplexity")!, gemRow = rows.find((r) => r.platform === "gemini")!; expect(perpRow.citation_urls).toEqual([]); expect(perpRow.metadata.citationsObserved).toBe(true); expect(perpRow.metadata.observationMode).toBe("standardized_response"); expect(gemRow.citation_urls).toBeNull(); expect(gemRow.metadata.citationsObserved).toBe(false); expect(rows.some((r) => (r.search_queries ?? []).length > 0)).toBe(true); });
  ...
  it("gives a changed served model its OWN history row, never a merge", async () => { const rows: PromptAnswerObservation[] = []; const store = memStore(); await promptObservationUnit(deps(store, rows, "claude-3-5-sonnet"), plan(prompts))("tp", cur(), 60_000);
    await promptObservationUnit({ ...deps(store, rows, "claude-4"), now: () => 5_000_000 + 24 * 3600 * 1000 }, plan(prompts, DAY_B))("tp", cur(), 60_000); expect(new Set(rows.filter((r) => r.platform === "claude" && r.prompt_id === "p1").map((r) => r.id)).size).toBe(2); expect(rows.filter((r) => r.platform === "chatgpt").every((r) => r.metadata.scraper === true && r.metadata.observationMode === "consumer_search")).toBe(true); }); // pre-6I readers still see the flag
