...
describe("research funnel - SERP current set, freshness, and recovery", () => {
  ...
  it("reserves each priority search THREE DISTINCT PUBLISHERS, admits ONE substitute only for the search whose own page was unreadable, and never exceeds eighteen page attempts", async () => { const s = emptyFunnelState("tw", BASIS); const at = new Date(NOW).toISOString(); const PRIORITY = ["iranian actors", "iranian films", "iranian food"]; const q = (query: string, hosts: string[]) => ({ query, cacheKey: null, status: "done" as const, observedAt: at, organic: hosts.map((h, i) => ({ rank: i + 1, url: `https://${h}/p`, domain: h, title: null })) });
    ...
    expect((await run([], /never/)).urls).not.toContain("https://en.wikipedia.org/p"); const all = await run(PRIORITY, /wikipedia|[bcd]\.com|[fg]\d/); expect([all.tried.slice(0, 3), all.tried.slice(9, 12)]).toEqual([["https://en.wikipedia.org/p", "https://b.com/p", "https://c.com/p"], ["https://d.com/p", "https://f4.com/p", "https://g4.com/p"]]); // global weight alone gave the searches under investigation NOTHING; named, each keeps ONE reserve slot per publisher (the second wikipedia page never eats the third opinion's slot) and FOCUS COMES BEFORE BREADTH: its own substitute is read before any exploration page
