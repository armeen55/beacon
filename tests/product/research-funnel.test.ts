...
const serp = (organic: (Omit<ParsedSerp["organic"][number], "snippet"> & { snippet?: string | null })[]): ParsedSerp => ({ organic: organic.map((o) => ({ snippet: null, ...o })), aiOverview: null, paaQuestions: [], relatedSearches: [] });
...
const err = (disposition: FailureDisposition, cacheKey: string | null = "ck-old"): CachedCallResult => ({ state: "error", cacheKey, disposition, detail: "the provider could not finish it" }); function memStore(seed?: FunnelState, history: CanonicalPairObservation[] = []) {
...
describe("research funnel - basis-scoped discovery + isolation", () => {
  ...
  it("a blocked POST stops the batch on the FIRST refusal, leaves every row exactly as it was, and pauses the run for review", async () => { const store = memStore(); let calls = 0; const pairs = () => store.peek("tp", BASIS)!.prompts.pairs; const d = mk(store, { callProvider: async () => { calls += 1; return err("blocked", "ck-blocked"); } });
    ...
    const store = seedWith([{ promptId: "p1", engine: "chatgpt", mode: "consumer_search", day: DAY_A, cacheKey: "ck-q", status: "unsupported", observedAt: new Date(NOW).toISOString() }]); const rows: AiObservationRecord[] = []; let touched = 0; const boom = async () => { touched += 1; return err("blocked"); }; // Aug 4: two quarantined pairs were terminal in the funnel state and still `failed` on the canonical row. Terminalizing happens where the disposition LANDS, and a pair already set aside never crosses that code again (not posted, so the collect skips it; not pending, so the post skips it), so the whole-day gate read 138 of 140 on every window, for four fires.
    ...
    rows.length = 0; await unit(d, [{ promptId: "p2", version: 1, text: "where to buy saffron", engine: "gemini", slot: 0, day: DAY_A }]); expect(rows.some((r) => r.prompt_id === "p1")).toBe(false);}); // AND ONLY WHILE THE PLANNER STILL SAYS IT IS OWED: an identity whose stored row is already terminal never reaches this plan, so nothing is rewritten and nothing is bought.
...
describe("research funnel - SERP current set, freshness, and recovery", () => {
  ...
  it("plans from historical canonical fan-outs, ignores transient pairs, and stops before any purchase when history is unreadable", async () => {
    const seed = retainedState([]), store = memStore(seed, [canon({ fanOutQueries: ["saffron grades"] }), canon({ reportingDay: DAY_B, fanOutQueries: ["saffron storage"] })]);
    seed.prompts.pairs = [{ promptId: "transient", promptText: "unrelated computer parts", engine: "chatgpt", status: "done", cacheKey: null }];
    const asked: string[] = []; const deps: FunnelDeps = { ...store.deps, ...serpBase, getAccount: async () => ({ domain: "own.example" } as Account), callProvider: async (_cap, input) => { asked.push(String((input as { keyword: string }).keyword)); return waiting("paid-key"); } };
    await serpAnalysisUnit(deps)("ts", cur(), 60_000); expect(asked).toEqual(expect.arrayContaining(["saffron grades", "saffron storage"])); expect(asked).not.toContain("unrelated computer parts");
    const before = structuredClone(store.peek("ts", BASIS)); asked.length = 0; deps.loadCanonicalObservations = async () => { throw new Error("history unavailable"); };
    expect((await serpAnalysisUnit(deps)("ts", cur(), 60_000)).status).toBe("failed"); expect((await winningPagesUnit(deps)("ts", cur(), 60_000)).status).toBe("failed"); expect(asked).toEqual([]); expect(store.peek("ts", BASIS)).toEqual(before);
  });
  ...
  it("keeps every row, its words, the answer box, the PAA answers and the overview the look paid for, projects them all, tells an outstanding overview from a reported none, and buys the overview only on a search that was named", async () => {
    ...
    const full: ParsedSerp = { ...serp(Array.from({ length: 20 }, (_v, i) => ({ rank: i + 1, domain: `d${i}.com`, url: `https://d${i}.com/x`, title: `T${i}`, snippet: `what d${i} says` }))),
      itemTypes: ["organic", "featured_snippet", "people_also_ask"], featuredSnippet: { url: "https://d0.com/x", domain: "d0.com", title: "T0", text: "the answer box's own words" },
      paaQuestions: [{ question: "who wins this", answeringDomain: "d3.com", answer: "d3 answers it this way" }], aiOverview: { present: true, references: [], excerpt: "the overview's own words", asynchronous: false } };
    ...
    const st = store.peek("ts", BASIS)!, named = st.serps.queries.find((q) => q.query === "named search")!;
    expect([named.organic!.length, named.organic![19]!.snippet, named.featured?.text, named.paa![0]!.answer, named.paa![0]!.answeringDomain, named.aiOverviewText, named.aiOverviewState, named.itemTypes]).toEqual([20, "what d19 says", "the answer box's own words", "d3 answers it this way", "d3.com", "the overview's own words", "observed", ["organic", "featured_snippet", "people_also_ask"]]);
    const row = projectFunnelEvidence(st, NOW).serpEvidence.find((e) => e.query === "named search")!; // the projection carries every one of them, or the writer's packet never sees a word of it
    expect([row.organic.length, row.organic[0]!.snippet, row.featured?.text, row.paa[0]!.answer, row.aiOverviewText, row.aiOverviewState, row.itemTypes]).toEqual([20, "what d0 says", "the answer box's own words", "d3 answers it this way", "the overview's own words", "observed", ["organic", "featured_snippet", "people_also_ask"]]);
    const overviewOf = async (aiOverview: ParsedSerp["aiOverview"]) => { const s2 = memStore(retainedState([])); await serpAnalysisUnit({ ...s2.deps, ...serpBase, loadPageQueries: async () => [{ query: "one search", impressions: 900 }], callProvider: async () => ok({ ...serp([]), itemTypes: ["organic"], aiOverview }) })("ts", cur(), 60_000);
      return projectFunnelEvidence(s2.peek("ts", BASIS)!, NOW).serpEvidence[0]!; };
    expect([(await overviewOf({ present: true, references: [], excerpt: null, asynchronous: true })).aiOverviewState, (await overviewOf(null)).aiOverviewState]).toEqual(["pending", "absent"]); // a stub still being fetched is never an observed absence
  ...
  it("attributes each winning page to its OWN engines/prompts, excludes the own domain, and reuses a cached extract before any fetch", async () => { const s = emptyFunnelState("tw", BASIS); s.discovery.counts = { raw: 10, normalized: 8, retained: 1, rejected: 0 };
    ...
    s.winningPages = [{ url: "https://a.com/x", domain: "a.com", engines: [], examplePrompts: [], appearances: [], extract: null }]; // q1 already holds a page of its own, so the GLOBAL WEIGHT ORDER alone decides this read set; the reserve a search nothing has read earns is pinned on its own below
    const cite = { url: "https://b.com/y", domain: "b.com", title: null }; // the SAME citation seen through both ChatGPT modes
    const history = (["standardized_response", "consumer_search"] as const).map((observationMode) => canon({ observationId: `obs_${observationMode}`, promptId: "pr", promptText: "best persian restaurant", observationMode, modelServed: "gpt-4o", citations: [cite] })); history.push(canon({ observationId: "obs_second_day", promptId: "pr", promptText: "best persian restaurant", reportingDay: DAY_B, citations: [cite] }));
    const store = memStore(s, history); let fetchCalls = 0; const out = await winningPagesUnit({ ...store.deps, loadProfile: async () => emptyBusinessProfile("tw"), getAccount: async () => ({ domain: "own.com" } as Account), now: () => NOW,
    ...
    const answers = win[0]!.appearances.filter((a) => a.kind === "ai_answer"); expect(answers.map((a) => [a.observationMode, a.reportingDay, a.observationId])).toEqual([["consumer_search", DAY_A, "obs_consumer_search"], ["consumer_search", DAY_B, "obs_second_day"]]);
  });
...
it("reserves each priority search FIVE RANKED PAGES, admits ONE substitute only for the search whose own page was unreadable, and never exceeds eighteen page attempts", async () => { const s = emptyFunnelState("tw", BASIS); const at = new Date(NOW).toISOString(); const PRIORITY = ["iranian actors", "iranian films", "iranian food"]; const q = (query: string, hosts: string[]) => ({ query, cacheKey: null, status: "done" as const, observedAt: at, organic: hosts.map((h, i) => ({ rank: i + 1, url: `https://${h}/p`, domain: h, title: null })) });
s.serps.queries = [q(PRIORITY[0]!, ["en.wikipedia.org", "simple.wikipedia.org", "b.com", "c.com", "d.com", "e.com", "h.com"]), q(PRIORITY[1]!, ["f1.com", "f2.com", "f3.com", "f4.com", "f5.com", "f6.com", "f7.com"]), q(PRIORITY[2]!, ["g1.com", "g2.com", "g3.com", "g4.com", "g5.com", "g6.com", "g7.com"])]; const history = Array.from({ length: 18 }, (_, i) => canon({ observationId: `obs_${i}`, promptId: `pr${i}`, promptText: "q", engine: "chatgpt", observedAt: at, citations: [{ url: `https://ai${i}.com/p`, domain: `ai${i}.com`, title: null }] }));
    s.winningPages = ["en.wikipedia.org", "f1.com", "g1.com"].map((h) => ({ url: `https://${h}/p`, domain: h, engines: [], examplePrompts: [], appearances: [], extract: null })); // each of the three searches already holds ONE page on file, so what is measured here is the global weight order and nothing else
    const run = async (priority: string[], unreadable: RegExp) => { const st = memStore(s, history); const tried: string[] = []; // tried = every page I actually went out and read, in the order I read them
    ...
    const none = await run([], /never/); expect([none.tried.length, none.tried.some((u) => u.includes("simple.wikipedia.org")), none.tried.some((u) => u.includes("ai"))]).toEqual([15, true, false]); // a partial competitor set still owes the other exact leaders, not unrelated exploration
    const all = await run(PRIORITY, /wikipedia|[bcdeh]\.com|[fg]\d/); expect([all.tried.slice(0, 3), all.tried.slice(15)]).toEqual([["https://en.wikipedia.org/p", "https://f1.com/p", "https://g1.com/p"], ["https://e.com/p", "https://f6.com/p", "https://g6.com/p"]]); // five ranked pages per case, interleaved; one substitute per blocked case
    expect([all.tried.length, all.tried.some((u) => u.includes("ai")), all.urls.some((u) => /f7|g7|h\.com/.test(u))]).toEqual([18, false, false]); // the fixed attempt ceiling includes the substitutes, never a second bench page
    ...
    const one = await run(PRIORITY, /wikipedia/); expect([one.tried.length, one.tried.includes("https://d.com/p"), one.tried.some((u) => /f4|f5|g4|g5/.test(u))]).toEqual([16, true, true]); }); // every exact leader is reserved; only the blocked case earns one substitute
  it.each([{ t: "tq", tag: "quarry", q: "granite quarry tools" }, { t: "ta", tag: "aviary", q: "aviary netting sizes" }])("reads the pages of a search nothing has ranked without being asked for it, and leaves a search that already holds one to the global order, on $t", async (h) => {
    const at = new Date(NOW).toISOString(), u = (n: string, k = 0) => `https://${n}${k || ""}-${h.tag}.example/p`, named = ["first", "second", "third"], FORUM = "https://a.reddit.com/p";
    const seed = (represented: boolean, searches = 1) => { const s = emptyFunnelState(h.t, BASIS);
      s.serps.queries = Array.from({ length: searches }, (_, k) => ({ query: `${h.q}${k || ""}`, cacheKey: null, status: "done" as const, observedAt: new Date(Date.parse(at) - k * 86_400_000).toISOString(), organic: named.map((n, i) => ({ rank: i + 1, url: u(n, k), domain: `${n}${k || ""}-${h.tag}.example`, title: null })) }));
      const history = Array.from({ length: 18 }, (_, i) => canon({ observationId: `obs_${i}`, promptId: `p${i}`, promptText: "q", engine: "chatgpt", observedAt: at, citations: [{ url: i ? `https://ai${i}.example/p` : FORUM, domain: i ? `ai${i}.example` : "a.reddit.com", title: null }] }));
      if (represented) s.winningPages = [{ url: u("first"), domain: `first-${h.tag}.example`, engines: [], examplePrompts: [], appearances: [], extract: null }]; return { s, history }; };
    const run = async (represented: boolean, searches = 1) => { const seeded = seed(represented, searches), st = memStore(seeded.s, seeded.history), tried: string[] = [];
      await winningPagesUnit({ ...st.deps, loadProfile: async () => emptyBusinessProfile(h.t), getAccount: async () => ({ domain: "own.example" } as Account), now: () => NOW, parse, readPageExtract: async () => null, writePageExtract: async () => {},
        fetchPage: (async (url: string) => { tried.push(url); return { ok: true, html: "<html><body><h1>H</h1><p>what this page says about its own subject</p></body></html>", status: 200 }; }) as unknown as FunnelDeps["fetchPage"] })(h.t, cur(), 60_000);
      return { tried, urls: st.peek(h.t, BASIS)!.winningPages.map((w) => w.url) }; };
    const owed = await run(false); expect([named.map((n) => u(n)).every((x) => owed.tried.includes(x)), owed.urls.filter((x) => x.includes(h.tag)).length, owed.tried.slice(0, 3)]).toEqual([true, 3, named.map((n) => u(n))]); // nobody asked for this search and its pages are read anyway, first, ahead of every page that has been winning for weeks
    const held = await run(true); expect([held.tried.some((x) => x.includes(h.tag)), held.tried.every((x) => x.includes("ai")), held.tried.includes(FORUM), held.tried.length]).toEqual([true, false, false, 15]); // one stored competitor does not discharge the other leaders; noise and the total read ceiling are unchanged
    const four = await run(false, 4); expect([four.tried.length, [0, 1, 2].every((k) => named.every((n) => four.tried.includes(u(n, k)))), named.some((n) => four.tried.includes(u(n, 3)))]).toEqual([15, true, false]); }); // FOUR searches owe a reading, one pass reserves for the three newest, and the NINE pages it reads are exactly the nine the runtime receipt counts: the fourth search's pages are left to the global order, lose it, and are promised to nobody
  it.each([{ t: "tk", tag: "kiln", q: "kiln firing schedule" }, { t: "tv", tag: "vellum", q: "vellum preparation" }])("keeps a reading the next ranking no longer favours, drops one whose search left the file, and still reserves the newest search nothing has ranked, on $t", async (h) => {
    const on = (k: number) => new Date(NOW - k * 86_400_000).toISOString(), u = (n: string) => `https://${n}-${h.tag}.example/p`;
    const search = (name: string, k: number, hosts: string[]) => ({ query: `${name} ${h.q}`, cacheKey: null, status: "done" as const, observedAt: on(k), organic: hosts.map((x, i) => ({ rank: i + 1, url: u(x), domain: `${x}-${h.tag}.example`, title: null })) });
    const s = emptyFunnelState(h.t, BASIS); s.serps.queries = [search("older", 3, ["a1", "a2", "a3"]), search("kept", 2, ["b1", "b2", "b3"])];
    const history = Array.from({ length: 18 }, (_, i) => canon({ observationId: `obs_${i}`, promptId: `p${i}`, promptText: "q", engine: "chatgpt", observedAt: on(0), citations: [{ url: `https://ai${i}.example/p`, domain: `ai${i}.example`, title: null }] }));
    const st = memStore(s, history), tried: string[] = [];
    const pass = async () => { tried.length = 0;
      await winningPagesUnit({ ...st.deps, loadProfile: async () => emptyBusinessProfile(h.t), getAccount: async () => ({ domain: "own.example" } as Account), now: () => NOW, parse, readPageExtract: async () => null, writePageExtract: async () => {},
        fetchPage: (async (url: string) => { tried.push(url); return { ok: true, html: `<html><body><main><h1>H</h1><p>what this page says about its own subject at ${url}</p></main></body></html>`, status: 200 }; }) as unknown as FunnelDeps["fetchPage"] })(h.t, cur(), 60_000);
      return new Map(st.peek(h.t, BASIS)!.winningPages.map((w) => [w.url, (w.extract?.mainText ?? "").length > 0])); };
    const first = await pass(); expect([u("a1"), u("b1")].map((x) => first.get(x)), `${h.t}: the pass that holds no reading reserves one for each search nothing has ranked`).toEqual([true, true]);
    const second = await pass(); expect([tried.every((x) => x.includes("ai")), ["a1", "a2", "a3", "b1", "b2", "b3"].map((n) => second.get(u(n)))], `${h.t}: the next ranking puts eighteen cited pages above every page of both searches and reads only those, and not one reading is thrown away by that order`).toEqual([true, [true, true, true, true, true, true]]);
    st.peek(h.t, BASIS)!.serps.queries = [search("kept", 2, ["b1", "b2", "b3"]), search("newest", 0, ["c1", "c2", "c3"])];
    const third = await pass(); expect([["a1", "a2", "a3"].some((n) => third.has(u(n))), third.get(u("b1")), ["c1", "c2", "c3"].every((n) => tried.includes(u(n)))], `${h.t}: a reading whose only search left the file goes with it, a reading whose search is still on file stays, and the search nothing has ranked is still reserved its own reads`).toEqual([false, true, true]); });
...
const body = (url: string) => url.includes("h3") ? { h1: "H", faqCount: 0, headings: [], title: "T", wordCount: 5 } : url.includes("h4") ? { ...full, title: "CHANGED" } : url.includes("h6") ? { ...full, fetchedAt: PROV } : url.includes("h8") ? { ...full, wordCount: 0 } : full; // the same body in another key order, a changed title, the provider's own fetch time, and no words at all
...
expect([paid, ["no.com", "h8.com", "h9.com"].map((h) => [row(h).extract, row(h).readOutcome!.state])]).toEqual([["h2.com", "h3.com", "h4.com", "h5.com", "h6.com", "h8.com"].map(u), [[null, "robots_blocked"], [null, "provider_unavailable"], [null, "temporarily_unavailable"]]]); // a robots denial is NEVER sent through the provider, an ordinary refusal earns exactly ONE paid read and the sixth is the last a cycle buys; the ranked URL survives either way and a denial, an empty parse and a spent ceiling are each named honestly
  ...
  it("consults the cache for every ranked row the look bought, re-reads a banked prefix once and holds the whole reading, keeps rows a short drive never reached, and keeps a fresh reading while its search is re-bought", async () => {
    const at = new Date(NOW).toISOString(), s = emptyFunnelState("tc", BASIS), u = (n: number) => `https://a${String(n).padStart(2, "0")}.example/p`, deep = `<html><body><main><h1>H</h1><h2>Deep</h2><p>${"word ".repeat(3_000)}</p><h2>Deeper</h2><p>the deep section words</p></main></body></html>`;
    s.serps.queries = [{ query: "q", cacheKey: null, status: "done", observedAt: at, organic: Array.from({ length: 20 }, (_, i) => ({ rank: i + 1, url: u(i + 1), domain: `a${i + 1}.example`, title: null })) }]; // twenty rows bought; the reserve takes ranks 1 to 5 and the global fill runs by address, so a20 sits past the fifteen fetch slots
    const banked = new Map<string, Record<string, unknown>>([[u(1), { title: "PREFIX", h1: null, wordCount: 5, headings: [], mainText: "x".repeat(12_000), truncated: true, heldChars: 12_000, totalChars: 30_000 }], [u(20), { title: "CACHED20", h1: null, wordCount: 5, headings: [], mainText: "the twentieth page's own words", truncated: false, heldChars: 30, totalChars: 30 }]]);
    const st = memStore(s), tried: string[] = [], pass = async (budgetMs = 60_000, priority: string[] = []) => { tried.length = 0; return winningPagesUnit({ ...st.deps, loadProfile: async () => emptyBusinessProfile("tc"), getAccount: async () => ({ domain: "own.example" } as Account), now: () => NOW, parse,
      readPageExtract: async (url: string) => { const e = banked.get(url); return e ? { extract: e, contentHash: "h", fetchedAt: at } : null; }, writePageExtract: async (url: string, extract: Record<string, unknown>) => { banked.set(url, extract); },
      fetchPage: (async (url: string) => { tried.push(url); return { ok: true, html: deep, status: 200 }; }) as unknown as FunnelDeps["fetchPage"] }, priority)("tc", cur(), budgetMs); };
    await pass(); const rows = () => st.peek("tc", BASIS)!.winningPages, row = (n: number) => rows().find((w) => w.url === u(n))!, whole = banked.get(u(1))!;
    expect([tried.length, tried.includes(u(1)), tried.includes(u(20)), row(20).extract!.title, rows().length], "fifteen fetches including the prefix re-read; the cached twentieth row lands with its reading and no fetch").toEqual([15, true, false, "CACHED20", 16]);
    expect([whole.heldChars === whole.totalChars && (whole.totalChars as number) > 12_000, whole.truncated, row(1).extract!.mainText!.length, row(1).extract!.truncated, row(1).extract!.sections!.map((c) => [c.heading, c.text.length <= 1_200, c.text])], "the cache holds the whole reading; the row holds the comparison ceiling of the text, says it is cut, and carries only the sections past that prefix, each cut to the window the comparison opens").toEqual([true, false, 12_000, true, [["Deeper", true, "the deep section words"]]]);
    await pass(); expect(tried, "only last pass's two bench pages are read now; a reading at the crawler's own ceiling is never re-read for length").toEqual([u(6), u(7)]);
    banked.delete(u(3)); await pass(33_000); expect([tried, rows().length, rows().every((w) => !!w.extract?.mainText)], "a short drive reads at most one page and never shrinks the projection: the rows it never reached stay as they were").toEqual([[u(3)], 18, true]);
    st.peek("tc", BASIS)!.serps.queries = [{ query: "q", cacheKey: "ck", status: "posted", organic: [] }]; await pass(); expect([rows().length, row(1).extract!.mainText!.length], "while the search is re-bought its winners keep their fresh readings instead of vanishing until the look lands").toEqual([18, 12_000]);
    st.peek("tc", BASIS)!.serps.queries = []; await pass(); expect(rows(), "a reading whose search left the agenda goes with it").toEqual([]); });
  const OWN = "https://own.com/p", W1 = "https://a.com/x", W2 = "https://b.com/g", ASK = { topicKey: "t1", ask: { pages: [W2, W1], exclude_pages: [OWN] } }; // ── the ONE paid page-by-page comparison the winners earn ──
  ...
  it("buys ONE comparison for the WHOLE page set, never buys a landed one twice, and a resumed crash costs nothing", async () => { const asks: unknown[] = [];
    ...
    expect([winners.status, winners.cursor, asks.length]).toEqual(["advanced", { stage: "compare" }, 0]); const heldComparison = await winningPagesUnit(cmpDeps(st, call), [], ASK, OWN)("tx", { ...cur(), ...winners.cursor, freeOnly: true }, 60_000); expect([heldComparison.status, heldComparison.cursor, asks.length, st.peek("tx", BASIS)!.pageComparisons]).toEqual(["waiting", { stage: "compare" }, 0, []]);
  ...
  const run = async (store: ReturnType<typeof memStore>, now: number, answer: unknown, ownedUrl: string | null = U, tenant = "to", over: Partial<FunnelDeps> = {}, bodies = new Map<string, ReturnType<typeof captured>>(), budgetMs = 25_000) => { const tried: string[] = [], saved: string[][] = [], paid: string[] = [];
    const out = await winningPagesUnit({ ...store.deps, loadProfile: async () => emptyBusinessProfile(tenant), getAccount: async () => ({ domain: "own.com" } as Account), now: () => now, parse, readPageExtract: async () => null, callProvider: (async (cap: CapabilityKey) => { paid.push(cap); return ok(serp([])); }) as FunnelDeps["callProvider"],
      fetchPage: (async (url: string) => { tried.push(url); return answer; }) as unknown as FunnelDeps["fetchPage"], readOwnedBodies: async (_t, urls, misses) => { for (const url of urls) if (!bodies.has(canonicalUrlKey(url))) misses?.set(canonicalUrlKey(url), "no_capture"); return bodies as never; }, readIncompleteAttempt: async () => null, writeOwnedPage: async (snap, t) => { saved.push([snap.id, snap.url, t]); bodies.set(canonicalUrlKey(snap.url), captured(now)); }, ...over }, [], null, ownedUrl, null, null, ownedUrl !== null)(tenant, cur(), budgetMs);
  ...
  const THEMES = ["baby names", "flag history", "national holidays", "given name origins"]; // A GENERIC reference site: no vertical token is privileged anywhere in the gate.
  ...
  it("checks a tracked question through its observed fan-out, else through its own approved words, and says which is which", () => {
    ...
    expect(out.sources).toEqual({ "most popular baby names 2026": "fan_out", "which flags changed in 1979": "prompt" }); }); // Both enter the agenda, neither wearing the other's name: a search the engine ran is not a question I asked.
  ...
  it("buys the searches an open question is stuck on FIRST, verbatim, and never more than its bound", () => {
    ...
    expect(out.queries[4]).toBe("female baby names"); expect(agenda({ priorityQueries: Array.from({ length: 12 }, (_, i) => `stuck query ${String(i).padStart(2, "0")}`) }, 40).queries.filter((q) => q.startsWith("stuck query"))).toHaveLength(12); // every stuck search the caller names is bought; only the 40 runaway stop remains
...
describe("research funnel - the CASE-SCOPED keyword universe", () => {
  ...
  const disc = (store: ReturnType<typeof memStore>, over: Partial<FunnelDeps> = {}, plan = PLAN) => keywordDiscoveryUnit({
    ...
    callProvider: async (cap: CapabilityKey) => ok(cap === "labs_ranked_keywords" ? ranked([["price saffron", "https://own.com/a", 4], ["price saffron", "https://own.com/b", 9], ["saffron threads", "https://own.com/t", 7], ["saffron threads", "https://own.com/t", 2]]) : parsedKw([])), ...over }, plan)("tc", cur(), 60_000); // The last two rows are ONE page of mine reported twice for one search: rows, not pages.
...
describe("research funnel - the journey behind a keyword", () => {
  ...
  it("traces a BOUGHT candidate back to the pull it arrived on and the theme it was asked for", async () => {
    const store = memStore(seeded()); // The free routes each carry their lineage. The paid ones are where the money went, and a keyword that arrives with no journey at all can never tell the operator which request they paid for produced it.
  ...
  it("stamps the fingerprint of the readings it actually harvested, moves it when a reading lands, and leaves it alone when the read fails", async () => {
    const store = memStore(seeded()); const wm = () => store.peek("tj", BASIS)!.discovery.consumedAnalyses; const held = [ask(), canon({ observationId: "obs_read", promptId: "p9", analysis: { topicEntities: ["iranian saffron"] }, analysisHash: "h9" })]; // A verdict stored on an answer already on file moved no keyword and no decision, so nothing could see it as owed. This watermark is what the debt is measured against.
  ...
  it("keeps everything it already found when the answers cannot be read, and never calls that a clean pass", async () => {
    const said: string[] = []; const spy = vi.spyOn(console, "warn").mockImplementation((...a: unknown[]) => void said.push(String(a[0]))); // The old source was the pass's own memory and could not fail. This one is a table read that times out, and a failed read used to wipe every keyword it could not re-derive.
  ...
  it("counts the arrivals it dropped ONCE, however many passes read the same eight", async () => {
    const eight = Array.from({ length: 8 }, (_, i) => ask({ observationId: `obs_${i}`, promptId: `q${i}`, promptText: `question ${i}` })); const after = (store: ReturnType<typeof memStore>) => { const r = rowsOf(store).get(FAN)!; return [r.origins!.length, r.moreOrigins]; }; // The count used to be the largest carried count PLUS this pass's own overflow, so an arrival dropped by one pass and recovered by the next was counted twice: eight real arrivals read as ten, then twelve.
