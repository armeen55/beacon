...
vi.mock("@/app/(shell)/worklist-data", () => ({ refreshWorklistSurface: async () => { calls.push("worklist"); } }));
vi.mock("@/app/(shell)/changes-data", () => ({ buildChangesViewUncached: async () => { calls.push("changes"); return changes; } }));
vi.mock("@/app/(shell)/today-view-data", () => ({ buildTodayCompositeFromChanges: async () => { calls.push("today"); return today; } }));
vi.mock("@/app/(shell)/today-newpages-data", () => ({ buildNewPagesData: async () => { calls.push("new-pages"); return newPages; } }));
...
import { refreshCustomerSurface } from "@/app/(shell)/surface-release";
