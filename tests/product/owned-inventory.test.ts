/** EVIDENCE - the owned-page inventory (discovery through the site's OWN answer inside hard bounds, first_seen kept, a refusing page held back) and the
 *  whole-page read that can finally answer "no". ONE in-memory stand-in for the two tables, with filters, ordering and paging applied for real, so a
 *  query that forgot its tenant scope shows up here as another account's row coming back. No network. */
...
import { discoverUrls, MAX_DISCOVERED_URLS } from "@/domains/evidence/scanning/in-process-scan"; import { completenessOf } from "@/domains/evidence/scanning/crawl-frontier";
...
const T = "tenant-own", OTHER = "tenant-other", NOW = new Date("2026-08-03T00:00:00.000Z");
const at = (days: number) => new Date(NOW.getTime() + days * 86_400_000);
const serve = (map: Record<string, string>) => (async (u: string) => (map[String(u)] == null
...
beforeEach(() => { db.owned = []; db.snaps = []; db.missing = ""; db.fails = false; });
...
describe("the owned-page inventory: what the site says it has, and what my read of it found", () => {
  it("follows the Sitemap: directive robots.txt publishes, three levels of index nesting deep, and inventories what it found", async () => {
    const out = await discover({ // the sitemap is at NEITHER conventional path: only the directive finds it
      ...
      "https://own.com/sm/root.xml": index(["https://own.com/sm/mid.xml"]), "https://own.com/sm/mid.xml": index(["https://own.com/sm/leaf.xml", "https://own.com/sm/deeper.xml"]),
    ...
    expect([out.source, out.truncated, out.pages.every((p) => p.via === "robots_sitemap")]).toEqual(["sitemap", 0, true]);
    ...
    expect((await readInventory(T, { limit: 10 })).map((r) => [r.url, r.discovered_via, r.crawl_state, r.completeness]))
      .toEqual([["https://own.com/a", "robots_sitemap", "uncrawled", "missing"], ["https://own.com/b", "robots_sitemap", "uncrawled", "missing"]]);
  ...
  it("gives a refusal a bounded retry date, holds the page back until it passes, and backs off further each time", async () => {
    ...
    const locked = () => readInventory(T).then((rs) => rs.find((r) => r.url.endsWith("/locked"))!);
    expect([(await locked()).crawl_state, (await locked()).completeness, (await locked()).blocked_until]).toEqual(["blocked", "blocked", at(1).toISOString()]);
    ...
    await markBlocked(T, "https://own.com/locked", 429, at(2)); expect((await locked()).blocked_until).toBe(at(9).toISOString()); // a day was not enough: a week
    await markBlocked(T, "https://own.com/locked", 401, at(9)); expect((await locked()).blocked_until).toBe(at(39).toISOString()); }); // then a month, the ceiling
});
