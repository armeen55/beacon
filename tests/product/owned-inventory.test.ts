...
describe("evidence - my own page's actual words, read narrowly", () => {
  ...
  it("holds the whole de-chromed page, hashes exactly the text it holds, and only then answers a whole-page question with no", async () => {
    ...
    expect([body.completeness, pageContains(body, "Zephyr Archive"), pageContains(body, "a fact this page never states"), body.sourceCapture, body.headings, body.passages.includes(long.replace(/\s+/g, " ").trim())]).toEqual(["complete", "yes", "no", snap.content_capture, ["Earlier detail", "Later section", "Follow-up"], true]); expect(body.heldNote).not.toMatch(/[—–]/); db.snaps = [snapRow({ url: "https://own.com/deep", word_count: snap.word_count, body_text: snap.body_text, content_capture: { ...JSON.parse(JSON.stringify(snap.content_capture)), complete: false } })]; const partial = await read("https://own.com/deep"); expect([partial.completeness, pageContains(partial, "Zephyr Archive"), pageContains(partial, "a fact this page never states"), partial.heldNote]).toEqual(["partial", "yes", "unknown", expect.stringContaining("saved main-content capture is incomplete")]); });
