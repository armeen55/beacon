...
describe("the replayed evidence reaches the REAL decision kernel", () => {
  ...
  it("judges the replayed gap, says what it is still missing, and drafts the change the evidence earned", async () => {
    ...
    expect([res.outcome, seam.calls(), res.proposals.every((p) => p.status === "needs_review"), res.noDraft]).toEqual(["proposals_persisted", 3, true, 1]); // THREE drafting calls, where this pass used to buy five. Every one of them was planned and priced before the pass spent anything: the whole-page rewrite refused without drafting, its one-field fallback drew on the SAME allowance, and the description card holds an allowance of its own because the page still had no row. Three charged calls per deliverable is one draft, its judge and the final reviewer, so a refused card has no fed-back retry left and the plan moves on. A draft the gates refuse is WITHDRAWN, and every queue row sits at needs_review
  });
