...
import { loadChangesViewWithSwr } from "@/app/(shell)/changes-data";
...
describe("ONE rebuild body (loader consolidation, 2026-07-21)", () => {
  it("changes-data.ts holds no private rebuild lane - the only single-flight key is the customer release's", () => {
    const changesSource = readFileSync(resolve(__dirname, "../../src/app/(shell)/changes-data.ts"), "utf8");
    const refreshSource = readFileSync(resolve(__dirname, "../../src/app/(shell)/surface-release.ts"), "utf8");
...
describe("Changes background rebuild tenant-source wiring", () => {
  it("passes the explicit tenant through every ambient-capable source", () => {
    const changesSource = readFileSync(resolve(__dirname, "../../src/app/(shell)/changes-data.ts"), "utf8");
    const worklistSource = readFileSync(resolve(__dirname, "../../src/app/(shell)/worklist-data.ts"), "utf8");
