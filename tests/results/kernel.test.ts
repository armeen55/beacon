...
describe("window evaluation and reporting lag", () => {
  it("marks a window waiting before its calendar close", () => {
    // Shipped 2 days ago; no window has closed.
    const windows = evaluateWindows("2026-05-30", NOW, "2026-05-31");
    expect(windows.every((w) => w.state === "waiting")).toBe(true);
  });

  it("marks a closed calendar window pending_data until Google finalizes it", () => {
    // 7-day window closed 2026-05-08, but finalized data only reaches 2026-05-05.
    const windows = evaluateWindows("2026-05-01", NOW, "2026-05-05");
    const w7 = windows.find((w) => w.day === 7)!;
    expect(w7.state).toBe("pending_data");
  });

  it("marks a window closed once finalized data passes its close date", () => {
    const w7 = CLOSED_WINDOWS.find((w) => w.day === 7)!;
    expect(w7.state).toBe("closed");
    expect(w7.closesOn).toBe(addDays("2026-05-01", 7));
  });
});
...
describe("overlap and confounding honesty", () => {
  it("flags same-page overlapping windows", () => {
    const overlaps = detectOverlaps([
      { id: "a", path: "/x", shippedAt: "2026-05-01" },
      { id: "b", path: "/x", shippedAt: "2026-05-10" },
      { id: "c", path: "/y", shippedAt: "2026-05-05" },
    ]);
  ...
  it("does NOT flag same-page changes more than 28 days apart", () => {
    const overlaps = detectOverlaps([
      { id: "a", path: "/x", shippedAt: "2026-05-01" },
      { id: "b", path: "/x", shippedAt: "2026-07-01" },
    ]);
  ...
  it("produces a bundle group read for overlapping same-page changes", () => {
    ...
    expect(bundles[0].changeIds.sort()).toEqual(["a", "b"]);
    expect(bundles[0].verdict).toBe("directional_improvement");
...
describe("historical records are preserved end to end", () => {
  it("maps every historical record to exactly one read", () => {
    ...
    expect(new Set(reads.map((r) => r.id)).size).toBe(12);
  });
  ...
  it("tolerates a legacy record with missing optional fields", () => {
    ...
    expect(input.baselineImpressions).toBe(0);
    expect(input.windows).toEqual([]);
    const read = evaluateChange(input, CLOSED_WINDOWS, []);
    expect(read.verdict).toBe("waiting");
  });
});
...
describe("ranking outcome signal", () => {
  ...
  it("stays neutral below the sample floor", () => {
    const priors = rankingPriors([{ actionType: "meta", read: { rankingSignal: 0.6 } }]);
    expect(priors.has("meta")).toBe(false);
  });
});

describe("metric selection and vocabulary", () => {
  it("judges snippet plays on CTR, rank plays on position, else clicks", () => {
    expect(metricFor("edit_title")).toBe("ctr");
    expect(metricFor("internal_link")).toBe("position");
    expect(metricFor("content")).toBe("clicks");
  });

  it("exposes a phrase for every verdict with no dashes", () => {
    for (const v of [
      "waiting", "insufficient_evidence", "directional_decline",
      "no_clear_movement", "directional_improvement", "stronger_improvement", "confounded",
    ] as const) {
