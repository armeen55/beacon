...
<header><h1>How to set a nowruz table</h1></header>
...
type Fetcher = (url?: string) => Promise<{ ok: true; html: string; status: number; finalUrl?: string } | { ok: false; reason: "robots_blocked" | "fetch_failed"; detail?: string }>;
const serve = (html: string, over: { status?: number; finalUrl?: string } = {}): Fetcher =>
  async (url = URL_) => ({ ok: true, html, status: 200, finalUrl: url, ...over });
...
describe("what Beacon can see on the live page, component by component", () => {
  ...
  it("reads all canonical main roots, excludes hidden/template copy and accepts ordinary CMS formatting", async () => { const copy = "A rainwater bowl & flowers: نوروز costs 5%.", formatted = PAGE.replace("</main>", `<p>A <strong>rain</strong>water bowl &amp; flowers: نوروز costs 5%.</p></main>`), excluded = ["nav", "template", 'div style="display:none"', 'div style="visibility:hidden"'];
    expect([await state("section_add", "What goes on the table at Nowruz and why"), ...await Promise.all(excluded.map((tag) => state("section_add", copy, serve(PAGE.replace("<main>", `<${tag}>${copy}</${tag.split(" ")[0]}><main>`))))), ...await Promise.all(excluded.map((tag) => state("h1", copy, serve(PAGE.replace("<main>", `<main><${tag}><h1>${copy}</h1></${tag.split(" ")[0]}>`))))), await state("paragraph_correction", copy, serve(formatted)), await state("paragraph_correction", copy.replace("5%", "5"), serve(formatted)), await state("section_add", copy, serve(PAGE.replace("</body>", `<main><p>${copy}</p></main></body>`)))]).toEqual(["not_verified", ...excluded.map(() => "not_verified"), ...excluded.map(() => "changed_differently"), "verified", "not_verified", "verified"]);});
  ...
  it("confirms actual entity properties, never just a schema type, pooled names or truncated answer", async () => {
    ...
    for (const bad of ["{", json({ ...faq(answer), "@context": { "@vocab": "https://schema.org/" } }), json({ ...faq(answer), "@type": 9 }), json({ "@id": "https://own.com/named-graph", "@graph": [faq(answer)] })]) expect((await check([{ kind: "title", after: "How to set a nowruz table" }, { kind: "schema_add", after: wanted }], live(bad))).status).toBe("partially_verified");
  });
  it("reads complete standard graphs and ID relationships, without conflating entities or certifying unsupported expansion", async () => {
    ...
    const graph = { "@graph": [{ ...node, "@type": ["https://schema.org/WebPage", "Article"], author: { "@id": "https://own.com/#writer" } }, node.author] }, live = (html: string) => serve(PAGE.replace(/<script[\s\S]*?<\/script>/, html));
  ...
  it("reads a sitemap change in the sitemap itself, and refuses to grade one it could not read", async () => { const listing = (...locs: string[]) => `<urlset>${locs.map((l) => `<url><loc>${l}</loc></url>`).join("")}</urlset>`;
    const graded = async (xml: string | null) => (await check([{ kind: "navigation", after: "I would add /nowruz to the sitemap you already publish." }], (async (u: string) => (!u.endsWith("/sitemap.xml") ? { ok: true, html: PAGE, status: 200, finalUrl: u } : xml == null ? { ok: false, reason: "fetch_failed" } : { ok: true, html: xml, status: 200, finalUrl: u })) as unknown as Fetcher)).components[0]!;
...
describe("what Beacon says overall, and what it refuses to say", () => {
  ...
  it("asks Google nothing for a body change, and nothing at all when no provider is on file", async () => { let asked = 0; const count = async () => { asked += 1; return []; };
    ...
    expect(blanked, "keys blanked: page evidence is still read; without an implementation stamp it is not learning permission").toEqual({ checkerContract: 4, proof: null, status: "verified", checkedAt: new Date(NOW).toISOString(), checks: 1, reason: null, recheckAfter: null, components: [{ kind: "title", state: "verified", note: "Your page title matches the prepared wording exactly." }] }); vi.unstubAllEnvs();});
  it("requires the observed intended address before banking content or buying search verification", async () => { const wrote = vi.fn(async () => {}), search = vi.fn(async () => []), observed = (finalUrl?: string) => verifyShipment(T, { id: "address", url: URL_, components: [{ kind: "title", after: "How to set a nowruz table" }], targetQueries: ["nowruz table"] }, { ...base, fetchPage: serve(PAGE, { finalUrl }), writeOwnedPage: wrote, readSerp: search });
    expect([(await observed()).status, (await observed("https://own.com/login")).status, wrote.mock.calls.length, search.mock.calls.length, (await observed(URL_)).status, wrote.mock.calls.length, search.mock.calls.length]).toEqual(["blocked", "blocked", 0, 0, "verified", 1, 1]);});});
describe("the pass: what is due, how much of it runs, and what it writes", () => {
  ...
  it("stops reading an address inside the same pass once an answer for it could not be saved", async () => {
    ...
    const written = await verifyDueShipments(T, { ...base, record: async () => false,
      fetchPage: (async (u: string) => { reads += 1; return { ok: true as const, html: PAGE, status: 200, finalUrl: u }; }),});
