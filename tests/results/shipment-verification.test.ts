...
describe("a historical record no page can answer is reconciled from the record, never re-crawled", () => {
  ...
  /** AND THE OTHER HALF OF THE SAME RULE: a record closed for naming nothing is closed only while that is still true. `applied_wording_missing` is not a verdict about the page, it is this code's own answer to "does the record name anything to look for", so a reader that learns to read a wording it once could not must re-derive the records it wrote off, through the ordinary pass and never a hand edit. Measured on the live ledger (2026-09-05): shp_6de7016c5cdf7e078f597a28f5a8975b held the exact words "Pallas's Cat facts" and stayed written off for ever after the apostrophe guard was fixed, because the door excluded that reason by name and the fix was forward-only. */
