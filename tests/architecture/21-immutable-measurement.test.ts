...
// ---------------------------------------------------------------------------
// D4 — sampling-status guard observability (measurement demotions are
// logged loud + the demotion metadata is a typed, reused field). Merged
// from sampling-guard-observability-log.
// ---------------------------------------------------------------------------

const SAMPLING_SRC = readFileSync(
  join(REPO_ROOT, "src/domains/attribution/url-change-outcome.ts"),
  "utf-8",
);
const VERDICT_TYPE_SRC = readFileSync(
  join(REPO_ROOT, "src/domains/attribution/url-verdict.ts"),
  "utf-8",
);

describe("D4 — materializer emits a structured log.warn on sampling-guard demotion", () => {
  const MSG = "[verdict-engine] sampling-status guard demoted verdict";
  it("emits the operator-locked warn with all 7 required fields, inside the demotion branch", () => {
    expect(SAMPLING_SRC.includes(MSG)).toBe(true);
    for (const field of [
      "tenantId,",
      "changeId:",
      "url:",
      ...
      expect(SAMPLING_SRC.includes(field), field).toBe(true);
    const guardIdx = SAMPLING_SRC.indexOf("computed.verdict.sampling_guard_demoted");
    const logIdx = SAMPLING_SRC.indexOf(MSG);
    expect(guardIdx).toBeGreaterThan(0);
    expect(logIdx).toBeGreaterThan(guardIdx);
    expect((SAMPLING_SRC.slice(guardIdx, logIdx).match(/^\s*\}\s*$/gm) ?? []).length).toBe(0);
    expect(SAMPLING_SRC.slice(Math.max(0, logIdx - 80), logIdx).includes("log.warn")).toBe(true);
  });
});

describe("D4 — UrlVerdict exposes sampling_guard_demoted metadata (typed + reused)", () => {
  it("declares the optional field + the SamplingGuardDemotion union", () => {
    expect(
      VERDICT_TYPE_SRC.match(/sampling_guard_demoted\?\s*:\s*SamplingGuardDemotion/),
    ).not.toBeNull();
    expect(VERDICT_TYPE_SRC.includes("export type SamplingGuardDemotion")).toBe(true);
    expect(VERDICT_TYPE_SRC.includes('"proof_day_in_post_window"')).toBe(true);
    expect(VERDICT_TYPE_SRC.includes('"no_full_days_in_post_window"')).toBe(true);
  });
});
