/** DataForSEO env truth + the ONE shared HTTP transport core. The money policy (configured / breaker / atomic reservation / single-flight cache) is pinned end to end in tests/sources/evidence-cache.test.ts; this file pins what every call shares: credential resolution, the fail-safe monthly cap and transport status/cost extraction. */
...
describe("the shared transport core", () => {
  it("returns the body with the provider-reported cost (falling back to the estimate) and never throws", async () => {
    ...
    const ok = await runDataForSeoTransport({ url: "https://x/v3/e", payload: [], estCostUsd: 0.01, env: ENV, fetchImpl: fetchImpl as never }); expect(ok).toEqual({ ok: true, body: { cost: 0.004, tasks: [] }, costUsd: 0.004 });
    ...
    const threw = await runDataForSeoTransport({ url: "https://x/v3/e", payload: [], estCostUsd: 0.01, env: ENV, fetchImpl: (async () => { throw new Error("net down"); }) as never }); expect(threw.ok).toBe(false); });
  it("GET mode sends no body (the free Standard task_get path)", async () => {
    ...
    await runDataForSeoTransport({ url: "https://x/v3/task_get/1", payload: [], estCostUsd: 0, env: ENV, fetchImpl: fetchImpl as never, method: "GET" }); expect(captured?.method).toBe("GET"); expect(captured?.body).toBeUndefined(); });
});
