...
const ENV = { DATAFORSEO_AUTH_B64: "abc" } as unknown as NodeJS.ProcessEnv, NOW = new Date("2026-07-25T12:00:00.000Z"), IDS = { tenantId: "t", unitKey: "u" }, BASE = "https://api.dataforseo.com/v3/";
const modelsEnv = (...models: Record<string, unknown>[]): ProviderEnvelope => ({ status_code: 20000, tasks: [{ status_code: 20000, result: models as unknown[] }] } as ProviderEnvelope), modelsFor = (name: string, post: boolean) => modelsEnv({ model_name: name, web_search_supported: true, task_post_supported: post }), STD = modelsFor("gpt-4o", true);
const atomicSpend = (over: Record<string, unknown> = {}) => ({ reserve: async (i: { estimatedUsd: number }) => ({ outcome: "reserved", attemptId: "attempt-1", attemptOrdinal: 1, state: "reserved", reportingDay: "2026-07-25", estimatedUsd: i.estimatedUsd, actualUsd: null, providerTaskId: null }), read: async () => null,
  claimTransmission: async () => "claimed", markAmbiguous: async () => true, release: async () => true, reconcile: async () => true, ...over });
...
function harness(fetchBody: unknown, over: Record<string, unknown> = {}) {
  const { modelsBody = fetchBody, rawBody = fetchBody, ...depsOver } = over;
  const calls = { fetch: [] as string[], bodies: [] as Record<string, unknown>[][], writes: [] as Record<string, unknown>[], upserts: [] as Record<string, unknown>[] };
  const deps = { env: ENV, now: () => NOW,
    fetchImpl: vi.fn(async (url: string, init?: RequestInit) => {
      ...
      return new Response(JSON.stringify(url.endsWith("/models") ? modelsBody : url.endsWith("/raw_html") ? rawBody : fetchBody), { status: 200 });
    }) as unknown as typeof fetch,
    claimEvidenceFetch: async () => ({ outcome: "claimed", payload: null, providerTaskId: null, modelServed: null, readyAt: null, costUsd: 0 }),
    spend: atomicSpend(), cacheUpsert: async (_k: string, row: Record<string, unknown>) => { calls.upserts.push(row); }, authorizeRepost: async () => true, breaker: async () => ({ tripped: false }),
...
describe("exact task-status taxonomy (docs.dataforseo.com/v3/appendix/errors)", () => {
  it("stores full HTML under the paid Instant Pages receipt and replays it without another paid call", async () => {
    ...
    const recovery = harness(paid, { rawBody: raw, claimEvidenceFetch: async () => ready(paid), spend: atomicSpend({ reserve: async () => { throw new Error("paid reserve on recovery"); } }) }); expect((await runWithoutSpending(() => providerCall("onpage_rendered_html", { url }, IDS, recovery.deps))).state).toBe("hit"); expect(recovery.task()).toEqual([`${BASE}on_page/raw_html`]);
    const banked = harness(paid, { rawBody: raw, cacheRead: async (key: string) => key === first.cacheKey ? { status: "ready", payload: paid, ready_at: NOW.toISOString(), expires_at: new Date(NOW.getTime() + 86_400_000).toISOString() } : null, claimEvidenceFetch: async () => ready(paid), spend: atomicSpend({ reserve: async () => { throw new Error("paid reserve on held replay"); } }) });
    expect((await providerCall("onpage_rendered_html", { url }, { ...IDS, bankedAfter: new Date(NOW.getTime() - 1000).toISOString() }, banked.deps)).state).toBe("hit"); expect(banked.task()).toEqual([`${BASE}on_page/raw_html`]);
    expect((await providerCall("onpage_rendered_html", { url }, { ...IDS, bankedAfter: NOW.toISOString() }, banked.deps)).state).toBe("capped");
    expect((await providerCall("onpage_rendered_html", { url, revision: "changed" }, { ...IDS, bankedAfter: new Date(NOW.getTime() - 1000).toISOString() }, banked.deps)).state).toBe("capped"); expect(banked.task()).toEqual([`${BASE}on_page/raw_html`]);
    const changed = harness(paid, { rawBody: raw }), next = await providerCall("onpage_rendered_html", { url, revision: "changed-owned-capture" }, IDS, changed.deps); expect(next.cacheKey).not.toBe(first.cacheKey); expect(changed.calls.bodies[0]).toEqual(h.calls.bodies[0]);
    const ambiguous = vi.fn(async () => true), release = vi.fn(async () => true), timed = harness({}, { spend: atomicSpend({ markAmbiguous: ambiguous, release }), fetchImpl: async (_u: string, init: RequestInit) => { expect(init.signal).toBeInstanceOf(AbortSignal); throw new DOMException("render deadline", "TimeoutError"); } });
  ...
  it("refuses wrong page, task, charge, status or HTML while retaining the paid receipt", async () => {
    ...
    const shellPaid = { ...paid, tasks: [{ ...paid.tasks![0], result: [{ crawl_progress: "finished", items: [{ status_code: 200, url, fetch_time: "2026-07-25 12:00:00 +00:00", meta: { content: { plain_text_word_count: 100 }, htags: { h1: ["Library"], h2: ["One", "Two", "Three", "Four"] } } }] }] }] };
    const near = (heads: string) => `<html><body><main><h1>Library</h1>${heads}<p>${'complete '.repeat(94)}</p></main></body></html>`;
    const duplicatePaid = structuredClone(shellPaid); duplicatePaid.tasks[0].result[0].items[0].meta.htags.h2 = ["One", "One", "Two", "Three", "Four"];
    const h4Paid = structuredClone(shellPaid); (h4Paid.tasks[0].result[0].items[0].meta.htags as Record<string, string[]>).h4 = ["Finer detail"]; let mismatch: Record<string, unknown> | undefined;
    for (const [fragment, source] of [[shell, shellPaid], [near("<h2>One</h2><h2>Two</h2><h2>Three</h2>"), shellPaid], [near("<h2>One</h2><h2>Two</h2><h2>Three</h2><h3>Four</h3>"), shellPaid], [near("<h2>One</h2><h2>Two</h2><h2>Three</h2><h2>Four</h2>"), duplicatePaid], [near("<h2>One</h2><h2>Two</h2><h2>Three</h2><h2>Four</h2>"), h4Paid]] as const) {
      const shellRead = harness(source, { rawBody: { ...raw, tasks: [{ ...raw.tasks[0], result: [{ items: { html: fragment } }] }] } });
      expect((await providerCall("onpage_rendered_html", { url }, IDS, shellRead.deps)).state).toBe("error"); expect(shellRead.task()).toEqual([`${BASE}on_page/instant_pages`, `${BASE}on_page/raw_html`]);
      mismatch ??= shellRead.calls.writes.find((w) => String(w.error_detail ?? "").startsWith("raw_html_mismatch:")); }
    expect(mismatch?.error_detail).toMatch(/^raw_html_mismatch:task:/);
    const again = harness(shellPaid, { claimEvidenceFetch: async () => ({ outcome: "pending", payload: null, providerTaskId: null, modelServed: null, readyAt: null, costUsd: 0 }), cacheRead: async () => ({ ...mismatch, quarantined_at: NOW.toISOString(), spend_attempt_id: "attempt-1" }), spend: atomicSpend({ read: async () => ({ state: "reconciled", resultPayload: shellPaid, accountedUsd: 0.0015 }) }) });
    expect((await providerCall("onpage_rendered_html", { url }, IDS, again.deps)).state).toBe("error"); expect(again.task()).toEqual([]);
  });
...
describe("exact provider paths + DYNAMIC method routing", () => {
  it("a Standard AI POST hits /task_post, the tag carries its unique spend attempt, and the model rides back; Perplexity is Live", async () => {
    ...
    if (res.state !== "waiting") throw new Error(res.state); expect(res.modelRequested).toBe("gpt-4o"); expect(chat.task()[0]).toBe(BASE + "ai_optimization/chat_gpt/llm_responses/task_post"); expect(chat.calls.bodies[0][0].tag).toBe("attempt-1");
  ...
  it("resumes Standard via /task_get/{id}, scraper via /task_get/advanced/{id}, and a raw 404 fails closed", async () => {
    const settled = atomicSpend({ read: async () => ({ state: "reconciled", providerTaskId: "abc-123", actualUsd: 0.03, estimatedUsd: 0.04 }) });
    const a = harness(llmResponsesTaskGet, { spend: settled, cacheRead: async () => taskRow("ai_optimization/chat_gpt/llm_responses/task_post", { spend_attempt_id: "attempt-1" }) }); expect((await collectCapability("k", a.deps)).state).toBe("ok"); expect(a.calls.fetch[0]).toBe(BASE + "ai_optimization/chat_gpt/llm_responses/task_get/abc-123"); const b = harness({ status_code: 20000, tasks: [{ status_code: 20000, id: "sc-1", cost: 0.03, result: [{ items: [] }] }] }, { spend: settled, cacheRead: async () => taskRow("ai_optimization/chat_gpt/llm_scraper/task_post", { provider_task_id: "sc-1", spend_attempt_id: "attempt-1" }) }); expect((await collectCapability("k", b.deps)).state).toBe("ok"); expect(b.calls.fetch[0]).toBe(BASE + "ai_optimization/chat_gpt/llm_scraper/task_get/advanced/sc-1");
...
describe("web-enabled request bodies per engine (only documented fields)", () => {
  ...
  it("keys an AI reading on the day it belongs to and the sample slot it is, and sends NEITHER to the provider", async () => {
    ...
    expect(slot0.key).not.toBe(tomorrow.key);    // and tomorrow is a new question, so a 23:00 answer is never served as tomorrow's // Neither field is a provider field: the request body is identical except for the unique reservation tag used for free recovery.
...
describe("keyword ideas: one request per 200 seeds, and nothing missing turned into a zero", () => {
  ...
  it("reserves BEFORE the call, reconciles down to the provider's actual, and reuses a cached equivalent instead of buying it twice", async () => {
    const order: string[] = []; let reserved = 0, actual = 0; const h = harness(ideas, { spend: atomicSpend({ reserve: async (i: { estimatedUsd: number }) => (order.push("reserve"), reserved = i.estimatedUsd, { outcome: "reserved", attemptId: "a", attemptOrdinal: 1, state: "reserved", reportingDay: "2026-07-25", estimatedUsd: i.estimatedUsd, actualUsd: null, providerTaskId: null }), claimTransmission: async () => (order.push("call-ready"), "claimed"), reconcile: async (_id: string, cost: number) => (order.push("reconcile"), actual = cost, true) }) });
    ...
    expect(order).toEqual(["reserve", "call-ready", "call", "reconcile"]); expect(reserved).toBeGreaterThanOrEqual(0.132); // reserve and atomic claim precede the wire
    expect(actual).toBeCloseTo(0.096, 5); // the provider's exact receipt is reconciled once
    let suggested = 0; const suggestion = harness(ideas, { spend: atomicSpend({ reserve: async (i: { estimatedUsd: number }) => (suggested = i.estimatedUsd, { outcome: "reserved", attemptId: "s", attemptOrdinal: 1, state: "reserved", reportingDay: "2026-07-25", estimatedUsd: i.estimatedUsd, actualUsd: null, providerTaskId: null }) }) }); await providerCall("labs_keyword_suggestions", { keyword: "persian names", limit: 1000 }, IDS, suggestion.deps); expect(suggested).toBeGreaterThanOrEqual(0.132);
...
describe("page intersection: ONE paid comparison for the whole page set", () => {
  ...
  it("sends ONE request numbering every page with the owner excluded, derives ONE identity from any order or unstated default, and re-serves it warm", async () => {
    ...
    let reserved = 0; const cap = harness(answer(), { spend: atomicSpend({ reserve: async (i: { estimatedUsd: number }) => (reserved = i.estimatedUsd, { outcome: "reserved", attemptId: "a", attemptOrdinal: 1, state: "reserved", reportingDay: "2026-07-25", estimatedUsd: i.estimatedUsd, actualUsd: null, providerTaskId: null }) }) }); await providerCall("labs_page_intersection", { pages: [W1, W2], limit: 5000 }, IDS, cap.deps);
...
describe("envelope parsing + method-aware resolution", () => {
  it("the Labs fixture through providerCall parses to nonempty keyword items, and a hit parses identically", async () => {
    ...
    let reserved = 0; const ov = harness(labsKeywordsForSiteLive, { spend: atomicSpend({ reserve: async (i: { estimatedUsd: number }) => (reserved = i.estimatedUsd, { outcome: "reserved", attemptId: "a", attemptOrdinal: 1, state: "reserved", reportingDay: "2026-07-25", estimatedUsd: i.estimatedUsd, actualUsd: null, providerTaskId: null }) }) }); await providerCall("labs_keyword_overview", { keywords: ["a"] }, IDS, ov.deps); expect(reserved).toBeGreaterThanOrEqual(700 * 0.0003); }); // 150 keywords really charged $0.02988, so a FULL 700-keyword batch lands near $0.21: never reserved under it
