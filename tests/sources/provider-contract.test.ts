...
describe("exact provider paths + DYNAMIC method routing", () => {
  ...
  it("resumes Standard via /task_get/{id}, scraper via /task_get/advanced/{id}, and a 404 is repost_once", async () => {
    ...
    const dead = await collectCapability("k", c.deps);
    expect(dead.state === "error" && dead.disposition).toBe("repost_once");
  });
  it("derives every family's FREE tasks_ready path and recovers a quarantined task by tag", async () => {
    for (const family of ["serp/google/organic", "serp/google/ai_mode", "ai_optimization/chat_gpt/llm_responses", "ai_optimization/claude/llm_responses", "ai_optimization/gemini/llm_responses", "ai_optimization/chat_gpt/llm_scraper"]) {
      ...
      expect(g.calls.fetch[0]).toBe(`${BASE}${family}/tasks_ready`); // FREE GET, verified per family on docs.dataforseo.com
      expect(g.calls.fetch.some((u) => u.includes("task_post"))).toBe(false); // never a paid repost
    }
  });
});
...
describe("web-enabled request bodies per engine (only documented fields)", () => {
  it("ChatGPT + Claude carry user_prompt + model_name + max_output_tokens + web_search (+ force + country when asked)", async () => {
    ...
    expect(cl.calls.bodies[0][0]).toMatchObject({ user_prompt: "q", model_name: "claude-sonnet-4-20250514", max_output_tokens: 2048, web_search: true });
  ...
  it("Gemini carries web_search ONLY; Perplexity never sends web_search; the scraper is keyword-based with no token field", async () => {
    ...
    expect(gb).toMatchObject({ user_prompt: "g", model_name: "gemini-2.5-pro", max_output_tokens: 2048, web_search: true });
    ...
    expect(pb).toMatchObject({ user_prompt: "p", model_name: "sonar", max_output_tokens: 2048, web_search_country_iso_code: "US" });
    ...
    expect(sc.calls.bodies[0][0].max_output_tokens).toBeUndefined(); // undocumented on llm_scraper: never sent
    const bad = harness({});
