...
describe("citation normalization (Slice 6I)", () => {
  it("resolves a wrapper through wrapper-only hops, keeps order/mode, never requests the final host or non-wrappers", async () => {
    ...
    const out = await resolveCitationTargets([app("https://plain.com/a"), app(WRAP, { observationMode: "consumer_search" })], impl); expect(out.map((a) => a.citedUrl)).toEqual(["https://plain.com/a", REAL]);
  ...
  it("keeps the raw wrapper on failure, and ranking never makes it a winning page", async () => {
    const { impl, calls } = stubFetch({ [WRAP]: { status: 500 } }); const out = await resolveCitationTargets([app(WRAP)], impl);
    expect(out[0]!.citedUrl).toBe(WRAP); expect(out[0]!.viaUrl).toBeUndefined(); expect(calls).toHaveLength(1); expect(rankWinningPages([...out, app(REAL)], "mysite.com", 10).map((c) => c.url)).toEqual([REAL]); });
  it("dedupes per URL, stops at the per-call resolution budget, and stops resolving past the deadline", async () => {
    const one = stubFetch({ [WRAP]: { status: 302, location: REAL } }); await resolveCitationTargets([app(WRAP), app(WRAP)], one.impl);
    expect(one.calls.map((c) => c.url)).toEqual([WRAP]); // one chain for both appearances; the final host is never requested
    const budget = stubFetch({}); await resolveCitationTargets(Array.from({ length: 12 }, (_, i) => app(`${WRAP}/${i}`)), budget.impl);
