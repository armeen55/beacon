...
describe("three opportunities waiting on their own results page", () => {
  ...
  it("14: the free collections its hub rows are waiting on are finished in front of the walk, the row those collections unlocked is written on that same drive, and eleven lower-ranked readings take the room behind the walk", async () => {
    ...
    expect([reasoningAsked.some((a) => a.kind === "body_edit" && a.ask.includes(QUERY)), lower.some((n) => boughtAt(n.query) > walkAt), lower.filter((n) => boughtAt(n.query) > walkAt).every((n) => acquisitions(runs[runs.length - 1]!).some((a) => a.query === n.query && a.outcome === "not_read" && a.detail.includes("nothing was researched"))), lower.every((n) => boughtAt(n.query) > walkAt || stillOwed.has(n.query)), !meter.requests.some((r) => r.kind === "page" && r.url.includes("/lower-")) && !reasoningAsked.some((a) => lower.some((n) => a.ask.includes(n.query)))],
      "the last dependency hires the hub writer on this drive; invalid lower-page obligations make no provider call or research claim, and every unattempted one remains owed for a later drive").toEqual([true, true, true, true, true]);
