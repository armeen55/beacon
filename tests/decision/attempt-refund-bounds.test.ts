...
describe("a refund gives back the attempt the door took, and never more", () => {
  ...
  it.each(SITES)("$t: an answer that was not served from the cache is never refunded", (s) => {
    expect([{ status: "drafted" }, { status: "validation_failed" }, null, undefined, { cached: false }].map((a) => door(s, a)), "none of these says nothing left the process, so every one keeps the attempt it took").toEqual(Array.from({ length: 5 }, () => "charged, calls 0, dollars 0"));
  });
});
...
describe("the pass's diagnosis purse", () => {
  ...
  it.each(SITES)("$t: a cached, refused or switched-off reading costs the pass no unit, and a real call costs exactly one", async (s) => {
    ...
    expect(spent, "a unit pays for a reading that actually left the process, so the model being off, the day cap refusing the call and the cache serving it all hand the unit back for the next case, and only the call that reached the provider is charged").toEqual(["no ruling, units 0, back 1, left 2", "no ruling, units 0, back 1, left 2", "ruled, units 0, back 1, left 2", "ruled, units 1, back 0, left 1"]);
  });
  it.each(SITES)("$t: and a reading that names real dollars keeps its unit, whatever its stamp says", async (s) => {
    expect(await reading(s, { status: "drafted", cached: true, attempts: 1, costUsd: 0.004, value: RULING }),
      "the cost guard reaches this door now: money on the receipt means the call left the process whatever the stamp says, so the unit it bought stays spent").toBe("ruled, units 1, back 0, left 1");
  });
});
...
describe("the AI-case meter's own left, through the one shared give-back", () => {
  ...
  it.each(SITES)("$t: never falls below zero, and one drawn unit comes back exactly once however often it is asked for", (s) => { const meter = AI_CASE_COPY.aeoMeter(1), drew = meter.draw(), atFloor = meter.left;
    for (let i = 0; i < 4; i += 1) DRAFT_BUDGET.refundIfNoCallMade(meter, { status: "blocked_budget", costUsd: 0 }); const again = meter.draw(); expect([drew, atFloor, meter.left, meter.spent(), again], `${s.t}: the drawn unit is handed back once, the pass may spend it again, and the second draw empties the purse for good`).toEqual([true, 0, 0, { funded: 1, attempted: 1, givenBack: 1, left: 0 }, true]); });
  it.each(SITES)("$t: a receipt naming real dollars keeps its unit at the shared door, whatever its stamp says", (s) => { const meter = AI_CASE_COPY.aeoMeter(3); meter.draw(); DRAFT_BUDGET.refundIfNoCallMade(meter, { status: "drafted", cached: true, attempts: 0, costUsd: 0.004 });
    expect([meter.left, meter.spent().givenBack], `${s.t}: the dollars are asked in front of all four stamps, so a cached receipt that was billed pays for its reading`).toEqual([2, 0]); });
  it.each(SITES)("$t: the writing purse's own slice cannot be lifted past what it drew either", (s) => { const key = DRAFT_BUDGET.keyOf({ pageUrl: s.url }), budget = DRAFT_BUDGET.plan({ jobs: [{ key, family: "editor", impact: 10, calls: 6 }], candidates: 1, calls: 6 }), slice = budget.draw(key, 2), before = slice?.left ?? -1;
