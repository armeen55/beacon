...
describe("structured-drafter strict transport", () => {
  ...
  it("drafts a VALUE, retries a recoverable answer once and no more, and never pays twice for one answer", async () => {
    ...
    const cached = await callStructuredLLM({ ...REQ, complete: hit.complete, cacheImpl, attempts: { left: 0 }, stopBy: 0 }); for (const ceiling of [0, 1, 2]) { const a = { left: ceiling }, paid = seam([{ value: {} }, { value: VALID_ATOMIC_EDIT }]), out = await callStructuredLLM({ ...REQ, attempts: a, complete: async (r) => ({ ...await paid.complete(r), httpAttempts: 1 }) }); expect([paid.calls(), a.left, out.status, out.status === "validation_failed" || out.status === "drafted" ? out.attempts : -1], "each actual request, including retry, takes the shared allowance before the wire").toEqual([ceiling, 0, ceiling === 2 ? "drafted" : "validation_failed", ceiling]); } const at = Date.now(), clock = vi.spyOn(Date, "now").mockReturnValue(at); try { let calls = 0; const out = await callStructuredLLM({ ...REQ, stopBy: at + 1, complete: async () => (calls += 1, clock.mockReturnValue(at + 2), { value: {}, httpAttempts: 1 }) }); expect([calls, out.status, out.status === "validation_failed" && out.attempts], "a failed first response cannot start a retry after the caller deadline").toEqual([1, "validation_failed", 1]); } finally { clock.mockRestore(); }
...
describe("structured body delivery", () => {
  it("retains complete units, practical steps and preservation beyond atomic field limits", async () => {
    ...
    const units = [{ kind: "heading", level: 3, text: "When should you leave?" }, { kind: "paragraph", text: before.replaceAll("return route", "**return route**") }, { kind: "ordered_list", items: ["Check the return route before entering."] }, { kind: "unordered_list", items: [fact] }];
    ...
    const out = await draftAtomicEditStructured({ field: "answer_block", query: "return route", pageLabel: "Return route", currentValue: null, unmarkPhrase: "return route", replaces: before, outline: [], evidenceHints: [before], tenantId: "body-fixture" }, { complete, bypassCache: true });
