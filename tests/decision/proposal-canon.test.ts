...
describe("promotion asks the canon's own quality status, not just its verdict", () => {
  ...
  it("refuses an unsupported national claim before a reviewer can cost money", async () => { const reader = await import("@/domains/evidence/pages/owned-context"), { canonicalUrlKey } = await import("@/domains/evidence/snapshot"), page = "Iran's national animal is the Asiatic cheetah.", candidate = proposal({ status: "needs_review", pagePath: "/asiatic-cheetah", pageUrl: "https://fixture.example/asiatic-cheetah", primaryQuery: "iran national animal", recommendedChange: { kind: "existing_edit", field: "title", before: "Meet the Asiatic Cheetah | Iran Animals", after: "Asiatic Cheetah: Iran's National Animal" }, claims: [{ text: page, supportedBy: ["page-copy-1"] }], supportFacts: [{ id: "page-copy-1", fact: page }] }), loaded = vi.spyOn(reader, "loadOwnedPageBodies").mockImplementation(async (_tenant, urls) => new Map(urls.map((url) => [canonicalUrlKey(url), { url, title: "Meet the Asiatic Cheetah | Iran Animals", h1: "Asiatic Cheetah", metaDescription: null, headings: [], passages: [page], vocabulary: page, completeness: "complete", version: "current", contentHash: "current-source", fetchedAt: PROMOTE.at }])) as never); try { expect(await preflightReviewedProposal(T, candidate, new Date(PROMOTE.at))).toBe("This change states a fact with no source behind it. It is held until a source is added."); } finally { loaded.mockRestore(); } });
