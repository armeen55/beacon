...
const db = vi.hoisted(() => {
  const state = { rows: [] as Row[], legacy: [] as Row[], shipments: [] as Array<{ tenant_id: string; id: string; proposal_id: string; proposal_version: string; implemented_at: string }>, missing: false, rpcMissing: false, breakWrite: false, rpcCalls: 0, raceForeign: "", race: null as null | (() => void) };
  const client: Record<string, unknown> = {
    rpc(name: string, args: Record<string, unknown>) { state.rpcCalls += 1;
      const run = (): { data: string | null; error: { message: string; code?: string } | null } => {
        ...
        if (name === "answer_change_proposal_review") {
          const at = state.rows.findIndex((r) => r.tenant_id === args.p_tenant_id && r.id === args.p_id);
          state.race?.(); state.race = null; const held = state.rows[at];
...
import { dismissChangeProposal, loadChangeProposal, loadChangeProposals, answerReviewedProposal, saveChangeProposal, transitionProposalToImplemented, withdrawChangeProposal } from "@/domains/decision/proposal-store";
...
import { componentIdOf, sameComponentId, deserializeChangeProposal, serializeChangeProposal, type ChangeBundle, type ChangeProposal } from "@/domains/decision/contracts";
...
beforeEach(() => { db.state.rows = []; db.state.legacy = []; db.state.shipments = []; db.state.missing = false; db.state.rpcMissing = false; db.state.breakWrite = false; db.state.rpcCalls = 0; db.state.raceForeign = ""; db.state.race = null; });
...
describe("canonical proposal persistence", () => {
  ...
  it("never retires a change the operator already acted on: a newer draft for that hypothesis is refused and the applied row keeps its place", async () => {
    expect(await saveChangeProposal(proposal())).toBe("saved");
    seedShipment("shp_seed", proposal().id); expect(await transitionProposalToImplemented(T, proposal().id, "shp_seed", "sv-1")).toBe(true); // done is reachable only through the transaction
...
describe("done is only ever reached with a record behind it", () => {
  ...
  it("refuses the flip with no record named, and lands it with one", async () => {
    ...
    seedShipment("rec-1", DONE_ID); expect([await transitionProposalToImplemented(T, DONE_ID, "rec-1", "sv-1"), db.state.rows[0]!.status]).toEqual([true, "implemented_pending_verification"]); });
  it("refuses a real shipment belonging to another proposal or applied-copy version", async () => {
    ...
    expect([await transitionProposalToImplemented(T, DONE_ID, "rec-other", "sv-other"), db.state.rows[0]!.status]).toEqual([false, "ready"]);
    db.state.shipments[0]!.proposal_id = DONE_ID;
    expect([await transitionProposalToImplemented(T, DONE_ID, "rec-other", "sv-wrong"), db.state.rows[0]!.status]).toEqual([false, "ready"]); });
  it("never clears a terminal withdrawal or dismissal, even for a matching shipment", async () => {
    ...
    seedShipment("rec-1", DONE_ID); expect([await transitionProposalToImplemented(T, DONE_ID, "rec-1", "sv-1"), db.state.rows[0]!.status, db.state.rows[0]!.terminal_disposition]).toEqual([false, "ready", "withdrawn"]);
    Object.assign(seed(), { terminal_disposition: "dismissed" });
    seedShipment("rec-2", DONE_ID); expect([await transitionProposalToImplemented(T, DONE_ID, "rec-2", "sv-1"), db.state.rows[0]!.terminal_disposition]).toEqual([false, "dismissed"]); });
  it("does not overwrite a newer proposal version that lands after the operator read the card", async () => {
    ...
    seedShipment("rec-1", DONE_ID); expect(await transitionProposalToImplemented(T, DONE_ID, "rec-1", "sv-1")).toBe(false);
...
describe("promotion asks the canon's own quality status, not just its verdict", () => {
  ...
  it("still promotes the sound row: needs_review only because a human look is owed, and the quality itself is ready", async () => { const initial = held("Nowruz Traditions"), peerUrl = new URL("/secondary-traditions", initial.pageUrl!).href, peer = { ...initial.bundle!.components[0]!, page: peerUrl, after: "Nowruz traditions include a Haft-Seen table whose seven symbolic items have Persian names beginning with the letter seen, arranged for the spring equinox celebration." }, base = { ...initial, bundle: { ...initial.bundle!, components: [...initial.bundle!.components, peer] }, claims: [...initial.claims!, { text: peer.after, supportedBy: ["fact-2"], of: componentIdOf(peer, 1) }], supportFacts: [...initial.supportFacts!, { id: "fact-2", fact: peer.after }] }, row = { ...base, semanticReview: { ...initial.semanticReview, of: copyKey(base), version: REVIEW_CONTRACT, claims: base.claims.map((c, i) => ({ i, by: [...c.supportedBy], entailed: true })) } }, availability = { peer: false }, reader = await import("@/domains/evidence/pages/owned-context"), { canonicalUrlKey } = await import("@/domains/evidence/snapshot"), loaded = vi.spyOn(reader, "loadOwnedPageBodies").mockImplementation(async (tenant, urls) => { expect(tenant).toBe(T); return new Map(urls.filter(url => availability.peer || url !== peerUrl).map(url => [canonicalUrlKey(url), { url, title: "Nowruz", h1: "Nowruz", metaDescription: "Nowruz", headings: ["Traditions"], passages: ["Nowruz begins spring."], vocabulary: "Nowruz begins spring.", completeness: "complete", version: "current", contentHash: "current-source", fetchedAt: PROMOTE.at }])) as never; }); try { await saveChangeProposal(row); const priorVersion = db.state.rows.find(r => r.id === row.id)!.proposal_version, missing = await answerReviewedProposal(T, row.id, confirmedVersion(row), row.basis ?? null, PROMOTE); expect([missing.status, missing.refusal?.startsWith(peerUrl), db.state.rows.find(r => r.id === row.id)!.status, db.state.rows.find(r => r.id === row.id)!.proposal_version]).toEqual(["refused", true, "needs_review", priorVersion]); availability.peer = true; const result = await answerReviewedProposal(T, row.id, confirmedVersion(row), row.basis ?? null, PROMOTE); expect([result, loaded.mock.calls.map(([, urls]) => urls)]).toEqual([{ status: "promoted" }, [[row.pageUrl, peerUrl], [row.pageUrl, peerUrl]]]); } finally { loaded.mockRestore(); } }); });
