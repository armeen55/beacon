...
describe("structured data answers to its own gate", () => {
  it("keeps question-answer association through linked identity and leaves uncertain capture as evidence debt", async () => {
    ...
    const markup = JSON.stringify({ "@context": "https://schema.org", "@graph": [{ "@type": "FAQPage", "@id": "#faq", mainEntity: { "@id": "#q" }, identifier: "retain" }, { "@type": "Question", "@id": "#q", name: "When do seals rest?", acceptedAnswer: { "@id": "#a" } }, { "@type": "Answer", "@id": "#a", text: "Seals rest at low tide.", identifier: "retain-answer" }] });
    ...
    const judge = (pageCapture: unknown, after = markup) => validateProposal({ ...p, recommendedChange: { ...p.recommendedChange, after } as ChangeProposal["recommendedChange"] }, { pageBodyText: capture.faqs.map((pair) => `${pair.question} ${pair.answer}`).join(" "), pageCapture } as never);
    ...
    expect(graph.nodes.map((node) => [node["@id"], node.identifier])).toEqual([["#faq", "retain"], ["#q", undefined], ["#a", "retain-answer"]]);
    ...
    for (const held of [null, { ...capture, version: "stale_known_good" }, { ...capture, contentHash: null }, { ...capture, url: "https://other.example/" }, { ...capture, faqs: capture.faqs.map((pair) => ({ ...pair, answerComplete: false })) }, { ...capture, faqs: capture.faqs.map((pair) => ({ ...pair, source: "jsonld" })) }]) {
