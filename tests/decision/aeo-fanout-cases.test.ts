...
describe("stage copy is honest and the diagnosis owns the treatment", () => {
  ...
  it("a refusing diagnosis mints no card, scatter hires on its own passages, missing information never hires, and an unruled case names no treatment at all", () => {
    ...
    const missing = dx({ kind: "missing_information", treatment: "add_answer_section", evidenceIds: ["ans-1"], missing: "the date rule" }), identity = claimIdentity("the date rule", "", "missing"); expect(gateOf(missing)).toMatchObject({ emit: true, hire: false, next: expect.stringContaining("acquires an authoritative source") }); expect([sourceDebt(missing, "which date rule applies", OWN.url, "proposal-1"), sourceDebt(missing, "which date rule applies", OWN.url, "proposal-1", false)], "the locked card exposes executable exact debt with no rival shortcut; the unlocked card drops only that debt and preserves the same proposition identity for its writer packet").toEqual([{ factIdentity: identity, obligation: { kind: "evidence", need: { kind: "factual_source", query: "which date rule applies", url: OWN.url, missingTopic: "the date rule", proposalId: "proposal-1", reasonCode: "source_support_unconfirmed" } } }, { factIdentity: identity }]); // MISSING INFORMATION NEVER HIRES UNSOURCED: the writer would have to STATE the proposition, and nothing binds that exact statement to the facts that support it. ONCE AN AUTHORIZED FACT ELSEWHERE ON THE PAGE IS NOT ENOUGH: only the exact missing proposition unlocks this card.
...
describe("the gap reader is exact, fail-closed and metered", () => {
  ...
  it("refuses ids nobody supplied, thin scatter, and every claim the packet cannot carry", async () => {
    ...
    for (const verdict of [
      { kind: "already_answered", ownedIds: ["ans-1"] },
      { kind: "missing_information", evidenceIds: ["own-1"], missing: "the rule" },
      { kind: "already_answered", ownedIds: ["faq-1"] },
    ...
    for (const capture of [
      ...
      { passages: Array.from({ length: 41 }, () => "Visible answer.") },
      { version: "stale_known_good" }, { version: undefined },
    ]) { net.body = page(capture); expect(await ask()).toMatchObject({ kind: "unknown", treatment: null, completeness: "partial" }); }
    net.body = page({ passages: ["Visible answer. ".repeat(50)] });
    expect(await ask()).toMatchObject({ kind: "missing_information", completeness: "complete" });
  ...
  it("binds the exact packet, funds every uncached attempt, and buys nothing unfunded", async () => {
    ...
    net.body = page({ contentHash: null, passages: [...Array.from({ length: 40 }, () => "Visible passage."), "old unshown answer"] });
    const unshown = (await ask())!;
    net.body = page({ contentHash: null, passages: [...Array.from({ length: 40 }, () => "Visible passage."), "changed unshown answer"] });
    expect((await ask({ banked: unshown }))!.packet).not.toBe(unshown.packet);
    const meter = aeoMeter(5); net.answer = drafted({ kind: "already_answered", ownedIds: ["own-99"] }); // every one refuses
