...
describe("a subject I own no page for becomes ONE researched page, and nothing else does", () => {
  ...
  it("builds exactly ONE new page from the earned verdict, carrying the WHOLE page, and holds it for the look it owes", async () => {
    ...
    expect([openHold(page).defects, openHold(page).advisories.map((a) => a.kind).filter((k) => k === "single_source")], "REPLACES the unsupported-claim hold and the source it minted: a whole page built from what wins this search is a reasonable, explainable page whose claims stand on its own words, so it is offered with the caveat naming what it rests on").toEqual([[], ["single_source"]]);
