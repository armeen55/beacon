...
/** A completion fn that replays a fixed queue (last response repeats). The seam returns a
 *  PARSED structured VALUE (never text); an error carries its retryability. */
...
const VALID_ATOMIC_EDIT = {
  ...
  evidenceRefs: [{ source: "gsc", detail: "many impressions for nowruz traditions with a low click rate" }], confidence: "high",
  risks: ["keep the title concise"], operatorSteps: ["Replace the page title field with the new value"], proofPlan: PROOF };
const VALID_CREATE_PAGE = {
  ...
  risks: ["describe the items qualitatively, not by count"], operatorSteps: ["Create a new page with this title and outline"],
  proofPlan: { metrics: ["AI citations"], windowsDays: [7, 14, 28], controls: "comparable topic pages" } };
const EXISTING_INPUT: EvidenceInput = {
  tenantId: "referencepedia", page: { path: "/nowruz", url: "https://fixture-content.example/nowruz", label: "Nowruz" },
...
const NEW_PAGE_INPUT: EvidenceInput = {
  tenantId: "referencepedia", page: { path: null, url: null, label: "Haft-Seen table" }, opportunity: { query: "haft-seen table", kind: "new_page", opportunityType: "Win AI citations" },
...
function baseProposal(over: Partial<ChangeProposal> = {}): ChangeProposal {
  return {
    ...
    pageLabel: "X", primaryQuery: "nowruz traditions", opportunityType: "Capture clicks", changeFamily: "title", status: "proposed",
...
describe("existing-page cold proposal", () => {
  it("generates, validates, persists (serialize), and re-loads (deserialize)", async () => {
    ...
    expect(p.whyItMatters).toMatch(/customs/i); expect(p.evidence.evidenceRefCount).toBe(1);
    expect(p.primaryQuery).toBe("nowruz traditions"); expect(p.opportunityType).toBe("Capture clicks");
    expect(out.validation.verdict).toBe("ready"); expect(p.status).toBe("proposed"); // validated safe → proposed, not rejected
    const loaded = deserializeChangeProposal(serializeChangeProposal(p)); // persisted + loadable: round-trips + re-validates
    expect(loaded).not.toBeNull(); expect(loaded).toEqual(p);
...
describe("rankProposals orders by value", () => {
  it("puts the higher honest upside first and sinks the rejected one", () => {
    const low = baseProposal({ id: "low", upsidePerMonth: 10, impactScore: 30 }), high = baseProposal({ id: "high", upsidePerMonth: 90, impactScore: 30 });
