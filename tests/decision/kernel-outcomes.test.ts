...
const VALID_CREATE_PAGE = {
  ...
  confidence: "high", risks: ["describe the items qualitatively, not by count"], operatorSteps: ["Create a new page with this title and outline"],
...
const EXISTING_INPUT: EvidenceInput = {
  ...
  page: { path: "/nowruz", url: "https://fixture-content.example/nowruz", label: "Nowruz" }, opportunity: { query: "nowruz traditions", kind: "existing_edit", field: "title", opportunityType: "Capture clicks", currentValue: "Nowruz", intent: "what" },
...
const NEW_PAGE_INPUT: EvidenceInput = {
  tenantId: "referencepedia",
  page: { path: null, url: null, label: "Haft-Seen table" }, opportunity: { query: "haft-seen table", kind: "new_page", opportunityType: "Win AI citations" },
...
function baseProposal(over: Partial<ChangeProposal> = {}): ChangeProposal {
  return {
    ...
    pagePath: "/x", pageUrl: "https://fixture-content.example/x", pageLabel: "X", primaryQuery: "nowruz traditions", opportunityType: "Capture clicks", changeFamily: "title", status: "proposed",
...
const edited = (field: "title" | "meta", before: string | null, after: string): ChangeProposal =>
  baseProposal({ recommendedChange: { kind: "existing_edit", field, before, after } });
describe("existing-page cold proposal", () => {
  it("generates, validates, persists (serialize), and re-loads (deserialize)", async () => {
    ...
    expect(out.validation.verdict).toBe("ready"); expect(p.status).toBe("proposed");
    // persisted + loadable: serialize → deserialize round-trips + re-validates
    const loaded = deserializeChangeProposal(serializeChangeProposal(p));
...
describe("new-page cold brief", () => {
  it("generates a full brief and validates it", async () => {
    ...
    expect(p.kind).toBe("new_page"); expect(p.pagePath).toBeNull(); // no existing page
