...
describe("a subject I own no page for becomes ONE researched page, and nothing else does", () => {
  ...
  it("builds exactly ONE new page from the earned verdict, carrying the WHOLE page, and holds it for the look it owes", async () => {
    ...
    const pack = page.bundle!.components.find((c) => c.kind === "source_pack")!.after; expect(pack).toContain(page.supportFacts![0]!.fact); expect(pack).not.toContain("You pick the exact source for this one");
...
describe("the unruled review pass", () => { // ── a pass whose review never ruled keeps its hands off a banked reading ───────
  ...
  it("rewrites no reviewed row, and still lands the owed card where nothing is on file", async () => { reset(SEEN());
    ...
    const rx = out.paid.receipts.find((r) => r.key === "/x" || r.key.startsWith("/x::")); const owed = nextObligation(env.store.get(reviewed.id)!); expect([rx?.outcome !== "evidence_required", owed?.kind, owed?.kind === "redraft" ? owed.instruction : ""], "REPLACES the source the row was sent shopping for: its real remaining debt is the corrective draft its own glued label owes, read off the typed step, and no reading is bought for editorial eligibility").toEqual([true, "redraft", expect.stringContaining("glued phrase")]); reset(SEEN()); fenv.cards = [mint()]; fenv.review = (cards) => cards; // THE SIBLING: the same unruled pass still lands the owed card on a page with nothing on file.
