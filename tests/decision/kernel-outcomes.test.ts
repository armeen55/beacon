...
const cited = (user: string): string => ["owned-page-1", "page-copy-1", "page-heading-1", "page-title"].find((id) => user.includes(`"id":"${id}"`)) ?? "page-title";
...
describe("a subject I own no page for becomes ONE researched page, and nothing else does", () => {
  ...
  it("builds exactly ONE new page from the earned verdict, carrying the WHOLE page, and holds it for the look it owes", async () => {
    ...
    expect(seam.kinds).toEqual(["new_page_brief", ...[0, ...BRIEF.sections].flatMap(() => ["body_edit", "editor_judgement"]), "page_acceptance"]); // each piece then one complete publication review
    ...
    expect([page.status, page.pagePath, page.publish, validateProposal(page).verdict], JSON.stringify({ faults: page.faults, holds: openHold(page), limitations: page.limitations })).toEqual(["ready", null, "manual", "ready"]);
    ...
    expect(page.bundle!.components.some((c) => c.kind === "source_pack")).toBe(false); expect(page.supportFacts!.every(f => f.fact && f.sources?.some(source => source.url)), JSON.stringify(page.supportFacts)).toBe(true); expect(page.operatorSteps).toContain("Check the evidence behind the claims and add the cited links in your site editor");
    expect([page.claims!.every((c) => c.supportedBy.every((id) => id.startsWith("owned-page-"))), page.informationGain?.pageWhole, page.semanticReview?.scope]).toEqual([true, true, "whole_page"]);
    ...
    const { reviewFinishedCopy } = await import("@/domains/decision/drafted-copy"); const rejected = (await reviewFinishedCopy(page, { tenantId: page.tenantId, now: NOW, bypassCache: true, judge: async (d) => ({ ...judged(""), wouldHandToCustomer: false, resolution: "structural_synthesis", notes: "The first section must answer its heading rather than describe the writing.", repairs: [{ component: 3, instruction: "Explain the table itself in the first section." }], claims: d.claims.map((c, i) => ({ i, by: [...c.supportedBy], entailed: true })) }) as never })).row!; env.store.set(page.id, { ...rejected, status: "needs_review" });
    const repairing: string[] = []; const repaired = await produceProposalsForTenant("fixture-tenant", { now: NOW, complete: async (r) => { repairing.push(r.kind); if (r.kind === "new_page_brief") throw new Error("never rebrief a rejected complete page"); return { value: (r.kind === "body_edit" ? publicationDraft({ ...sectionDraft(r.user), after: sectionDraft(r.user).after.replace("It says what belongs there and why", "It explains what each item means and how families arrange it") }) : judged(r.user)) as never }; } }); const landed = repaired.proposals.find((p) => p.id === page.id)!;
    expect([repairing, landed.status, nextObligation(landed), landed.newPageDraft?.repair]).toEqual([["body_edit", "editor_judgement", "page_acceptance"], "ready", null, undefined]); expect(landed.newPageDraft!.pieces[1]!.after).not.toBe(page.newPageDraft!.pieces[1]!.after); for (const n of [0, 2, 3]) expect(landed.newPageDraft!.pieces[n]).toEqual(page.newPageDraft!.pieces[n]);
  ...
  it.each(["tenant-one", "tenant-two"])("stops a twelve-call page build at its share of the walk's box and files it in the box's own words rather than as a cost block [%s]", async () => {
    reset(snap([GAP], READY({ topicKey: keyOf(READY()) }), DEMAND));
    vi.useFakeTimers({ toFake: ["Date"] }); const at = Date.now(); let asked = 0; const inner = briefSeam(); // every ask of the writer costs 80 seconds of a 200-second box, so the share runs out inside the page build
    const greedy: CompleteFn = async (r) => (asked += 1, vi.setSystemTime(at + asked * 80_000), inner.complete(r));
    const out = await produceProposalsForTenant("fixture-tenant", { complete: greedy, now: NOW, produce: true, stopBy: at + 200_000 }); vi.useRealTimers();
    const topic = out.paid.receipts.find((r) => r.key.startsWith("topic:"))!;
    expect([asked, topic.outcome, topic.why], "the build takes the asks its share covers and starts no more, and its receipt names the drive's box, so the next drive owes it again at its own rank instead of reading a time stop as a purse that ran out").toEqual([
      2 /* two asks, not three: the page-only body draft no longer runs before its winners are read (2026-09-15) */, "retryable_blocked", "this job's share of the drive's box ended before it finished, so the calls it made are remembered and it is owed again at its own rank This work waited for the reading of the pages that win its subject, which this same pass funded and decided first."]); });
  it("banks unfinished new-page research without offering it as delivered copy, and uses actual stored sources", async () => {
    ...
    const unfinished = partial.proposals.find((p) => p.kind === "new_page")!; expect([unfinished.status, unfinished.researchOnly, !!unfinished.newPageDraft?.brief, nextObligation(unfinished)?.kind, env.saved.some((p) => p.id === unfinished.id)]).toEqual(["needs_review", true, true, "sections", true]);
    ...
    expect([held.status, held.bundle!.components.find((c) => c.kind === "source_pack")!.after.includes("You pick the exact source for this one")]).toEqual(["ready", false]);
    expect(held.bundle!.components.find((c) => c.kind === "source_pack")!.after).toContain(held.supportFacts![0]!.fact); });
  ...
  it("a partial page survives another topic's verdict, resumes without a second brief, and completes as one page", async () => {
    ...
    reset(snap([GAP], READY({ topicKey: keyOf(READY()) }), DEMAND));
    const inner1 = pageSeam(BRIEF); const partialSeam: CompleteFn = async (r) => r.kind === "body_edit" && ![BRIEF.proposedTitle, BRIEF.sections[0]!.heading].includes(/Write the section headed "(.*?)"/.exec(r.user)?.[1] ?? "") ? { value: {} as never } : inner1(r);
    ...
    expect([page.status, page.researchOnly === true, nextObligation(page) ?? null, (page.operatorSteps ?? []).length > 0, (page.bundle?.components ?? []).length >= 1 + BRIEF.sections.length],
      "the finished page is CUSTOMER-ACTIONABLE: Ready with whole-page acceptance, real copy, no owed work, operator steps and every planned component").toEqual(["ready", false, null, true, true]);
    ...
    expect(nextObligation(page)?.kind === "sections", "no sections owed: one complete page a person can publish").toBe(false); env.store.set(page.id, { ...page, status: "needs_review" }); const noCalls = vi.fn(async () => { throw new Error("completed late review promotes at zero provider cost"); }); await produceProposalsForTenant("fixture-tenant", { complete: noCalls as never, now: NOW, zeroSpend: true }); expect([noCalls.mock.calls.length, env.store.get(page.id)?.status, nextObligation(env.store.get(page.id)!) ?? null]).toEqual([0, "ready", null]); });
...
describe("why this page loses the click, one named cause at a time", () => { it("blames the wording only where the results page accuses it, and says what it beat and what would kill it", () => {
  ...
  it("promotes only earned recurrence using canonical citation ownership and the pass's clock", async () => {
    ...
    const blobs = vi.spyOn(await import("@/lib/persistence/json-store"), "readStore").mockResolvedValue([{ query: "unowned recurrence", passes: 1, refusedPages: ["/guide"], lastSeen: NOW.toISOString() }]), topics = vi.spyOn(await import("@/domains/evidence/topic-investigation"), "buildTopicInvestigations").mockReturnValue([]); vi.useFakeTimers({ toFake: ["Date"] }); vi.setSystemTime(new Date(NOW.getTime() + 90 * 86400000));
