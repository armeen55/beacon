...
describe("a subject I own no page for becomes ONE researched page, and nothing else does", () => {
  ...
  /** A JOB THAT IS NOT THE EDITOR STILL SPENDS INSIDE ITS SHARE OF THE WALK'S BOX (live 13:00Z drive on production, 2026-09-05). The editor batch asks its stop before every card and every retry; a twelve-call rewrite asked nothing, ran 108 seconds through its share and out past the walk's own box, and the drive's backstop timer then threw away every receipt the walk had filed. The share is spent the way the money is spent: past it the allowance reports empty, no further attempt is started, what is in flight finishes and is filed, and the receipt says the BOX ended rather than the money running out. */
