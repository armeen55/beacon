...
describe("what the evidence justifies before anything is drafted", () => { it("leaves a page that already beats the clicks its positions earn alone, however big it is", () => {
  ...
  it("funds the body answer worth more before the description, and leaves the description only what the answer did not take", () => { const j = (key: string, impact: number) => ({ key, family: "editor", impact, calls: DRAFT_BUDGET.DELIVERABLE_CALLS, workKey: `${key}::w` }), keys = (calls: number) => DRAFT_BUDGET.plan({ jobs: [j("/p::meta", 2), j("/p2::body::answer", 9)], candidates: 4, calls }).funded.map((f) => f.key); const topicWorth = proposalValueScore(baseProposal({ id: "job::topic:hardest-language", pagePath: "topic:hardest-language", pageUrl: "topic:hardest-language", primaryQuery: "", opportunityType: "", changeFamily: "single", status: "needs_review", researchOnly: true, recommendedChange: { kind: "existing_edit", field: "section", before: null, after: "" }, whyItMatters: "", estimatedEffortMinutes: 15, riskLevel: "low", confidence: "low", limitations: [], evidence: { query: "", hints: [], evidenceRefCount: 0 }, impactScore: 27100 / 100, upsidePerMonth: null })); const memory = (key: string, m: { calls: number; last: string; settled: boolean }) => ({ [`${key}::w`]: m }); expect([keys(DRAFT_BUDGET.DELIVERABLE_CALLS * 2), keys(DRAFT_BUDGET.DELIVERABLE_CALLS / (1 + DRAFT_BUDGET.RETRIES)), topicWorth, DRAFT_BUDGET.plan({ jobs: [j("pattern:hardest-language", topicWorth), j("/p::answer", 2.84)], candidates: 4 }).funded.map((f) => f.key), DRAFT_BUDGET.plan({ jobs: [j("pattern:hardest-language", topicWorth), j("/p::answer", 2.84)], candidates: 1, memory: memory("pattern:hardest-language", { calls: 1, last: "deterministic_refusal", settled: true }) }).funded.map((f) => f.key), DRAFT_BUDGET.plan({ jobs: [j("/hi", 9), j("/lo", 2)], candidates: 1 }).declined.map((d) => d.key), DRAFT_BUDGET.plan({ jobs: [j("/hi", 9), j("/lo", 2)], candidates: 2, memory: memory("/hi", { calls: 1, last: "provider_blocked", settled: false }) }).funded.map((f) => f.key)], "no metadata band and no body band: ONE order, and fast metadata fills what is left over rather than crowding out the section worth more. ONE VALUE CALCULATION: a topic worth 27,100 searches a month reaches the manifest through the SAME receipt an editor card does, at 5.01 and never at 271, and it still leads the 2.84 answer because 27,100 searches collected at the undiagnosed share is more than 2.84, not because nobody discounted it. A DEPENDENCY BEACON'S OWN RULE ALREADY REFUSED cannot take the one slot again: the answer takes it. AND WORK THE MONEY NEVER REACHED KEEPS ITS PRIORITY across two continuations, while the job that really was attempted and finished nothing is demoted behind it.").toEqual([["/p2::body::answer", "/p::meta"], ["/p2::body::answer"], 5.01, ["pattern:hardest-language", "/p::answer"], ["/p::answer"], ["/lo"], ["/lo", "/hi"]]); });
