...
describe("openAIStructuredResponse: what a failed call says, and what it stops", () => {
  ...
  it("names the provider's own code, carries Retry-After, lets no free text out of the door, and holds an empty balance before the network", async () => {
    ...
    expect([refused.kind, held.capture.calls, stopped.seen, refused.kind === "blocked_credit" && refused.reason.includes("credit is exhausted")]).toEqual(["blocked_credit", 0, [], true]); // every caller inherits the stop, and a HELD account claims no probe and reaches no network
  ...
  it("reports zero requests for every refusal decided before the network, and one once the request is on the wire", async () => {
    ...
    expect([held.kind, noTenant.kind, probeLost.kind, life.seen]).toEqual(["blocked_credit", "invalid_response", "blocked_credit", ["reserve", "release:true"]]);
...
describe("a due probe is spent on the provider call itself, never on a guard in front of it", () => {
  ...
  it("survives every guard unspent, is claimed by the one request that leaves the process, and makes no call at all while held", async () => {
    try {
      ...
      expect([await g.creditBreakerHeld(T), await g.creditBreakerHeld(T), probe()]).toEqual([false, false, null]); const wire = fakeFetch(completedEnvelope(JSON.stringify({ title: "T", score: null }))); // A DUE PROBE READS AS NOT HELD to every guard, however many times they ask, and none of them stamps it.
