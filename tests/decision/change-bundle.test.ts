...
beforeEach(() => { process.env.OPENAI_API_KEY = "test-key"; store.rows.clear(); sent.length = 0; kinds.length = 0; }); afterEach(() => { delete process.env.OPENAI_API_KEY; }); describe("produceBundleForSnapshot", () => {
  it("repairs the page with the biggest proven gap, in exact drafted copy, on a receipt every component cites", async () => { const out = await produceBundleForSnapshot(snapshot(), { complete: seam, ...OPTS }); expect(out.status).toBe("bundled"); if (out.status !== "bundled") return; const p = out.proposal; const bundle = p.bundle!; expect(p.id).toBe(`${TENANT}::/rain-barrels::existing_edit::title-family`); expect(p.pagePath).toBe("/rain-barrels"); expect(p.changeFamily).toBe("title-family"); // the family, in the id AND the stamp: a snippet rewrite and a body rebuild are two changes, and the ledger must be able to tell them apart expect(p.kind).toBe("existing_edit"); // the 570-click gap, not the 55-click one
    ...
    expect(bundle.components[0].evidenceKeys).toEqual(["demand-exact", "copy-current", "serp1", "serp-owned", "serp-pattern"]); expect(bundle.receipt.items.find((i) => i.key === "copy-current")!.fact).toBe('When I read this page on 2026-07-20 it was titled "Rain Barrels" and ran 900 words across 3 sections.'); expect(bundle.receipt.items.find((i) => i.key === "serp-owned")!.fact).toBe('On that results page Google shows this page worded "Rain Barrels".'); expect(bundle.receipt.items.find((i) => i.key === "serp-pattern")!.fact).toBe('Across the other pages that come up for "rain barrel sizing", "barrel", "rain", "sizing" recur in the wording Google shows.'); expect(bundle.confidenceReasons.some((r) => r.startsWith("I checked the results page:"))).toBe(true); // the diagnosis itself, in the operator's words
  ...
  it("puts what the answers SAID on the receipt, dated and attributed, and never lets an engine's claim become source material", async () => {
    ...
    if (out.status !== "bundled") throw new Error("expected a change"); const items = out.proposal.bundle!.receipt.items; const at = (k: string) => items.find((i) => i.key === k); expect([at("named1")!.fact, at("named1")!.observationId, at("named1")!.observedAt]).toEqual(["The answers to 2 of the questions I track name waterwise as an option for this.", "obs_p1_chatgpt", "2026-07-22T00:00:00.000Z"]); // the count says how many QUESTIONS stand behind it; the id and the date say which answer it quotes
    expect(at("format1")!.fact).toBe("One answer I hold here asks for a sizing table."); // one answer is said as one answer, never as agreement
    expect([at("missing1")!.fact, at("brandnamed")!.fact]).toEqual(["The answers to 2 of the questions I track leave this unanswered: what a first flush diverter costs", "Not one of the 2 answers I hold here names your own site."]); expect([at("missing2"), items.some((i) => i.fact.includes("currently"))]).toEqual([undefined, false]); // the change SHIPS, and the line that claimed the present is the only thing left out
    ...
    expect([at("covered1")!.fact, items.some((i) => i.fact.includes("Overflow"))]).toEqual(['The answers to 2 of the questions I track cover "Roof area".', false]); expect(items.map((i) => i.fact).join(" ")).not.toContain("600 gallons"); // an engine's own claim and its caveats are never grounding for a word of copy
    expect(items.filter((i) => ["named1", "format1", "missing1", "covered1", "brandnamed"].includes(i.key)).every((i) => !!i.observedAt)).toBe(true); // dated, or the one line saying "currently" refuses the whole change on its own
    expect([at("brandnamed")!.observationId, at("brandnamed")!.observedAt]).toEqual([undefined, "2026-07-22T00:00:00.000Z"]); // an absence across every answer quotes none of them, so it borrows no answer's identity, and a date is not an identity
    expect(out.proposal.bundle!.components.every((c) => !c.evidenceKeys.includes("missing1"))).toBe(true); expectCleanCopy(out.proposal); // a title is not a piece that covers a gap, so the omission never props it up
    // IDENTITY IS MATERIAL: the same sentence read off a different answer is different evidence, while a row that carries no id at all computes exactly what it always did.
    const swap = (id: string): ChangeProposal => ({ ...out.proposal, bundle: { ...out.proposal.bundle!, receipt: { ...out.proposal.bundle!.receipt, items: items.map((i) => (i.key === "named1" ? { ...i, observationId: id } : i)) } } });
    const bare: ChangeProposal = { ...out.proposal, bundle: { ...out.proposal.bundle!, receipt: { ...out.proposal.bundle!.receipt, items: items.map(({ observationId: _drop, ...rest }) => rest) } } };
    // THE STORED-ROW COMPATIBILITY CONTRACT, as a literal: a receipt item carrying NO answer id must hash to exactly what it hashed to before ids existed, or every proposal
    // already on file is rewritten once to say the identical thing. Appending the id unconditionally (`?? null`) moves this string, which is the whole point of pinning it.
    expect(proposalFingerprint(bare)).toBe("da60e4f198cbcb0f"); expect(new Set([out.proposal, swap("obs_somebody_else"), bare].map(proposalFingerprint)).size).toBe(3); expect(deserializeChangeProposal(serializeChangeProposal(out.proposal))!.bundle!.receipt.items.find((i) => i.key === "named1")!.observationId).toBe("obs_p1_chatgpt"); });
  it("lets an omission the answers keep leaving ADD support to a piece that stands on its own, and never rescue one that stands on nothing", async () => {
    ...
    expect(b.alternatives.find((a) => a.option === "Rescued by an omission")!.reason).toContain("I could not show you the evidence behind that one"); expect(b.components[0]!.evidenceKeys).toEqual(["demand-page", "missing1"]); }); // and the omission still ADDS itself to the piece that already stood up
  it("spends nothing on a search whose results page it has never looked at, and says exactly that", async () => {
    ...
    expect(out.status).toBe("none"); if (out.status !== "none") return; expect(called).toBe(0); expect(out.reason).toBe("I have not looked at the results page for that search yet, so I cannot tell you what to change. It is first in line on my next research pass."); });
...
describe("what a receipt will and will not accept", () => { it("takes the results page bought under the same words in any order, and no winner a shared domain alone vouches for", async () => {
  ...
  it("withholds a link it cannot place, and never calls my own data alone high confidence", async () => {
    ...
    expect(b.components.some((c) => c.kind === "internal_links")).toBe(false); expect(b.receipt.items.some((i) => i.kind === "internal_link")).toBe(false); expect(b.alternatives.find((a) => a.option === "Links out to your own pages")!.reason).toContain("I can see 1 of your own page worth linking to");
    const reasons = b.confidenceReasons.join(" "); expect(reasons).toContain("your own search data"); expect(reasons).toContain("the page as I last read it"); expect(reasons).not.toMatch(/\d+ pieces/); expect(confidenceFor({ gsc: true, ownedCopy: true, serp: false, winners: 0, body: false })).toBe("low"); }); // my own demand rows and copy are still only my own data
  ...
  it("reads two winners as medium, names the one step left, and claims only a pattern both of them share", async () => {
    ...
    expect(items.filter((i) => i.kind === "winning_page").map((i) => i.fact).join(" ")).not.toMatch(/elsewhere/); expect(items.find((i) => i.key === "winpattern")!.fact).toBe('Of the 2 pages I read that come up for "rain barrel sizing", 2 of them answer it in a question and answer block, and this page has none, and the middle one runs 1,400 words against this page\'s 900.'); expect(out.proposal.status).toBe("ready"); expect(out.proposal.confidence).toBe("medium"); expectCleanCopy(out.proposal); expect(out.proposal.limitations).toContain("I do not hold this page's full body text, so I checked every draft against its title and section headings only."); expect(confidenceFor({ gsc: true, ownedCopy: true, serp: true, winners: 2, body: true })).toBe("high"); }); // reading the page itself is the only step left to certain
   ...
   *  another engine's answer: below, only the answer this account's lineage actually names may join, and its neighbour's searches stay its neighbour's. */
  ...
  /** A CHANGE MAY NOT OUTLIVE ITS OWN EXPLANATION. This used to DEMOTE the row to needs_review, which is "look at this first" and still fully actionable, so an
   *  unsupported change kept its place on the operator's list under a quieter name. It is taken back now, and a page that still earns an action is untouched. */
  ...
  /** P0. THE SNAPSHOT IS FAIL-SOFT BY DESIGN: every leg catches to empty, so one database blip reads as an account with no pages. The retire loop then proved nothing
   *  about anything, took back EVERY live change, and the refusal kept them shut afterwards. A pass may only speak about a page it actually read. */
...
describe("the coverage verdict never invents a page this account already owns", () => {
  ...
  it("says WHY a page of mine is unread and names the STORED retry date, identically however often it is asked, and never a date already past", async () => {
    ...
    expect(shut.explanation).toContain(`Your site's robots rules stopped me from reading ${ONE.url}`); expect(shut.explanation).toContain("I will check again on 2026-08-25"); expect(down.explanation).toContain(`I could not read ${ONE.url}`); expect(down.explanation).toContain("I will try it again on 2026-07-27"); // the SAME date on every visit, because the outcome is persisted
  ...
  it("carries the one purchase that would change a refusal, and stops asking once buying has stopped paying", async () => {
    ...
    expect(spent.explanation).toContain("There is nothing left I could buy or look up that would change that");
