...
beforeEach(() => { process.env.OPENAI_API_KEY = "test-key"; store.rows.clear(); sent.length = 0; kinds.length = 0; }); afterEach(() => { delete process.env.OPENAI_API_KEY; }); describe("produceBundleForSnapshot", () => {
  ...
  it("puts what the answers SAID on the receipt, dated and attributed, and never lets an engine's claim become source material", async () => {
    ...
    expect(proposalFingerprint(bare)).toBe("da60e4f198cbcb0f"); expect(new Set([out.proposal, swap(["obs_p1_chatgpt", "obs_somebody_else"]), bare].map(proposalFingerprint)).size).toBe(3); expect(proposalFingerprint(swap(["obs_p2_gemini", "obs_p1_chatgpt"]))).toBe(proposalFingerprint(out.proposal)); // the same support in another order is the same support
...
describe("what a receipt will and will not accept", () => { it("takes the results page bought under the same words in any order, and no winner a shared domain alone vouches for", async () => {
  ...
  it("withholds a link it cannot place, and never calls my own data alone high confidence", async () => {
    ...
    const reasons = b.confidenceReasons.join(" "); expect(receiptComposition(b.receipt.items)).toBe("13 checks: the live results page (3), what the results page told me about the cause (3), your search data (3), AI answers I watched, monthly search counts, the page as I last read it, winning pages I read"); expect(reasons).toContain("I built this from 13 checks:"); expect(receiptComposition([])).toBe("nothing I can show you"); expect(reasons).not.toMatch(/\d+ pieces/); expect(confidenceFor({ gsc: true, ownedCopy: true, serp: false, winners: 0, body: false })).toBe("low"); }); // my own demand rows and copy are still only my own data
  ...
  it("hands over nothing at all rather than a change it can show you nothing for", async () => {
    ...
    const two = looked({ ...topicSnapshot(), ownedPages: [topicSnapshot().ownedPages[1]!, twin] }); const one = only(two)[0]!; expect(only(topicSnapshot())).toEqual([]); // no results page for that exact search, so no suggestion at all
    ...
    expect([one.opportunityType, one.operatorSteps]).toEqual(['Move "compost bin sizing" into the first 60 characters of the title', ["Open your site editor on /compost", "Replace the title with the copy above", "Come back here and mark it done, and I start measuring"]]); // the exact move, where it happens, what I do next
    ...
    const quiet = looked({ ...topicSnapshot(), ownedPages: [soft], research: asked }); const soften = compileCandidates(quiet)[0]!; const frame = "That is under my bar for a proven change, so this is a quick test, and I will measure what it does.";
    expect(soften.reason).toContain("that search is worth about 25 clicks, under the 50 clicks on 500 searches I act on. I am watching it instead of making you work.");
    expect(["and that is only about 7 clicks, under the 50 I act on.", "and that gap is 1.5 percent, under the 2.0 percent I act on.", "and that is too little search to act on yet (I want 500 impressions on one query)."]
      .map((t) => suggestedEdits(quiet, [{ ...soften, reason: `Scope. Rates, ${t} I am watching it instead of making you work.` }], { now: OPTS.now!, basis: "b" })[0]!.whyItMatters.split(" This line says")[0]!))
    ...
    const ranked = rankProposals([res.proposals[0]!, one]); expect(only(quiet)[0]!.whyItMatters).toContain("AI answers about this topic credit compostpro, never you.");
    expect([!!ranked[0]!.whyRankedAboveNext, ranked[0]!.rankingReceipt!.basis.length > 0, ranked[1]!.rankingReceipt!.factors.length > 0, ranked[1]!.whyRankedAboveNext, ledgerProofLine({ windows: [] }),
      ledgerProofLine({ windows: [{ day: 7, ran: true, controlsUsed: 3, adjustedLift: 12.4 }, { day: 28, ran: true, controlsUsed: 4, adjustedLift: -3.2 }, { day: 56, ran: false, controlsUsed: 0 }] })]).toEqual([true, true, true, undefined, null, "clicks -3 against similar pages I did not change"]);
