...
describe("what is wrong with how a page is served", () => { it("names every fault it can prove, on a concrete address, with the exact fix", () => { const found = readTechnicalFindings(SERVED);
...
describe("one score orders every kind of change, and says why", () => { it("puts the lever the evidence named above a bigger one it did not, on the same page", () => { const named = prop({ id: "title-fix", impactScore: 120, diagnosisCause: "ctr_snippet", bundle: bundleOf([comp({ kind: "title" })]) });
  ...
  it("never reads as ready while it leaves its own diagnosed cause unsettled", () => { const NAMED = ["iranopedia.com/persian-female-first-names", "iranopedia.com/persian-names"];
  ...
  it("never drops a word the page earns clicks on, and demand vocabulary overrides the banned list", () => { const body = "Persian boy names with meanings, a list of classic and modern Iranian names for boys.";
    ...
    const farsi = (demand: string[]) => deliverableFailures({ targetUrl: "https://www.iranopedia.com/persian-male-first-names", // The Farsi ruling: banned everywhere EXCEPT where a real stored search for this page carries it.
      ...
      claims: [{ text: "a list of classic and modern Iranian names for boys in Farsi", supportedBy: ["page-copy-1", "demand-1"] }], finalCopy: "Persian boy names with meanings: classic and modern Iranian names for boys in Farsi." } as never, pk({ preserve: [], vocabulary: demand }, ["Farsi"], 'people search "persian boy names in farsi" 480 times in 90 days') as never);
  ...
  it("a focused key funds first and past skip, and a spend on one family never declines the different job the same key owes", () => { // LIVE 2026-08-30: a standing card ONE review from Ready could not be funded by any means, because fresh page candidates outrank it and skip only declines. Focus aims the same money, never more of it.
    const j = (key: string, impact: number) => ({ key, family: "editor", impact, calls: 2 }); const out = DRAFT_BUDGET.plan({ jobs: [j("/big", 900), j("/stalled", 1)], candidates: 1, calls: 4, skip: ["/stalled::editor"], focus: ["/stalled"] });
    expect([out.funded.map((f: { key: string }) => f.key), out.declined.some((d) => d.key === "/big")], "the named card takes the one slot; the bigger page waits its turn").toEqual([["/stalled"], true]);
    // ALREADY BOUGHT NEVER BLOCKS A DIFFERENT OBLIGATION (operator, 2026-09-02): the day's memory carries `key::family`, so a page whose DRAFT was spent today can still fund the READING it owes on the same key.
    const owed = DRAFT_BUDGET.plan({ jobs: [j("/p::meta", 5), { key: "/p::meta", family: "correction_review", impact: 5, calls: 2 }], candidates: 2, calls: 20, skip: ["/p::meta::editor"] });
    const spent = DRAFT_BUDGET.plan({ jobs: [j("/p::meta", 5)], candidates: 2, calls: 20, skip: ["/p::meta::editor"] });
    expect([owed.funded.map((f: { key: string }) => f.key), spent.funded.length, (spent.declined[0]?.reason ?? "").includes("already spent on this exact job")], "the review is funded although today already spent this key's draft, and the draft alone is still declined in its own words").toEqual([["/p::meta"], 0, true]); });
  ...
  describe("grounded writing: mechanical placement, evidence-first copy, corrective retries", () => {
    ...
    it("hands the writer the CAUSE it must solve, never the card's traffic prose, and refuses body copy for a row that carries neither", async () => {
      ...
      await applyDraftedCopy([bare as never], { tenantId: TENANT, snapshot: snap as never, now: NOW, refusals: said, complete: async ({ user }: { user: string }) => (asked.push(user), { value: GOOD }), budget: DRAFT_BUDGET.plan({ jobs: [{ key: DRAFT_BUDGET.keyOf(bare as never), family: "editor", impact: 9, calls: DRAFT_BUDGET.DELIVERABLE_CALLS }], candidates: 1, calls: 30 }) } as never);
    ...
    it("the diagnosis is the whole assignment: told what to add and what only to look at, shaped by the gap not the query, kept on the retry, a list-led draft refused and a fact-led one continued additively", async () => { const asked: string[] = []; const { canonicalUrlKey: ck6 } = await import("@/domains/evidence/snapshot"); bodyStore.map = new Map([[ck6(BODY.url), BODY]]);
      ...
      const card = prop({ id: `${TENANT}::/funny-farsi-phrases::existing_edit::art-false-tips-promise`, pagePath: "/funny-farsi-phrases", pageUrl: BODY.url, changeFamily: "section", status: "needs_review" as const, researchOnly: false, primaryQuery: "funny persian phrases", whyItMatters: "This page is shown 8,898 times in 90 days.", causeFinding: { cause: "retrieved_not_cited", action: null, evidenceKeys: ["k1"], competingExplanations: [], notConsidered: [], falsifier: "f", explanation: "The introduction promises usage tips but the page only lists phrases." } as never, limitations: [], evidence: { query: "funny persian phrases", hints: [], evidenceRefCount: 1 }, recommendedChange: { kind: "existing_edit" as const, field: "section" as const, before: null, after: "Jeegareto bokhoram is affection. Moosh bokhoradet is playful." } });
      const LEAD = "\u0161oma is the deferential or formal you, so use it with anyone you address respectfully.", snap6 = { ownedPages: [{ url: BODY.url, content: { wordCount: 400, title: BODY.title, h1: BODY.h1, outline: BODY.headings }, search: null }], research: {}, sources: [], scope: { tenantId: TENANT, site: "iranopedia.com" } };
      const listLed = { ...GOOD, after: `${P1}\n${P2}\n${P3}\n${LEAD}`, claims: [...GOOD.claims, { text: LEAD, supportedBy: ["fact-1"] }] }, factLed = { ...GOOD, after: `${LEAD} Jeegareto bokhoram is one phrase said between close friends.`, claims: [{ text: LEAD, supportedBy: ["fact-1"] }, { text: "Jeegareto bokhoram is said between close friends.", supportedBy: ["page-copy-2"] }] };
...
describe("a change earns ready on its own evidence, its whole version, and words that still stand", () => {
  ...
  it("will not preserve banked copy today's rules would refuse, or copy whose support moved underneath it", () => {
    ...
    expect([why(), why({}, null), why(copy(GOOD.replace("International ", "")))[0], why({ supportFacts: [{ id: "card-1", fact: SHIP }, { id: "card-2", fact: "An entirely different reading nobody wrote this copy from." }] })[0], why({ supportFacts: [{ id: "card-1", fact: SHIP }] })[0], why({}, { ...page, metaDescription: "A description this page no longer carries." })[0], why({ claims: undefined })])
...
describe("the pass walks the order the money was committed in", () => {
  ...
  const drive = async (cards: readonly ChangeProposal[], over: Record<string, unknown> = {}, throwing = false) => { vi.resetModules(); sent.length = 0; vi.doMock("@/domains/decision/producers/extra", () => ({ extraQueuePass: async () => ({ run: { cards, complete: true, held: [], needsOwnPage: [], families: ["ai_answer_gap"] }, unitLoad: null }) })); if (throwing) vi.doMock("@/domains/decision/drafted-copy", async (orig) => ({ ...(await orig<Record<string, unknown>>()), applyDraftedCopy: async () => { throw new Error("the writer transport died"); } })); const { produceProposalsForTenant: run } = await import("@/domains/decision/produce-proposals"); env.snap = snapshot({ ownedPages: SLUGS.map((x) => page({ url: `fixture-content.example/${x}` })) });
    return run(TENANT, { complete: seam, ...OPTS, produce: true, ...over }); };
  ...
  it("funds the ranked order, walks every funded key, spends nothing on the unfunded, and resumes the owed one ahead of the untried one", async () => {
    ...
    const next = await drive(cards, { maxDrafts: 2, skipKeys: mine.map((k) => `${k}::editor`) }); expect(next.paid.funded.filter(keyed), "the owed one resumes first and the untried one follows it").toEqual(["/charlie::body::charlie sizing", "/delta::body::delta sizing"]); }); // the day's own memory, exactly as the runtime keeps it
  it("names a throw on the key it happened to, and gives a funded key its real reason when no row can spend it", async () => {
    ...
    const threw = out.paid.receipts.filter((r) => (r.why ?? "").includes("the writer transport died")); expect([threw.length, threw.every((r) => r.outcome === "retryable_blocked")], "every card in the run that threw carries the error on its own key, and none of them is written off for it").toEqual([2, true]);
    const refused = gap("charlie", 30, { id: `${TENANT}::/charlie::existing_edit::missing_description`, changeFamily: "meta", researchOnly: false, recommendedChange: { kind: "existing_edit", field: "meta", before: "Old.", after: "A description of the charlie page." } }); store.rows.set(refused.id, refused); const aimed = await drive([], { maxDrafts: 3, focusKeys: ["/charlie::meta", "/ghost-page"] });
    expect(aimed.paid.declined?.find((d) => d.key === "/charlie::meta"), "a card the diagnosis refuses is named with its own reason and never funded, so no slot is held open for work that cannot be bought").toMatchObject({ family: "editor", reason: expect.stringContaining("does not treat it") }); expect(aimed.paid.receipts.find((r) => r.key === "/ghost-page"), "and a funded key no row on file carries settles with the truth instead of a fault sentence").toMatchObject({ outcome: "deterministic_refusal", why: expect.stringContaining("no row on file carries this job") });
  ...
  it("buys nothing at all for work its own gates already settled", async () => {
    ...
    store.rows.set(settled.id, settled); const out = await drive([], { maxDrafts: 2, focusKeys: [DRAFT_BUDGET.keyOf(settled)] }), row = store.rows.get(settled.id)!;
    expect([(row.recommendedChange as { after: string }).after, row.obligation?.kind], "the settled row keeps its words and its verdict").toEqual(["A finished section for the alpha page, written and settled.", "terminal"]);
    expect(out.paid.receipts.filter((r) => r.key === DRAFT_BUDGET.keyOf(settled)).every((r) => r.providerCalls === 0 && r.costUsd === 0), "and replaying it costs nothing").toBe(true); });
});
...
describe("the reading taken over finished copy", () => {
  ...
  it("banks the exact rulings, refuses to bank a clerical mismatch as anything at all, writes the evaluator's own objection when the COPY is what failed, and reads nothing off a row that banks nothing", async () => {
    ...
    const bare = await read({ ...PASSED, claims: [] }, finished({ claims: [], supportFacts: [] }));
    expect([bare.row, bare.detail.includes("banks no claims or no sources")], "a row with no claims and no sources has nothing to read them against, and nobody is paid to find that out").toEqual([null, true]); });});
