...
describe("one score orders every kind of change, and says why", () => { it("puts the lever the evidence named above a bigger one it did not, on the same page", () => { const named = prop({ id: "title-fix", impactScore: 120, diagnosisCause: "ctr_snippet", bundle: bundleOf([comp({ kind: "title" })]) });
  ...
  it("a focused key funds first and past skip, so a named card can actually be finished", () => { // LIVE 2026-08-30: a standing card ONE review from Ready could not be funded by any means, because fresh page candidates outrank it and skip only declines. Focus aims the same money, never more of it.
    const j = (key: string, impact: number) => ({ key, family: "editor", impact, calls: 2 }); const out = DRAFT_BUDGET.plan({ jobs: [j("/big", 900), j("/stalled", 1)], candidates: 1, calls: 4, skip: ["/stalled"], focus: ["/stalled"] });
