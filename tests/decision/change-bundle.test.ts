...
describe("one score orders every kind of change, and says why", () => { it("puts the lever the evidence named above a bigger one it did not, on the same page", () => {
  ...
  describe("grounded writing: mechanical placement, evidence-first copy, corrective retries", () => {
    ...
    const BODY = { url: "https://www.iranopedia.com/funny-farsi-phrases", title: "Funny Farsi Phrases" as string | null, h1: "Funny Farsi Phrases" as string | null, metaDescription: null, vocabulary: "", headings: ["Playful Persian expressions"], passages: [P1, P2, P3] };
  ...
  it("the cards that reached a customer are refused before a model is asked", () => { const pk = (bodyText: string, bannedTerms: string[] = []) => ({ targetUrl: "https://www.iranopedia.com/x", title: "T", h1: "H", metaDescription: null, bodyText, headings: [], evidence: { "page-copy-1": bodyText }, trackedQuestion: "Q", ownedPaths: ["/x"], bannedTerms, demand: { preserve: [], vocabulary: [] } });
    ...
    expect(RECV)
      ...
      .toEqual([["it tells a reader this page offers \"Topoli\", \"Gooz\", \"Bikhial\", and this page's own evidence shows no such thing", "it points at the page instead of answering", "it uses words this account does not publish: Farsi", "where it goes is two page elements glued together, which nobody can find on the rendered page", "where it goes is taken from the crawl's own markers, not from the page", "it tells a reader this page offers \"Chert-o-Pert\", \"gives brief meanings\", \"typical contexts\", and this page never puts those words together", "its copy names \"Pedar\", \"Sag\", and nothing on file about this page mentions them"], ["it points at the page instead of answering", "its copy names \"Finglish\", and nothing on file about this page mentions them"], ["where it goes is a paragraph rather than a place on the page", "where it goes is two page elements glued together, which nobody can find on the rendered page", "its copy names \"Iranian\", \"Cyrus\", \"Great\", and nothing on file about this page mentions them"]]) });
  // THE READY CARD ON THE LIVE ACCOUNT, 2026-08-15: the description sells "modern designs" and the page shows neither. It slipped a bag-of-words corpus because one stored sentence says "timeless designs" and another says "modern fashion", so every part of the phrase is on file and the phrase itself is not. THE PAGE'S OWN ORDER CATCHES IT, and DECLARING IT CANNOT LAUNDER IT: the second case adds the exact claim the writer would have had to write, and the refusal stands, because a claim is the writer's own bookkeeping and this page still never says those words together. Real stored body, real persisted claims, real shipped copy.
  it("refuses copy that recombines the page's words into something it never says", () => {
