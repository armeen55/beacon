...
describe("one score orders every kind of change, and says why", () => { it("puts the lever the evidence named above a bigger one it did not, on the same page", () => {
  ...
  it("calls a deliverable finished only when it is the work, by type", () => {
    ...
    expect([g("Cover the pot with a lid so it steams for ten minutes, then fluff the rice with a fork."), g("Fill in the form online, then pay the fee at a designated bank branch."), g("Include a copy of your passport photo page when you apply."), g("Link building for a Persian culture site works best through museums and university pages.", "meta"), g("Add Saffron to Your Rice: A Persian Cook's Guide", "title")])
      .toEqual(Array(5).fill(undefined)); });
  // THE EDITOR'S OWN HOMEWORK IS CHECKED AGAINST THE STORED PAGE, NEVER AGAINST ITS SPELLING. A faithful paraphrase whose every claim names real stored evidence is FINISHED, which is the exact case the old containment word list refused: not one of "survived", "point to" or "carrying" is printed on this page. Evidence nobody holds, a replacement of words the page does not carry, a placement that is nowhere on it, and the tracked question pasted back as a heading are each refused by name.
  it("an editor's deliverable is finished only when the stored page carries it", () => {
    const P = { targetUrl: "https://www.iranopedia.com/iran-flags/achaemenid-empire-flag", title: "Achaemenid Empire Flag (550 to 330 BCE)", h1: null, headings: ["Explore More"], trackedQuestion: "What are funny Persian phrases and idioms?", ownedPaths: ["/iran-flags/achaemenid-empire-flag", "/persian-female-first-names"], evidence: { "page-copy-1": "Though no official flag design has been preserved, ancient reliefs and inscriptions suggest banners with eagle and winged sun symbols" }, bodyText: "Though no official flag design has been preserved, ancient reliefs and inscriptions suggest banners with eagle and winged sun symbols. The Achaemenid Empire Flag is often depicted with a red background and a golden Faravahar or eagle emblem, from 550 to 330 BCE." };
    const D = { actionType: "meta" as const, targetUrl: P.targetUrl, placementAnchor: "Achaemenid Empire Flag", beforeText: null, naturalHeading: null, evidenceIdsUsed: ["page-copy-1"], uncertaintyOrOmitted: [], implementationMinutes: 3, measurementTarget: "clicks on this page", claims: [{ text: "No Achaemenid flag has survived", supportedBy: ["page-copy-1"] }], finalCopy: "No Achaemenid flag has survived. Reliefs and inscriptions point to a red banner carrying a golden Faravahar or eagle, used from 550 to 330 BCE." };
    expect([deliverableFailures(D, P), deliverableFailures({ ...D, claims: [{ text: "Cyrus raised it himself", supportedBy: ["made-up-7"] }] }, P)[0], deliverableFailures({ ...D, beforeText: "a line this page never carried" }, P)[0], deliverableFailures({ ...D, placementAnchor: "a heading nowhere on the page" }, P)[0], deliverableFailures({ ...D, actionType: "answer_block", naturalHeading: P.trackedQuestion }, P)[0]])
      .toEqual([[], "it names evidence that is not on file: made-up-7", "the words it says it replaces are not on the stored page", "the place it says it lands is not on the stored page", "its heading is the tracked question said back word for word"]); });
