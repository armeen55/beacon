...
describe("one score orders every kind of change, and says why", () => { it("puts the lever the evidence named above a bigger one it did not, on the same page", () => {
  ...
  it("the cards that reached a customer are refused before a model is asked", () => { const pk = (bodyText: string, bannedTerms: string[] = []) => ({ targetUrl: "https://www.iranopedia.com/x", title: "T", h1: "H", metaDescription: null, bodyText, headings: [], evidence: { "page-copy-1": bodyText }, trackedQuestion: "Q", ownedPaths: ["/x"], bannedTerms, demand: { preserve: [], vocabulary: [] } });
    ...
    expect([d({ P: pk(chrome, ["Farsi"]), placementAnchor: chrome, finalCopy: "Funny Persian phrases and idioms are common Farsi insults and playful slang such as Pedar Sag, Topoli, Gooz, Bikhial, and Chert-o-Pert. This page lists those expressions, gives brief meanings and typical contexts. See the headings below for each example and its short meaning." }),
      ...
      .toEqual([["it tells a reader this page offers \"Topoli\", \"Gooz\", \"Bikhial\", and this page's own evidence shows no such thing", "it points at the page instead of answering", "it uses words this account does not publish: Farsi", "where it goes is two page elements glued together, which nobody can find on the rendered page", "where it goes is taken from the crawl's own markers, not from the page",
        "it tells a reader this page offers \"Topoli\", \"Gooz\", \"Bikhial\", and no claim on this card carries it", "its copy says \"idiom\", \"playful\", \"pedar\", which no claim it makes and no evidence those claims name carries"],
        ["it points at the page instead of answering", "its copy says \"beginner\", \"simple\", \"sentence\", which no claim it makes and no evidence those claims name carries"],
        ["where it goes is a paragraph rather than a place on the page", "where it goes is two page elements glued together, which nobody can find on the rendered page", "its copy says \"iranian\", \"today\", \"cyru\", which no claim it makes and no evidence those claims name carries"]]); });
  ...
  it("refuses copy that asserts something none of its own claims carries", () => {
    ...
    expect([d(), d({ claims: [...claims, { text: "The page shows modern designs", supportedBy: ["page-copy-1"] }] })])
      .toEqual([["it tells a reader this page offers \"modern designs\", and no claim on this card carries it"], []]); });
