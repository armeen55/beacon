...
import type { CauseFinding } from "@/domains/decision/diagnosis"; import { effortMinutesFor, fieldForComponent, type ProducerCtx } from "@/domains/decision/producers/contract";
...
describe("the causes that had no copy now write one, or refuse in words", () => {
  ...
  it("buys no rewrite without its current complete original and a structure-preserving composition path", async () => { const causes = ["weak_opening", "incomplete_coverage"] as const; let calls = 0; const draft = { ...whole(), section: async () => { calls += 1; return null; } }; for (const body of [null, { ...wholeCtx().body!, completeness: "sample_only" as const }, { ...wholeCtx().body!, version: "stale_known_good" as const }, { ...wholeCtx().body!, passages: [] }]) { const result = await produceFullRewriteRecommendation(ctxOf({ body, draft }), causes); expect([result.components.length, result.requirement?.kind]).toEqual([0, "page_source"]); } const result = await produceFullRewriteRecommendation(wholeCtx({ draft: { ...draft, compose: undefined } }), causes); expect(result.refusal).toContain("no rewrite is bought"); expect(calls).toBe(0); });
  it("rebuilds only when causes agree and banks interrupted pieces as one unchanged replacement assignment", async () => {
    ...
    const causes = ["weak_opening", "incomplete_coverage", "weak_opening"] as const; const many = await produceFullRewriteRecommendation(wholeCtx({ draft: whole() }), causes);
    const c = many.components[0]!; expect([c.kind, c.risk, answered(c)]).toEqual(["full_rewrite", "review", true]); expect([c.after.startsWith(OPENING), c.after.includes("How much water a roof collects"), c.after.includes("Choosing a barrel size"), c.after.includes("still owed")]).toEqual([true, true, true, false]);
    expect([c.objective!.includes("a guide that answers the question from end to end"), c.mechanism!.includes("2 things are wrong at once")]).toEqual([true, true]); expect([componentRefusals(validate(many.components)), validate(many.components).verdict]).toEqual([[], "ready"]);
    expect(c.preserves!.keeps).toEqual([]); expect(c.preserves!.losses.map((l) => l.what)).toEqual([wholeCtx().body!.passages[0], "How much rain a roof collects", "Barrel sizes", ...LINKS.map(link => `${link.anchorText}: ${link.href}`)]); expect(c.preserves!.keeps).not.toContain("Roof area"); expect(c.preserves!.losses.every((l) => l.why.includes("no deletion is authorized"))).toBe(true); expect([c.before, c.target, c.units?.filter((u) => u.kind === "heading").length]).toEqual([wholeCtx().body!.passages.join("\n\n"), { mode: "whole_body", anchorKind: null, anchor: null }, 2]); const silent = validate([{ ...c, preserves: { keeps: [], losses: [] } }]);
    expect([silent.verdict, silent.reasons.some((r) => r === 'The rebuild drops "Barrel sizes" and never says why, so it stays held rather than offered.')]).toEqual(["rejected", true]); const bare = { ...bundleOf(many.components), receipt: { items: KEYS.map((key) => ({ key, kind: "gsc_demand" as const, fact: FACTS[0]!, observedAt: null })), missing: [], freshestObservedAt: null } };
    ...
    const four = { pattern: { ...PATTERN, commonHeadings: PLANNED.map((heading, i) => ({ heading, seenOn: [i] })) } }; const partial = await produceFullRewriteRecommendation(wholeCtx({ ...four, draft: whole(2) }), causes); const bank = JSON.parse(JSON.stringify(partial.draftBank)); // Pure recovery control; the dispatcher fixture below separately exercises real piece authorization and canonical decoding.
    expect([partial.components.map(x => x.kind), bank.pieces.map((p: { slot: number }) => p.slot), bank.brief.owed]).toEqual([["full_rewrite"], [1, 2], [0, 3, 4, 5, 6, 7, 8]]); const draft = whole(), asked: (string | null)[] = []; const resumed = await produceFullRewriteRecommendation(wholeCtx({ ...four, draftBank: bank, draft: { ...draft, section: async i => { asked.push(i.heading); return draft.section(i); } } }), causes); expect([asked, resumed.draftBank!.brief.owed, resumed.components[0]!.after.startsWith(OPENING)]).toEqual([PLANNED.slice(2), [], true]); const forbidden = async () => { throw Error("unchanged bank must not draft"); }; expect((await produceFullRewriteRecommendation(wholeCtx({ ...four, draftBank: resumed.draftBank, draft: { ...whole(), section: forbidden, openingAnswer: forbidden } }), causes)).components).toEqual(resumed.components);
    const mute = await produceFullRewriteRecommendation(wholeCtx({ draft: { ...whole(), openingAnswer: async () => null } }), causes); expect([mute.components.map(x => x.kind), mute.draftBank!.pieces.length, mute.draftBank!.brief.owed]).toEqual([["full_rewrite"], 2, [0]]); let openingCalls = 0; const completed = await produceFullRewriteRecommendation(wholeCtx({ draftBank: mute.draftBank, draft: { ...whole(), section: forbidden, openingAnswer: async () => { openingCalls++; return OPENING; } } }), causes); expect([openingCalls, completed.draftBank!.brief.owed]).toEqual([1, []]); const changed = await produceFullRewriteRecommendation(wholeCtx({ primary: "changed query", draftBank: mute.draftBank, draft: { ...whole(), section: forbidden, openingAnswer: forbidden } }), causes); expect(changed.refusal).toContain("different page, evidence or assignment"); // No cache assumption or full-page approval inherited from pieces.
    const blind = await produceFullRewriteRecommendation(wholeCtx({ pattern: null, draft: whole() }), causes); expect([blind.components.length, blind.refusal!.includes("side by side")]).toEqual([0, true]); const capture = { version: 1 as const, complete: true, mainHtml: '<h1>Rain</h1><p><a href="/roof">Roof area</a> determines capture.</p><h2>Overflow</h2><p>Overflow needs a safe outlet.</p><figure><img src="/roof-before.jpg" alt="Roof collection diagram"></figure>', jsonLd: [] as string[] }, original = { ...wholeCtx().body!, sourceCapture: capture }, scoped = await produceFullRewriteRecommendation(wholeCtx({ body: original, draft: { ...whole(), openingAnswer: async () => null } }), causes), saved = JSON.parse(JSON.stringify(scoped.draftBank)), bankBefore = JSON.stringify(saved); let writes = 0; for (const mainHtml of [capture.mainHtml.replace('/roof"', '/changed-roof"'), capture.mainHtml.replace('<h2>Overflow</h2>', '<h3>Overflow</h3>'), capture.mainHtml.replace('<p>Overflow needs a safe outlet.</p>', '<ul><li>Overflow needs a safe outlet.</li></ul>'), capture.mainHtml.replace('roof-before.jpg', 'roof-after.jpg')]) { const stopped = await produceFullRewriteRecommendation(wholeCtx({ body: { ...original, sourceCapture: { ...capture, mainHtml } }, draftBank: saved, draft: { ...whole(), section: async () => { writes++; return null; }, openingAnswer: async () => { writes++; return null; } } }), causes); expect([stopped.components, stopped.refusal, writes, JSON.stringify(saved)]).toEqual([[], expect.stringContaining('different page, evidence or assignment'), 0, bankBefore]); } const reused = await produceFullRewriteRecommendation(wholeCtx({ body: { ...original, fetchedAt: '2026-09-14T00:00:00Z', sourceCapture: { jsonLd: ['{"@context":"https://schema.org","@type":"Article"}'], mainHtml: capture.mainHtml, complete: capture.complete, version: capture.version } }, draftBank: saved, draft: { ...whole(), section: forbidden, openingAnswer: async () => { writes++; return OPENING; } } }), causes); expect([writes, reused.draftBank!.brief.owed, reused.draftBank!.pieces.slice(1), JSON.stringify(saved)]).toEqual([1, [], saved.pieces, bankBefore]); }); // no reading of the pages that win means no rebuild, however many causes fired
