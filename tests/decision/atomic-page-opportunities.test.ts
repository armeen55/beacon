...
describe("two audiences losing clicks on one page are two rows", () => {
  ...
  it.each(SITES)("$t: a settled row whose winners nobody has read is re-minted owing that reading, and one whose winners were read and carry nothing stays settled", async (s) => {
    ...
    expect([[nothing.winnersOnFile, sibling.winnersOnFile, own.winnersOnFile], nextObligation(preferFinished(nothing, settled(nothing))), nextObligation(preferFinished(sibling, settled(sibling))), nextObligation(preferFinished(own, settled(own))), nextObligation(preferFinished({ ...nothing, winnersOnFile: undefined }, settled(nothing)))],
      "the mint says what is on file for THIS row's own search: nothing at all, a results page for a sibling phrasing of the group whose winner joins no comparison this row will ever see, or a page winning this very search read whole. The sweep re-mints the settled row through that same comparison and the store's own recompute turns the first two settlements into the reading each one owes, named by what is already on file: the results page where none was ever bought, and the winners themselves where that page is on file and nothing off it has been read, while the third keeps its honest refusal, and a row no producer ever stamped is left exactly as it was because absent decides nothing")
      .toEqual([["none", "unread", "read"], owes, reads, stands, stands]);
  });
});
