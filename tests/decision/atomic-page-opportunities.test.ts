...
describe("a recovery card reads as a person would say it", () => {
  it("names the page by its title, dates the windows in words, never shouts LOST and never offers a bare zero as within reach", async () => { const s = SITES[0]!, hist = { earlyClicksPerDay: 1.9, recentClicksPerDay: 0.67, lostClicksPerMonth: 37, earlyImpressions: 9000, recentImpressions: 2000, priorTopPage: url(s), currentTopPage: url(s), pageSwapped: false, earlyPosition: 2.3, recentPosition: 5, pageEarlyPosition: 2.3, pageRecentPosition: 5, pageShareEarly: 1, pageShareRecent: 1 };
    ...
    const out = await (await import("@/domains/decision/producers/demand-recovery")).demandRecoveryCards({ tenantId: s.t, snapshot: snapshot(s, [page(s, [[s.big, 900]])]), now: NOW, preloaded: { units: [unit as never], historyWindow: { earlyDays: 390, recentDays: 90, earlyFrom: "2025-05-26", earlyTo: "2026-06-20" } } as never });
    expect([out.cards.length, out.cards[0]!.opportunityType, out.cards[0]!.pageLabel, out.cards[0]!.whyItMatters, /LOST|about 0|treatment/.test(JSON.stringify(out.cards[0]))]).toEqual([1, `Win back "${s.big}" on ${s.title}: the page slipped from position 2.3 to 5.0 and is down 37 clicks a month`, s.title, `Searches for ${s.big} brought this site about 57 clicks a month from May 2025 to June 2026 and bring about 20 now, down 37 a month. The page itself slid from position 2.3 to 5.0 for this audience's searches, so something better took its ground: the fix is stronger content, not a sharper line.`, false]); });
});
