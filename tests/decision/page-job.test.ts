...
describe("the three things a reading says about a page, and what each one changes", () => {
  ...
  it.each(["tenant-one", "tenant-two"])("hands the writer the capability a reader still cannot get here and the things the page sells, and asks for both to be left standing [%s]", async (tenant) => {
    ...
    expect([prompt.includes(`${SUBJECT}: ${ANSWER}`), prompt.includes("Judged as an answer this page does not carry, which one publisher that was read for it may stand behind, rated likely")], "and the checked statement reaches the writer under its own id, saying which bar admitted it, so a card that stands on an addition can be told from one that replaces the page's own words").toEqual([true, true]);
