...
const seam = (value: unknown): { complete: CompleteFn; calls: () => number } => { let calls = 0; return { calls: () => calls, complete: async () => budget.allowed ? (calls += 1, { value, httpAttempts: 1 }) : { error: "blocked_budget", failure: "budget", retryable: false, httpAttempts: 0 } }; }; // The injected gateway seam enforces the same pre-wire cap refusal as the atomic production door.
...
/** WHAT THE POOL PAYS FOR A CALL THAT NEVER LEFT THE PROCESS (reviewer, 2026-09-06, seventh pass). The charge asked the STATUS, and the six refusals the gateway gives in front of the wire all arrive as `validation_failed` with no request and no dollars, so an account with no credit burned a unit a page and stepped its rotation over every page nobody read. It asks the one rule now. Two accounts with unrelated subjects, through the real `loadPageJobs` and the real drafter. */
const POOL = [{ t: "acct-reef", e: { url: "https://acct-reef.example/tide-pool-guide", title: "Tide pools", h1: "Tide pools", headings: ["What to bring", "When to go"], wordCount: 740 } },
  { t: "acct-loom", e: { url: "https://acct-loom.example/blackwork-stitches", title: "Blackwork", h1: "Points de blackwork", headings: ["Ordre des points", "Tension"], wordCount: 620 } }] as const;
...
describe("the page-reading pool", () => {
  ...
  it.each(POOL)("$t: and a request that did reach the provider and came back unusable keeps its unit spent", async (s) => { budget.allowed = true; const pool = { left: 3 };
    await loadPageJobs(s.t, [s.e], { complete: (async () => ({ value: { ...READING, topics: ["one", "two"] }, httpAttempts: 1 })) as never, store: store(false), reads: pool }); expect(pool.left, "a reading the provider answered and the schema could not use was still bought, so its unit stays spent and only the calls nobody made come back").toBe(2); });
...
describe("page identification is not absence evidence", () => {
  ...
  it.each(["tenant-one", "tenant-two"])("hands the writer the evidenced diagnosis and conversion actions, never the old metadata guess [%s]", async (tenant) => {
    ...
    const { claimIdentity, pageHashOf } = await import("@/domains/evidence/pages/fact-check-run"), { rulesVersionFor } = await import("@/domains/evidence/pages/fact-checks");
    ...
    const card = { id: `${tenant}::/hand-loom::existing_edit::missing_answer`, tenantId: tenant, kind: "existing_edit", pagePath: "/hand-loom", pageUrl: url, pageLabel: "The Hand Loom", primaryQuery: "hand loom", opportunityType: "Capture clicks", changeFamily: "section", status: "needs_review", researchOnly: true,
      ...
      causeFinding: { cause: "ai_citation_gap", action: "section", evidenceKeys: ["ai-citations"], competingExplanations: [], notConsidered: [], falsifier: "f", explanation: "e", payload: { cause: "ai_citation_gap", engine: "chatgpt", promptText: "q", missing: "how one hand loom differs from another", aeoKind: "missing_information" } }, factIdentity: claimIdentity(SUBJECT, "", "missing") };
...
describe("a verdict that depends on the template siblings is one verdict, whatever order they arrive in", () => {
  ...
  it.each(["tenant-one", "tenant-two"])("reads every sibling that shares the shell, so twelve of them in one order and in the reverse give one answer [%s]", async (tenant) => {
    ...
    expect([asked([...all]).some((r) => r.includes(SPUN)), asked([...all]).some((r) => r.includes("were not all read")), asked([self, ...kin.slice(0, 3)]).includes(NOT_READ), asked([self, ...kin.slice(0, 3)]).some((r) => r.includes(SPUN))],
      "handed the account's own page list, a whole family answers and says nothing about being unread, and a family three of twelve read says so and rules on nothing: one verdict or none, never two").toEqual([true, false, true, false]);
    ...
    expect([asked([...kin, ...strangers]).includes(UNREAD_SELF), asked([...kin, ...strangers]).some((r) => r.includes(SPUN)), asked([]).includes(UNREAD_SELF)],
      "with other pages in hand and this one absent, the rule holds the row as unread and rules on nothing; with no page read at all it says nothing, because a read that never happened is not this page's absence").toEqual([true, false, false]); });
});
