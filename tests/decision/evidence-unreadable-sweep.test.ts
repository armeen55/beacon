...
const env = vi.hoisted(() => ({ rpc: {} as Record<string, RpcAnswer[]>, calls: [] as Array<{ name: string; args: unknown }>, snapshot: null as unknown, schemaBody: undefined as unknown,
  wrote: [] as string[], saved: new Map<string, ChangeProposal>(), store: new Map<string, unknown>(), withdrawn: [] as string[], captureId: "cap-a",
...
vi.mock("@/domains/evidence/pages/owned-context", async (orig) => { const actual = (await orig()) as typeof import("@/domains/evidence/pages/owned-context");
  return { ...actual, loadOwnedPageBodies: async (...args: Parameters<typeof actual.loadOwnedPageBodies>) => env.schemaBody === undefined ? actual.loadOwnedPageBodies(...args) : env.schemaBody === "fail" ? Promise.reject(new Error("body read failed")) : new Map(env.schemaBody ? [["fixture.example/a", env.schemaBody]] : []) }; });
vi.mock("@/domains/decision/proposal-store", async (orig) => { const actual = (await orig()) as typeof import("@/domains/decision/proposal-store");
  return { ...actual, loadChangeProposals: async () => { if (!proposalReads.current) throw new Error("current queue unavailable"); return new Map(env.store as Map<string, ChangeProposal>); },
    ...
    withdrawChangeProposal: async (p: ChangeProposal, _why?: string, capture?: Record<string, unknown>) => { env.calls.push({ name: "withdrawChangeProposal", args: capture }); if (p.id.includes("::missing_answer") && capture?.id !== env.captureId) return "blocked" as const; env.withdrawn.push(p.id); return "retired" as const; },
...
beforeEach(() => { env.rpc = {}; env.calls = []; env.snapshot = null; env.schemaBody = undefined; env.store = new Map(); env.withdrawn = []; env.captureId = "cap-a"; env.aiWindow = []; env.dispositions = new Map(); env.upserts = 0; proposalReads.current = true; proposalReads.terminal = true; });
...
describe("the sweep only retires what a producer that FINISHED rewrote", () => {
  ...
  it("sweeps only finished families and an exact answered capture, never an AI outage, fifth unanswered query, failed read or recapture race", async () => {
    const now = new Date("2026-08-12T09:00:00Z"), url = "https://fixture.example/a", base = snapshotWith("fresh") as ReturnType<typeof buildEvidenceSnapshot>, title = "Basic Persian Phrases and Greetings", queries = ["farsi greetings", "which road reaches the valley", "when do trains leave the city", "how deep is the nearby lake", "which bird nests in the forest", "which mountain is tallest"]; env.snapshot = { ...base, ownedPages: [{ url, content: { title, h1: title, metaDescription: null, h2: ["How to say hi/hello in Persian", "How to say good morning in Persian"], outline: [], schemaTypes: [], hasFaq: false, faqCount: 0, wordCount: 335, internalLinks: [], fetchedAt: now.toISOString() }, search: { clicks90d: 10, impressions90d: 1000, ctr90d: .01, position90d: 8, topQueries: queries.map((query, i) => ({ query, impressions: i === 0 ? 25 : i === 5 ? 20 : 300 - 40 * i, clicks: 1, position: 8 })) }, engagement: null, friction: null, aiCitations: { count: 0, distinctPrompts: 0, engines: [] } }] };
    const sourceCapture = { version: 1, complete: true, mainHtml: `<main><h1>${title}</h1><h2>How to say hi/hello in Persian</h2><p>سلام</p><h2>How to say good morning in Persian</h2><p>صبح بخیر</p></main>`, jsonLd: [] }, capture = { id: "cap-a", page_id: "page-a", url, final_url: null, fetched_at: now.toISOString(), content_hash: "hash-a", extraction_certainty: "confirmed", body_text: "How to say hi/hello in Persian سلام How to say good morning in Persian صبح بخیر", word_count: 20, content_capture: sourceCapture };
    const body = { url, pageId: "page-a", captureId: "cap-a", latestCaptureId: "cap-a", captureStates: [capture], finalUrl: null, title, h1: title, metaDescription: null, headings: ["How to say hi/hello in Persian", "How to say good morning in Persian"], passages: ["How to say hi/hello in Persian", "سلام", "How to say good morning in Persian", "صبح بخیر"], passageMeta: [{ id: "hello#0", heading: "How to say hi/hello in Persian" }, { id: "hello#1", heading: "How to say hi/hello in Persian" }, { id: "morning#0", heading: "How to say good morning in Persian" }, { id: "morning#1", heading: "How to say good morning in Persian" }], vocabulary: capture.body_text, openingSample: null, cardTexts: [], faqs: [], entityNames: [], internalLinks: [], fetchedAt: now.toISOString(), completeness: "complete", version: "current", contentHash: "hash-a", sourceCapture }; env.schemaBody = body;
    const answered = { ...openCard("missing_answer"), id: `${TENANT}::/a::existing_edit::missing_answer`, pagePath: "/a", pageUrl: url, primaryQuery: "farsi greetings", demandImpressions90d: 57, basis: "basis_fx" }, fifth = { ...answered, id: `${answered.id}@fifth`, primaryQuery: queries[5]! }, stale = openCard("title"), theirs = openCard("ai_answer_gap"), rows = (other = false) => new Map([[answered.id, answered], [fifth.id, fifth], ...(other ? [[stale.id, stale], [theirs.id, theirs]] as const : [])]); env.aiWindow = "fail"; env.store = rows(true); await produceProposalsForTenant(TENANT, { now, zeroSpend: true, maxDrafts: 0 }); expect(env.withdrawn.sort()).toEqual([answered.id, stale.id].sort()); expect(env.calls.some((c) => c.name === "withdrawChangeProposal" && (c.args as { id?: string })?.id === "cap-a")).toBe(true);
    for (const failure of ["read_failed", "recaptured"] as const) { env.withdrawn = []; env.store = rows(); env.schemaBody = failure === "read_failed" ? "fail" : body; env.captureId = failure === "recaptured" ? "cap-next" : "cap-a"; await produceProposalsForTenant(TENANT, { now, zeroSpend: true, maxDrafts: 0 }); expect(env.withdrawn, failure).toEqual([]); }
  });
