 ...
 *
 * MERGED (2026-07-21): the body-section PUSH path contract
 * (push-service-body.test.ts, BEACON_500 item 2) now lives in this file —
 * see the "body-section push path" block at the bottom. The body file's
 * Ritz hard-block and daily-cap cases were dropped as duplicates: both
 * gates fire in executePush BEFORE any route dispatch (field / create /
 * schema / body), so the field-path cases here already pin them for every
 * route. All route-specific body pins (snapshot-first, merge semantics,
 * RICOS, fail-closed rails, idempotent re-push, revert) were kept.
 */
...
// Body-section route (BEACON_500 item 2): null = no body mapping → the
// body card refuses and stays paste-ready.
...
// route's verify-by-re-read genuinely observes the write (not a canned
// answer). When null, the field/create/schema fixtures above drive the mock.
...
//     the JSON string.
//   - 1MB ceiling + fail-closed snapshot + dry-run rails on the BODY route.
//   - no body mapping -> the card refuses and stays paste-ready.
//   - revert restores the exact prior body through buildRevertEdit.
// (The Ritz hard-block + daily-cap cases lived here too; both gates fire
// before route dispatch and are pinned above, so they were not duplicated.)
...
describe("body-section push path", () => {
  ...
  describe("happy path (plain body)", () => {
    ...
    it("add_faq APPENDS the section after the existing body", async () => {
      ...
      const r = await executePush({
        tenantId: "tenant-iranopedia",
        edit: bodyEdit({ action_type: "add_faq" as RecommendedEditRow["action_type"], proposed_text: faqDraft }),
      });
      expect(r.kind).toBe("pushed");
      const written = String(_updateCalls[0]!.value);
  ...
  describe("RICOS object bodies", () => {
    ...
    it("replace_section on a RICOS body fails closed with the paste receipt", async () => {
      const r = await executePush({
        tenantId: "tenant-iranopedia",
        edit: bodyEdit({ target_element_key: "section:History", proposed_text: "History\n\nLong replacement text here." }),
      });
      expect(r.kind).toBe("refused");
  ...
  describe("fail-closed rails", () => {
    ...
    it("a merged item past the 1MB ceiling refuses locally (body route)", async () => {
    ...
    it("a failed Wix write lands push_failed in the ledger (body route)", async () => {
      _updateResult = { ok: false, reason: "api_error", detail: "http_500" };
      const r = await executePush({ tenantId: "tenant-iranopedia", edit: bodyEdit() });
      expect(r.kind).toBe("refused");
      const ledger = (_stores.get("push-ledger") ?? []) as Array<{ result: string }>;
    ...
    it("dry-run runs the full guard path with ZERO side effects (body route)", async () => {
      const r = await executePush({ tenantId: "tenant-iranopedia", edit: bodyEdit(), dryRun: true });
      expect(r.kind).toBe("dry_run");
  ...
  describe("idempotent re-push (no duplicate on re-click)", () => {
    ...
    it("add_faq re-push does not stack a second FAQ section either", async () => {
      const faqDraft = "Frequently asked questions\nHow long does koobideh take? About 20 minutes on the grill.";
      const faqEdit = bodyEdit({ action_type: "add_faq" as RecommendedEditRow["action_type"], proposed_text: faqDraft });
  ...
  describe("rollback restores the prior body", () => {
    ...
    it("a revert still restores when the prior body was much shorter than the merged value", async () => {
      ...
      const original = bodyEdit({ proposed_text: bigDraft });
      const push = await executePush({ tenantId: "tenant-iranopedia", edit: original });
      expect(push.kind).toBe("pushed");
    ...
    it("an empty prior body refuses to build a revert (blanking needs a human)", async () => {
      ...
      const revert = buildRevertEdit(snap, bodyEdit(), new Date());
