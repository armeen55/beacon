/**
 * decide-action - THE single decision authority (Core 100K Phase 6 trim of
 * tests/domains/changes/decide-action.test.ts). Every input dimension
 * (RecommendationAction / UnifiedKind / changeFamily) maps to exactly ONE of
 * the six decisions, and the cannibalization directive is always a single
 * action, never a "X or Y" fork.
 */
...
describe("every input dimension maps to exactly one of the six decisions", () => {
  it("RecommendationAction mapping (incl. the demand-gated split)", () => {
  ...
  it("UnifiedKind + changeFamily mappings", () => {
    const kinds: Array<[UnifiedKind, ChangeDecision]> = [
      ["edit", "edit_existing"],
      ["link", "edit_existing"],
      ["fix", "edit_existing"],
    ...
    for (const [kind, expected] of kinds) {
      expect(decisionFromUnifiedKind(kind)).toBe(expected);
    }
...
describe("decideChangeAction - the CanonicalChange entry always returns one of six + a one-to-one CTA", () => {
  ...
  it("family/type defaults: new_page creates, meta edits, merge consolidates", () => {
  ...
  it("every decision has a defined label and a defined (or null) CTA; act-decisions are exact", () => {
    for (const d of SIX) {
