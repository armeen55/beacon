...
describe("a change detail hands over the whole investigation and the controls to act on it", () => {
  ...
  it("offers no Copy control for prepared factual wording while the change is held", async () => { const b = proposal().bundle!, held = proposal({ bundle: { ...b, components: [{ ...b.components[0]!, kind: "factual_correction", label: "Correction", risk: "review" }] } }); const html = await renderDetail(held); expect([html.includes("Prepared wording, not ready to paste"), html.includes('data-copy-after="true"')]).toEqual([true, false]); });
