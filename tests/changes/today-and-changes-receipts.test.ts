...
describe("a change detail hands over the whole investigation and the controls to act on it", () => {
  ...
  it("turns a stored source address into an inspectable link without making the evidence sentence itself the destination", async () => {
    const row = proposal(), receipt = row.bundle!.receipt; row.bundle = { ...row.bundle!, receipt: { ...receipt, items: receipt.items.map((item) => ({ ...item, fact: `${item.fact} Read https://standards.example/nowruz.` })) } }; const html = await renderDetail(row); expect([html.includes('href="https://standards.example/nowruz"'), html.includes("Open source ↗"), html.includes("1,200 impressions and 9 clicks")]).toEqual([true, true, true]); });
