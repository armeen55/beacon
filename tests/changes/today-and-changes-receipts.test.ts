...
describe("a change detail hands over the whole investigation and the controls to act on it", () => {
  ...
  it("the investigation carries the cause, what it beat, what would kill it, and what could not be tested", async () => {
    ...
    const legacy = proposal({ id: "t::/iran-flags/iran-islamic-republic-flag-history::existing_edit::takbir_22", pagePath: "/iran-flags/iran-islamic-republic-flag-history", primaryQuery: "why is allahu akbar written 22 times on the iran flag", whyItMatters: "The page states the phrase is repeated twenty-two times but never says why.", impactScore: 84, demandImpressions90d: 31_863, rankingReceipt: { score: 84, directional: false, basis: "84 clicks over 28 days measured as missing", factors: [{ name: "visibility", input: "84 clicks over 28 days measured as missing", contribution: 84, max: 120 }] } }), detail = await renderDetail(legacy);
    expect([detail.includes("No attributable click figure backs this saved ranking receipt"), detail.includes("84 clicks over 28 days")]).toEqual([true, false]); }); // A saved page-wide score does not regain authority on the detail route.
  it("shows Beacon's next research step rather than CMS instructions before copy is complete", async () => {
    const pending = proposal({ bundle: undefined, researchOnly: true, operatorSteps: ["Open your site editor", "Add section"], research: { missing: "Add section about the unsupported causal claim.", next: "Beacon checks the exact source before writing the answer." }, obligation: { kind: "evidence", need: { kind: "factual_source", query: "why nowruz", reasonCode: "source_support_unconfirmed" } }, recommendedChange: { kind: "existing_edit", field: "section", before: null, after: "The exact wording has not been written yet." } }), html = await renderDetail(pending); expect([html.includes("What Beacon does next"), html.includes("Add section about the unsupported causal claim."), html.includes("Beacon will check a source for the missing claim"), html.includes("Open your site editor"), html.includes("Add section"), html.includes("Mark done"), html.includes("Prepare best edit on this page")]).toEqual([true, false, true, false, false, false, true]); const terminal = await renderDetail({ ...pending, obligation: { kind: "terminal", reason: "no substantive gap named" }, createdAt: SEEN }); expect([terminal.includes("no substantive gap named"), terminal.includes("Beacon checks the exact source"), terminal.includes("Prepare best edit on this page")]).toEqual([true, false, false]); const sections = proposal({ bundle: undefined, researchOnly: true, recommendedChange: { kind: "existing_edit", field: "meta", before: "Pallas Cat in Iran", after: "The exact wording has not been written yet." }, newPageDraft: { brief: { kind: "body_meta", identity: "a".repeat(64), owed: [0, 1] }, pieces: [] } }), sectionsHtml = await renderDetail(sections); expect([sectionsHtml.includes("Prepare best edit on this page"), sectionsHtml.includes("Mark done"), sectionsHtml.includes("Beacon will finish and check the publication copy")]).toEqual([true, false, true]); const whole = await renderDetail({ ...sections, changeFamily: "full_rewrite" }); expect(whole.includes("Prepare best edit on this page")).toBe(false); const bundled = await renderDetail(proposal({ researchOnly: true, research: pending.research, obligation: pending.obligation })); expect([bundled.includes("Prepared pieces, not ready to paste"), bundled.includes("Next: Beacon will check a source"), bundled.includes("The exact edits")]).toEqual([true, true, false]); const malformedRow = proposal({ status: "ready", researchOnly: true, modeledOn: SHAPE, research: pending.research }), { BundleDetail } = await import("@/app/(shell)/changes/[id]/bundle-detail"), malformed = renderToStaticMarkup(createElement(BundleDetail, { proposal: malformedRow, bundle: malformedRow.bundle!, recorded: new Set<string>() })); expect([malformed.includes("Prepared pieces, not ready to paste"), malformed.includes("Mark done"), malformed.includes("After you make it"), malformed.includes('data-copy-after="true"')]).toEqual([true, false, false, false]); const b = proposal().bundle!, incompleteRow = proposal({ status: "ready", researchOnly: false, modeledOn: SHAPE, bundle: { ...b, components: [{ ...b.components[0]!, after: "" }] } }), incomplete = renderToStaticMarkup(createElement(BundleDetail, { proposal: incompleteRow, bundle: incompleteRow.bundle!, recorded: new Set<string>() })); expect([incomplete.includes("Prepared pieces, not ready to paste"), incomplete.includes("Mark done"), incomplete.includes('data-copy-after="true"')]).toEqual([true, false, false]); const finished = atomic(), ready = await renderDetail({ ...finished, bundle: undefined, recommendedChange: { ...finished.recommendedChange, where: "The page title field" } as ChangeProposal["recommendedChange"] }); expect([ready.includes("Where it goes: The page title field"), ready.includes("What Beacon does next")]).toEqual([true, false]); const same = { after: finished.recommendedChange.kind === "existing_edit" ? finished.recommendedChange.after : "", retiredBecause: "Old placement", at: SEEN, attempts: 0 }, repeat = await renderDetail({ ...finished, bundle: undefined, previousCopy: same }), distinct = await renderDetail({ ...finished, bundle: undefined, previousCopy: { ...same, after: "Earlier words." } }), heldRepeat = await renderDetail({ ...finished, bundle: undefined, status: "needs_review", previousCopy: same }); expect([repeat.includes('data-previous-copy="true"'), distinct.includes('data-previous-copy="true"'), heldRepeat.includes('data-previous-copy="true"')]).toEqual([true, true, true]); const { cardCaveats } = await import("@/app/(shell)/changes/types"), notes = ["Old formatting note", "Old placement note"], producer = "The source link list might have changed since capture.", link = { ...finished, bundle: undefined, recommendedChange: { kind: "existing_edit", field: "section", before: null, after: "The earlier Aster flag was the Aster Blue Flag.", where: "Immediately after the captured block.", target: { mode: "after_block", anchorKind: "passage", anchor: "The lion stands before the rising sun." }, linkTo: "/flags/aster-blue-flag", anchorText: "Aster Blue Flag" }, previousCopy: { ...same, after: "The earlier Aster flag was the Aster Blue Flag.", retiredBecause: "the publication address, placement or predecessor changed" }, limitations: [producer, ...notes], draftNotes: notes, claims: [{ text: "The earlier Aster flag was the Aster Blue Flag.", supportedBy: ["owned-page-target-1"] }], supportFacts: [{ id: "owned-page-target-1", fact: "Aster Blue Flag, an earlier Aster flag." }], semanticReview: { version: 7, of: "reviewed", claims: [{ i: 0, by: ["owned-page-target-1"], entailed: true }], editor: { pageFit: true, usefulAndNatural: true, placementCorrect: true, resolvesDiagnosis: true, implementableNow: true, improvesPage: true, wouldHandToCustomer: true, notes: "Exact link placement reviewed." } } } as ChangeProposal; const { copyKey } = await import("@/domains/decision/proof"), { openHold } = await import("@/domains/decision/completeness"), reviewedLink = { ...link, semanticReview: { ...link.semanticReview!, of: copyKey(link) } }, staleLink = { ...reviewedLink, semanticReview: { ...reviewedLink.semanticReview!, version: 6 } }, emptyUnkeyed = { ...reviewedLink, claims: [], semanticReview: { ...reviewedLink.semanticReview!, claims: [] } }, emptyClaims = { ...emptyUnkeyed, semanticReview: { ...emptyUnkeyed.semanticReview, of: copyKey(emptyUnkeyed) } }, refusalLink = { ...reviewedLink, previousCopy: { ...reviewedLink.previousCopy!, retiredBecause: "source evidence failed" } }, faultedLink = { ...reviewedLink, faults: ["source support failed"] }, caveats = (p: ChangeProposal) => cardCaveats(p, p.limitations, openHold(p).settledPriorReceipt); expect([caveats(reviewedLink), caveats(link), caveats(staleLink), caveats(emptyClaims), caveats(refusalLink), caveats(faultedLink), caveats({ ...reviewedLink, status: "needs_review" }), caveats({ ...reviewedLink, previousCopy: { ...reviewedLink.previousCopy!, after: "Different retired words." } }), caveats({ ...reviewedLink, recommendedChange: { ...reviewedLink.recommendedChange, target: { mode: "after_heading", anchorKind: "heading", anchor: "Flags" } } as unknown as ChangeProposal["recommendedChange"] })]).toEqual([[producer], [producer, ...notes], [producer, ...notes], [producer, ...notes], [producer, ...notes], [producer, ...notes], [producer, ...notes], [producer, ...notes], [producer, ...notes]]); const { SimpleDetail } = await import("@/app/(shell)/changes/[id]/bundle-detail"), receipt = (p: ChangeProposal) => renderToStaticMarkup(createElement(SimpleDetail, { proposal: p })), settledDetail = receipt(reviewedLink), refusedDetail = receipt(refusalLink), staleDetail = receipt(staleLink), emptyDetail = receipt(emptyClaims), faultedDetail = receipt(faultedLink); expect([settledDetail.includes('data-previous-copy="true"'), refusedDetail.includes('data-previous-copy="true"'), staleDetail.includes('data-previous-copy="true"'), emptyDetail.includes('data-previous-copy="true"'), faultedDetail.includes('data-previous-copy="true"')]).toEqual([false, true, true, true, true]); });
  it("shows saved support neutrally while a draft is held and preserves inspectable links for Ready", async () => {
    const old = "https://old.example/takbir", phrase = "The Takbir appears 22 times on the flag.", row = proposal({ bundle: undefined, status: "needs_review", recommendedChange: { kind: "existing_edit", field: "section", before: null, after: phrase, where: "Under Flag meaning" }, claims: [{ text: phrase, supportedBy: ["fact-old"] }], supportFacts: [{ id: "fact-old", fact: `Earlier Wikipedia reading ${old} says: "${phrase}"`, sources: [{ url: old, kind: "publisher" }] }], limitations: ["Backed by 1 checked source. Add a second before publishing if being wrong here would cost you."], obligation: { kind: "evidence", need: { kind: "factual_source", query: "why Takbir 22", reasonCode: "source_support_unconfirmed" } } }), held = await renderDetail(row), reviewOnly = await renderDetail({ ...row, obligation: { kind: "review" } }); const { SimpleDetail } = await import("@/app/(shell)/changes/[id]/bundle-detail"), malformed = renderToStaticMarkup(createElement(SimpleDetail, { proposal: { ...row, status: "ready", researchOnly: true, obligation: undefined } })), { cardCaveats } = await import("@/app/(shell)/changes/types"), readyCaveats = cardCaveats({ ...row, status: "ready", obligation: undefined }, row.limitations); const readyRow = atomic(), receipt = readyRow.bundle!.receipt; readyRow.bundle = { ...readyRow.bundle!, receipt: { ...receipt, items: receipt.items.map((item) => ({ ...item, fact: `${item.fact} Read https://standards.example/nowruz.` })) } }; const ready = await renderDetail(readyRow); expect([held.includes("Backed by 1 checked source"), held.includes("Sources saved with this unfinished draft"), held.indexOf(old) > held.indexOf('data-held-support="true"'), reviewOnly.includes("Sources saved with this unfinished draft"), reviewOnly.includes("This unfinished draft cites saved source material"), malformed.includes("Backed by 1 checked source"), malformed.includes("Sources saved with this unfinished draft"), readyCaveats.some((line) => line.startsWith("Backed by 1 checked source")), ready.includes('href="https://standards.example/nowruz"'), ready.includes("Open source ↗")]).toEqual([false, true, true, true, true, false, true, true, true, true]); });
...
describe("a finished change is read, decided and pasted without being opened", () => {
  ...
  it.each(SITES)("$t: a small entry correction does not inherit its page's traffic, a substantive section competes with no proven lift, and the family a change belongs to never sets the order", async (s) => {
    ...
    const section = finished(s, { ...big, id: `${s.t}::${s.path}::existing_edit::section`, changeFamily: "section", diagnosisCause: "incomplete_coverage", impactScore: 40, impactAttribution: { page: big.pageUrl, query: s.q, members: [s.q], clicks28d: 40, impressions90d: 1200, sourceDay: new Date().toISOString().slice(0, 10) },
