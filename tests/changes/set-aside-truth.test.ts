...
describe("an empty Changes queue reads as a decision, not an empty screen", () => {
  ...
  it("prepares the selected page only with disclosed ceilings and never calls a reservation an invoice", async () => {
    const { finishOneProposalAction } = await import("@/app/(shell)/changes/actions"), { SetAsideChange } = await import("@/app/(shell)/changes/change-controls");
    prepareRun.mockResolvedValueOnce({ success: false, reason: "no_new_ready_substantive_edit", preferredRetiredReason: "blocked", captured: true, allowance: { modelCalls: 0, externalCalls: 0, modelReservedUsd: 0.2, externalReservedUsd: 0.002 }, evidenceOwed: [{ kind: "factual_source", query: "regional weave", proposalId: ID }, { kind: "serp", query: "unrelated private query", proposalId: "other" }] }).mockResolvedValueOnce({ success: false, reason: "proof_admission_replayed", allowance: null }).mockResolvedValueOnce({ success: false, reason: "openai_not_configured_in_this_runtime", allowance: null }).mockResolvedValueOnce({ success: false, reason: "no_new_ready_substantive_edit", preferredRetiredReason: "missing", captured: false, allowance: { modelCalls: 0, externalCalls: 0, modelReservedUsd: 0, externalReservedUsd: 0 } }).mockResolvedValueOnce({ success: false, reason: "no_new_ready_substantive_edit", preferredOutcome: "evidence_required", captured: true, allowance: { modelCalls: 0, externalCalls: 0, modelReservedUsd: 0, externalReservedUsd: 0 }, evidenceOwed: [{ kind: "serp", query: "named source", proposalId: ID }, { kind: "serp", query: "private other", proposalId: "other" }] });
    const failed = await finishOneProposalAction({ proposalId: ID, prepare: true, authorizationId: AUTH }), used = await finishOneProposalAction({ proposalId: ID, prepare: true, authorizationId: AUTH }), missing = await finishOneProposalAction({ proposalId: ID, prepare: true, authorizationId: AUTH }), retired = await finishOneProposalAction({ proposalId: ID, prepare: true, authorizationId: AUTH }), owed = await finishOneProposalAction({ proposalId: ID, prepare: true, authorizationId: AUTH });
