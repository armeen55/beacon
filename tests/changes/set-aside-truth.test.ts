...
describe("an empty Changes queue reads as a decision, not an empty screen", () => {
  ...
  it("shows every open opportunity with zero ready, never calls a draft finished, and counts the same on both screens", async () => {
    ...
    expect(html).toContain("1 change has draft copy but is not ready to apply, and 1 opportunity does not yet have a finished change."); expect(html).toContain("Research is paused, so no new opportunity is being worked on"); // Outstanding counts never claim that paused work is running.
    ...
    expect([today.readyTotal, today.nextOpportunities, today.topEdit, today.headerSentence, today.preparing]).toEqual([0, [], undefined, "No finished change is ready today. When Beacon finishes a change, it appears here.", { written: 1, researching: 1 }]); // Today previews finished work alone and carries the same two counts
  ...
  it("collapses 12 preparing opportunities into one counted sentence, never essays, never detail links and never Ready", async () => {
    ...
    expect([html.match(/data-lane-preparing="true"/g)?.length, html.match(/data-research-detail="true"/g)?.length ?? 0, html.includes("12 opportunities do not yet have a finished change."),
  ...
  it("prepares the selected page only with disclosed ceilings and never calls a reservation an invoice", async () => {
    const { finishOneProposalAction } = await import("@/app/(shell)/changes/actions"), { SetAsideChange } = await import("@/app/(shell)/changes/change-controls"), { loadChangeProposal, resolveCurrentBasis } = await import("@/domains/decision");
    const question = "why is allahu akbar written 22 times on the iran flag", debt = { ...bundled(NOW), bundle: undefined, status: "needs_review", researchOnly: false, primaryQuery: question, recommendedChange: { kind: "existing_edit", field: "section", before: null, after: "Unfinished answer." }, obligation: { kind: "evidence", need: { kind: "factual_source", query: question, proposalId: ID, reasonCode: "causal_answer_source_unconfirmed" } } } as ChangeProposal, missingOutcome = { success: false, proposalId: ID, reason: "no_new_ready_substantive_edit", preferredRetiredReason: "missing", captured: true, allowance: { modelCalls: 0, externalCalls: 0, modelReservedUsd: 0, externalReservedUsd: 0 } }; vi.mocked(resolveCurrentBasis).mockResolvedValue(NOW); vi.mocked(loadChangeProposal).mockResolvedValueOnce(null).mockResolvedValueOnce(debt).mockResolvedValueOnce(null).mockResolvedValueOnce({ ...debt, basis: "old_basis" });
    prepareRun.mockResolvedValueOnce({ ...missingOutcome, preferredRetiredReason: "blocked", allowance: { ...missingOutcome.allowance, modelReservedUsd: 0.2, externalReservedUsd: 0.002 }, evidenceOwed: [{ kind: "factual_source", query: "regional weave", proposalId: ID }, { kind: "serp", query: "unrelated private query", proposalId: "other" }] }).mockResolvedValueOnce({ success: false, reason: "proof_admission_replayed", allowance: null }).mockResolvedValueOnce({ success: false, reason: "openai_not_configured_in_this_runtime", allowance: null }).mockResolvedValueOnce(missingOutcome).mockResolvedValueOnce({ ...missingOutcome, preferredRetiredReason: undefined, preferredOutcome: "evidence_required", evidenceOwed: [{ kind: "serp", query: "named source", proposalId: ID }, { kind: "serp", query: "private other", proposalId: "other" }] }).mockResolvedValueOnce(missingOutcome).mockResolvedValueOnce({ ...missingOutcome, evidenceOwed: [{ kind: "factual_source", query: question, proposalId: ID }] }).mockResolvedValueOnce({ ...missingOutcome, evidenceOwed: [{ kind: "factual_source", query: question, proposalId: ID }] });
    const failed = await finishOneProposalAction({ proposalId: ID, prepare: true, authorizationId: AUTH }), used = await finishOneProposalAction({ proposalId: ID, prepare: true, authorizationId: AUTH }), missing = await finishOneProposalAction({ proposalId: ID, prepare: true, authorizationId: AUTH }), retired = await finishOneProposalAction({ proposalId: ID, prepare: true, authorizationId: AUTH }), owed = await finishOneProposalAction({ proposalId: ID, prepare: true, authorizationId: AUTH }), active = await finishOneProposalAction({ proposalId: ID, prepare: true, authorizationId: AUTH }), staleReceipt = await finishOneProposalAction({ proposalId: ID, prepare: true, authorizationId: AUTH }), wrongBasis = await finishOneProposalAction({ proposalId: ID, prepare: true, authorizationId: AUTH }); expect(prepareRun).toHaveBeenCalledWith({ tenantId: "t", proposalId: ID, currentBasis: NOW, maxOpenAiCalls: 8, maxOpenAiUsd: 2, maxDataForSeoCalls: 3, maxDataForSeoUsd: 0.4, authorizationId: AUTH });
    expect([failed.success, failed.error?.includes("not invoices"), failed.error?.includes("regional weave"), failed.error?.includes("unrelated private query"), failed.error?.includes("exact edit is held"), failed.error?.includes("complete current page capture"), failed.error?.includes("0 provider calls"), failed.error?.includes("Research stays paused"), used.error?.includes("already used"), missing.error?.includes("not configured"), retired.error?.includes("no longer includes this exact edit"), owed.error?.includes("named evidence"), owed.error?.includes("named source"), owed.error?.includes("private other"), active.error?.includes("Still needs factual source"), active.error?.includes("no longer includes this exact edit"), staleReceipt.error?.includes("no longer includes this exact edit"), wrongBasis.error?.includes("no longer includes this exact edit")]).toEqual([false, true, true, false, true, true, true, true, true, true, true, true, true, false, true, false, true, true]);
    publish.allowed = false; const denied = await finishOneProposalAction({ proposalId: ID, prepare: true, authorizationId: AUTH }); publish.allowed = true; expect([denied.success, prepareRun.mock.calls.length]).toEqual([false, 8]);
  ...
  it("revalidates a stored release against the exact current basis before serving any row", async () => {
    ...
    const faulted = { ...bundled(NOW, "t::faulted"), faults: ["it points at the page instead of answering"] } as ChangeProposal, byFault = withCurrentBasisOnly({ ...emptyView(0), proposals: [faulted], ready: [faulted], summary: { todo: 0, ready: 1, measuring: 0, results: 0 } } as ChangesView, { tenantId: "t", currentBasis: NOW }); // the current bar rechecks a stored Ready stamp even when all proposal rows pass the basis check
  ...
  it("keeps an independently paged Ready change when the global first page contains a stale row", async () => { const { withCurrentBasisOnly } = await vi.importActual<typeof import("@/app/(shell)/changes-data")>("@/app/(shell)/changes-data"); const ready = bundled(NOW, "t::off-page-ready"), stale = { ...bundled("basis_old::d2", "t::first-page-stale"), status: "needs_review", researchOnly: true } as ChangeProposal, release = { ...emptyView(0), proposals: [stale], ready: [ready], research: [stale], summary: { ...emptyView(0).summary, ready: 1, todo: 19, research: 157 } } as ChangesView, shown = withCurrentBasisOnly(release, { tenantId: "t", currentBasis: NOW }); expect([shown.proposals.map((p) => p.id), shown.ready.map((p) => p.id), shown.summary.ready, shown.summary.todo, shown.summary.research, shown.laneById?.[ready.id], (await renderChanges(shown)).includes(EXACT)]).toEqual([[ready.id], [ready.id], 1, 19, 156, "ready", true]); });
});
