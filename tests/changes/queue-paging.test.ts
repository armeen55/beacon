...
describe("one release identity, or no release at all", () => {
  ...
  it("builds the one order without touching the live ranking, commits ranking and surface together or not at all, and pages no change whose receipt stopped resolving", async () => {
    ...
    expect(await committed()).toEqual(["rel-9", "rel-9"]);
    const first100 = view.ready.slice(0, 100), index = ALL.findIndex((p) => !first100.some((x) => x.id === p.id)), research = proposal(index, { status: "needs_review", researchOnly: true }), independent = { ...args("rel-11", "rel-9"), content: { ...args("rel-11", "rel-9").content, manifest: [...first100.map((p) => ({ id: p.id, lane: "ready" as const })), { id: research.id, lane: "research" as const }], changes: { proposals: first100, ready: first100, toDo: [], research: [research] } } }; db.rows[index]!.payload = JSON.parse(serializeChangeProposal(research)); expect(await publishCustomerRelease(independent)).toBe("rel-11");
