...
describe("Sprint 7 Phase 7.4 — middleware tenant injection", () => {
  ...
  // 2026-07-18 incident follow-up (merged from src/lib/auth/supabase-middleware.test.ts):
  // a stale beacon_tenant cookie naming a NON-member tenant must never be injected;
  // the successful lookup path scrubs it so it cannot be honored during a later blip.
