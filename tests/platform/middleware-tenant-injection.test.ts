...
describe("middleware account injection — one login, one account, fail-closed", () => {
  ...
  it.each([
    ...
    // A read that did not ANSWER is not an account verdict. With no signed account to fall back on, a request that is not a document (a server action POST, an RSC fetch) is told the check is busy and can retry; it is never bounced to /login, which is what ate a Mark done press. A forged cookie still buys nothing.
  ...
  it("a signed account answers without a query, so a starved pool never bounces a page or eats a server action POST", async () => {
    ...
    const first = await updateSession(makeRequest("/changes"));
    const signed = cookieValue(first, "beacon_acct");
    ...
    const headers = { cookie: `beacon_acct=${signed}` };
    const warm = await updateSession(makeRequest("/changes", { headers }));
    expect(warm.status).toBe(200);
    expect(injectedTenant(warm)).toBe("tenant-mine");

    const post = new NextRequest(new URL("/changes", "https://beacon-bice.vercel.app"), { method: "POST", headers });
    const acted = await updateSession(post);
    expect(acted.status).toBe(200);
    expect(acted.headers.get("location"), "a Mark done POST must never be redirected to /login").toBeNull();
  ...
  it("with no signed account, a page reloads itself once and only the second failure reaches login, carrying where they were going", async () => {
    ...
    const first = await updateSession(makeRequest("/changes", { headers: { accept: "text/html" } }));
