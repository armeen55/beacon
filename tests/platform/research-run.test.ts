...
describe("the cycle finishes stored work before it buys exploratory evidence", () => {
  ...
  it("writes a page off only on its own settled receipt, and never on a blocked, unreached or unreadable pass", async () => {
    const legacy = { id: "legacy", kind: "existing_edit", status: "needs_review", researchOnly: false, pageUrl: "https://example.com/legacy", pagePath: "/legacy", primaryQuery: "legacy answer", changeFamily: "answer_block", recommendedChange: { kind: "existing_edit", field: "answer_block", before: null, after: "" }, evidence: { query: "legacy answer", hints: ["old row"], evidenceRefCount: 2 }, obligation: { kind: "redraft", attempt: 1, instruction: "try again" } };
    const focus = { id: "focus", kind: "existing_edit", status: "needs_review", researchOnly: true, pageUrl: "https://example.com/focus", pagePath: "/focus", primaryQuery: "qualified answer", diagnosisCause: "ai_citation_gap", treatment: "add_answer_section", changeFamily: "answer_block", recommendedChange: { kind: "existing_edit", field: "answer_block", before: null, after: "" }, evidence: { query: "qualified answer", hints: ["saved comparison"], evidenceRefCount: 3 }, obligation: { kind: "draft" } };
    ...
    try {
      ...
      expect([Object.entries(mem.jobs).map(([k, m]) => [k, m.calls, m.settled, (m as { waited?: number }).waited ?? 0]), M.asked.length, M.asked[0]!.produce, M.asked[0]!.bypassCache, M.asked[0]!.maxDrafts, M.asked[0]!.focusPage], "the walk carries the highest-ranked page and one-draft bound while preserving the per-work receipt memory").toEqual([[["/a::wc3::e1", 1, false, 0], ["/b::wc3::e1", 0, false, 0], ["/c::wc3::e1", 0, false, 0], ["/d::wc3::e1", 0, false, 0], ["/e::wc3::e1", 0, false, 0]], 1, true, false, 1, focus.pageUrl]);
