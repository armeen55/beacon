...
describe("the cycle finishes stored work before it buys exploratory evidence", () => {
  ...
  it("loads only the named scoped change and distinguishes saved partial review progress from complete acceptance", async () => {
    ...
    try { const { defaultSteps: live } = await import("@/domains/runtime/ops/research-steps"); const cases: Array<[string, typeof base | null, string, boolean, boolean]> = [["complete", full, "saved", true, true], ["partial", { ...full, semanticReview: { ...review, claims: review.claims.slice(0, 1) } }, "saved", true, false], ["old", { ...full, semanticReview: { ...review, version: REVIEW_CONTRACT - 1 } }, "saved", true, false], ["wrong placement", { ...full, semanticReview: { ...review, editor: { ...editor, placementCorrect: false } } }, "saved", true, false], ["not implementable", { ...full, semanticReview: { ...review, editor: { ...editor, implementableNow: false } } }, "saved", true, false], ["wrong source", { ...full, semanticReview: { ...review, claims: [{ ...review.claims[0]!, by: ["fact-2"] }, review.claims[1]!] } }, "saved", true, false], ["stale copy", { ...full, semanticReview: { ...review, of: "old-copy" } }, "saved", true, false], ["free record repair", base, "saved", true, false], ["provider interrupted", null, "saved", false, false], ["save failed", full, "failed", false, false], ["already saved", full, "unchanged", true, true]];
      ...
      S.loaded = []; S.reads = 0; S.saved = []; const bare = await live.acquireEvidence(T, { kind: "semantic_review", query: "well depth", workKey: "review-without-row" }, "b1", 5_000); expect([bare.acquired, bare.detail.includes("names no change"), S.loaded, S.reads, S.saved]).toEqual([false, true, [], 0, []]); S.load = base; S.row = { ...full, semanticReview: { ...review, claims: review.claims.slice(0, 1) } }; S.save = "saved"; S.reads = 0; S.saved = []; const rows = withRun({ current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready"] } } }), need = { key: "/well::section", kind: "semantic_review" as const, query: "well depth", proposalId: ID, reasonCode: "review_owed", reason: "The written copy still owes its review.", workKey: "well-copy" }; let walks = 0; await run({ ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }), acquireEvidence: live.acquireEvidence, replenishReady: async () => (walks += 1, { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: [need] }) }); expect([S.reads, walks, rows[0]!.progress.acquisitions?.map((a) => a.outcome), rows[0]!.progress.evidenceOwed?.map((n) => [n.key, n.reason.includes("Review progress is saved"), n.reason.includes("stronger one")])], "real saved partial review stays owed, records incomplete acquisition, and starts no replacement drafting walk").toEqual([1, 1, ["read_not_usable"], [[need.key, true, false]]]); S.row = full; S.reads = 0; S.saved = []; const completeRows = withRun({ current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready"] } } }), counters: number[] = []; await run({ ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }), acquireEvidence: live.acquireEvidence, replenishReady: async (_t, _n, seen) => (counters.push(seen?.callsSpent ?? 0), { ready: S.saved.length, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: S.saved.length ? [] : [need] }) }); expect([S.reads, counters, completeRows[0]!.progress.acquisitions?.map((a) => a.outcome), completeRows[0]!.progress.evidenceOwed], "a completed saved review transfers its actual request receipt into the very next walk allowance").toEqual([1, [0, 1], ["unlocked"], []]); for (const advances of [false, true]) { S.row = { ...full, semanticReview: { ...review, claims: review.claims.slice(0, 1) } }; S.reads = 0; S.saved = []; const before = { ...need, unlocks: { proposalId: ID, step: "review" as const, beforeMicros: true as const } }, after = { ...before, workKey: advances ? "well-copy::read:qualified-component" : before.workKey }, persisted = withRun({ current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready"] }, evidenceOwed: [before] } }), steps: Partial<ResearchCycleSteps> = { ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }), acquireEvidence: live.acquireEvidence, replenishReady: async () => ({ ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: [after] }) }; await run(steps); expect([S.reads, persisted[0]!.progress.evidenceOwed?.[0]?.workKey, persisted[0]!.progress.evidenceOwed?.[0]?.boughtOn ?? null], "a free same-drive replay cannot stamp the remaining identity as already purchased").toEqual([1, after.workKey, advances ? null : reportingDay(NOW)]); S.row = full; S.reads = 0; S.saved = []; await run({ ...steps, replenishReady: async () => ({ ready: S.saved.length, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: S.saved.length ? [] : [after] }) }); expect([S.reads, persisted.at(-1)!.progress.evidenceOwed?.length], "only qualified progress resumes on the next same-day drive; an unchanged incomplete result stays held").toEqual([advances ? 1 : 0, advances ? 0 : 1]); }
