...
describe("dueWork: what is genuinely owed, computed from persisted state only", () => {
  ...
  /** WINNERS ON FILE THAT WERE NEVER READ AS PAGES. An extract can be banked before the reading itself existed, and `pageExtractFromRecord` decodes exactly that row as `mainText` null with `truncated` null; the comparison that settles a body case then sees every winner as unread and the case settles as "nothing to say" against pages nobody ever opened. Measured on production 2026-09-06: 20 winners on file, 14 carrying an extract, not one carrying a word of its page, and today's passes planned no phase that would read them. TWO SYNTHETIC ACCOUNTS with unrelated subjects, so a rule that holds for one set of pages is not a rule. */
  const WINNERS = [
  ...
  /** A RESULTS PAGE ON FILE WHOSE OWN PAGES NOTHING HAS RANKED (production 11:02Z, 2026-09-06). Three body rows owed their own results page and the purchase loop bought all three at the head of the next funded drive; a bought results page is not a winner until the winning-pages unit ranks it, so nothing counted as unread, no pass was planned to read it, and the pages that win those searches waited for tomorrow's full cycle while the rows went on owing a reading nobody planned. */
...
describe("the cycle finishes stored work before it buys exploratory evidence", () => {
  ...
  /** THE TWO DEADLINE ANSWERS REACH THE ROW, AND THEY ARE NOT ONE SENTENCE (2026-09-05). A drive that stopped WAITING leaves its walk running, so the job in flight saves its row after the box and the calls it made are already spent; a walk that STOPPED refused to start funded work the drive could no longer pay for, so nothing is running and that work is the next drive's first. Both read as "the drive ran out of time" before this, and the answer to each is different. */
  ...
  it("reads an unreached named body gap before micros", async () => {
    ...
    try { await run({ ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }),
      ...
      expect(order[0]).toBe("winners"); expect(order.filter((x) => x === "winners")).toHaveLength(1);
  ...
  it("never lets higher-ranked new-page debt consume the existing-edit proof, including an unstamped legacy row", async () => {
    ...
    const edit = { key: "/guide::body::answer", kind: "serp" as const, query: "existing guide answer", reasonCode: "no_exact_serp", reason: "The existing section needs its results page.", workKey: "/guide::existing_edit::section", rank: 2, delivery: "existing_page_edit" as const };
    const rows = withRun({ current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready"] }, evidenceOwed: [whole, edit] } }), asked: string[] = [];
    await run({ ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }),
      ...
      replenishReady: async () => ({ ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: [whole, edit] }) });
    expect(asked, "rank cannot make whole-page research admissible in the existing-edit phase").toEqual([edit.query]);
    expect((rows[0]!.progress.evidenceOwed ?? []).map((need) => need.key), "the edit remains eligible and the legacy whole-page debt is removed from this run").toEqual([edit.key]);
  });
