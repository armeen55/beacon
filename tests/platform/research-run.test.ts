...
describe("the cycle finishes stored work before it buys exploratory evidence", () => {
  ...
  it("writes a page off only on its own settled receipt, and never on a blocked, unreached or unreadable pass", async () => {
    ...
    try {
      ...
      expect([Object.entries(mem.jobs).map(([k, m]) => [k, m.calls, m.settled, (m as { waited?: number }).waited ?? 0]), M.asked.length, M.asked[0]!.produce, M.asked[0]!.bypassCache, "maxDrafts" in M.asked[0]!], "P3: A JOB NOBODY STARTED IS UNSPENT AND STILL OWED. Four of the five were funded and never reached: not one is written off, each is remembered at ZERO calls and unsettled so it is owed at its own rank next drive, and the only thing recorded against it is how many drives running it has been funded and never begun, which is what stops the resume rule holding the head of the queue for one of them for ever. And no count gates the buy: ONE call is the whole pass, dispatched to produce, and nothing inventory-shaped rides it. A FRESH PAID TAKE IS FOR A RETRY: this day has tried nothing yet, so an ask identical to one already paid for and validated is served from the cache").toEqual([[["/a::wc3::e1", 1, false, 0], ["/b::wc3::e1", 0, false, 1], ["/c::wc3::e1", 0, false, 1], ["/d::wc3::e1", 0, false, 1], ["/e::wc3::e1", 0, false, 1]], 1, true, false, false]);
