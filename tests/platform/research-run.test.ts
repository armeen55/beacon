...
describe("the canonical run order is the RUNTIME order", () => { it("walks fact_check ahead of every paid phase, so a type-union edit alone can never move it", async () => {
  ...
  /** AND THE MARK THAT BUYS THE TURN IS CLEARED BY THE STEP THAT STARTS, NEVER BY THE ANSWER IT GIVES (round-twelve reviewer residual 1, kept), AND THE DRIVE A STEP GOT ITS TURN ON IS NEVER THE DRIVE THE PHASE IS LEFT ON (round fourteen). A mark left standing would put the step in front of the walk on a second consecutive drive at the same phase, which is the standing hold twelve consecutive ticks of 2026-09-04 already proved starves the walk; and both doors out of this phase count the same per-phase number, so granting the turn and yielding the phase on one drive would buy the step exactly nothing. A step that answers `advanced`, `failed` or `waiting` is a step that RAN. */
  it.each([T, U])("clears %s's owed turn the moment the step starts, leaves the phase on no drive its own step ran, and leaves it on the next drive that finds it waiting", async (t) => {
    const drive = async (phase: "serp_analysis" | "winning_pages", unit: "plan_cases" | "read_winner_pages", answer: "failed" | "waiting") => {
      const rows = freshRepo(); let at = NOW; const started: string[] = [], boxes: number[] = [], seen: { waited: unknown; phase: string }[] = []; rows.push(mk({ tenant_id: t, cycle_key: ckey(t, NOW), current_phase: phase, progress: { plan: { units: ["replenish_ready", unit] }, waited: { phase, drives: 1, unpaid: true }, replenish: MEASURED(t) } }));
      for (let n = 0; n < 2; n += 1) { at = NOW;
        await runResearchCycle(t, { now: () => new Date(at), deadlineMs: 195_400, steps: { ...BENIGN, ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready", unit] as DueWork["due"] }),
          replenishReady: async (_i, _n, _seen, by) => { started.push("walk"); boxes.push((by ?? at) - NOW); at = answer === "failed" ? (by ?? at) + 40_000 : NOW + 100_000; return null; }, // one walk spends its whole box, the other is closed early by the day's own memory, so the step behind it can still be started
          funnelUnit: async (_p, _u, _c, budgetMs) => (started.push("unit"), boxes.push(budgetMs), at += 20_000, { status: answer, cursor: null, progress: {}, detail: "the provider had nothing to give this pass" }) } });
        seen.push({ waited: rows.at(-1)!.progress?.waited, phase: rows.at(-1)!.current_phase }); } return { seen, boxes, started }; };
    expect([await drive("serp_analysis", "plan_cases", "failed"), await drive("winning_pages", "read_winner_pages", "waiting")],
      "the first drive owes the step a turn, runs it FIRST on 110,400 ms of the box and says so on the row in one word. A step that answers failed is a step that ran, so the row keeps the count and drops the mark, and the second drive hands the walk the whole 155,400 ms box back and leaves the phase on the count it already carried. A step that answers waiting is a step that ran too: the phase is NOT left on that drive though its count has already reached two, and the next drive, which finds the same phase still behind its provider, leaves it so the rest of the day runs").toEqual([
      { seen: [{ waited: { phase: "serp_analysis", drives: 1, turn: "ran" }, phase: "serp_analysis" }, { waited: { phase: "serp_analysis", drives: 2, unpaid: true }, phase: "done" }], boxes: [110_400, 155_400], started: ["unit", "walk"] },
      { seen: [{ waited: { phase: "winning_pages", drives: 2, turn: "ran" }, phase: "winning_pages" }, { waited: { phase: "winning_pages", drives: 3 }, phase: "done" }], boxes: [110_400, 155_400, 95_400], started: ["unit", "walk", "unit"] }]); });
