...
describe("the canonical run order is the RUNTIME order", () => { it("walks fact_check ahead of every paid phase, so a type-union edit alone can never move it", async () => {
  ...
  it.each([T, U])("buys one reading a drive across the gap at the head of %s's order, only for work the last walk reached, and never a second", async (t) => {
    const day = ckey(t, NOW).slice(-10), need = (key: string, rank: number, servedToday = false) => ({ key, kind: "factual_source" as const, query: `q${key}`, url: key, rank, reasonCode: "acquire_factual_source", reason: "owed", workKey: `${key}::wk`, ...(servedToday ? { boughtOn: day } : {}) });
    const drive = async (owed: readonly ReturnType<typeof need>[], reached: readonly string[]) => { const bought: string[] = [];
      const rows = freshRepo(); rows.push(mk({ tenant_id: t, cycle_key: ckey(t, NOW), current_phase: "fact_check", progress: { plan: { units: ["replenish_ready", "check_page_facts"] }, evidenceOwed: [...owed],
        replenish: { day, jobs: {}, outcomes: { readySaved: 0, evidenceBanked: 0, refused: 0, blocked: 0, unreached: 0, stuck: [], receipts: owed.map((n) => ({ key: n.key, workKey: n.workKey, outcome: reached.includes(n.key) ? "produced" : "not_reached" })) } } } }));
    ...
    expect([await drive([need("/one", 1, true), need("/two", 17)], ["/two"]), await drive([need("/one", 40, true), need("/far", 145)], ["/one"]), await drive([need("/one", 1, true), need("/two", 17), need("/three", 18)], ["/two", "/three"]), await drive([need("/one", 1, true), need("/two", 17)], []), await drive([need("/one", 1, true), need("/58", 58), need("/84", 84)], ["/84"])],
      "the reading bought across the gap is the best-ranked one the last walk got PAST, never the count of the receipts it happened to file: a reached row at rank 17 behind a served head is bought at its own rank; a need at 145 behind a walk that stopped at 40 still waits, with the reason it always had; a drive buys ONE such reading and says so about the next; a walk that reached nothing at all buys none; and the live case, where the walk reached the row ranked 84 and the row ranked 58 it got past on the way is bought although no receipt names it").toEqual([
...
describe("research-run: a recovery pass runs what it was opened for", () => {
  ...
  it.each([T, U])("reads the winners on file and buys no results page, crawl or keyword to do it, on %s", async (t) => {
    ...
    await runResearchCycle(t, { now: () => new Date(NOW), steps: { ...BENIGN, ...spy(touched), dueWork: async () => ({ ...SOMETHING_DUE, due: ["read_winner_pages"], winners: { unread: 4, unranked: 0 } }) } });
...
describe("dueWork: what is genuinely owed, computed from persisted state only", () => {
  ...
  const file = (winners: unknown[], serps: unknown[] = []) => async () => ({ ...NO_WINNERS, winners, serps }), bought = (query: string, hosts: string[], status = "done", from = 1) => ({ query, status, organic: hosts.map((h, i) => ({ rank: i + from, url: `https://${h}/p`, domain: h, title: null })) });
