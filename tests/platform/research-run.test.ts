...
describe("the canonical run order is the RUNTIME order", () => { it("walks fact_check ahead of every paid phase, so a type-union edit alone can never move it", async () => {
  ...
  it.each([T, U, "stale-basis", "wrong-raw"])("keeps every receipt %s's walk had filed when the drive's own box cut it off, remembers the job that was still running with the calls it made, and still checks no stock and buys no evidence in its place", async (t) => {
    ...
    await vi.advanceTimersByTimeAsync(1_000_000); await drive; vi.useRealTimers(); const row = rows.at(-1)!, rep = row.progress?.replenish; expect([rep?.jobs, rep?.waiting, (rep?.outcomes?.receipts ?? []).length, ((rep?.outcomes?.receipts ?? []) as { providerCalls?: number }[]).map((r) => r.providerCalls), rep?.outcomes?.preparedMs, bought, row.status, row.current_phase]).toEqual([ filed.jobs, ["wk-next"], 2, [17, 0], 54_503, 1, "paused", "fact_check"]);
  ...
  it("checks a saved thin-meta source owner before any paid read, including after a changed or retired row", async () => { const url = "https://example.com/joojeh", id = `${T}::/joojeh::existing_edit::missing_description`, decision = await import("@/domains/decision"), owner = { id, tenantId: T, kind: "existing_edit", pagePath: "/joojeh", pageUrl: url, pageLabel: "Joojeh Kabob", primaryQuery: "Joojeh Kabob", opportunityType: "", changeFamily: "meta", status: "needs_review", basis: "basis_test::d9", publish: "manual", createdAt: iso(), recommendedChange: { kind: "existing_edit", field: "meta", before: null, after: "Joojeh Kabob is chicken marinated with saffron." }, whyItMatters: "Describe the dish", estimatedEffortMinutes: 5, riskLevel: "low", confidence: "low", limitations: [], evidence: { query: "Joojeh Kabob", hints: [], evidenceRefCount: 0 }, impactScore: null, upsidePerMonth: null, obligation: { kind: "redraft", attempt: 3, instruction: "Read the article first." } } as const, need = { key: "/joojeh::meta", kind: "page_source" as const, query: owner.primaryQuery, url, proposalId: id, ownerVersion: decision.confirmedVersion(owner as never), reasonCode: "thin_meta_page_source_owed", reason: "current article owed", workKey: "wk-meta" }, lookup = vi.spyOn(decision, "loadChangeProposal"), current = vi.spyOn(decision, "resolveCurrentBasis").mockResolvedValue("basis_test::d9");
    for (const state of ["current", "changed", "implemented", "stale-basis", "unreadable"] as const) { const rows = freshRepo(); rows.push(mk({ tenant_id: T, cycle_key: ckey(T, NOW), current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready"] }, evidenceOwed: [need], replenish: { day: reportingDay(NOW), jobs: {}, outcomes: { receipts: [{ key: need.key, outcome: "evidence_required" }] } } as never } })); lookup.mockResolvedValue((state === "unreadable" ? null : state === "changed" ? { ...owner, recommendedChange: { ...owner.recommendedChange, after: "A different draft." } } : state === "implemented" ? { ...owner, status: "implemented_pending_verification" } : state === "stale-basis" ? { ...owner, basis: "basis_test::d8" } : owner) as never); let bought = 0; await runResearchCycle(T, { now: () => new Date(NOW), deadlineMs: 260_000, steps: { ...BENIGN, dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }), acquireEvidence: async () => (bought++, { acquired: true, detail: "current article saved" }), replenishReady: async () => null } }); expect([state, bought, rows[0]!.progress.acquisitions?.[0]?.preflight ?? null]).toEqual([state, state === "current" ? 1 : 0, state === "current" ? null : state === "unreadable" ? "owner_unreadable" : state === "stale-basis" ? "basis_mismatch" : "owner_changed"]); } expect(lookup.mock.calls.every(([, , o]) => o?.canonicalOnly === true)).toBe(true); lookup.mockRestore(); current.mockRestore(); });
