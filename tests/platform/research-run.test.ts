...
describe("the daily scheduler: one guarded door, the same lease, the same cycle", () => {
  ...
  it("claims each due account in turn and drives it through the real cycle, and its receipt carries counts and nothing else", async () => {
    ...
    expect(rows.map((r) => [r.tenant_id, r.status, r.cycle_key.slice(-10)])).toEqual([[T, "completed", today()], [U, "completed", today()]]); expect(await dispatch(NO_PHASE)).toEqual(R()); }); // a finished day is claimed by nobody
...
describe("the cycle finishes stored work before it buys exploratory evidence", () => {
  ...
  it("loads only the named scoped change and distinguishes saved partial review progress from complete acceptance", async () => {
    ...
    const editor = { pageFit: true, usefulAndNatural: true, placementCorrect: true, resolvesDiagnosis: true, implementableNow: true, improvesPage: true, wouldHandToCustomer: true, notes: "Complete reader-facing answer" }, review = { of: copyKey(base), version: REVIEW_CONTRACT, editor, claims: [{ i: 0, by: ["fact-1"], entailed: true }, { i: 1, by: ["fact-2"], entailed: true }] }, full = { ...base, semanticReview: review }, S = { loaded: [] as string[], saved: [] as typeof base[], reads: 0, load: base, row: full as typeof base | null, save: "saved" };
    vi.doMock("@/domains/decision/proposal-store", () => ({ loadChangeProposal: async (_t: string, id: string) => (S.loaded.push(id), S.load), saveChangeProposal: async (p: typeof base) => (S.saved.push(p), S.save) })); vi.doMock("@/domains/decision/drafted-copy", () => ({ reviewFinishedCopy: async (_p: typeof base, opts: { attempts?: { left: number; record?: (r: unknown) => void }; stopBy?: number }) => (S.reads += 1, opts.attempts?.record?.({ status: "drafted", attempts: 1, costUsd: 0.01 }), { row: S.row, detail: "banked review progress" }) }));
