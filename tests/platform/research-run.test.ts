...
describe("the canonical run order is the RUNTIME order", () => { it("walks fact_check ahead of every paid phase, so a type-union edit alone can never move it", async () => {
  ...
  it.each([T, U])("leaves %s's phase behind on the second drive its own step could not be paid for, so the run moves though every slice goes to the walk, and the row says what was not started and when it runs", async (t) => {
    ...
    expect([await drive(36_000), await drive(18_600), started],
      ...
      { phase: "prompt_observations", status: "paused", waited: { phase: "prompt_observations", drives: 1, unpaid: true }, blocker: "The next research step needs 40 seconds and this drive had 36 left, so nothing was started for it. The next pass runs it first.", endedAt: 164_000 },
      { phase: "serp_analysis", status: "paused", waited: { phase: "serp_analysis", drives: 1, unpaid: true }, blocker: "The next research step needs 40 seconds and this drive had 19 left, so nothing was started for it. The next pass runs it first.", endedAt: 181_400 },
      ["walk", "walk"]]); });
  /** AND THE PHASE'S OWN UNIT GETS ITS TURN ON THE DRIVE AFTER ONE THAT COULD NOT PAY FOR IT (round-eleven reviewer's option 2, measured 2026-09-05). Yielding stops the run being parked for ever and buys the unit no work at all: on a drive whose walk takes the slice no new AI check is posted, and this account owes a stock top-up on every drive, so it banks no reserve and the walk's box ends at the deadline. The drive that could not pay now MARKS the row, and the next drive at that phase holds the unit's own minimum plus the starting margin out of the walk's box, which is the box the fact check has always reserved and is exactly what a walk that spends its whole grace can take back out of it. It is dropped by the same guard the fact check's hold is dropped by, and the mark is cleared the moment the unit starts, so it can never stand. */
  it.each([T, U])("holds %s's walk back for the phase's own unit on the drive after one that could not pay for it, drops the hold where the walk cannot afford it, and takes nothing on any other drive", async (t) => {
    const drive = async (deadlineMs: number, waited?: NonNullable<RR.ResearchRunProgress["waited"]>) => {
      const rows = freshRepo(); let at = NOW; const started: string[] = []; let stopBy = 0;
      rows.push(mk({ tenant_id: t, cycle_key: ckey(t, NOW), current_phase: "prompt_observations", progress: { plan: { units: ["replenish_ready", "daily_observations"] }, ...(waited ? { waited } : {}),
        replenish: { day: ckey(t, NOW).slice(-10), jobs: {}, outcomes: { readySaved: 0, evidenceBanked: 0, refused: 0, blocked: 0, unreached: 0, stuck: [], preparedMs: 44_616 } } } }));
      await runResearchCycle(t, { now: () => new Date(at), deadlineMs, steps: { ...BENIGN, ...healthySteps([]),
        dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready", "daily_observations"] as DueWork["due"] }),
        ...
        funnelUnit: async () => (started.push("unit"), at += 60_000, { status: "waiting" as const, cursor: null, progress: {} }) } });
      return { started, stopBy, endedAt: at - NOW }; };
    const mark = { phase: "prompt_observations" as const, drives: 1, unpaid: true as const };
    expect([await drive(195_400), await drive(195_400, mark), await drive(123_650, mark), await drive(195_400, { phase: "prompt_observations" as const, drives: 1 })],
      "with nothing owed the walk keeps the whole box and the unit is not started, which is this account's every drive; on the drive after one the unit could not be paid for, 85,000 ms of that box is held back and the unit takes its turn; at the bottom of the band the same hold would leave the walk under the box it needs to begin one job, so it is dropped and the walk keeps everything; and a drive counted for a provider's own wait buys no hold at all, because the mark and the count are two different facts").toEqual([
