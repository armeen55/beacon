...
describe("the canonical run order is the RUNTIME order", () => { it("walks fact_check ahead of every paid phase, so a type-union edit alone can never move it", async () => {
  ...
  it("serves a current rank-20 review before unrelated rank-100 exact sources", async () => { const decision = await import("@/domains/decision"), lookup = vi.spyOn(decision, "loadChangeProposal").mockImplementation(async (_t, id) => ({ id, tenantId: T, status: "needs_review", basis: "basis_test::d9", workKey: id, obligation: { kind: "evidence", need: { kind: "page_source", reasonCode: "acquire_page_source", url: `https://example.com/${id}`, query: "source" } } } as never)), current = vi.spyOn(decision, "resolveCurrentBasis").mockResolvedValue("basis_test::d9"), sources = Array.from({ length: 104 }, (_, i) => ({ key: `source-${i}`, kind: "page_source" as const, query: "source", url: `https://example.com/source-${i}`, rank: 100 + i, reasonCode: "acquire_page_source", reason: "owed", workKey: `source-${i}`, unlocks: { proposalId: `source-${i}`, step: "draft" as const, beforeMicros: true as const } })), reviewNeed = { key: "review", kind: "semantic_review" as const, query: "review", proposalId: "review", rank: 20, reasonCode: "review_owed", reason: "owed", workKey: "review", unlocks: { proposalId: "review", step: "review" as const } };
    ...
    lookup.mockRestore(); current.mockRestore(); }); it.each([T, U])("takes %s's walk floor from the last walk's own measured preparation, so the units keep their reservation in the band a padded constant took it away in, and a slower account funds nothing rather than a manifest it cannot begin", async (t) => {
