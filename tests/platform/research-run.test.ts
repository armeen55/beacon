...
describe("the canonical run order is the RUNTIME order", () => { it("walks fact_check ahead of every paid phase, so a type-union edit alone can never move it", async () => {
  ...
  it.each([T, U])("keeps every receipt %s's walk had filed when the drive's own box cut it off, remembers the job that was still running with the calls it made, and still checks no stock and buys no evidence in its place", async (t) => {
    ...
    expect(row.progress.evidenceOwed).toEqual([expect.objectContaining({ ...source, unlocks: { ...source.unlocks, beforeMicros: true } })]); const order: string[] = [], preferred: unknown[] = []; const decision = await import("@/domains/decision"), lookup = vi.spyOn(decision, "loadChangeProposal").mockResolvedValue(t === T ? { id: source.unlocks.proposalId, tenantId: t, status: "needs_review", basis: "basis_test", workKey: source.workKey, obligation: { kind: "evidence", need: source } } as never : null);
    await runResearchCycle(t, { now: () => new Date(NOW), deadlineMs: t === T ? 260_000 : 90_000, steps: { ...BENIGN, ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready", "check_page_facts"] }), acquireEvidence: async (_id, need) => (order.push(need.kind), { acquired: true, detail: "current complete capture on file" }),
      replenishReady: async (_id, _now, seen) => (order.push("writer"), preferred.push(seen?.preferred), { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: [] }) } }); expect([order, preferred, t === U ? row.progress.acquisitions?.at(-1)?.outcome : null, row.progress.evidenceOwed?.length]).toEqual(t === T ? [["page_source", "writer"], [{ proposalId: source.unlocks.proposalId, workKey: source.workKey }], null, 0] : [["writer"], [undefined], "deferred", 1]); expect(lookup.mock.calls.every(([, , opts]) => opts?.canonicalOnly === true)).toBe(true); lookup.mockRestore(); });
