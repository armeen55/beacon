...
describe("dueWork: what is genuinely owed, computed from persisted state only", () => {
  ...
  it("names each owed unit from the ONE persisted fact that proves it", async () => {
    ...
    expect(await due({ run: async () => ({ open: false, progress: { decided: { basis: "b1", rowVersion: 7 }, focus: parked(NOW - 1), replenish: { day: reportingDay(NOW), fingerprint: "b1::v7::settled", attempted: [], closed: "candidates_exhausted" as const } } }) })).toEqual(["acquire_case_evidence"]);  // A retry date that PASSED is the same-day unlock: the promise I made has come due.
    expect(await due({ evidenceVersion: async () => 8, run: async () => ({ open: false, progress: { decided: { basis: "b1", rowVersion: 7 }, focus: parked(NOW + DAY), replenish: { day: reportingDay(NOW), fingerprint: "b1::v8::settled", attempted: [], closed: "candidates_exhausted" as const } } }) })).toEqual(["plan_cases", "decide_and_prepare"]);  // The notes moved past what the last decision consumed: new evidence, so a plan and a decision are owed.
    ...
    expect(await due({ run: async () => ({ open: true, progress: { decided: { basis: "b1", rowVersion: 7 }, replenish: { day: reportingDay(NOW), fingerprint: "b1::v7::settled", attempted: [], closed: "candidates_exhausted" as const } } }) })).toEqual(["plan_cases"]);  // An OPEN run with no plan bound to this basis owes one; an idle account with no plan owes nothing.
    expect(await due({ run: async () => ({ open: false, progress: { decided: { basis: "b1", rowVersion: 7 }, replenish: { day: reportingDay(NOW), fingerprint: "b1::v7::settled", attempted: [], closed: "candidates_exhausted" as const } } }) })).toEqual([]); });
  it("treats a completed pass's frozen plan as a receipt, not a standing queue: only an arrived retry date or a still-open run makes a topic owed", async () => {
    ...
    const decided = { basis: "b1", rowVersion: 7 }; const due = async (open: boolean) => (await dueWork(T, new Date(NOW), { ...base, run: async () => ({ open, progress: { decided, focus: plan, replenish: { day: reportingDay(NOW), fingerprint: "b1::v7::settled", attempted: [], closed: "candidates_exhausted" as const } } }) })).due;
    ...
    expect((await dueWork(T, new Date(NOW), { ...base, run: async () => ({ open: false, progress: { decided, focus: parked(NOW - 1), replenish: { day: reportingDay(NOW), fingerprint: "b1::v7::settled", attempted: [], closed: "candidates_exhausted" as const } } }) })).due) .toEqual(["acquire_case_evidence"]); });  // And a date I promised that has ARRIVED is owed whether or not a run is open.
...
describe("the cycle finishes stored work before it buys exploratory evidence", () => {
  ...
  /** ONLY A SETTLED MANIFEST HOLDS A DAY. A count never did and never may: the target is a floor, so a day that merely reached five is still owed whatever evidenced work stands behind it. Exhaustion is the one closure; a day carrying none is asked again, which is free when the stock really is still there. */
  it("lets a proven exhaustion hold the day, and asks again when nothing closed it", async () => { const day = ckey(T, NOW).slice(-10); let calls = 0;
  ...
  it("writes a page off only on its own settled receipt, and never on a blocked, unreached or unreadable pass", async () => {
    const M = { ready: 0, declared: ["/a", "/b", "/c", "/d", "/e"], out: [] as { key: string; outcome: string; why?: string; cost?: number }[], outcome: "proposals_persisted", throws: false, owed: [] as unknown[], lands: true }; vi.resetModules();
    ...
    vi.doMock("@/domains/decision", () => ({ resolveCurrentBasis: async () => "b", stockOf: (rows: unknown[]) => rows.length, loadProposalQueue: async () => (M.ready < 0 ? Promise.reject(new Error("queue unreadable")) : { ready: Array.from({ length: M.ready }, () => ({})) }),
      produceProposalsForTenant: async (_t: string, o: { maxDrafts?: number; skipKeys?: readonly string[] }) => { if (M.throws) throw new Error("the provider fell over");
        ...
        M.ready += M.lands ? receipts.filter((r) => r.outcome === "produced").length : 0; // `lands: false` is the live falsifier: the store answered saved and the ready queue cannot see the row
        return { persisted: 0, held: [], outcome: M.outcome, paid: { declared: M.declared, funded, attemptUnitsSpent: funded.length * 3, receipts, evidenceOwed: M.owed } }; } }));
    try {
      const { defaultSteps: live } = await import("@/domains/runtime/ops/research-steps"); let mem: { fingerprint: string | null; attempted: string[] } = { fingerprint: null, attempted: [] };
      ...
      M.out = [{ key: "/a", outcome: "retryable_blocked" }, { key: "/b", outcome: "retryable_blocked" }]; expect(await drive()).toEqual([0, "retryable_blocked", 0]); // 2. a transient failure on one page before the others are reached writes off neither
      ...
      M.declared = ["/j"]; M.out = [{ key: "/j", outcome: "deterministic_refusal" }]; mem = { fingerprint: null, attempted: [] };
      ...
      M.owed = []; M.lands = false; M.declared = ["/k"]; M.out = [{ key: "/k", outcome: "produced" }]; mem = { fingerprint: null, attempted: [] };
      const unseen = await live.replenishReady(T, new Date(NOW), mem);
      expect([unseen!.reason, unseen!.outcomes!.stuck.some((s) => s.includes("produced_but_absent"))], "a claim the queue cannot see settles nothing and is named on the receipt").toEqual(["retryable_blocked", true]); M.lands = true;
      ...
      const { DRAFT_BUDGET } = await import("@/domains/decision/draft-budget"); expect(last!.fingerprint).toContain(`::p${DRAFT_BUDGET.POLICY}::`); // 7. AND A DRAFTING-POLICY CHANGE REOPENS WHAT THE OLD POLICY SETTLED, the same day: a page written off because the old allowance ran out mid-deliverable says nothing about the new one, so it is asked again rather than skipped.
      const stale = { fingerprint: last!.fingerprint.replace(`::p${DRAFT_BUDGET.POLICY}::`, "::pw4r2c6::"), attempted: ["/basic-persian-phrases", "/funny-farsi-phrases"] }; // The two pages this matters for by name, under the EXACT policy they were attempted on: `w4r2c6` scored a rewrite target against thousand-character crawler chunks that matched nothing, so both were written off against a target that did not exist. Bumping the policy is what lets a funded pass reach them again instead of answering "already tried those" and spending the proof somewhere else.
      ...
      L.seq = [1.0, 1.02]; M.ready = 0; M.declared = ["/h"]; mem = { fingerprint: null, attempted: [] }; // 8. THE LEDGER'S OWN ANSWER RIDES BESIDE THE RECEIPTS, both directions. A world that moved more than the receipts explain is REPORTED as unreconciled, never patched. (The COMPLETENESS of a receipt is proved
  ...
  it("buys exactly the search a funded candidate was refused for, even on a run that opened only for the stock", async () => {
    ...
    void rows; const asked2: string[] = []; withRun({ current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready"] }, evidenceOwed: [{ key: "/persian-female-first-names", kind: "serp" as const, query: "persian girl names", reasonCode: "no_exact_serp", reason: "No results page is on file", workKey: "wk-1", boughtOn: ckey(T, NOW).slice(-10) }] } }); await run({ ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }), acquireEvidence: async (_t, need) => { asked2.push(need.query); return { acquired: true, detail: "bought" }; }, replenishReady: async () => ({ ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, fingerprint: "m1", attempted: [], evidenceOwed: [{ key: "/persian-female-first-names", kind: "serp" as const, query: "persian girl names", reasonCode: "no_exact_serp", reason: "No results page is on file", workKey: "wk-1" }] }) }); expect(asked2, "a reading bought today that left the same refusal standing is not bought again today, before the walk or after it").toEqual([]); });
  ...
  it("carries only what settled into the day's memory, so an unreached page is asked again immediately", async () => {
    ...
    const drive = async (attempted: string[]) => { await run({ ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }),
