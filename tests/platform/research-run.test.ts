...
describe("the canonical run order is the RUNTIME order", () => { it("walks fact_check ahead of every paid phase, so a type-union edit alone can never move it", async () => {
  ...
  /** AND THE GAP IS CROSSED ON THE ONE RULER THE RANK IS MEASURED ON (run p1, eight drives, 2026-09-06). A need's rank is its row's position among every stored row, and reach was a COUNT of the last walk's receipts: a walk that filed 33 of them deferred the needs ranked 84, 85, 87, 125, 129 and 141 as beyond what it reached while it had filed a REACHED receipt for those very rows, and the three body rows owing their own results page at ranks 58, 63 and 66 could never be bought before a walk on any drive, so the comparison chain waiting on them stalled at the purchase for ever. Reach is the deepest RANK the walk got past: the owed rows whose own key it filed a receipt for that is not `not_reached`, and nothing where it reached none of them. */
  it.each([T, U])("buys one reading a drive across the gap at the head of %s's order, only for work the last walk reached, and never a second", async (t) => {
    const day = ckey(t, NOW).slice(-10), need = (key: string, rank: number, servedToday = false) => ({ key, kind: "factual_source" as const, query: `q${key}`, url: key, rank, reasonCode: "acquire_factual_source", reason: "owed", workKey: `${key}::wk`, ...(servedToday ? { boughtOn: day } : {}) });
    const drive = async (owed: readonly ReturnType<typeof need>[], reached: readonly string[], silent: readonly string[] = []) => { const bought: string[] = [];
      const rows = freshRepo(); rows.push(mk({ tenant_id: t, cycle_key: ckey(t, NOW), current_phase: "fact_check", progress: { plan: { units: ["replenish_ready", "check_page_facts"] }, evidenceOwed: [...owed],
        replenish: { day, jobs: {}, outcomes: { readySaved: 0, evidenceBanked: 0, refused: 0, blocked: 0, unreached: 0, stuck: [], receipts: owed.map((n) => ({ key: n.key, workKey: n.workKey, ...(silent.includes(n.key) ? {} : { outcome: reached.includes(n.key) ? "produced" : "not_reached" }) })) } } } }));
      await runResearchCycle(t, { now: () => new Date(NOW), deadlineMs: 200_000, steps: { ...BENIGN, ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready", "check_page_facts"] }),
        acquireEvidence: async (_i, n) => (bought.push(String(n.query)), { acquired: true, detail: "the reading landed" }), replenishReady: async () => null } });
      return { bought, deferred: (rows.at(-1)!.progress?.acquisitions ?? []).filter((a) => a.outcome === "deferred").map((a) => `${a.key}: ${a.detail.split(",")[0]}`) }; };
    expect([await drive([need("/one", 1, true), need("/two", 17)], ["/two"]), await drive([need("/one", 40, true), need("/far", 145)], ["/one"]), await drive([need("/one", 1, true), need("/two", 17), need("/three", 18)], ["/two", "/three"]), await drive([need("/one", 1, true), need("/two", 17)], []), await drive([need("/one", 1, true), need("/58", 58), need("/84", 84)], ["/84"]), await drive([need("/one", 1, true), need("/two", 17)], [], ["/two"])],
      "the reading bought across the gap is the best-ranked one the last walk got PAST, never the count of the receipts it happened to file: a reached row at rank 17 behind a served head is bought at its own rank; a need at 145 behind a walk that stopped at 40 still waits, with the reason it always had; a drive buys ONE such reading and says so about the next; a walk that reached nothing at all buys none; the live case, where the walk reached the row ranked 84 and the row ranked 58 it got past on the way is bought although no receipt names it; and a receipt carrying a key and NO outcome at all says nothing about what the walk reached, so it never lifts reach and the reading behind the gap waits").toEqual([
      { bought: ["q/two"], deferred: [] },
      { bought: [], deferred: ["/far: the work ranked above this reading needs no reading at all"] },
      { bought: ["q/two"], deferred: ["/three: this drive has already bought the one reading it may buy out of the ranking's order"] },
      { bought: [], deferred: ["/two: the work ranked above this reading needs no reading at all"] },
      { bought: ["q/58"], deferred: ["/84: this drive has already bought the one reading it may buy out of the ranking's order"] },
    { bought: [], deferred: ["/two: the work ranked above this reading needs no reading at all"] }]); });
  ...
  it.each([T, U])("ends %s's drive inside its own slice when the walk takes the slice, writing the walk's row and starting no step the deadline cannot pay for", async (t) => {
    ...
    const row = rows.at(-1)!; expect([started, at - NOW, row.status, row.current_phase, (row.progress?.replenish?.outcomes?.receipts ?? []).length, row.progress?.replenish?.waiting, row.progress?.state?.blocker],
  ...
  /** The walk outcomes one row shares (a drive that spent its whole box on a blocked manifest), and the measured free half every drive here budgets on. */
  ...
  /** AND A PHASE THAT COULD NOT PAY FOR ITS OWN STEP EVEN FIRST, ON A SECOND CONSECUTIVE DRIVE, YIELDS TO THE REST OF THE DAY (round eleven's door, narrowed by round fourteen). The step now goes AHEAD of the walk on the drive after one that could not pay for it, so this door answers a narrower question than it used to: not "the walk took the slice again", which the order answers, but "even this drive's first turn could not be started". The row says which of the two happened, in one typed word beside the count. */
  it.each([T, U])("leaves %s's phase behind on the second drive that could not pay for its own step even first, and says on the row that the owed turn was stopped", async (t) => {
  ...
  /** AND THE PHASE'S OWN STEP TAKES THE DRIVE'S FIRST TURN AFTER A DRIVE THAT COULD NOT PAY FOR IT (round fourteen, on production's own 04:02Z reading of 2026-09-06). Round twelve bought that turn by RESERVING room out of the walk's box, and the reservation was dropped by its own guard on every room this account arrives with: at winning_pages the row read the same 28,296 ms against a 40,000 ms floor on two consecutive drives and the phase yielded with the winners never read. The turn is an ORDER now and it is taken at EVERY phase: the step runs first, inside the fact unit's own box and never under its own minimum, and the walk takes what is left. It is still only ever the drive after one the step got nothing on. */
  it.each([T, U])("gives %s's phase its own bounded first turn ahead of the walk on the drive after one that could not pay for it, hands the walk what is left, and takes nothing on any other drive", async (t) => {
    ...
    expect([await drive(195_400), await drive(195_400, mark), await drive(123_650, mark), await drive(195_400, { phase: "prompt_observations" as const, drives: 1 }), await drive(195_400, { phase: "keyword_discovery" as const, drives: 1, unpaid: true }, true), await drive(260_000, mark)],
      "with nothing owed the walk keeps the whole box and the step is not started at all, which is this account's every drive; on the drive after one the step could not be paid for, the step goes FIRST on 110,400 ms of the 195,400 and the walk follows it with the 135,400 that are left, and the phase is NOT left on the very drive its own step ran; at the bottom of the band the step still gets the fact unit's own 45,000 ms minimum and the walk still runs behind it; a drive counted for a provider's own wait buys no turn at all, because the mark and the count are two different facts; and a drive opened for the stock alone buys no turn either, because such a drive buys no research evidence at all and a turn taken there would be exactly that purchase; and on the whole 260-second dispatch the turn stops at the fact unit's own two-minute box and hands the other 200 seconds to the walk, because a turn is a turn and never the drive").toEqual([
  ...
  /** AND THE MARK THAT BUYS THE TURN IS CLEARED BY THE STEP THAT STARTS, NEVER BY THE ANSWER IT GIVES (round-twelve reviewer residual 1, kept), AND THE DRIVE A STEP GOT ITS TURN ON IS NEVER THE DRIVE THE PHASE IS LEFT ON (round fourteen). A mark left standing would put the step in front of the walk on a second consecutive drive at the same phase, which is the standing hold twelve consecutive ticks of 2026-09-04 already proved starves the walk; and both doors out of this phase count the same per-phase number, so granting the turn and yielding the phase on one drive would buy the step exactly nothing. A step that answers `advanced`, `failed` or `waiting` is a step that RAN. AND THE WALK THE ORDER SKIPPED STILL RUNS BEHIND A STEP THAT CAME BACK WAITING (production, run p2 at prompt_observations, 13:30Z on 2026-09-06): the step took the turn, answered waiting on its provider 89 seconds in, and the drive paused with no walk and no read-back purchase on it at all. The loop goes back through the phase once, for the stock-first block and nothing else, and only where the room genuinely left can pay the walk's own floor; under that floor the row says what was not started and the drive pauses as it always did. */
  it.each([T, U])("clears %s's owed turn the moment the step starts, leaves the phase on no drive its own step ran, leaves it on the next drive that finds it waiting, and still runs behind that step the walk its turn skipped, once and only on the room genuinely left", async (t) => {
    ...
    expect([await drive("serp_analysis", "plan_cases", "failed"), await drive("winning_pages", "read_winner_pages", "waiting"), await drive("winning_pages", "read_winner_pages", "waiting", 123_650), await drive("serp_analysis", "plan_cases", "advanced"), await drive("winning_pages", "read_winner_pages", "waiting", 195_400, false)],
      "the first drive owes the step a turn, runs it FIRST on 110,400 ms of the box and says so on the row in one word. A step that answers failed is a step that ran, so the row keeps the count and drops the mark, and the second drive hands the walk the whole 155,400 ms box back and leaves the phase on the count it already carried. A step that answers waiting is a step that ran too: the phase is NOT left on that drive though its count has already reached two, and the walk its turn skipped still runs on the 135,400 ms behind it and files its receipt before the drive pauses, so no drive is spent without one. At the bottom of this account's band the same step leaves 78,650 ms against a walk floor of 85,000, so the block is NOT started, the row says which work that was and what it needed, and the next drive walks first. A step that answers advanced already sent the loop back, so its drive walks ONCE and the wait behind that walk starts no second one. And a wait with no turn behind it is untouched: the walk goes first as always, the first wait pauses and the second leaves the phase. The pass count is what bounds the loop back to one: the phase's own spending gate is reached twice on the drive that goes back for its walk, three times across the pair, and never once more than the block it went back for").toEqual([
      { seen: [{ waited: { phase: "serp_analysis", drives: 1, turn: "ran" }, phase: "serp_analysis", blocker: "the provider had nothing to give this pass", filed: 0 }, { waited: { phase: "serp_analysis", drives: 2, unpaid: true }, phase: "done", blocker: "The next research step needs 40 seconds and this drive had 0 left, so nothing was started for it. The next pass runs it first.", filed: 1 }], boxes: [110_400, 155_400], started: ["unit", "walk"], passes: 2 },
      ...
      { seen: [{ waited: { phase: "winning_pages", drives: 2, turn: "ran" }, phase: "winning_pages", blocker: "Preparing more finished changes needs 85 seconds and this drive had 79 left, so nothing was started for it. The next pass runs it first.", filed: 0 }, { waited: { phase: "winning_pages", drives: 3, unpaid: true }, phase: "done", blocker: "The next research step needs 40 seconds and this drive had 24 left, so nothing was started for it. The next pass runs it first.", filed: 1 }], boxes: [45_000, 83_650], started: ["unit", "walk"], passes: 2 },
      ...
      { seen: [{ waited: { phase: "winning_pages", drives: 1 }, phase: "winning_pages", blocker: null, filed: 1 }, { waited: { phase: "winning_pages", drives: 2 }, phase: "done", blocker: null, filed: 1 }], boxes: [155_400, 95_400, 155_400, 95_400], started: ["walk", "unit", "walk", "unit"], passes: 2 }]); });
  /** AND THE WINNERS ARE READ (production, 04:02Z on 2026-09-06). Run p10 was opened for the winning-pages phase alone; on the 03:30Z drive and again on the 04:00Z drive the walk and one owed reading took the slice, the row read "leftMs 28296, needsMs 40000" both times, and the phase yielded after two drives with the winners never read. This is that pair of drives end to end: the first leaves the turn owed, and the second reads the winners first inside a bounded box, hands the walk what is left, and finishes the phase. */
  ...
  it.each([T, U])("takes %s's whole walk when it lands inside the lease's own tail, and keeps the filed snapshot when it does not", async (t) => {
    ...
    expect([rep(late), rep(lost)],
      ...
      [2, [], 1], [1, ["wk-b"], 0]]); });
  /** AND AN ATTEMPT THAT DID NOT LAND IS COUNTED WHATEVER IT CAME BACK WITH (live runs tenant-iranopedia:p2 of 2026-09-05, read 11:37Z, and of 2026-09-06). A purchase that fails stamps nothing on the need, so one page check was bought SIX times on one run row and another NINE times across the day's two runs. Counting the repeat of one SENTENCE closed that case and left the next one wide open: the rank-15 fact check for one flag page came back "advanced, 1 banked; the answer to ..." at 13:00Z and "failed, 0 banked; the answer to ..." at 14:00Z, the wording moved, the count restarted at 1, and that one reading took a slot on every drive for ever, while the results pages ranked 30, 58, 63 and 66 that the whole competitor comparison waits on were deferred every drive for want of a slot. What a purchase can still teach this day is bounded by the ATTEMPT, never by the wording of the answer: two attempts under one work identity in one day spend the reading, the latest answer stays on the row so it still says what happened, and a moved work identity or a new day starts the count again. */
  it.each([T, U])("spends %s's reading after two attempts that did not land, whatever each of them came back with, buys it again after only one, and starts the count again when the work identity moves or the day turns", async (t) => {
    ...
    expect([one.asked, two.asked, three.asked, one.owed[0]?.tried?.count, two.owed[0]?.tried?.count, one.attempts, two.attempts, three.attempts, three.owed[0]?.boughtOn ?? null],
      "the first attempt is made, the second spends the reading, and the third is refused: the need carries the count and the last answer it got, every receipt says which attempt it was, the door behind the walk stops offering the reading the moment it is spent, and nothing is ever stamped as bought, because it was not").toEqual([1, 1, 0, 1, 2, [1, 1], [2], [], null]);
...
describe("research-run: a recovery pass runs what it was opened for", () => {
  ...
  /** A PASS OPENED ONLY TO READ WINNERS ALREADY ON FILE BUYS NO SEARCH. It carries no case and therefore no focus query, so the results-page phase would take its queries from the broad agenda and buy pages nothing asked for; it is also one of the two phases that can hold a whole drive, ahead of the read the pass exists for. */
...
describe("dueWork: what is genuinely owed, computed from persisted state only", () => {
  ...
  it.each(WINNERS)("owes the winner read for a results page whose own pages nothing has ranked, and counts only the pages one pass could read, on $t", async (w) => {
    ...
    const many = [1, 2].map((n) => bought(`${w.query} ${n}`, Array.from({ length: 10 }, (_, i) => `h${n}${i}-${w.tag}.example`))); expect((await dueWork(w.t, new Date(NOW), { ...base, winnerRows: file([], many) })).winners).toEqual({ unread: 0, unranked: 15 });  // twenty pages are owed and fifteen is what one pass reads: the receipt never promises more than a pass can do
  ...
  it.each(WINNERS)("owes nothing more for a search that already holds a page of its own on file, on $t", async (w) => {
    ...
    expect([held.due, held.winners]).toEqual([[], { unread: 0, unranked: 0 }]); });  // one page of that search read whole is the search represented; the rest of its top ten takes what the global order gives it
