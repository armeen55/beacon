...
describe("the canonical run order is the RUNTIME order", () => { it("walks fact_check ahead of every paid phase, so a type-union edit alone can never move it", async () => {
  ...
  /** AND ONE READING A DRIVE CROSSES THE GAP AT THE HEAD, FOR WORK THE LAST WALK REALLY REACHED (measured on run tenant-iranopedia:p3, 2026-09-05). Eight readings have sat on that run row since round eight with `tried` written on none of them: the head of the order is served, the gap behind it stops the loop every time, and the purchase loop AFTER the walk asks for 60,000 ms a walk whose box ends at the deadline never leaves. Ranks 17, 145, 149, 153, 155, 156 and 158 were deferred with that same sentence on four consecutive drives and not one was ever bought. The original case was money going to needs whose rows ranked 144th and worse, so the crossing is bounded by what the last walk itself got to: one reading, at its own rank, inside that walk's own funded count. */
  it.each([T, U])("buys one reading a drive across the gap at the head of %s's order, only for work the last walk reached, and never a second", async (t) => {
    const day = ckey(t, NOW).slice(-10), need = (key: string, rank: number, servedToday = false) => ({ key, kind: "factual_source" as const, query: `q${key}`, url: key, rank, reasonCode: "acquire_factual_source", reason: "owed", workKey: `${key}::wk`, ...(servedToday ? { boughtOn: day } : {}) });
    const drive = async (owed: readonly ReturnType<typeof need>[], walked: number) => {
      const rows = freshRepo(); rows.push(mk({ tenant_id: t, cycle_key: ckey(t, NOW), current_phase: "fact_check", progress: { plan: { units: ["replenish_ready", "check_page_facts"] }, evidenceOwed: [...owed],
        replenish: { day, jobs: {}, outcomes: { readySaved: 0, evidenceBanked: 0, refused: 0, blocked: 0, unreached: 0, stuck: [], receipts: Array.from({ length: walked }, (_x, i) => ({ key: `/r${i}`, outcome: "not_reached" })) } } } }));
      const bought: string[] = [];
    ...
    expect([await drive([need("/one", 1, true), need("/two", 17)], 35), await drive([need("/one", 1, true), need("/far", 145)], 35), await drive([need("/one", 1, true), need("/two", 17), need("/three", 18)], 35), await drive([need("/one", 1, true), need("/two", 17)], 0)],
      "a head already served and a gap behind it no longer sends the drive away empty: the best-ranked reading the last walk actually reached is bought at its own rank; a need ranked far past that walk still waits, with the reason it always had; a drive buys ONE such reading and says so about the next; and a drive with no walk behind it on the row reaches for none at all").toEqual([
  ...
  it.each([T, U])("reads %s's winners first on the drive after the one whose walk took the slice, inside a bounded box, and still hands the walk what is left", async (t) => {
    ...
    const drive = async (): Promise<{ phase: string; status: string; waited: unknown; blocker: string | null | undefined }> => { at = NOW;
      await runResearchCycle(t, { now: () => new Date(at), deadlineMs: 200_000, steps: { ...BENIGN, ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["read_winner_pages"] as DueWork["due"], winners: { unread: 20 } }),
