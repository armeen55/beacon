...
import { driveClaimed, runResearchCycle, ensureResearchRunOnVisit, RESEARCH_CYCLE_DEADLINE_MS, type ResearchCycleSteps } from "@/domains/runtime/ops/on-visit-refresh";
...
function memRepo(): { repo: RR.ResearchRunRepo; rows: RR.ResearchRun[] } { const rows: RR.ResearchRun[] = [];
  ...
  const patchProgress = (t: string, id: string, patch: RR.ResearchRunProgress, key?: string, day?: string): RR.ResearchRunProgress | null => { const r = find(id, t); if (!r) return null; const before = r.progress ?? {}, held = key ? (before as Record<string, { day?: string; count?: number } | undefined>)[key] ?? null : null;
    r.progress = { ...before, ...patch, ...(key ? { [key]: { day, count: held != null && held.day === day ? (held.count ?? 0) + 1 : 1 } } : {}) }; if (patch.dispatch) Object.assign(r, { next_dispatch_at: patch.dispatch.at, dispatch_reason: patch.dispatch.reason, dispatch_plan: patch.dispatch.plan, dispatch_attempts: patch.dispatch.attempts }); return r.progress; }; const repo: RR.ResearchRunRepo = {
    ...
    async claimDue({ owner, limit, leaseSeconds }) {  // claim_due_research_work: enumerate ACTIVE, not-paused accounts whose current reporting day still owes work, then claim each THROUGH the claim above, so the lease stays the only mechanism.
      ...
      for (const t of queue) { if (out.length >= limit) break; if (statusOf(t) !== "active" || PAUSED.has(t)) continue; const done = newestFirst(t).find((x) => x.status === "completed" && x.completed_at && reportingDay(Date.parse(x.completed_at)) === today), open = openRun(t);
        ...
        const claimed = await repo.claim({ tenantId: t, owner, leaseSeconds }); if (claimed) { out.push(claimed); continue; }
        if (done?.next_dispatch_at && Date.parse(done.next_dispatch_at) <= NOW) { const opened = await repo.startPass({ tenantId: t, owner, leaseSeconds, day: today, progress: done.dispatch_plan?.length ? { plan: { units: done.dispatch_plan, openedFor: true } } : {} }); if (opened) { Object.assign(done, { next_dispatch_at: null, dispatch_reason: null, dispatch_plan: null }); out.push(opened); } } }
      return out; },
    ...
    async finish({ tenantId, id, owner, outcome, errorInfo, spendUsd }) { const r = find(id, tenantId), done = outcome === "completed", failed = !done && errorInfo != null; if (!r || !live(r, owner) || !(r.status === "running" || r.status === "paused")) return false; const attempts = failed ? Math.min(r.dispatch_attempts + 1, 10) : 0, wake = iso(NOW + (failed ? Math.min(12 * 3600_000, 10 * 60_000 * 2 ** Math.max(0, attempts - 1)) : !done && !r.progress?.providerWait ? 0 : 10 * 60_000));
...
describe("the canonical run order is the RUNTIME order", () => { it("walks fact_check ahead of every paid phase, so a type-union edit alone can never move it", async () => {
  ...
  it.each([T, U, "stale-basis", "wrong-raw"])("keeps every receipt %s's walk had filed when the drive's own box cut it off, remembers the job that was still running with the calls it made, and still checks no stock and buys no evidence in its place", async (t) => {
    const source = { key: "/next", kind: "page_source" as const, query: "next question", url: "https://example.com/next", reasonCode: "acquire_page_source", reason: "complete source owed", workKey: "wk-next", unlocks: { proposalId: `${t}::/next::existing_edit::missing_answer`, step: "draft" as const } }, rows = freshRepo(); rows.push(mk({ tenant_id: t, cycle_key: ckey(t, NOW), current_phase: "fact_check", progress: { plan: { units: ["replenish_ready", "check_page_facts"] }, evidenceOwed: [{ key: "/owed", kind: "factual_source" as const, query: "owed reading", url: "/owed", rank: 1, reasonCode: "acquire_factual_source", reason: "owed", workKey: "wk-owed" }], replenish: { day: ckey(t, NOW).slice(-10), jobs: {}, outcomes: { readySaved: 0, evidenceBanked: 0, refused: 0, blocked: 0, unreached: 0, stuck: [], receipts: [{ key: "/owed", outcome: "prepared", providerCalls: 1 }] } } as never } }));
    const filed = { ready: 0, deficit: 5, persisted: 2, satisfied: false, reason: "retryable_blocked" as const, jobs: { "wk-begun": { calls: 1, last: "retryable_blocked", settled: false } }, waiting: ["wk-next"], evidenceOwed: [source],
      outcomes: { readySaved: 0, evidenceBanked: 0, refused: 0, blocked: 1, unreached: 1, stuck: [], preparedMs: 54_503, receipts: [{ key: "/rewrite", workKey: "wk-begun", outcome: "retryable_blocked", providerCalls: 17, costUsd: 0.181417, ms: 108_000 }, { key: "/next", workKey: "wk-next", outcome: "evidence_required", providerCalls: 0, costUsd: 0, ms: 0 }] } };
    ...
    expect([rep?.jobs, rep?.waiting, (rep?.outcomes?.receipts ?? []).length, ((rep?.outcomes?.receipts ?? []) as { providerCalls?: number }[]).map((r) => r.providerCalls), rep?.outcomes?.preparedMs, bought, row.status, row.current_phase]).toEqual([ filed.jobs, ["wk-next"], 2, [17, 0], 54_503, 1, "paused", "fact_check"]);
    expect(row.progress.evidenceOwed).toEqual([expect.objectContaining({ ...source, unlocks: { ...source.unlocks, beforeMicros: true } })]); const order: string[] = [], preferred: unknown[] = []; const decision = await import("@/domains/decision"), lookup = vi.spyOn(decision, "loadChangeProposal").mockResolvedValue(t === U ? null : { id: source.unlocks.proposalId, tenantId: t, status: "needs_review", basis: t === "stale-basis" ? "basis_test::d8" : t === "wrong-raw" ? "other::d9" : "basis_test::d9", workKey: source.workKey, obligation: { kind: "evidence", need: source } } as never), current = vi.spyOn(decision, "resolveCurrentBasis").mockResolvedValue(t === "wrong-raw" ? "other::d9" : "basis_test::d9");
    await runResearchCycle(t, { now: () => new Date(NOW), deadlineMs: t === T ? 260_000 : 90_000, steps: { ...BENIGN, ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready", "check_page_facts"] }), acquireEvidence: async (_id, need) => (order.push(need.kind), { acquired: true, detail: "current complete capture on file" }),
      replenishReady: async (_id, _now, seen) => (order.push("writer"), preferred.push(seen?.preferred), { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: [] }) } }); expect([order, preferred, t !== T ? row.progress.acquisitions?.at(-1)?.outcome : null, row.progress.evidenceOwed?.length]).toEqual(t === T ? [["page_source", "writer"], [{ proposalId: source.unlocks.proposalId, workKey: source.workKey, strict: true }], null, 0] : [[], [], "deferred", 1]); expect(lookup.mock.calls.every(([, , opts]) => opts?.canonicalOnly === true)).toBe(true); lookup.mockRestore(); current.mockRestore(); });
  it("serves one exact source before an unpaid keyword turn, then pauses without buying that unit", async () => { const id = `${T}::/next::existing_edit::missing_answer`, need = { key: "/next", kind: "page_source" as const, query: "next question", url: "https://example.com/next", reasonCode: "acquire_page_source", reason: "complete source owed", workKey: "wk-next", rank: 10, unlocks: { proposalId: id, step: "draft" as const, beforeMicros: true as const } }, rows = freshRepo(); rows.push(mk({ tenant_id: T, cycle_key: ckey(T, NOW), current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready", "consume_analyses"] }, waited: { phase: "keyword_discovery", drives: 1, unpaid: true }, evidenceOwed: [{ key: "free", kind: "serp", query: "already posted", rank: 100, reasonCode: "no_exact_serp", reason: "free collection", workKey: "free", postedOn: reportingDay(NOW) }, need, { key: "later", kind: "serp", query: "unrelated paid read", rank: 101, reasonCode: "no_exact_serp", reason: "later", workKey: "later" }], replenish: { day: reportingDay(NOW), jobs: {} } } })); const order: string[] = [], decision = await import("@/domains/decision"), lookup = vi.spyOn(decision, "loadChangeProposal").mockResolvedValue({ id, tenantId: T, status: "needs_review", basis: "basis_test::d9", workKey: need.workKey, obligation: { kind: "evidence", need } } as never), current = vi.spyOn(decision, "resolveCurrentBasis").mockResolvedValue("basis_test::d9"); const steps: ResearchCycleSteps = { ...BENIGN, dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready", "consume_analyses"] }), acquireEvidence: async (_t, n, basis) => (order.push(n.kind === "serp" ? n.key ?? "" : `source:${basis}`), { acquired: true, ...(n.kind === "page_source" ? { attempted: false as const } : {}), detail: "capture saved" }), replenishReady: async () => (order.push("writer"), { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: [] }), funnelUnit: async () => (order.push("keyword"), { status: "done", cursor: null, progress: {} }) }; await runResearchCycle(T, { now: () => new Date(NOW), deadlineMs: 260_000, steps }); expect([order, rows[0]!.status, rows[0]!.progress.waited?.unpaid, rows[0]!.progress.acquisitions?.map((a) => a.outcome)]).toEqual([["free", "source:basis_test", "writer"], "paused", true, ["unlocked", "unlocked"]]); NOW += 3600_000; await runResearchCycle(T, { now: () => new Date(NOW), deadlineMs: 260_000, steps }); expect(order).toEqual(["free", "source:basis_test", "writer", "keyword", "writer", "later", "writer"]); const staleRows = freshRepo(); staleRows.push(mk({ tenant_id: T, cycle_key: ckey(T, NOW), current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready", "consume_analyses"] }, waited: { phase: "keyword_discovery", drives: 1, unpaid: true }, evidenceOwed: [need], replenish: { day: reportingDay(NOW), jobs: {} } } })); lookup.mockResolvedValue({ id, tenantId: T, status: "needs_review", basis: "basis_test::d8", workKey: need.workKey, obligation: { kind: "evidence", need } } as never); order.length = 0; let firstWalk = true; const staleSteps: ResearchCycleSteps = { ...steps, replenishReady: async () => { order.push("writer"); if (firstWalk) { NOW += 120_000; firstWalk = false; return null; } return { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: [need] }; } }; await runResearchCycle(T, { now: () => new Date(NOW), deadlineMs: 150_000, steps: staleSteps }); expect([order, staleRows[0]!.status, staleRows[0]!.progress.evidenceOwed?.[0]?.priorityHeld, staleRows[0]!.progress.acquisitions?.map((a) => a.outcome)]).toEqual([["keyword"], "completed", "owner_unconfirmed", ["deferred"]]); NOW += 3600_000; await runResearchCycle(T, { now: () => new Date(NOW), deadlineMs: 260_000, steps: staleSteps }); expect(order).toEqual(["keyword", "writer", "keyword"]); lookup.mockRestore(); current.mockRestore(); });
  it("records a paid partial source, then advances the next exact source without revisiting it", async () => { const needs = [1, 2].map((n) => ({ key: `/shell-${n}`, kind: "page_source" as const, query: `question ${n}`, url: `https://example.com/shell-${n}`, reasonCode: "acquire_page_source", reason: "complete source owed", workKey: `/shell-${n}::wk`, unlocks: { proposalId: `${T}::/shell-${n}::existing_edit::answer`, step: "draft" as const, beforeMicros: true as const } })), rows = freshRepo(); rows.push(mk({ tenant_id: T, cycle_key: ckey(T, NOW), current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready"] }, evidenceOwed: needs, replenish: { day: reportingDay(NOW), jobs: {} } } }));
    const decision = await import("@/domains/decision"), lookup = vi.spyOn(decision, "loadChangeProposal").mockImplementation(async (_tenant, id) => { const need = needs.find((n) => n.unlocks.proposalId === id)!; return { id, tenantId: T, status: "needs_review", basis: "basis_test::d9", workKey: need.workKey, obligation: { kind: "evidence", need } } as never; }), current = vi.spyOn(decision, "resolveCurrentBasis").mockResolvedValue("basis_test::d9"), order: string[] = [];
    let at = NOW; const steps: ResearchCycleSteps = { ...BENIGN, dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }), acquireEvidence: async (_t, need) => (order.push(need.key ?? ""), need.key === "/shell-1" ? { acquired: false, preflight: "unchanged_incomplete", detail: "paid task yielded a durable partial capture" } : { acquired: true, detail: "current capture saved" }), replenishReady: async () => (order.push("writer"), { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: [] }) };
    await runResearchCycle(T, { now: () => new Date(at), deadlineMs: 260_000, steps }); expect([order, rows[0]!.progress.acquisitions?.at(-1)?.outcome, rows[0]!.progress.evidenceOwed?.[0]?.priorityHeld]).toEqual([["/shell-1"], "not_read", "unchanged_incomplete"]); at += 3600_000; await runResearchCycle(T, { now: () => new Date(at), deadlineMs: 260_000, steps });
    expect([order, rows[0]!.progress.evidenceOwed?.map((n) => [n.key, n.priorityHeld])]).toEqual([["/shell-1", "/shell-2", "writer"], [["/shell-1", "unchanged_incomplete"]]]); lookup.mockRestore(); current.mockRestore(); });
  it("serially reconciles multiple exact sources and leaves a failed source owed before unrelated work", async () => { const needs = [1, 2, 3].map((n) => ({ key: `/page-${n}`, kind: "page_source" as const, query: `question ${n}`, url: `https://example.com/page-${n}`, reasonCode: "acquire_page_source", reason: "complete source owed", workKey: `/page-${n}::wk`, unlocks: { proposalId: `${T}::/page-${n}::existing_edit::answer`, step: "draft" as const, beforeMicros: true as const } })), rows = freshRepo(); rows.push(mk({ tenant_id: T, cycle_key: ckey(T, NOW), current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready", "consume_analyses"] }, waited: { phase: "keyword_discovery", drives: 1, unpaid: true }, evidenceOwed: needs, replenish: { day: reportingDay(NOW), jobs: {} } } }));
    const decision = await import("@/domains/decision"), lookup = vi.spyOn(decision, "loadChangeProposal").mockImplementation(async (_tenant, id) => { const need = needs.find((n) => n.unlocks.proposalId === id)!; return { id, tenantId: T, status: "needs_review", basis: "basis_test::d9", workKey: need.workKey, obligation: { kind: "evidence", need } } as never; }), current = vi.spyOn(decision, "resolveCurrentBasis").mockResolvedValue("basis_test::d9"), order: string[] = [], preferred: string[] = [];
    await runResearchCycle(T, { now: () => new Date(NOW), deadlineMs: 260_000, steps: { ...BENIGN, dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready", "consume_analyses"] }), acquireEvidence: async (_t, need) => (order.push(String(need.key)), { acquired: need.key !== "/page-3", detail: need.key === "/page-3" ? "provider failed" : "capture saved" }), replenishReady: async (_t, _now, seen) => (order.push("writer"), preferred.push(seen?.preferred?.proposalId ?? ""), { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: [], preferred: seen?.preferred ? { ...seen.preferred, retiredReason: "missing" as const } : undefined }), funnelUnit: async () => (order.push("keyword"), { status: "done", cursor: null, progress: {} }) } });
    expect([order, preferred, rows[0]!.progress.evidenceOwed?.map((n) => n.key), rows[0]!.progress.acquisitions?.map((a) => a.outcome), rows[0]!.status]).toEqual([["/page-1", "writer", "/page-2", "writer", "/page-3"], [needs[0]!.unlocks.proposalId, needs[1]!.unlocks.proposalId], ["/page-3"], ["unlocked", "unlocked", "not_read"], "paused"]);
    const failedRows = freshRepo(); failedRows.push(mk({ tenant_id: T, cycle_key: ckey(T, NOW), current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready", "consume_analyses"] }, evidenceOwed: needs.slice(0, 2), replenish: { day: reportingDay(NOW), jobs: {} } } })); order.length = 0; preferred.length = 0; await runResearchCycle(T, { now: () => new Date(NOW), deadlineMs: 260_000, steps: { ...BENIGN, dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready", "consume_analyses"] }), acquireEvidence: async (_t, need) => (order.push(String(need.key)), { acquired: need.key !== "/page-1", detail: need.key === "/page-1" ? "provider failed" : "capture saved" }), replenishReady: async (_t, _now, seen) => (order.push("writer"), preferred.push(seen?.preferred?.proposalId ?? ""), { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: [] }), funnelUnit: async () => (order.push("keyword"), { status: "done", cursor: null, progress: {} }) } }); expect([order, preferred, failedRows[0]!.progress.evidenceOwed?.map((n) => n.key), failedRows[0]!.progress.acquisitions?.map((a) => a.outcome)]).toEqual([["/page-1"], [], ["/page-1", "/page-2"], ["not_read"]]); lookup.mockRestore(); current.mockRestore(); });
  it("retries a transient unreadable owner next drive without buying a source on the failed preflight", async () => { const id = `${T}::/retry::existing_edit::answer`, need = { key: "/retry", kind: "page_source" as const, query: "reader question", url: "https://example.com/retry", reasonCode: "acquire_page_source", reason: "source owed", workKey: "/retry::wk", unlocks: { proposalId: id, step: "draft" as const, beforeMicros: true as const } }, rows = freshRepo(); rows.push(mk({ tenant_id: T, cycle_key: ckey(T, NOW), current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready", "consume_analyses"] }, evidenceOwed: [need], replenish: { day: reportingDay(NOW), jobs: {} } } })); const decision = await import("@/domains/decision"), lookup = vi.spyOn(decision, "loadChangeProposal").mockResolvedValue(null), current = vi.spyOn(decision, "resolveCurrentBasis").mockResolvedValue("basis_test::d9"); let at = NOW, asked = 0; const steps: ResearchCycleSteps = { ...BENIGN, dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready", "consume_analyses"] }), acquireEvidence: async () => (asked++, { acquired: true, detail: "capture saved" }), replenishReady: async () => (at += 120_000, null) };
    await runResearchCycle(T, { now: () => new Date(at), deadlineMs: 150_000, steps }); expect([asked, rows[0]!.progress.acquisitions?.at(-1)?.preflight, rows[0]!.progress.evidenceOwed?.[0]?.priorityHeld, rows[0]!.progress.evidenceOwed?.[0]?.preflightMisses?.count]).toEqual([0, "owner_unreadable", undefined, 1]); lookup.mockResolvedValue({ id, tenantId: T, status: "needs_review", basis: "basis_test::d9", workKey: need.workKey, obligation: { kind: "evidence", need } } as never); at += 3600_000; await runResearchCycle(T, { now: () => new Date(at), deadlineMs: 260_000, steps: { ...steps, replenishReady: async () => ({ ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: [] }) } }); expect([asked, rows.at(-1)!.progress.acquisitions?.at(-1)?.outcome]).toEqual([1, "unlocked"]); lookup.mockRestore(); current.mockRestore(); });
  it.each([T, U])("buys ahead of %s's walk only the free collections and the readings that unlock rows the last walk reached, and leaves every other one to the loop behind it", async (t) => {
    ...
    const drive = async (deadlineMs: number, owed: readonly ReturnType<typeof need>[], reached: readonly string[] = [], spendMs = 0): Promise<{ bought: string[]; deferred: string[]; stopBy: number; noRoom: boolean }> => {
      const rows = freshRepo(); rows.push(mk({ tenant_id: t, cycle_key: ckey(t, NOW), current_phase: "fact_check", progress: { plan: { units: ["replenish_ready", "check_page_facts"] }, evidenceOwed: [...owed],
        replenish: { day, jobs: {}, outcomes: { readySaved: 0, evidenceBanked: 0, refused: 0, blocked: 0, unreached: 0, stuck: [], receipts: reached.map((key) => ({ key, outcome: "prepared", providerCalls: 1 })) } } as never } }));
      const bought: string[] = []; let at = NOW, walk = { stopBy: 0, noRoom: false };
      await runResearchCycle(t, { now: () => new Date(at), deadlineMs, steps: { ...BENIGN, ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready", "check_page_facts"] }),
        acquireEvidence: async (_i, n) => (at += spendMs, bought.push(String(n.query)), { acquired: false, detail: "still owed" }),
  ...
  it.each([T, U])("serves %s's free receipt, then stops lower purchases after the highest ranked review fails", async (t) => {
    const day = ckey(t, NOW).slice(-10), owed = [
      { key: "/hub", kind: "serp" as const, query: "hub-call", rank: 60, reasonCode: "no_exact_serp", reason: "owed", workKey: "/hub::wk" },
      { key: "/hub", kind: "competitor_page" as const, query: "hub-fetch", url: "https://rival.example/x", rank: 60, reasonCode: "winner_unread", reason: "owed", workKey: "/hub::wk" },
      { key: "/hub", kind: "serp" as const, query: "hub-posted", rank: 60, reasonCode: "no_exact_serp", reason: "owed", workKey: "/hub::wk", postedOn: day },
      { key: "/near", kind: "serp" as const, query: "near-one", rank: 10, reasonCode: "no_exact_serp", reason: "owed", workKey: "/near::wk" },
      { key: "/near", kind: "serp" as const, query: "near-two", rank: 11, reasonCode: "no_exact_serp", reason: "owed", workKey: "/near::wk" },
      { key: "/solo", kind: "serp" as const, query: "solo-last", rank: 90, reasonCode: "no_exact_serp", reason: "owed", workKey: "/solo::wk" }, { key: "/review", kind: "semantic_review" as const, query: "review-last", proposalId: "review", rank: 1, reasonCode: "review_owed", reason: "owed", workKey: "/review::wk" }];
    const rows = freshRepo(); rows.push(mk({ tenant_id: t, cycle_key: ckey(t, NOW), current_phase: "fact_check", progress: { plan: { units: ["replenish_ready", "check_page_facts"] }, evidenceOwed: owed,
      replenish: { day, jobs: {}, outcomes: { readySaved: 0, evidenceBanked: 0, refused: 0, blocked: 0, unreached: 0, stuck: [], receipts: [{ key: "/hub", outcome: "produced", providerCalls: 1 }] } } as never } }));
    const bought: string[] = []; await runResearchCycle(t, { now: () => new Date(NOW), deadlineMs: 260_000, steps: { ...BENIGN, ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready", "check_page_facts"] }),
    ...
    expect([bought, (rows.at(-1)!.progress?.acquisitions ?? []).filter((a) => a.outcome === "deferred").length]).toEqual([
      ["hub-posted", "review-last"], 0]); });
  it("serves a current rank-20 review before unrelated rank-100 exact sources", async () => { const decision = await import("@/domains/decision"), lookup = vi.spyOn(decision, "loadChangeProposal").mockImplementation(async (_t, id) => ({ id, tenantId: T, status: "needs_review", basis: "basis_test::d9", workKey: id, obligation: { kind: "evidence", need: { kind: "page_source", reasonCode: "acquire_page_source", url: `https://example.com/${id}`, query: "source" } } } as never)), current = vi.spyOn(decision, "resolveCurrentBasis").mockResolvedValue("basis_test::d9"), sources = Array.from({ length: 104 }, (_, i) => ({ key: `source-${i}`, kind: "page_source" as const, query: "source", url: `https://example.com/source-${i}`, rank: 100 + i, reasonCode: "acquire_page_source", reason: "owed", workKey: `source-${i}`, unlocks: { proposalId: `source-${i}`, step: "draft" as const, beforeMicros: true as const } })), reviewNeed = { key: "review", kind: "semantic_review" as const, query: "review", proposalId: "review", rank: 20, reasonCode: "review_owed", reason: "owed", workKey: "review", unlocks: { proposalId: "review", step: "review" as const } };
    for (const [reached, outcome] of [[false, "success"], [true, "success"], [false, "deferred"], [false, "failed"], [false, "postwalk"]] as const) { const bought: string[] = [], rows = freshRepo(); let at = NOW; rows.push(mk({ current_phase: "fact_check", progress: { plan: { units: ["replenish_ready", "check_page_facts"] }, waited: outcome === "postwalk" ? undefined : { phase: "fact_check", drives: 1, unpaid: true }, evidenceOwed: outcome === "postwalk" ? [] : [reviewNeed, ...sources], replenish: { day: reportingDay(NOW), jobs: {}, outcomes: { receipts: reached ? [{ key: "review", outcome: "evidence_required" }] : [] } } as never } })); await runResearchCycle(T, { now: () => new Date(at), deadlineMs: 260_000, steps: { ...BENIGN, ...healthySteps([]), currentBasis: async () => "basis_test", dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready", "check_page_facts"] }), factCheck: async () => (bought.push("broad_check"), at += outcome === "postwalk" ? 0 : 120_000, { status: "done", banked: 0, bankedPages: [], pagesComplete: 0 }), acquireEvidence: async (_t, n) => (bought.push(n.kind), n.kind === "semantic_review" && outcome === "deferred" ? { acquired: false, attempted: false, detail: "owner changed" } : n.kind === "semantic_review" && outcome === "failed" ? { acquired: false, detail: "review failed" } : { acquired: true, detail: "saved" }), replenishReady: async (_t, _now, seen) => (bought.push(`writer:${seen?.preferred?.proposalId ?? "none"}`), { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked", jobs: {}, evidenceOwed: outcome === "postwalk" && !seen?.preferred ? [reviewNeed, ...sources] : [] }) } }); expect([bought.filter((x) => x === "semantic_review").length, outcome === "success" || outcome === "postwalk" ? bought.slice(0, 2) : bought.filter((x) => x === "page_source")], `review owns the next turn without lower paid source after ${outcome}/${reached}`).toEqual([1, outcome === "success" ? ["semantic_review", "writer:review"] : outcome === "postwalk" ? ["writer:none", "semantic_review"] : []]); }
    lookup.mockRestore(); current.mockRestore(); });
  ...
  it.each([T, U])("leaves %s's walk the whole 190-second room at prompt_observations, and still keeps the fact check its units' own floor out of the same room", async (t) => {
    ...
    expect([await drive("prompt_observations", "daily_observations"), await drive("fact_check", "check_page_facts")]).toEqual([
  ...
  it.each([T, U])("ends %s's drive inside its own slice when the walk takes the slice, writing the walk's row and starting no step the deadline cannot pay for", async (t) => {
    ...
    const row = rows.at(-1)!; expect([started, at - NOW, row.status, row.current_phase, (row.progress?.replenish?.outcomes?.receipts ?? []).length, row.progress?.replenish?.waiting, row.progress?.state?.blocker]).toEqual([
  ...
  const BLOCKED_WALK = { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, outcomes: { readySaved: 0, evidenceBanked: 0, refused: 0, blocked: 0, unreached: 1, stuck: [] } }, MEASURED = (t: string) => ({ day: ckey(t, NOW).slice(-10), jobs: {}, outcomes: { readySaved: 0, evidenceBanked: 0, refused: 0, blocked: 0, unreached: 0, stuck: [], preparedMs: 44_616 } });
  it.each([T, U])("yields %s's FIRST provider wait and resumes one row across 48 ticks", async (t) => {
    { const enabled = true;
      ...
      expect(order).toEqual(enabled ? ["write", "prompt_observations", "winning_pages"] : ["write", "prompt_observations"]); expect(rows[0]!.progress.waited?.drives).toBe(1); expect(rows[0]!.current_phase).toBe("prompt_observations");
      ...
      if (enabled) { for (let tick = 1; tick < 48; tick += 1) { NOW += 30 * 60_000; if (tick === 1) { rows[0]!.current_phase = "winning_pages"; rows[0]!.phase_cursor = null; } await runResearchCycle(t, { now: () => new Date(NOW), steps }); }
        expect(rows).toHaveLength(1); expect(rows[0]!.id).toBe(id); expect(rows[0]!.completed_at).toBeNull(); expect(order.filter((p) => p === "winning_pages")).toHaveLength(48); expect(rows[0]!.lease_owner).toBeNull();
  ...
  it.each([T, U])("keeps %s's pending provider cursor when the resume walk eats the slice", async (t) => {
    const units: DueWork["due"] = ["daily_observations", "replenish_ready", "read_winner_pages"], order: string[] = [];
    const rows = withRun({ tenant_id: t, current_phase: "prompt_observations", progress: { plan: { units } } });
    let tick = 0, at = NOW; const steps: ResearchCycleSteps = { ...BENIGN, dueWork: async () => ({ ...SOMETHING_DUE, due: units }),
      replenishReady: async () => { order.push("walk"); if (tick === 1) at = NOW + 175_000; return null; },
      funnelUnit: async (phase, _u, cursor) => { order.push(phase); if (phase === "prompt_observations" && tick > 0) expect(cursor).toMatchObject({ pendingKeys: ["task-a"] });
        return { status: phase === "prompt_observations" ? "waiting" : "done", cursor: { pendingKeys: ["task-a"] }, progress: {} }; } };
    for (; tick < 5; tick += 1) { at = NOW; order.length = 0; await runResearchCycle(t, { now: () => new Date(at), deadlineMs: 200_000, steps });
      expect(rows).toHaveLength(1); expect(rows[0]).toMatchObject({ id: "seed", current_phase: "prompt_observations", status: "paused", completed_at: null, last_error: null, lease_owner: null }); expect(rows[0]!.phase_cursor).toMatchObject({ phase: "prompt_observations", unit: { pendingKeys: ["task-a"] } });
      if (tick === 1) { expect(order).toEqual(["walk"]); expect(rows[0]!.progress.waited).toMatchObject({ phase: "prompt_observations", drives: 2, unpaid: true }); }
      if (tick === 2) expect(order.slice(0, 2)).toEqual(["prompt_observations", "walk"]);
      NOW += 30 * 60_000;
    }
  });
  it.each([T, U])("runs %s's newly due facts and publication on a held planned pass", async (t) => {
  ...
  it.each([T, U])("buys keywords for %s's mixed plan only when keyword work is explicitly authorized", async (t) => {
    for (const keyword of [null, "plan_cases", "consume_analyses"] as const) {
      ...
      expect(phases.includes("keyword_discovery")).toBe(keyword !== null); expect(phases).toContain("prompt_observations"); expect(phases.includes("winning_pages")).toBe(true);
    } vi.unstubAllEnvs();
  });
  it.each([T, U])("preserves %s's long winner turn after a 33-second read, including zero output and a held provider", async (t) => {
    for (const held of [false, true]) for (const unread of [9, 0]) for (const stage of ["read", "compare"]) for (const failed of [false, true]) { NOW = Date.parse("2026-09-07T12:00:00Z");
  ...
  it.each([T, U])("reads %s's winners first on the drive after the one whose walk took the slice, inside a bounded box, and still hands the walk what is left", async (t) => {
    ...
    const drive = async (): Promise<{ phase: string; status: string; waited: unknown; blocker: string | null | undefined }> => { at = NOW;
      await runResearchCycle(t, { now: () => new Date(at), deadlineMs: 200_000, steps: { ...BENIGN, ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["read_winner_pages"] as DueWork["due"], winners: { unread: 20, unranked: 0 } }),
  ...
  it.each([T, U])("hands %s's read-back only what is spare over the walk's own floor, so the walk keeps its funded window across the whole 123,650 to 195,400 ms band", async (t) => {
    ...
    expect([await drive(195_400), await drive(123_650)]).toEqual([
  ...
  it.each([T, U])("stops %s's drive at the first owed reading whose lease no longer holds, so nothing is bought on a dead lease and every reading re-proves it", async (t) => {
    ...
    expect([await drive(1), await drive(2), await drive(9)]).toEqual([
  ...
  it.each([T, U])("takes %s's whole walk when it lands inside the lease's own tail, and keeps the filed snapshot when it does not", async (t) => {
    ...
    expect([rep(late), rep(lost)]).toEqual([
      [2, [], 1], [1, ["wk-b"], 0]]); });
  it.each([T, U])("spends %s's reading after two attempts that did not land, whatever each of them came back with, buys it again after only one, and starts the count again when the work identity moves or the day turns", async (t) => {
    ...
    const drive = async (owed: readonly Owed[], detail: string, at = NOW): Promise<{ asked: number; owed: readonly Owed[]; attempts: readonly (number | undefined)[] }> => {
      ...
      await runResearchCycle(t, { now: () => new Date(at), deadlineMs: 260_000, steps: { ...BENIGN, ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready", "check_page_facts"] }),
        acquireEvidence: async () => (asked += 1, { acquired: detail === "defer_once" && asked > 1, ...(detail.includes("credit_held") || detail === "defer_once" && asked === 1 ? { attempted: false as const } : {}), detail }),
        replenishReady: async () => ({ ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "made_progress" as const, jobs: {}, evidenceOwed: (detail === "defer_once" && asked > 1 ? [] : [...owed]) as never }) } });
    ...
    expect([one.asked, two.asked, three.asked, one.owed[0]?.tried?.count, two.owed[0]?.tried?.count, one.attempts, two.attempts, three.attempts, three.owed[0]?.boughtOn ?? null]).toEqual([1, 1, 0, 1, 2, [1, 1], [2], [], null]);
    const moved = await drive(two.owed.map((n) => ({ ...n, workKey: "w2" })), SAME); expect([moved.asked, moved.owed[0]?.tried?.count, (await drive(moved.owed, SAME)).asked], "a new capture or a newly banked source gives the work a new identity, and the reading starts again from its first attempt: it is bought, the count is back to 1, and the drive behind that one buys it too, because the new identity has its own two attempts").toEqual([1, 1, 1]);
    ...
    const HELD = "fact check of /rug: failed (credit_held), 0 banked"; const h1 = await drive([need("w1")], HELD), h2 = await drive(h1.owed, HELD), h3 = await drive(h2.owed, HELD); expect([h1.owed[0]?.tried ?? null, h2.owed[0]?.tried ?? null, h3.asked], "a credit hold asked the provider nothing, so it is deferred and never spends the reading's two attempts").toEqual([null, null, 2]);
    const deferred = await drive([{ ...need("capture"), kind: "page_source" }], "defer_once");
    expect([deferred.asked, deferred.owed.length], "a short pre-walk deferral does not mask a qualified acquisition later in the same drive").toEqual([2, 0]);
...
describe("one working context per drive, and the funded work that outlives it", () => {
  it.each([T, U])("prepares %s's account once, rebuilds only the part its own writes moved, hands the work it never began to the next walk, and finishes it though the first connector failed", async (t) => {
    ...
    await runResearchCycle(t, { now: () => new Date(NOW), deadlineMs: 260_000, steps: { ...BENIGN, refreshSources: async () => ({ attempted: 1, succeeded: [], failures: [{ provider: "google_ga4", detail: "fetch failed" }] }),
      acquireEvidence: async () => ({ acquired: true, unlocked: true, detail: "the reading landed" }), reconcileCases: async (_i, _b, plan) => prepare(plan.shared, "evidence"), factCheck: async (_i, _b, _r, _f, shared) => (prepare(shared, "evidence"), prepare(shared, "learning"), { status: "done" as const, banked: 0, bankedPages: [], pagesComplete: 0 }),
    ...
    expect([new Set(maps).size, maps[0] instanceof Map, prepared, carried, rows[0]!.progress.replenish?.waiting, rows[0]!.status, (rows[0]!.progress.sourcesStale ?? "").includes("could not be refreshed")]).toEqual([1, true, ["evidence", "proposals", "learning", "evidence", "proposals"], [[], ["/b::wk"]], ["/b::wk"], "completed", true]); }); });
...
describe("research-run: a recovery pass runs what it was opened for", () => {
  ...
  it("runs exactly the owed set on a mixed debt, and a paid rotation only on its shift, whatever the pass was opened for", async () => { const rows = settled(); const touched: string[] = [];
    const steps = { ...spy(touched), dueWork: async () => ({ ...SOMETHING_DUE, due: ["crawl_pages", "verify_and_measure"] as DueWork["due"] }) };
    await run(steps); expect(touched, "pass 2 is off the verify shift (every 12th drive), so the crawl runs and the verify waits its turn even though this pass was opened carrying it").toEqual(["crawl", "publish"]);
    for (let n = rows.length; n < 12; n += 1) rows.push(mk({ id: `filler-${n}`, cycle_key: `${T}:p${n + 1}:${reportingDay(NOW)}`, status: "completed", completed_at: iso(), current_phase: "done", started_at: iso() }));
    touched.length = 0; await run(steps); expect([rows.at(-1)!.cycle_key.includes(":p13:"), touched], "pass 13 is the verify shift, so the whole owed set runs").toEqual([true, ["crawl", "verify", "measure", "publish"]]); });
  it.each([T, U])("reads the winners on file and buys no results page, crawl or keyword to do it, on %s", async (t) => {
    ...
    await runResearchCycle(t, { now: () => new Date(NOW), steps: { ...BENIGN, ...spy(touched), dueWork: async () => ({ ...SOMETHING_DUE, due: ["read_winner_pages"], winners: { unread: 4, unranked: 6 } }) } });
    expect(touched).toEqual(["winning_pages"]);  // the winner read, and not one phase or cent beside it
    expect([rows[1]!.status, rows[1]!.tenant_id, rows[1]!.progress.plan?.units, rows[1]!.progress.state?.winnersUnread, rows[1]!.progress.state?.winnersUnranked]).toEqual(["completed", t, ["read_winner_pages"], 4, 6]); });  // and the row carries why it opened, and the winner receipt the ONE due-work read produced, so a surface says the number instead of computing a second one
...
describe("research-run resume + status projection", () => {
  it("resumes at the persisted phase (done phases are not re-run) and a deadline pauses durably", async () => {
    ...
    expect(log).toEqual([]); expect([rows[0]!.status, rows[0]!.current_phase]).toEqual(["paused", "gsc_backfill_chunk"]); await run(healthySteps(log)); expect(log).toEqual(["backfill", "crawl", "publish"]); // never "refresh" (that phase was already done)
...
describe("research-run frozen investigation: ONE topic, and the lease the comparison spends under", () => {
  ...
  it("pauses a funnel phase before EVERYTHING when the account basis cannot be read, then reconciles first and proceeds once it can", async () => {
    ...
    expect([order, rows[0]!.status, rows[0]!.current_phase, rows[0]!.progress.focus]).toEqual([[], "paused", "serp_analysis", undefined]); expect(rows[0]!.last_error!.message).toContain("Confirmed business details"); // the existing pause vocabulary, not a new status
  ...
  it("requires the account basis at publish_surface too: no staleness check, no publication, no provider and no website call until it can be read", async () => {
    ...
    expect([order, rows[0]!.status, rows[0]!.current_phase, rows[0]!.progress.surfacePublished !== true]).toEqual([[], "paused", "publish_surface", true]); expect(rows[0]!.last_error!.message).toContain("Confirmed business details"); // the existing pause vocabulary, and no completed evidence phase re-run to get here
...
describe("research-run Today copy", () => {
  ...
  it("stops calling a dead process work in progress: a freshly-touched running row reads in progress, one untouched for ten minutes reads interrupted", () => {
    ...
    expect([dead.state, dead.pauseReason]).toEqual(["paused", "Research stopped part way through. The next daily round picks this back up."]); expect(dead.stepsDone).toBe(6);
  ...
  it("says whether research is alive at all: what the last pass produced, a day that owed nothing, and a silence with the press that ends it", () => {
    ...
    expect([seen({ progress: { funnel: { answersAnalyzed: 34 } } }), seen({ progress: { funnel: { spendUsd: 0 } }, spend_usd: 0.57519 }), RR.projectStatusView(mk({ status: "running", updated_at: iso(NOON_PT + 59 * 60_000), spend_usd: 0.1 }), NOON_PT + 3_600_000).liveness, RR.projectStatusView(mk({ status: "paused", updated_at: iso(NOON_PT + 30 * 60_000), spend_usd: 0.1 }), NOON_PT + 3_600_000).liveness, RR.projectStatusView(mk({ status: "paused", updated_at: at, spend_usd: 0.1, last_error: { phase: "keyword_discovery", message: "stopped", at } }), NOON_PT + 3_600_000).liveness, seen({}), seen({}, NOON_PT + 3 * DAY), RR.projectStatusView(null, NOON_PT).liveness]).toEqual([{ state: "productive", line: "Checked 34 new AI answers for mentions of this site today at 12:00 PM." }, { state: "quiet", line: "Research ran today at 12:00 PM, but saved no new usable evidence." }, { state: "quiet", line: "Research is still running. Paid work has not saved new usable evidence yet." }, { state: "quiet", line: "Research is queued to continue. Paid work has not saved new usable evidence yet." }, { state: "interrupted", line: "Research stopped partway today at 12:00 PM after paid work saved no new usable evidence. Open Today and press Update data." }, { state: "quiet", line: "Checked today at 12:00 PM. Nothing new was owed." }, { state: "silent", line: "No research has run since Thursday. Open Today and press Update data." }, { state: "silent", line: "No research has run for this account yet. Open Today and press Update data." }]); }); // canonical spend is never productive output; each persisted lifecycle still says whether the same no-output pass is active, queued or stopped
});
describe("research-run conflict-free research closure", () => {
  ...
  it("holds a lane that paid and stored nothing for the rest of the day, names it on the row, and releases it on a moved input or a new day", async () => {
    let reads = 0; const paidNothing = async () => (reads += 1, { attempted: 9, settled: 0, refused: 0, read: 0, outcomes: { provider_refused: 9 }, billed: 3 }), day = ckey(T).slice(-10), rows = withRun();
    await run({ analyzeAnswers: paidNothing }); await run({ analyzeAnswers: paidNothing }); // the first pass pays and stores nothing; the second pass of the same day is held on the same input
    expect([reads, rows[0]!.progress.zeroOutput, rows[1]!.progress.state?.blocker]).toEqual([1, { day, lanes: { readback: "0" } }, "Paid work in reading back AI answers produced nothing today; it resumes tomorrow or when its inputs change."]);
    await run({ analyzeAnswers: paidNothing, dueWork: async () => ({ ...SOMETHING_DUE, checks: { ...NO_CHECKS, done: 9, total: 9 } }), dayStanding: async () => ({ ...NO_CHECKS, done: 9, total: 9 }) }); NOW += 86_400_000; await run({ analyzeAnswers: paidNothing }); // new answers settled, then a new day: each releases the lane once
    expect([reads, rows[2]!.progress.zeroOutput?.lanes, rows[3]!.progress.zeroOutput?.day]).toEqual([3, { readback: "9" }, reportingDay(NOW)]);
    let walks = 0; const walked = withRun({ current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready", "plan_cases"] } } }), steps: Partial<ResearchCycleSteps> = { dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready", "plan_cases"] }),
      replenishReady: async () => (walks += 1, { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "made_progress" as const, jobs: {}, outcomes: { readySaved: 0, evidenceBanked: 0, refused: 0, blocked: 1, unreached: 0, stuck: [], receipts: [{ key: "/a", outcome: "provider_blocked", providerCalls: 2 }] } }) };
    await run(steps); await run(steps); // a walk that made two charged calls and saved no row is the leak's own shape, so the day's second pass does not fund it
    expect([walks, walked[0]!.progress.zeroOutput?.lanes, walked[1]!.progress.state?.blocker]).toEqual([1, { walk: "" }, "Paid work in preparing finished changes produced nothing today; it resumes tomorrow or when its inputs change."]); });
  it("keeps a day marker patched onto the row during a phase across the drive's own whole-object advances", async () => {
    const rows = withRun({ current_phase: "prompt_observations" }), retries = { day: ckey(T).slice(-10), counts: { "q1|chatgpt": 2 } };
    await run({ funnelUnit: async () => { rows[0]!.progress = { ...rows[0]!.progress, observationRetries: retries, extraSamples: { day: retries.day, granted: 1 } }; return { status: "done", cursor: null, progress: {} }; } }); // the observation unit patches the row's markers through patch_research_run_progress while the drive holds its own copy of progress
...
describe("a day of AI checks ends when the day ends, never when a batch does", () => {
  ...
  it("keeps ONE open run on the AI phase until every identity is settled, then completes on 140 of 140 carrying the whole day's breakdown", async () => {
    ...
    expect([rows.length, rows[0]!.status, rows[0]!.current_phase]).toEqual([1, "completed", "done"]); expect(rows[0]!.progress.state).toMatchObject({ checksDone: 140, checksTotal: 140, checksAnswers: 140 }); }); // the DAY's numbers, not the last batch's
...
describe("research-run fail-closed + render path", () => {
  ...
  it("does not run from a blank same-day memory when the prior-pass read fails", async () => {
    const base = memRepo().repo; RR.setResearchRunRepoForTests({ ...base,
      sameDay: async () => { throw new Error("same-day rows unreadable"); } });
    await expect(RR.claimRun(T, "owner")).rejects.toThrow("same-day rows unreadable");
  });
  it("does not run when inherited paid-work memory cannot be saved under the lease", async () => {
    const base = memRepo().repo, run = mk({ id: "fresh", status: "running", lease_owner: "owner", lease_expires_at: iso(NOW + LEASE), progress: {} });
    RR.setResearchRunRepoForTests({ ...base,
      claim: async () => ({ ...run }),
      sameDay: async () => [{ id: "prior", progress: { zeroOutput: { day: run.cycle_key.slice(-10), lanes: { walk: "basis-1" } } } }],
      advance: async () => false,
    });
    await expect(RR.claimRun(T, "owner")).rejects.toThrow("same-day research state could not be carried");
  });
});
describe("a visit may not open a research pass the account cannot pay for", () => {
  it("refuses a spent ceiling, allows a funded one, treats an unreadable budget as unaffordable, and probes a cost a ceiling can actually refuse", async () => {
    ...
    expect([refused.allowed, allowed.allowed, unreadable.allowed]).toEqual([false, true, false]); expect([refused.reason.includes("54.9913 / 55"), unreadable.reason.includes("could not be read")]).toEqual([true, true]); // the refusal carries the door's OWN words
...
describe("the due-work runtime: a day is not a unit of work", () => {
  ...
  const PERSISTED = { staleSources: async () => 0, checks: async () => ({ ...NO_CHECKS, done: 140, total: 140, answers: 140, due: 0 }),
    ...
    pagesToCrawl: async () => false, answersToAnalyze: async () => false, readyStock: async () => 5, winnerRows: async () => NO_WINNERS };
  ...
  it("does not buy evidence for a claim while the provider that has to judge it is out of credit", async () => { const owing = { ...PERSISTED, factDebt: async () => ({ owed: 33, everChecked: true }) };
    expect((await dueWork(T, new Date(NOW), { ...owing, creditHeld: async () => false })).due).toContain("check_page_facts"); expect((await dueWork(T, new Date(NOW), { ...owing, creditHeld: async () => true })).due).not.toContain("check_page_facts"); });
  ...
  it("closes a pass that opened with nothing due immediately and at zero cost, and that completion blocks no later pass", async () => {
    ...
    await run({ ...healthySteps(log), dueWork: async () => ({ ...NOTHING_DUE, checks: { ...NO_CHECKS, done: 40, total: 40, answers: 37, unavailable: 2, unsupported: 1 }, cases: { active: 0, parked: 2 }, nextDueAt: "2026-08-02T00:00:00.000Z", winners: { unread: 1, unranked: 3 } }) });
    expect([log, rows[0]!.status, rows[0]!.current_phase]).toEqual([[], "completed", "done"]); // not one phase ran
    expect(rows[0]!.progress.state).toMatchObject({ checksDone: 40, checksTotal: 40, checksAnswers: 37, checksUnavailable: 2, checksUnsupported: 1, casesParked: 2, nextDueAt: "2026-08-02T00:00:00.000Z", winnersUnread: 1, winnersUnranked: 3 });  // A pass that closed with nothing owed still says what it checked, what pages are still owed a reading, and when the waiting ends.
  ...
  it.each(["empty", "stale"] as const)("never closes a %s pass after its receipt advance failed", async (kind) => {
    const { repo, rows } = memRepo(); let finishes = 0;
    RR.setResearchRunRepoForTests({ ...repo, advance: async () => false, finish: async () => { finishes += 1; return true; } });
    rows.push(mk({ cycle_key: kind === "stale" ? `${T}:2026-01-01` : ckey(T, NOW), status: "running", lease_owner: "owner", lease_expires_at: iso(NOW + LEASE) }));
    const receipt = await driveClaimed({ ...rows[0]! }, "owner", kind === "empty" ? NOTHING_DUE : SOMETHING_DUE,
      () => new Date(NOW), NOW + RESEARCH_CYCLE_DEADLINE_MS, BENIGN);
    expect([receipt, finishes, rows[0]!.status]).toEqual(["failed", 0, "running"]);
  });
...
describe("the daily scheduler: one guarded door, the same lease, the same cycle", () => {
  ...
  it("tells a failure apart from a drive, hands the failed account's lease back, and names the expiry when it cannot", async () => {
    ...
    expect(await dispatch({ currentBasis: async () => { throw new Error("the cycle fell over"); } }))  // Basis resolution sits OUTSIDE the cycle's own phase recovery, so a throw here is a throw the dispatch sees.
      .toEqual(R({ claimed: 1, attempted: 1, failed: 1, released: 1, remaining: 1 }));
    ...
    expect(receipt).toEqual(R({ claimed: 1, attempted: 1, failed: 1, remaining: 1, leaseHeldUntil: [iso(NOW + LEASE)] })); expect(receipt.released).toBe(0); });
  it("re-reads the durable run before handing back a failed drive, so paid progress is not closed with its claim-time zero", async () => {
    const rows = freshRepo(); setAccountStatus(U, "pending_onboarding");
    await dispatch({ currentBasis: async () => { rows[0]!.progress = { ...rows[0]!.progress, funnel: { spendUsd: 0.37 } }; throw new Error("a later step failed"); } });
    expect([rows[0]!.status, rows[0]!.spend_usd]).toEqual(["paused", 0.37]);
  });
  it("counts a wait, a phase pause and a lost lease as themselves, and a clean cycle as exactly one success", async () => {
    ...
    expect(await waiting.receipt).toEqual(R({ claimed: 1, attempted: 1, paused: 1, remaining: 1 })); // durable provider work is pending, which is not a day's work done
  ...
  it("says 503 when it could not read what is owed, in one bounded word, and keeps 200 for a fleet that genuinely owes nothing", async () => {
    ...
    expect(rows).toHaveLength(0); ROUTE.receipt = R(); vi.stubEnv("CRON_SECRET", "s3cret"); const idle = await post({ authorization: "Bearer s3cret" }); expect([idle.status, await idle.json()]).toEqual([200, R()]); expect((await post({ authorization: "Bearer wrong" })).status).toBe(401); // and a bad bearer still claims nothing at all
    vi.unstubAllEnvs(); });
  ...
  it("continues ordinary pauses promptly, backs off repeated failures, and recovers expired leases", async () => {
    NOW = Date.parse("2026-09-23T12:25:00Z"); const rows = withRun({ status: "running", lease_owner: "o", lease_expires_at: iso(NOW + LEASE) }); setAccountStatus(U, "pending_onboarding"); const id = rows[0]!.id; await RR.finishRun(T, id, "o", "paused"); expect([rows[0]!.dispatch_reason, rows[0]!.dispatch_attempts, rows[0]!.next_dispatch_at]).toEqual(["continuation", 0, iso()]);
    ...
    await RR.claimRun(T, "o3"); await RR.finishRun(T, id, "o3", "paused"); expect([rows[0]!.dispatch_attempts, rows[0]!.next_dispatch_at]).toEqual([0, iso()]);
    NOW = Date.parse("2026-09-23T12:30:00Z"); expect((await dispatch({})).claimed).toBe(1); expect(await dispatch(NO_PHASE)).toEqual(R()); rows.length = 0; rows.push(mk({ status: "running", lease_owner: "dead", lease_expires_at: iso(NOW - 1), next_dispatch_at: iso(NOW + DAY) })); expect((await dispatch({})).claimed).toBe(1);
  });
...
describe("dueWork: what is genuinely owed, computed from persisted state only", () => {
  ...
  const base = { staleSources: async () => 0, checks: async () => ({ ...NO_CHECKS, done: 4, total: 4, answers: 4, due: 0 }), basis: async () => "b1", evidenceVersion: async () => 7, answersToAnalyze: async () => false, analysisFingerprint: async () => "fp1", consumedAnalyses: async () => "fp1",
    surfaceStale: async () => false, debt: async () => ({ measurable: 0, unverified: 0 }), pagesToCrawl: async () => false, factDebt: async () => ({ owed: 0, everChecked: true }), readyStock: async () => 5, creditHeld: async () => false, winnerRows: async () => NO_WINNERS,
  ...
  it("names each owed unit from the ONE persisted fact that proves it", async () => {
    ...
    expect(await due({ checks: async () => ({ ...NO_CHECKS, done: 2, total: 4, answers: 2, due: 2 }) })).toEqual(["daily_observations"]); expect(await due({ pagesToCrawl: async () => true })).toEqual(["crawl_pages"]); // a page of their own website I have never read is owed a batch; base says none is, which is the "nothing due" half
    ...
    expect(await due({ debt: async () => ({ measurable: 3, unverified: 0 }) })).toEqual(["verify_and_measure"]); expect(await due({ debt: async () => ({ measurable: 0, unverified: 1 }) })).toEqual(["verify_and_measure"]);  // A change marked implemented but never checked live owes the same unit. ONE ledger read answers both.
  ...
  it.each(WINNERS)("owes the winner read for pages banked before the reading existed, and says how many on the receipt, on $t", async (w) => {
    const asked: string[][] = [];
    const work = await dueWork(w.t, new Date(NOW), { ...base, winnerRows: async (t, basis) => (asked.push([t, basis]), { ...NO_WINNERS, winners: [w.read, w.legacy, w.wordless] }) });
    expect([work.due, work.readable, work.winners]).toEqual([["read_winner_pages"], true, { unread: 1, unranked: 0 }]);  // the one row that carries no reading, and the receipt says so
    expect(asked).toEqual([[w.t, "b1"]]);  // one read, scoped to this account and the basis it holds now
    const both = await dueWork(w.t, new Date(NOW), { ...base, winnerRows: file([w.legacy, holdOn(w.read, NOW - 1)]) });
    expect([both.due, both.winners]).toEqual([["read_winner_pages"], { unread: 2, unranked: 0 }]); });  // and a retry date I promised that has ARRIVED is owed again, exactly as a page of my own is
  it.each(WINNERS)("owes nothing for a winner that carries a reading or is waiting on a date I promised, on $t", async (w) => {
    const quiet = await dueWork(w.t, new Date(NOW), { ...base, winnerRows: file([w.read, w.wordless, holdOn(w.legacy, NOW + DAY)]) });
    expect([quiet.due, quiet.readable, quiet.winners]).toEqual([[], true, { unread: 0, unranked: 0 }]);  // words, an honest no-words reading, and a live hold: nothing here is owed
    expect((await dueWork(w.t, new Date(NOW), { ...base, winnerRows: async () => NO_WINNERS })).winners).toEqual({ unread: 0, unranked: 0 }); });  // no winners at all is an honest none, not a debt
  it.each(WINNERS)("owes the winner read for a results page whose own pages nothing has ranked, and counts only the pages one pass could read, on $t", async (w) => {
    const paid = bought(w.query, [`first-${w.tag}.example`, "own.example", "reddit.com", `second-${w.tag}.example`]), work = await dueWork(w.t, new Date(NOW), { ...base, winnerRows: file([], [paid]) });
    expect([work.due, work.readable, work.winners]).toEqual([["read_winner_pages"], true, { unread: 0, unranked: 2 }]);  // the two pages a pass would rank: a page of this account's own is never a winner and the forum page the reserve refuses is never one either
    const quiet = await dueWork(w.t, new Date(NOW), { ...base, winnerRows: file([], [bought(w.query, [`late-${w.tag}.example`], "done", 21), { ...paid, status: "pending" }]) });
    expect([quiet.due, quiet.winners]).toEqual([[], { unread: 0, unranked: 0 }]);  // a page ranked twenty-first is past the twenty rows bought and is not a page that wins the search, and a results page nobody has bought yet owes nothing at all
    const many = [1, 2].map((n) => bought(`${w.query} ${n}`, Array.from({ length: 10 }, (_, i) => `h${n}${i}-${w.tag}.example`))); expect((await dueWork(w.t, new Date(NOW), { ...base, winnerRows: file([], many) })).winners).toEqual({ unread: 0, unranked: 10 });  // Two searches reserve five ranked pages each inside the existing total budget.
    const four = [1, 2, 3, 4].map((n) => bought(`${w.query} ${n}`, ["first", "second", "third"].map((x) => `${x}${n}-${w.tag}.example`), "done", 1, `2026-02-0${5 - n}T00:00:00.000Z`));
    expect((await dueWork(w.t, new Date(NOW), { ...base, winnerRows: file([], four) })).winners).toEqual({ unread: 0, unranked: 9 }); });  // FOUR searches owe a reading and one pass reserves for three of them: the receipt counts the nine pages of the three newest, never the twelve of all four, because the fourth search's pages are left to the global order and lose it
  it.each(WINNERS)("owes the remaining comparison for a search holding only one ranked page, on $t", async (w) => {
    const paid = bought(w.query, [`first-${w.tag}.example`, `second-${w.tag}.example`]), held = await dueWork(w.t, new Date(NOW), { ...base, winnerRows: file([{ ...w.read, url: `https://second-${w.tag}.example/p` }], [paid]) });
    expect([held.due, held.winners]).toEqual([["read_winner_pages"], { unread: 0, unranked: 2 }]); });  // One banked result cannot stand in for the remaining comparison pages.
  it.each(WINNERS)("says unreadable rather than a quiet nothing when the research document cannot be read, on $t", async (w) => {
    ...
    expect([blind.readable, blind.due, blind.winners]).toEqual([false, [], { unread: null, unranked: null }]); });  // a count nothing could read is unknown, and unknown never authorizes or forbids the read
...
describe("the cycle finishes stored work before it buys exploratory evidence", () => {
  ...
  it("writes which of the two deadline answers each walk gave, with every receipt it filed, on two accounts", async () => {
    for (const [tenant, ended] of [[T, "boxed"], [U, "stopped"]] as const) {
      const rows = withRun({ tenant_id: tenant, cycle_key: ckey(tenant, NOW), current_phase: "keyword_discovery" });
      const answer = { ready: 1, deficit: 0, persisted: 1, satisfied: false, reason: "retryable_blocked" as const, jobs: { "/p::wc5::e1": { calls: 1, last: "retryable_blocked", settled: false } },
        outcomes: { readySaved: 0, evidenceBanked: 0, refused: 0, blocked: 1, unreached: 1, stuck: [], ended, receipts: [{ key: "/p", outcome: "retryable_blocked", providerCalls: 3 }] } };
      await runResearchCycle(tenant, { now: () => new Date(NOW), steps: { ...BENIGN, ...healthySteps([]), replenishReady: async () => answer } });
      const kept = rows.at(-1)!.progress?.replenish?.outcomes;
      expect([kept?.ended, (kept?.receipts ?? []).length, rows.at(-1)!.progress?.replenish?.closed], `${tenant}: the row says which of the two answers this walk gave, keeps every receipt it filed, and neither answer closes the day`).toEqual([ended, 1, undefined]);
    }});
  it("writes a page off only on its own settled receipt, and never on a blocked, unreached or unreadable pass", async () => {
    const legacy = { id: "legacy", kind: "existing_edit", status: "needs_review", researchOnly: true, pageUrl: "https://example.com/legacy", pagePath: "/legacy", primaryQuery: "legacy answer", diagnosisCause: "ai_citation_gap", treatment: "add_answer_section", changeFamily: "answer_block", recommendedChange: { kind: "existing_edit", field: "answer_block", before: null, after: "" }, evidence: { query: "legacy answer", hints: ["rivals cited, owned gap not diagnosed"], evidenceRefCount: 2 }, obligation: { kind: "draft" } };
    const focus = { id: "focus", kind: "existing_edit", status: "needs_review", researchOnly: true, pageUrl: "https://example.com/focus", pagePath: "/focus", primaryQuery: "qualified answer", diagnosisCause: "ai_citation_gap", treatment: "add_answer_section", causeFinding: { cause: "ai_citation_gap", payload: { cause: "ai_citation_gap", engine: "chatgpt", promptText: "qualified answer", aeoKind: "missing_information", missing: "Installation depth depends on the local water table" } }, changeFamily: "answer_block", recommendedChange: { kind: "existing_edit", field: "answer_block", before: null, after: "" }, evidence: { query: "qualified answer", hints: ["saved comparison"], evidenceRefCount: 3 }, obligation: { kind: "draft" } };
    const M = { ready: 0, ranked: [legacy, focus] as unknown[], declared: ["/a", "/b", "/c", "/d", "/e"], out: [] as { key: string; outcome: string; why?: string; cost?: number }[], outcome: "proposals_persisted", throws: false, owed: [] as unknown[], lands: true, evidence: "e1", asked: [] as { produce?: boolean; bypassCache?: boolean; maxDrafts?: number; focusPage?: string; memory?: Readonly<Record<string, { calls: number; last: string; settled: boolean }>> }[] }; vi.resetModules();
    const ledger: number[] = []; vi.doMock("@/lib/cost/budget-ledger-supabase", () => ({ getTenantSpentThisMonthUsd: async () => ledger.shift() ?? 0 }));
    ...
    vi.doMock("@/domains/decision", async () => ({ resolveCurrentBasis: async () => "b", stockOf: (rows: unknown[]) => rows.length, settledByRows: (await vi.importActual<typeof import("@/domains/decision/load-proposals")>("@/domains/decision/load-proposals")).settledByRows, /* THE REAL RULE, never a stub: the day memory is settled against the rows before anything is funded, so this harness proves what production does */ loadProposalQueue: async () => (M.ready < 0 ? Promise.reject(new Error("queue unreadable")) : { ranked: M.ranked, ready: Array.from({ length: M.ready }, () => ({})), toDo: [], research: [] }),
      produceProposalsForTenant: async (_t: string, o: { maxDrafts?: number; focusPage?: string; produce?: boolean; bypassCache?: boolean; memory?: Readonly<Record<string, { calls: number; last: string; settled: boolean }>>; handOver?: (ask: () => { paid: unknown; persisted: number }) => void }) => { if (M.throws) throw new Error("the provider fell over");
        ...
        const funded = M.declared.filter((k) => { const m = o.memory?.[wk(k)]; return m?.settled !== true && (m?.calls ?? 0) < 2; }).filter((k) => !o.focusPage || k === o.focusPage).slice(0, o.maxDrafts ?? Infinity); // the plan's own two questions, asked of the WORK and never of the page
    ...
    try {
      ...
      for (const row of [legacy, focus, { ...legacy, treatment: undefined }]) { M.ranked = [row]; await live.replenishReady(T, new Date(NOW), { jobs: {} }); expect(M.asked.at(-1)?.focusPage, "ranked work remains open across pages regardless of the first row").toBeUndefined(); } M.ranked = [legacy, focus]; M.asked.length = 0;
      { let handed: (() => unknown) | null = null; const prior = M.declared; M.declared = Array.from({ length: 143 }, (_, i) => `/waiting-${i}`); M.out = [{ key: "/waiting-0", outcome: "produced" }]; const whole = await live.replenishReady(T, new Date(NOW), { jobs: {}, filed: (f) => { handed = f; } }), boxed = (handed as unknown as (() => { jobs: unknown; waiting?: unknown; reason: string; satisfied: boolean; outcomes?: { receipts?: unknown[] } }))();
        const rows = withRun({ current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready"] } } }); await run({ ...healthySteps([]), replenishReady: async () => whole }); const waiting = JSON.parse(JSON.stringify(rows[0]!.progress.replenish?.waiting)) as string[];
        const { DRAFT_BUDGET } = await import("@/domains/decision/draft-budget"), tail = waiting.at(-1)!, job = (key: string, workKey: string) => ({ key, workKey, family: "editor", impact: 1, calls: 1 }); const choose = (memory = {}) => DRAFT_BUDGET.plan({ jobs: [job("/fresh", "/fresh::wc3::e1"), job("/waiting-142", tail)], candidates: 1, waiting, memory }).funded.map((f) => f.key);
        expect([boxed.jobs, boxed.waiting, (boxed.outcomes?.receipts ?? []).length, boxed.reason, boxed.satisfied], "an interrupted walk carries its full waiting list and receipts").toEqual([whole!.jobs, whole!.waiting, 143, "retryable_blocked", false]); expect([waiting.length, tail, choose(), choose({ [tail]: { calls: 1, last: "produced", settled: true } })]).toEqual([142, "/waiting-142::wc3::e1", ["/waiting-142"], ["/fresh"]]);
        M.declared = prior; M.ready = 0; mem = { jobs: {} }; M.out = []; M.asked.length = 0; } // the reading above is not one of the drives the sequence below counts
      ...
      expect([Object.entries(mem.jobs).map(([k, m]) => [k, m.calls, m.settled, (m as { waited?: number }).waited ?? 0]), M.asked.length, M.asked[0]!.produce, M.asked[0]!.bypassCache, M.asked[0]!.maxDrafts, M.asked[0]!.focusPage], "the walk preserves per-work receipts without imposing a single-page or single-draft output quota").toEqual([[["/a::wc3::e1", 1, false, 0], ["/b::wc3::e1", 0, false, 0], ["/c::wc3::e1", 0, false, 0], ["/d::wc3::e1", 0, false, 0], ["/e::wc3::e1", 0, false, 0]], 1, true, false, undefined, undefined]);
      ...
      expect([reopened!.reason, reopened!.ready, reopened!.outcomes!.readySaved, Object.keys(settledUnder).every((w) => reopened!.jobs[w]?.settled === true), Object.keys(reopened!.jobs).length], "one pass delivers BOTH eligible pages under changed evidence, while remembering old settlements; no single-draft or single-page quota").toEqual(["made_progress", 2, 2, true, 4]); M.evidence = "e1";
      ledger.push(1.0, 1.02); M.ready = 0; M.declared = ["/h"]; mem = { jobs: {} }; M.out = [{ key: "/h", outcome: "produced", cost: 0.02 }]; // Both exact and partially explained ledger deltas retain their receipt.
      ...
      ledger.push(1.0, 1.9); M.ready = 0; M.declared = ["/i"]; mem = { jobs: {} }; M.out = [{ key: "/i", outcome: "produced", cost: 0.02 }];
  ...
  it("carries an unlocked source into its exact writer ahead of old waiting and higher-impact untried work", async () => {
    const { DRAFT_BUDGET } = await import("@/domains/decision/draft-budget"), key = "/pools::body::tide", workKey = `${key}::wc21::basis::source`, proposalId = `${T}::/pools::existing_edit::missing_answer`; const need = { key, kind: "competitor_page" as const, query: "rock pool tides", reasonCode: "no_winner_to_read", reason: "Read the named gap", workKey, rank: 1, unlocks: { proposalId, step: "draft" as const } };
    const day = ckey(T, NOW).slice(-10), jobs = { [workKey]: { calls: 0, last: "not_reached", settled: false } }, order: string[] = [], selected: string[] = [], seenInputs: unknown[] = [], declined: unknown[] = []; const rows = withRun({ current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready"] }, evidenceOwed: [need], replenish: { day, jobs, waiting: [workKey] } } }); let walks = 0;
    await run({ ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }), acquireEvidence: async () => (order.push("source"), { acquired: true, detail: "Winner words banked" }),
      replenishReady: async (_t, _n, seen) => { order.push("walk"); if (++walks === 1) return { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs, waiting: [workKey], evidenceOwed: [need] };
        seenInputs.push(seen); const plan = DRAFT_BUDGET.plan({ jobs: [{ key: "/higher", family: "editor", impact: 900, calls: 13, workKey: "/higher::new" }, { key, family: "editor", impact: 1, calls: 13, workKey }], candidates: 1, memory: seen?.jobs, waiting: seen?.waiting, preferredWorkKey: seen?.preferred?.workKey }); selected.push(...plan.funded.map((f) => f.key)); declined.push(...plan.declined); return { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs, evidenceOwed: [] }; } });
    expect([order, seenInputs, selected, declined, rows[0]!.progress.acquisitions?.filter((a) => a.key === key).map((a) => a.outcome)]).toEqual([["walk", "source", "walk"], [expect.objectContaining({ preferred: { proposalId, workKey }, jobs, waiting: [workKey] })], [key], expect.any(Array), ["unlocked"]]);
  });
  it("buys exactly the search a funded candidate was refused for, even on a run that opened only for the stock", async () => {
    const asked: Array<{ kind: string; query: string; basis: string; runId: string; cycle: string }> = [];
    ...
    await run({ ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }),
      acquireEvidence: async (_t, need, basis, _budget, _review, _scope, receipt) => { asked.push({ kind: need.kind, query: need.query, basis: basis ?? "(none)", runId: receipt?.runId ?? "", cycle: receipt?.cycle ?? "" }); return { acquired: true, detail: "bought" }; },
    ...
    expect([asked[0]!.basis !== "(none)", asked[0]!.runId, asked[0]!.cycle]).toEqual([true, rows[0]!.id, rows[0]!.cycle_key]); // the REAL runner carries the basis and the run receipt through the owed-reading seam
  ...
  it("admits stamped new-page debt by rank while dropping unstamped legacy topic debt", async () => {
    const whole = { key: "topic:new-guide", kind: "serp" as const, query: "new guide", reasonCode: "no_exact_serp", reason: "The new page needs its results page.", workKey: "topic:new-guide", rank: 1 };
    const edit = { key: "/guide::body::answer", kind: "serp" as const, query: "existing guide answer", reasonCode: "no_exact_serp", reason: "The existing section needs its results page.", workKey: "/guide::existing_edit::section", rank: 2, delivery: "existing_page_edit" as const }, earned = { ...whole, key: "topic:earned-guide", query: "earned new guide", workKey: "topic:earned-guide", rank: 1.5, delivery: "whole_page" as const };
    const rows = withRun({ current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready"] }, evidenceOwed: [whole, edit, earned] } }), asked: string[] = [];
    await run({ ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }),
      acquireEvidence: async (_t, need) => { asked.push(need.query); return { acquired: true, unlocked: false, detail: "the reading landed but stronger evidence is still owed" }; },
      replenishReady: async () => ({ ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: [whole, edit, earned] }) });
    expect(asked, "unstamped legacy debt is refused; an earned page and an existing edit both advance by current rank").toEqual([earned.query, edit.query]);
    expect((rows[0]!.progress.evidenceOwed ?? []).map((need) => need.key), "the legacy whole-page debt is removed while both current obligations persist").toEqual([edit.key, earned.key]);
  });
  it.each([T, U])("names on the run row which row and which funding identity every reading was bought to unlock, buys one reading once however many rows owe it, and writes a reading that landed without moving its obligation onto the need instead of re-owing it in silence [%s]", async (tenant) => {
    ...
    const scopedRows = freshRepo(); scopedRows.push(mk({ tenant_id: tenant, cycle_key: ckey(tenant, NOW), current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready"] } } })); const scoped = ["topic:one", "topic:two"].map((key) => ({ ...owed[0]!, key, url: undefined, topic: { key, label: "Well depth" }, workKey: key })), scopes: string[] = []; await runResearchCycle(tenant, { now: () => new Date(NOW), steps: { ...BENIGN, ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }), acquireEvidence: async (_t, need) => { scopes.push(need.topic!.key); return { acquired: true, unlocked: false, detail: "exact topic read, stronger evidence owed" }; }, replenishReady: async () => ({ ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: scoped }) } }); expect([scopes, scopedRows[0]!.progress.evidenceOwed ?? [], scopedRows[0]!.progress.acquisitions], "legacy topic debt is classified as whole-page work before acquisition and removed from the existing-edit run").toEqual([[], [], undefined]);
  ...
  it("loads only the named scoped change and distinguishes saved partial review progress from complete acceptance", async () => {
    vi.resetModules(); const { deserializeChangeProposal, serializeChangeProposal, componentIdOf } = await import("@/domains/decision/contracts"), { copyKey, REVIEW_CONTRACT } = await import("@/domains/decision/proof"); const ID = `${T}::/well::existing_edit::section`, decoded = deserializeChangeProposal(JSON.stringify({ v: 1, proposal: { id: ID, tenantId: T, kind: "existing_edit", pagePath: "/well", pageUrl: "https://example.com/well", pageLabel: "Well", primaryQuery: "well depth", opportunityType: "coverage", changeFamily: "section", status: "needs_review", basis: "b1::d9", workKey: "review-work", publish: "manual", createdAt: "2026-09-13T00:00:00.000Z", recommendedChange: { kind: "existing_edit", field: "section", before: null, after: "The well is 20 feet deep. Its casing is steel." }, whyItMatters: "Answer the measured question", estimatedEffortMinutes: 5, riskLevel: "low", confidence: "medium", limitations: [], evidence: { query: "well depth", hints: [], evidenceRefCount: 2 }, impactScore: null, upsidePerMonth: null, claims: [{ text: "The well is 20 feet deep.", supportedBy: ["fact-1"] }, { text: "Its casing is steel.", supportedBy: ["fact-2"] }], supportFacts: [{ id: "fact-1", fact: "The well is 20 feet deep." }, { id: "fact-2", fact: "Its casing is steel." }] } })); expect(decoded).not.toBeNull(); const base = decoded!;
    const editor = { pageFit: true, usefulAndNatural: true, placementCorrect: true, resolvesDiagnosis: true, implementableNow: true, improvesPage: true, wouldHandToCustomer: true, notes: "Complete reader-facing answer" }, review = { of: copyKey(base), version: REVIEW_CONTRACT, editor, claims: [{ i: 0, by: ["fact-1"], entailed: true }, { i: 1, by: ["fact-2"], entailed: true }] }, full = { ...base, semanticReview: review }, S = { loaded: [] as string[], saved: [] as typeof base[], workKeys: [] as string[], reads: 0, load: base, row: full as typeof base | null, save: "saved" };
    vi.doMock("@/domains/decision/load-proposals", () => ({ resolveCurrentBasis: async () => "b1::d9" })); vi.doMock("@/domains/decision/proposal-store", () => ({ loadChangeProposal: async (_t: string, id: string) => (S.loaded.push(id), S.load), saveChangeProposal: async (p: typeof base, _transition: unknown, _keep: unknown, expected: typeof base) => (expect(expected).toBe(S.load), S.saved.push(p), S.save) })); vi.doMock("@/domains/decision/drafted-copy", () => ({ reviewFinishedCopy: async (_p: typeof base, opts: { proposalWorkKey: string; attempts?: { left: number; record?: (r: unknown) => void }; stopBy?: number }) => (S.reads += 1, S.workKeys.push(opts.proposalWorkKey), opts.attempts?.record?.({ status: "drafted", attempts: 1, costUsd: 0.01 }), { row: S.row, detail: "banked review progress" }) }));
    try { const { defaultSteps: live } = await import("@/domains/runtime/ops/research-steps"); const cases: Array<[string, typeof base | null, string, boolean, boolean]> = [["complete", full, "saved", true, true], ["partial", { ...full, semanticReview: { ...review, claims: review.claims.slice(0, 1) } }, "saved", true, false], ["old", { ...full, semanticReview: { ...review, version: REVIEW_CONTRACT - 1 } }, "saved", true, false], ["wrong placement", { ...full, semanticReview: { ...review, editor: { ...editor, placementCorrect: false } } }, "saved", true, false], ["not implementable", { ...full, semanticReview: { ...review, editor: { ...editor, implementableNow: false } } }, "saved", true, false], ["wrong source", { ...full, semanticReview: { ...review, claims: [{ ...review.claims[0]!, by: ["fact-2"] }, review.claims[1]!] } }, "saved", true, false], ["stale copy", { ...full, semanticReview: { ...review, of: "old-copy" } }, "saved", true, false], ["free record repair", base, "saved", true, false], ["provider interrupted", null, "saved", false, false], ["save failed", full, "failed", false, false], ["already saved", full, "unchanged", true, true]];
      for (const [label, row, saved, acquired, unlocked] of cases) { S.row = row && deserializeChangeProposal(serializeChangeProposal(row)); S.save = saved; S.loaded = []; S.saved = []; S.reads = 0; const got = await live.acquireEvidence(T, { kind: "semantic_review", query: "well depth", proposalId: ID, workKey: "review-work" }, "b1", 5_000); expect([got.acquired, got.unlocked === true, S.loaded, S.reads, S.saved], label).toEqual([acquired, unlocked, [ID], 1, row ? [S.row] : []]); }
      expect(S.workKeys).toEqual(Array(cases.length).fill("review-work"));
      for (const wrong of [{ ...base, tenantId: U }, { ...base, id: "different-change" }]) { S.load = wrong; S.reads = 0; S.saved = []; const got = await live.acquireEvidence(T, { kind: "semantic_review", query: "well depth", proposalId: ID, workKey: "review-work" }, "b1", 5_000); expect([got.acquired, S.reads, S.saved], "payload identity cannot buy another tenant or change a review").toEqual([false, 0, []]); }
      S.loaded = []; S.reads = 0; S.saved = []; const bare = await live.acquireEvidence(T, { kind: "semantic_review", query: "well depth", workKey: "review-without-row" }, "b1", 5_000); expect([bare.acquired, bare.detail.includes("names no change"), S.loaded, S.reads, S.saved]).toEqual([false, true, [], 0, []]); for (const stale of [{ ...base, basis: "b1::d8" }, { ...base, workKey: "source-changed" }, { ...base, status: "implemented_pending_verification" as const }]) { S.load = stale; S.reads = 0; S.saved = []; const got = await live.acquireEvidence(T, { kind: "semantic_review", query: "well depth", proposalId: ID, workKey: "review-work" }, "b1", 5_000); expect([got.acquired, got.attempted, S.reads, S.saved], "stale basis, changed source identity and shipped copy buy no review").toEqual([false, false, 0, []]); } S.load = base; S.row = { ...full, semanticReview: { ...review, claims: review.claims.slice(0, 1) } }; S.save = "saved"; S.reads = 0; S.saved = []; const rows = withRun({ current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready"] } } }), need = { key: "/well::section", kind: "semantic_review" as const, query: "well depth", proposalId: ID, reasonCode: "review_owed", reason: "The written copy still owes its review.", workKey: "review-work" }; let walks = 0; await run({ ...healthySteps([]), currentBasis: async () => "b1", dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }), acquireEvidence: live.acquireEvidence, replenishReady: async () => (walks += 1, { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: [need] }) }); expect([S.reads, walks, rows[0]!.progress.acquisitions?.map((a) => a.outcome), rows[0]!.progress.evidenceOwed?.map((n) => [n.key, n.reason.includes("Review progress is saved"), n.reason.includes("stronger one")])], "real saved partial review stays owed, records incomplete acquisition, and starts no replacement drafting walk").toEqual([1, 1, ["read_not_usable"], [[need.key, true, false]]]); S.row = full; S.reads = 0; S.saved = []; const completeRows = withRun({ current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready"] } } }), counters: number[] = []; await run({ ...healthySteps([]), currentBasis: async () => "b1", dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }), acquireEvidence: live.acquireEvidence, replenishReady: async (_t, _n, seen) => (counters.push(seen?.callsSpent ?? 0), { ready: S.saved.length, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: S.saved.length ? [] : [need] }) }); expect([S.reads, counters, completeRows[0]!.progress.acquisitions?.map((a) => a.outcome), completeRows[0]!.progress.evidenceOwed], "a completed saved review transfers its actual request receipt into the very next walk allowance").toEqual([1, [0, 1], ["unlocked"], []]); const parts: NonNullable<typeof base.bundle>["components"] = [{ kind: "section", label: "Depth", before: null, after: "The well is 20 feet deep.", evidenceKeys: ["fact-1"], risk: "safe" }, { kind: "section", label: "Casing", before: null, after: "Its casing is steel.", evidenceKeys: ["fact-2"], risk: "safe" }], bundle: NonNullable<typeof base.bundle> = { objective: "Answer well depth", metric: "Clicks", scope: { queries: ["well depth"], prompts: [] }, components: parts, receipt: { items: [], missing: [], freshestObservedAt: null }, alternatives: [], risks: [], confidenceReasons: [], measurementPlan: "Check clicks" }, bundled = { ...base, bundle, claims: base.claims!.map((c, i) => ({ ...c, of: componentIdOf(parts[i]!, i) })) }, partialOwner = { ...bundled, semanticReview: { ...review, of: copyKey(bundled), claims: [{ i: 0, by: ["fact-1"], entailed: true }] } }, progressKey = `review-work::read:${(await import("node:crypto")).createHash("sha256").update(JSON.stringify([partialOwner.semanticReview.of, [0]])).digest("hex")}`; expect((await import("@/domains/decision/obligation")).nextObligation(partialOwner)?.kind).toBe("review"); S.load = partialOwner; S.reads = 0; const forged = await live.acquireEvidence(T, { kind: "semantic_review", query: "well depth", proposalId: ID, workKey: `review-work::read:${"a".repeat(64)}` }, "b1", 5_000); expect([forged.acquired, forged.attempted, S.reads], "a plausible but false partial-review digest buys nothing").toEqual([false, false, 0]); for (const advances of [false, true]) { S.load = base; S.row = { ...full, semanticReview: { ...review, claims: review.claims.slice(0, 1) } }; S.reads = 0; S.saved = []; const before = { ...need, unlocks: { proposalId: ID, step: "review" as const, beforeMicros: true as const } }, after = { ...before, workKey: advances ? progressKey : before.workKey }, persisted = withRun({ current_phase: "keyword_discovery", progress: { plan: { units: ["replenish_ready"] }, evidenceOwed: [before] } }), steps: Partial<ResearchCycleSteps> = { ...healthySteps([]), currentBasis: async () => "b1", dueWork: async () => ({ ...SOMETHING_DUE, due: ["replenish_ready"] }), acquireEvidence: live.acquireEvidence, replenishReady: async () => ({ ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: [after] }) }; await run(steps); expect([S.reads, persisted[0]!.progress.evidenceOwed?.[0]?.workKey, persisted[0]!.progress.evidenceOwed?.[0]?.boughtOn ?? null], "a free same-drive replay cannot stamp the remaining identity as already purchased").toEqual([1, after.workKey, advances ? null : reportingDay(NOW)]); S.load = advances ? partialOwner : base; S.row = full; S.reads = 0; S.saved = []; await run({ ...steps, replenishReady: async () => ({ ready: S.saved.length, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {}, evidenceOwed: S.saved.length ? [] : [after] }) }); expect([S.reads, persisted.at(-1)!.progress.evidenceOwed?.length], "only qualified progress resumes on the next same-day drive; an unchanged incomplete result stays held").toEqual([advances ? 1 : 0, advances ? 0 : 1]); }
    } finally { vi.doUnmock("@/domains/decision/load-proposals"); vi.doUnmock("@/domains/decision/proposal-store"); vi.doUnmock("@/domains/decision/drafted-copy"); vi.resetModules(); } });
  it("carries the day's one ledger from drive to drive, and holds no entry for work nobody started", async () => {
    ...
    expect(rows.at(-1)!.progress?.replenish?.jobs).toEqual(ledger); expect(JSON.stringify(rows.at(-1)!.progress?.replenish)).not.toMatch(/attempted|tried|spent|fingerprint|deferred/); // the four page-keyed lists and the manifest fingerprint are deleted, not renamed
    await drive(ledger);
  ...
  it("keeps the stock owed across scheduler dispatches until five exist, and closes the day only at the target or on a proven exhaustion", async () => {
    ...
    const quiet = { staleSources: async () => 0, checks: async () => ({ ...NO_CHECKS, done: 1, total: 1, answers: 1, due: 0 }), basis: async () => "b1", evidenceVersion: async () => 1,
      ...
      consumedAnalyses: async () => "fp1", factDebt: async () => ({ owed: 0, everChecked: true }), creditHeld: async () => false, readyStock: async () => Q.ready, winnerRows: async () => NO_WINNERS };
