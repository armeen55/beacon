...
const SURF = vi.hoisted(() => [] as string[]); vi.mock("@/app/(shell)/surface-release", () => ({ invalidateCoreSurfaces: async (t: string) => void SURF.push(t), readCustomerSurface: async () => null, isCustomerSurfaceStale: () => false, refreshCustomerSurface: async () => ({}) }));
...
describe("research-run phase truth", () => { it("counts only synced sources as refreshed, and a connector that would not sync records its debt instead of consuming the whole drive", async () => {
...
describe("one working context per drive, and the funded work that outlives it", () => {
  it.each([T, U])("prepares %s's account once, rebuilds only the part its own writes moved, hands the work it never began to the next walk, and finishes it though the first connector failed", async (t) => {
    ...
    expect([new Set(maps).size, maps[0] instanceof Map, prepared, carried, rows[0]!.progress.replenish?.waiting, rows[0]!.status, (rows[0]!.progress.state?.blocker ?? "").includes("could not be refreshed")],
...
describe("research-run partial-success durability + deduped refreshed providers", () => { it("persists the providers that DID sync, never counts a failed one, and counts the later success on the pass that actually gets it", async () => {
    ...
    expect([rows[1]!.status, rows[1]!.progress.sourcesRefreshed, rows[1]!.progress.refreshedProviders, rows[1]!.progress.state?.blocker]).toEqual(["completed", 2, ["google_gsc", "google_ga4"], null]); }); // a debt that cleared leaves no stale claim behind
...
describe("the cycle finishes stored work before it buys exploratory evidence", () => {
  ...
  it("writes a page off only on its own settled receipt, and never on a blocked, unreached or unreadable pass", async () => {
    ...
    try {
      ...
      expect([Object.keys(mem.jobs), M.asked.length, M.asked[0]!.produce, M.asked[0]!.bypassCache, "maxDrafts" in M.asked[0]!], "P3: A JOB NOBODY STARTED IS UNSPENT. Four of the five were funded and never reached, at zero calls, so the ledger holds only the one really attempted and the other four are owed at their own rank next drive. And no count gates the buy: ONE call is the whole pass, dispatched to produce, and nothing inventory-shaped rides it. A FRESH PAID TAKE IS FOR A RETRY: this day has tried nothing yet, so an ask identical to one already paid for and validated is served from the cache").toEqual([["/a::wc3::e1"], 1, true, false, false]);
