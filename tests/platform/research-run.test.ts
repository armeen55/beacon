...
describe("the cycle finishes stored work before it buys exploratory evidence", () => {
  ...
  it("writes a page off only on its own settled receipt, and never on a blocked, unreached or unreadable pass", async () => {
    ...
    vi.doMock("@/domains/decision", () => ({ resolveCurrentBasis: async () => "b", stockOf: (rows: unknown[]) => rows.length, loadProposalQueue: async () => (M.ready < 0 ? Promise.reject(new Error("queue unreadable")) : { ready: Array.from({ length: M.ready }, () => ({})) }),
      produceProposalsForTenant: async (_t: string, o: { maxDrafts?: number; produce?: boolean; bypassCache?: boolean; memory?: Readonly<Record<string, { calls: number; last: string; settled: boolean }>> }) => { if (M.throws) throw new Error("the provider fell over");
    ...
    try {
      ...
      expect([Object.keys(mem.jobs), M.asked.length, M.asked[0]!.produce, M.asked[0]!.bypassCache, "maxDrafts" in M.asked[0]!], "P3: A JOB NOBODY STARTED IS UNSPENT. Four of the five were funded and never reached, at zero calls, so the ledger holds only the one really attempted and the other four are owed at their own rank next drive. And no count gates the buy: ONE call is the whole pass, dispatched to produce, and nothing inventory-shaped rides it").toEqual([["/a::wc3::e1"], 1, true, true, false]);
  ...
  /** P1, P2, P12. A FACT BANKED IN THIS DRIVE HIRES ITS WRITER IN THIS DRIVE. The walk runs once, at the first stock phase, and the fact check is a LATER phase, so a proposition researched at 02:29 could reach the writer no earlier than the next invocation, by which time the day had written its key off (live, cobra and jersey, 2026-09-03). */ it("walks again in the same drive when a fact lands for funded work, resumes exactly that work when the box ends first, and never lets one account's ledger reach another", async () => {
    ...
    const walks: Array<Record<string, unknown>> = [], bought: string[] = [], spent: number[] = []; let round = 0, shut2 = 0; const walk = (reason: "retryable_blocked" | "candidates_exhausted"): ResearchCycleSteps["replenishReady"] => async (_t, _n, was) => (walks.push({ ...(was?.jobs ?? {}) }), spent.push(was?.callsSpent ?? 0), round += 1, { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason, closedUnder: "basis_test::v", jobs: { ...owed, [`r${round}`]: { calls: round, last: "review_saved", settled: false } }, evidenceOwed: [need], outcomes: { readySaved: 0, evidenceBanked: 0, refused: 0, blocked: 1, unreached: 0, stuck: [], receipts: [{ providerCalls: 7 }] } });
    const steps = (reason: "retryable_blocked" | "candidates_exhausted", fact: ResearchCycleSteps["factCheck"]): Partial<ResearchCycleSteps> => ({ ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["check_page_facts"] }), acquireEvidence: async (_t, n) => (bought.push(n.query), { acquired: true, detail: "bought" }), replenishReady: walk(reason), factCheck: fact });
    let rows = seed(); await run(steps("retryable_blocked", async () => ({ status: "advanced" as const, banked: 1, bankedPages: [WOLF], pagesComplete: 0 })));
    expect([walks.length, Object.keys(walks[1] ?? {}).includes("r1"), bought, rows.at(-1)!.progress?.replenish?.awakened, spent], "P1: TWO walks in one drive with no day rollover, the second reading the ledger the first wrote, no reading re-bought, and the awakened list cleared by the walk that served it. R5: a walk is a PAID drafting pass, so the second is handed what the first metered and draws on the remainder of ONE ceiling for the drive, never a fresh one").toEqual([2, true, [], undefined, [0, 7]]);
    rows = seed(); walks.length = 0; let t = NOW; // P2. THE BOX ENDS BEFORE THE AWAKENED WALK: the exhaustion the first walk earned was reached before this evidence existed, so it is dropped, the woken work is persisted by name, and the next invocation walks it without acquiring anything again
    await runResearchCycle(T, { now: () => new Date(t), deadlineMs: 260_000, steps: { ...BENIGN, ...steps("candidates_exhausted", async () => { t += 210_000; return { status: "advanced" as const, banked: 1, bankedPages: [WOLF], pagesComplete: 0 }; }) } });
    const parked = rows.at(-1)!.progress; expect([walks.length, parked?.replenish?.awakened, parked?.replenish?.closed, (parked?.state?.blocker ?? "").includes("for 1 change already funded today, and this pass ran out of time")], "P2: one walk, the exhaustion withdrawn, the exact work persisted by ONE name (R6: a reading key and the workKey that opens with it are one funding slot, and the sentence the operator reads counts it once), and the stop reason on the row in their own words").toEqual([1, [WOLF], undefined, true]);
    walks.length = 0; await run(steps("retryable_blocked", async () => ({ status: "done" as const, banked: 0, bankedPages: [], pagesComplete: 1 })));
    expect([walks.length, bought, rows.at(-1)!.progress?.replenish?.awakened], "and the next invocation's first walk funds exactly that work, buys no reading again, and clears the list").toEqual([1, [], undefined]);
    ...
    shut(); await run({ ...steps("retryable_blocked", async () => ({ status: "done" as const, banked: 0, bankedPages: [], pagesComplete: 1 })), researchOwed: async () => [WOLF] }); shut2 = walks.length; shut(); await run({ ...steps("retryable_blocked", async () => ({ status: "advanced" as const, banked: 1, bankedPages: [WOLF], pagesComplete: 0 })), researchOwed: async (_t, p) => (p.includes(WOLF) ? [WOLF] : []) });
    const { dueWork: dw } = await import("@/domains/runtime/ops/due-work"), back = rows.at(-1)!.progress, owedNow = (await dw(T, new Date(NOW), { staleSources: async () => 0, checks: async () => ({ ...NO_CHECKS, done: 1, total: 1, answers: 1, due: 0 }), basis: async () => "basis_test", evidenceVersion: async () => null, answersToAnalyze: async () => false, analysisFingerprint: async () => "f", consumedAnalyses: async () => "f", surfaceStale: async () => false, debt: async () => ({ measurable: 0, unverified: 0 }), pagesToCrawl: async () => false, factDebt: async () => ({ owed: 0, everChecked: true }), readyStock: async () => 0, creditHeld: async () => false, run: async () => ({ open: false, progress: back! }) })).due.includes("replenish_ready");
    ...
    walks.length = 0; rows.push(mk({ tenant_id: U, cycle_key: ckey(U, NOW) })); // the first account row, ledger and owed reading all still stand in the same store
    await runResearchCycle(U, { now: () => new Date(NOW), steps: { ...BENIGN, ...healthySteps([]), dueWork: async () => ({ ...SOMETHING_DUE, due: ["check_page_facts"] }), acquireEvidence: async (_t, n) => (bought.push(n.query), { acquired: true, detail: "bought" }), factCheck: async () => ({ status: "done" as const, banked: 0, bankedPages: [], pagesComplete: 0 }),
      replenishReady: async (_t, _n, was) => (walks.push({ ...(was?.jobs ?? {}) }), { ready: 0, deficit: 5, persisted: 0, satisfied: false, reason: "retryable_blocked" as const, jobs: {} }) } });
