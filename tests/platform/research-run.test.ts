...
describe("the canonical run order is the RUNTIME order", () => { it("walks fact_check ahead of every paid phase, so a type-union edit alone can never move it", async () => {
  ...
  /** WHAT A DRIVE BUYS AHEAD OF ITS WALK IS EVERY OWED READING ITS OWN DEADLINE CAN PAY FOR (operator's correction, section 4, 2026-09-06). The rule this replaces took the contiguous head of
   *  the ranking and at most one reading across a gap, so three body rows owing their own results page at ranks 58, 63 and 66 were deferred on every drive and the comparison waiting on them
   *  never began. What still bounds the loop is the drive's own clock: a reading is never started on time the walk needs to begin its first job, and a drive that runs out says so on the row. */
  it.each([T, U])("buys every owed reading ahead of %s's walk that the deadline can pay for, and never out of the box the walk needs to begin a job", async (t) => {
    ...
    expect([await drive(200_000, [need("/one", 1, true), need("/two", 15), need("/three", 145)]), await drive(260_000, [need("/a", 1), need("/b", 2), need("/c", 9)]), await drive(125_000, [need("/a", 1)])],
      "a head already served this morning no longer holds the readings behind it back, so both are bought on the one drive and neither waits a half hour for its turn; three owed readings at the head are three purchases and not one; and a reading is never started on time the walk needs to begin, so a 125-second box begins its work and the reading takes the next drive, saying so on the row").toEqual([
      { bought: ["q/two", "q/three"], deferred: [], stopBy: 75_000, noRoom: false },
      { bought: ["q/a", "q/b", "q/c"], deferred: [], stopBy: 135_000, noRoom: false },
