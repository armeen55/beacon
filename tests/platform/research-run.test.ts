...
describe("the canonical run order is the RUNTIME order", () => { it("walks fact_check ahead of every paid phase, so a type-union edit alone can never move it", async () => {
  ...
  it.each([T, U])("orders %s's purchases by the work each one unlocks, then the row's rank, then what the reading costs", async (t) => {
    ...
    const owed = [
      ...
      { key: "/solo", kind: "serp" as const, query: "solo-last", rank: 90, reasonCode: "no_exact_serp", reason: "owed", workKey: "/solo::wk" }];
    const rows = freshRepo(); rows.push(mk({ tenant_id: t, cycle_key: ckey(t, NOW), current_phase: "fact_check", progress: { plan: { units: ["replenish_ready", "check_page_facts"] }, evidenceOwed: owed,
...
describe("research-run Today copy", () => {
  ...
  it("says whether research is alive at all: what the last pass produced, a day that owed nothing, and a silence with the press that ends it", () => {
    ...
    expect([seen({ progress: { funnel: { answersAnalyzed: 34 } } }), seen({}), seen({}, NOON_PT + 3 * DAY), RR.projectStatusView(null, NOON_PT).liveness]).toEqual([{ state: "productive", line: "Checked 34 new AI answers for mentions of this site today at 12:00 PM." }, { state: "quiet", line: "Checked today at 12:00 PM. Nothing new was owed." }, { state: "silent", line: "No research has run since Thursday. Open Today and press Update data." }, { state: "silent", line: "No research has run for this account yet. Open Today and press Update data." }]); }); // a count of checks can never say this: a quiet day and a week of silence both rendered as nothing at all
});
