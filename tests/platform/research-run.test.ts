...
describe("the cycle finishes stored work before it buys exploratory evidence", () => {
  ...
  /** P1, P2, P12. A FACT BANKED IN THIS DRIVE HIRES ITS WRITER IN THIS DRIVE. The walk runs once, at the first stock phase, and the fact check is a LATER phase, so a proposition researched at 02:29 could reach the writer no earlier than the next invocation, by which time the day had written its key off (live, cobra and jersey, 2026-09-03). */ it("walks again in the same drive when a fact lands for funded work, resumes exactly that work when the box ends first, and never lets one account's ledger reach another", async () => {
    ...
    expect([order, walks.length, bought, rows.at(-1)!.progress?.replenish?.awakened, spent], "P1: EVIDENCE BEFORE DRAFTING. The units run first inside their own box, the drive's ONE walk then hires the dependent writer at its own rank in the same drive with no rollover and no second walk, and no reading is re-bought. R5: a walk is a PAID drafting pass, so it draws on the remainder of ONE ceiling for the drive").toEqual([["facts", "walk"], 1, [], undefined, [0]]);
    ...
    expect([order, walks.length], "and a fact check that banks nothing still leaves the walk its turn: the stock is never held hostage to a check with nothing to bank").toEqual([["facts", "walk"], 1]);
    ...
    expect([order.slice(0, 2), rows.at(-1)!.progress?.replenish?.awakened], "and the next invocation puts its units first and then walks the work that was woken, which clears the list; whatever that walk re-mints is bought and redrafted behind it, exactly as any other reading is").toEqual([["facts", "walk"], undefined]);
