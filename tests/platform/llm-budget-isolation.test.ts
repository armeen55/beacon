/**
 * Per-account LLM budget isolation (closure, 2026-07-24).
 *
 * The promise: one account's LLM spend can never change another account's remaining budget or cap it, on EITHER layer (per-account file backstop + per-account durable ledger), and no budget operation runs without an explicit account.
 */
...
describe("per-account LLM budget isolation", () => {
  ...
  it("same-account max(file, durable) and the exact-cap boundary are unchanged", async () => {
    ...
    DURABLE.set(A, 30); // durable now reports AT cap for A
