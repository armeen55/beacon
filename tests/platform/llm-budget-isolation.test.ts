 ...
 *
 * The promise: one account's LLM spend can never change another account's
 * remaining budget or cap it, on EITHER layer (per-account file backstop +
 * per-account durable ledger), and no budget operation runs without an
 * explicit account.
 */
