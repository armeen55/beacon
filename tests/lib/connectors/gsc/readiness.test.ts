...
describe("loadGscReadiness — soft-fail (never throws)", () => {
  ...
  it("generic read error + connected → connected_no_data (degrades coherently)", async () => {
    _info = connectedInfo();
...
describe("describeGscReadiness — presenter copy", () => {
  ...
  it("needs_reconnect → reconnect prompt", () => {
  ...
  it("not_connected → minimal connect prompt, idle tone, no numbers", () => {
    const d = describeGscReadiness(readiness({ verdict: "not_connected" }));
    expect(d.headline).toBe("Not connected");
    expect(d.detail).toBe(
  ...
  it("ready WITHOUT coverage (defensive) → no fabricated span, falls back to honest copy", () => {
    const d = describeGscReadiness(
      readiness({ verdict: "ready", property: null, coverage: null }),
    );
    expect(d.headline).toBe("Connected and pulling search data");
    expect(d.detail).toBe("Search data is up to date.");
    // No fabricated dates/counts.
