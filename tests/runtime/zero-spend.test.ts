...
describe("the funded proof can touch exactly one stored candidate", () => {
  ...
  const drive = async (readbackStatus: string, startStatus = "needs_review", permissions = ["paused"], saveResult = "saved") => { const events: string[] = []; let stored = { ...row, status: startStatus }, pi = 0;
    const deps = { permission: async () => permissions[Math.min(pi++, permissions.length - 1)], load: async (_t: string, asked: string) => (events.push(`load:${asked}`), stored), version: () => "v1",
      providerConfigured: () => true, delivery: () => "existing_page_edit", obligation: () => ({ kind: "review" }),
  ...
  it("persists then rereads the same stable id and trusts only the stored Ready row", async () => { const good = await drive("ready"), bad = await drive("needs_review"), already = await drive("ready", "ready"), changed = await drive("ready", "needs_review", ["paused"], "blocked"); expect([good.out.success, bad.out.success, already.out.success, changed.out.success, changed.events.at(-1), good.events, already.events]).toEqual([true, false, false, false, "reconcile:0:false", [`load:${id}`, "admit", "claim", `review:${id}`, `save:${id}:true`, `load:${id}`, "reconcile:0:true"], [`load:${id}`]]);});
  it("admits one concurrent press and spends nothing after research resumes", async () => { const resumed = await drive("ready", "needs_review", ["paused", "running"]); expect([resumed.out.success, resumed.events.includes("review:" + id), resumed.events.at(-1)]).toEqual([false, false, "release"]);
    ...
    const base = { permission: async () => "paused", load: async () => row, version: () => "v1", providerConfigured: () => true, delivery: () => "existing_page_edit", obligation: () => ({ kind: "review" }), acceptable: () => true,
