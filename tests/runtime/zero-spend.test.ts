...
describe("the funded proof can touch exactly one stored candidate", () => {
  const id = "tenant-fx::/one::existing_edit::answer", stale = "it did not pass the re-read of a stored change against the rules that stand today: the reading on file was made under an older review contract, so it is read again before these words are offered", row = { id, tenantId: "tenant-fx", status: "needs_review", researchOnly: false, impactScore: 7, obligation: { kind: "review" }, faults: [stale], limitations: [stale] };
  const drive = async (readbackStatus: string, startStatus = "needs_review", permissions = ["paused"], saveResult = "saved", authorized = false) => { const events: string[] = []; let stored = { ...row, status: startStatus }, pi = 0;
    const deps = { permission: async () => permissions[Math.min(pi++, permissions.length - 1)], load: async (_t: string, asked: string) => (events.push(`load:${asked}`), stored), version: () => "v1",
      providerConfigured: () => true, reviewAuthorized: () => authorized, delivery: () => "existing_page_edit", obligation: () => ({ kind: "review" }),
  ...
  it("admits one concurrent press and spends nothing after research resumes", async () => { const resumed = await drive("ready", "needs_review", ["paused", "running"]); expect([resumed.out.success, resumed.events.includes("review:" + id), resumed.events.at(-1)]).toEqual([false, false, "release"]);
    ...
    const base = { permission: async () => "paused", load: async () => row, version: () => "v1", providerConfigured: () => true, reviewAuthorized: () => false, delivery: () => "existing_page_edit", obligation: () => ({ kind: "review" }), acceptable: () => true,
      ...
      review: async () => (events.push("review"), await wait, { row: { ...row, status: "needs_review" }, detail: "accepted" }), save: async () => "saved" };
    const proof = (await import("@/domains/runtime/ops/atomic-proof")).default, args = { tenantId: "tenant-fx", proposalId: id, maxOpenAiCalls: 1, maxOpenAiUsd: 0.05 }, first = proof.run(args, base as never);
    const noKey = await proof.run(args, { ...base, providerConfigured: () => false, spend: { ...base.spend, reserve: async () => { throw new Error("admission reached"); } } } as never); expect(noKey.reason).toBe("openai_not_configured_in_this_runtime");
