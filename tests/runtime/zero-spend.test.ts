...
describe("the funded proof can touch exactly one stored candidate", () => {
  ...
  const drive = async (readbackStatus: string, startStatus = "needs_review", permissions = ["paused"], saveResult = "saved", authorized = false, reviewRefresh = false) => { const events: string[] = []; let stored = { ...row, status: startStatus }, pi = 0;
    const deps = { permission: async () => permissions[Math.min(pi++, permissions.length - 1)], load: async (_t: string, asked: string) => (events.push(`load:${asked}`), stored), version: () => "v1",
      providerConfigured: () => true, reviewAuthorized: () => authorized, delivery: () => "existing_page_edit", obligation: () => ({ kind: "review" }), preflight: async () => ({ reason: null, reviewRefresh, captures: [] }),
    ...
    return { out: await proof.run({ tenantId: "tenant-fx", proposalId: id, currentBasis: "basis-current::d9", maxOpenAiCalls: 1, maxOpenAiUsd: 0.05 }, deps as never), events }; };
  it("persists then rereads the same stable id and trusts only the stored Ready row", async () => { const good = await drive("ready"), bad = await drive("needs_review"), already = await drive("ready", "ready"), changed = await drive("ready", "needs_review", ["paused"], "blocked"), free = await drive("ready", "needs_review", ["paused"], "saved", true), refreshed = await drive("ready", "needs_review", ["paused"], "saved", true, true); expect([good.out.success, bad.out.success, already.out.success, changed.out.success, changed.events.at(-1), good.events, already.events, free.out.success, free.out.meter, free.events, refreshed.out.success, refreshed.events.includes(`review:${id}`), refreshed.events.includes(`promote:${id}:reviewed`)]).toEqual([true, false, false, false, "reconcile:0:false", [`load:${id}`, "admit", "claim", `review:${id}`, `promote:${id}:reviewed`, `load:${id}`, "reconcile:0:true"], [`load:${id}`], true, { ops: 0, providerCalls: 0, costUsd: 0 }, [`load:${id}`, `promote:${id}:stored`, `load:${id}`], true, true, true]);});
  it("admits one concurrent press and spends nothing after research resumes", async () => { const resumed = await drive("ready", "needs_review", ["paused", "running"]); expect([resumed.out.success, resumed.events.includes("review:" + id), resumed.events.at(-1)]).toEqual([false, false, "release"]);
    ...
    const base = { permission: async () => "paused", load: async () => row, version: () => "v1", providerConfigured: () => true, reviewAuthorized: () => false, delivery: () => "existing_page_edit", obligation: () => ({ kind: "review" }), preflight: async () => ({ reason: null, reviewRefresh: false, captures: [] }), acceptable: () => true,
    ...
    const proof = (await import("@/domains/runtime/ops/atomic-proof")).default, args = { tenantId: "tenant-fx", proposalId: id, currentBasis: "basis-current::d9", maxOpenAiCalls: 1, maxOpenAiUsd: 0.05 }, first = proof.run(args, base as never);
    const preflight = await proof.run(args, { ...base, preflight: async () => ({ reason: "This change states a fact with no source behind it.", reviewRefresh: false, captures: [] }), spend: { ...base.spend, reserve: async () => { throw new Error("admission reached"); } }, review: async () => { throw new Error("review reached"); } } as never); expect([preflight.success, preflight.reason]).toEqual([false, "candidate_preflight:This change states a fact with no source behind it."]);
