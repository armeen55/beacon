...
describe("one paused page proof uses canonical production without renewing its budget", () => {
  it.each(["ready", "meta_ready", "link_ready", "link_review_failed", "link_unowned", "link_capture_failed", "link_readback_failed", "link_wrong_port", "link_query", "wrong_port", "readback_wrong_url", "readback_wrong_final", "authorized", "authorized_used", "malformed_authorization", "provider_missing", "old_ready", "foreign", "wrong_site", "capture_failed", "no_wire", "ambiguous", "unqualified", "admission_used", "boxed", "unreadable", "failed_gsc", "failed_pages", "failed_snapshot", "preferred_missing", "exact_owed", "producer_unreadable", "source_ready", "source_no_url", "source_unused", "source_failed", "source_mismatch", "source_foreign", "source_deadline", "source_repeat", "source_paused", "source_providerheld", "source_work_changed"])("reports %s from durable readback and keeps paid admission", async (outcome) => {
    ...
    let paid = 0, clock = Date.now(), acquired = 0, admitted = false, reviewed = false, promoted = false; const captured = new Map<string, { url: string; finalUrl: string; version: "current"; completeness: "complete"; contentHash: string; fetchedAt: string }>(); if (outcome === "producer_unreadable") captured.set(url, { url, finalUrl: url, version: "current", completeness: "complete", contentHash: "current-hash", fetchedAt: new Date(clock).toISOString() }); if (source) candidate.obligation = { kind: "evidence", need: { ...need, ...(outcome === "source_mismatch" ? { missingTopic: "different claim" } : {}) } };
    ...
    const produce = vi.fn<NonNullable<Deps["produce"]>>(async (_t, options = {}) => {
      ...
      if (!["preferred_missing", "exact_owed", "producer_unreadable"].includes(outcome)) expect(PROOF_SPEND.authorize(tenantId, "model", 0.1)).toBe(false); if (outcome === "producer_unreadable") return { ...produced([]), outcome: "evidence_unreadable" };
      if (source) {
    ...
    } else if (["no_wire", "preferred_missing", "exact_owed", "producer_unreadable"].includes(outcome)) {
