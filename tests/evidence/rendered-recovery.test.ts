...
describe("rendered recovery follows unresolved page identities", () => {
  ...
  it("replays a newer banked rendered task through an active source hold and clears it only after readback", async () => {
    ...
    let body = { version: "current", completeness: "partial", contentHash: raw.content_hash, vocabulary: "Loading names.", fetchedAt: current, finalUrl: unread, captureStates: [{ ...raw, id: "raw" }], latestCaptureId: "raw" } as Record<string, unknown>, ready = false;
    const called: string[] = [], writes: string[] = [], fetched: string[] = [], deps = { now: () => now, loadState: async () => ({ state, rowVersion: 1 }), saveState: async () => 2, getAccount: async () => ({ domain: "fixture.example" }), loadProfile: async () => null,
      readOwnedBodies: async () => new Map([[canonicalUrlKey(unread), body]]), readIncompleteAttempt: async () => null, fetchPage: async (url: string) => { fetched.push(url); return { ok: true, html: shell, status: 200, finalUrl: unread }; }, writeOwnedPage: async (snap: typeof raw) => { writes.push(snap.content_hash); body = { ...body, contentHash: snap.content_hash, completeness: snap.content_capture?.complete ? "complete" : "partial", fetchedAt: snap.fetched_at, finalUrl: snap.final_url, captureStates: [{ ...snap }], latestCaptureId: snap.id }; },
    ...
    const read = winningPagesUnit(deps, [], null, unread, null, null, true), cursor = { basis: "basis", runId: "run" }; expect((await read(tenant, { ...cursor, freeOnly: true }, 90_000)).status).toBe("failed"); expect([called, fetched, writes]).toEqual([[], [], []]); expect((await read(tenant, cursor, 90_000)).status).toBe("failed"); expect([called, (state as { ownedReads: unknown[] }).ownedReads]).toEqual([[held.attemptedAt], [held]]);
    ready = true; expect((await read(tenant, cursor, 90_000)).status).toBe("done"); expect([called, fetched, (state as { ownedReads: unknown[] }).ownedReads, body.completeness, writes.includes(rendered.content_hash)]).toEqual([[held.attemptedAt, held.attemptedAt], [], [], "complete", true]);
  });
  it("recovers a same-hash complete DOM from legacy text even after a false partial, but not from a partial alone", async () => {
    const url = "https://fixture.example/samanid", key = canonicalUrlKey(url), html = `<main><h1>Samanid Empire Flag</h1><p>${"The Samanid banner followed the Abbasid black standard. ".repeat(16)}</p></main>`, fresh = extractPageSnapshot(html, url, pageIdFor(key), tenant, 200, undefined, url), clock = Date.now();
    const legacy = { ...fresh, id: "legacy", fetched_at: new Date(clock - 10 * 86_400_000).toISOString(), content_capture: null }, poisoned = { ...fresh, id: "false-partial", fetched_at: new Date(clock - 60_000).toISOString(), extraction_certainty: "uncertain", content_capture: { ...fresh.content_capture!, complete: false } }; let rows: Row[] = [poisoned as Row, legacy as Row], writes = 0, htmlNow = html; const sb = supabaseFake({ rows: (name) => name === "page_snapshots" ? rows : [] }); vi.resetModules(); vi.doMock("@/lib/persistence/supabase", () => ({ getSupabaseAdmin: () => sb }));
    const { loadOwnedPageBodies } = await import("@/domains/evidence/pages/owned-context"), { winningPagesUnit } = await import("@/domains/evidence/funnel/winning-pages"), state = { ownedReads: [], ledger: { spentUsd: 0, cacheHits: 0 }, cycle: { runId: null, cycleKey: null, spentUsd: 0, cacheHits: 0 } } as never; const deps = { now: () => clock, loadState: async () => ({ state, rowVersion: 1 }), saveState: async () => 2, getAccount: async () => ({ domain: "fixture.example" }), loadProfile: async () => null, readOwnedBodies: loadOwnedPageBodies, fetchPage: async () => ({ ok: true, html: htmlNow, status: 200, finalUrl: url }), writeOwnedPage: async (snap: Row) => { writes++; rows.unshift(snap); }, callProvider: async () => { throw new Error("a page-source repair may not buy a provider"); } } as unknown as FunnelDeps; const read = winningPagesUnit(deps, [], null, url, null, null, true), cursor = { basis: "basis", freeOnly: true };
    const complete = () => (rows[0]?.content_capture as { complete?: boolean } | undefined)?.complete; expect((await read(tenant, cursor, 90_000)).status).toBe("done"); expect([(await loadOwnedPageBodies(tenant, [url])).get(key)?.completeness, complete(), writes]).toEqual(["complete", true, 1]); rows = [poisoned as Row, legacy as Row]; htmlNow = "<main><h1>Samanid Empire Flag</h1><p>Loading names.</p></main>"; expect((await read(tenant, cursor, 90_000)).status).toBe("failed"); expect([complete(), writes]).toEqual([false, 2]); rows = [poisoned as Row]; htmlNow = html; expect((await read(tenant, cursor, 90_000)).status).toBe("failed"); expect(writes).toBe(2);
    rows = [poisoned as Row, legacy as Row]; htmlNow = `<script src="/changed.js"></script>${html}`; expect((await read(tenant, cursor, 90_000)).status).toBe("failed"); expect([complete(), writes]).toEqual([false, 3]); rows = [poisoned as Row, legacy as Row]; htmlNow = html.replace("<p>", "<section><p>").replace("</p>", "</p></section>"); expect((await read(tenant, cursor, 90_000)).status).toBe("failed"); expect([complete(), writes]).toEqual([false, 4]);
    const rich = extractPageSnapshot(`<main><h1>Samanid Empire Flag</h1><p>${"A separate confirmed richer body still lives on this page. ".repeat(60)}</p></main>`, url, pageIdFor(key), tenant, 200, undefined, url); rows = [poisoned as Row, { ...rich, fetched_at: new Date(clock - 120_000).toISOString() } as Row, legacy as Row]; htmlNow = html; expect((await read(tenant, cursor, 90_000)).status).toBe("failed"); expect([complete(), writes]).toEqual([false, 4]); });
  it("stops an unchanged partial shell after cache expiry, reopens on an observed asset change, and holds a failed marker read", async () => {
    const url = unread, key = canonicalUrlKey(url), page = pageIdFor(key), shell = "<main><h1>Names</h1><p>Loading names.</p></main>", raw = (asset: string) => extractPageSnapshot(`<script src="/${asset}.js"></script>${shell}`, url, page, tenant), first = raw("v1"), changed = raw("v2"), stored: Row[] = []; let body = { version: "current", completeness: "partial", contentHash: first.content_hash, vocabulary: "Loading names.", fetchedAt: current, finalUrl: url, captureStates: [{ ...first }], latestCaptureId: first.id }, failed = false, calls = 0, clock = now;
    const sb = supabaseFake({ rows: (table) => table === "page_snapshots" ? stored : [], error: (table, op) => failed && table === "page_snapshots" && op === "select" ? { message: "read failed" } : null }); vi.resetModules(); vi.doMock("@/lib/persistence/supabase", () => ({ getSupabaseAdmin: () => sb })); const { renderUnreadOwnedPages } = await import("@/domains/evidence/pages/rendered-read"), { winningPagesUnit } = await import("@/domains/evidence/funnel/winning-pages");
    const state = { ownedReads: [], ledger: { spentUsd: 0, cacheHits: 0 }, cycle: { runId: null, cycleKey: null, spentUsd: 0, cacheHits: 0 } } as never; const deps = { now: () => clock, loadState: async () => ({ state, rowVersion: 1 }), saveState: async () => 2, getAccount: async () => ({ domain: "fixture.example" }), loadProfile: async () => null,
    ...
    let held = 0; const read = winningPagesUnit(deps, [], null, url, null, null, true), cursor = { basis: "basis", runId: "run" };
    ...
    const bustedAt = new Date(clock).toISOString(); failed = false; expect(await renderUnreadOwnedPages(tenant, 1, { url, rawSnapshot: first, bustedAt, deps, deadline: clock + 90_000 })).toBe(0); expect(calls).toBe(2);
  ...
  it("passes a cold loading shell to rendered work, then renews an identical trusted page free behind 2,001 snapshots", async () => {
    const snapshots = Array.from({ length: 2001 }, (_, i) => row(`new-${i}`, "page-a", recent, new Date(now - i * 1000).toISOString()));
    ...
    expect((await loadOwnedPageBodies(tenant, [unread])).has(canonicalUrlKey(unread))).toBe(false);
    ...
    const redirected = { ...trusted, final_url: `${unread}?preview=1` }; expect(await renderUnreadOwnedPages(tenant, 1, { url: unread, rawSnapshot: redirected, bustedAt: new Date(now + 1).toISOString(), deps, deadline: now + 90_000 })).toBe(0); expect(provider).toHaveLength(2);
  ...
  it("reaches URL 61 past 60 partial rendered failures, then reads back its hold without buying a provider call", async () => {
    ...
    const snaps = urls.slice(0, 60).map((url, i) => ({ ...row(`partial-${i}`, `page-${i}`, url, current), content_capture: { version: 1, complete: false, mainHtml: "<main><p>Partial copy.</p></main>", jsonLd: [] }, structural_warnings: ["rendered_read: partial capture"] }));
    const sb = supabaseFake({ rows: (name) => name === "owned_pages" ? rows : name === "page_snapshots" ? snaps : [] });
    vi.doMock("@/lib/persistence/supabase", () => ({ getSupabaseAdmin: () => sb })); const { renderUnreadOwnedPages } = await import("@/domains/evidence/pages/rendered-read");
    let clock = now;
