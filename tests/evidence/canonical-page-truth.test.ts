...
describe("one rule decides which capture is the page", () => {
  it("retains a populated Wix roster during prehydration without admitting hidden lists", async () => {
    const url = "https://iranopedia.com/famous-iranian-singers", card = (name: string) => `<div class="wixui-repeater__item"><h2>${name}</h2><p>${name} recorded influential Persian songs and performed for audiences worldwide.</p></div>`;
    const list = (style: string, items: number, cards: string) => `<div class="wixui-repeater"><fluid-columns-repeater role="list" items="${items}" style="${style}">${cards}</fluid-columns-repeater></div>`;
  ...
  it("carries same-sized material revisions from extraction through the stored projection into job identity, without clock churn", async () => {
    ...
    for (const repeats of [1500, 6000]) {
      db.rows = [capture(html.replace("When do seals rest?", "When do seals rest? ".repeat(30).trim()).replace("They rest at low tide. ".repeat(15), "They rest at low tide. ".repeat(repeats)))]; const wide = (await loadOwnedPageBodies("t", [url])).get("fixture-revision.example/page")!;
      expect([wide.faqs[0]!.answerComplete, wide.faqs[0]!.answer, wide.completeness, wide.faqs[0]!.question]).toEqual([repeats === 1500, repeats === 1500 ? `${"They rest at low tide. ".repeat(repeats)}June.` : before.faqs[0]!.answer_excerpt, "partial", "When do seals rest? ".repeat(30).trim()]); expect([wide.title ?? "", wide.metaDescription ?? "", ...wide.headings, ...wide.cardTexts, ...wide.entityNames].join(" ").length + wide.faqs.reduce((n, f) => n + f.question.length + f.answer.length, 0) + wide.passages.reduce((n, p) => n + p.length, 0)).toBeLessThanOrEqual(48000); }
    ...
    const winner = (rank: number, words = 100) => ({ url: `https://publisher-${rank}.example/seals`, domain: `publisher-${rank}.example`, engines: [], examplePrompts: [], appearances: [{ kind: "serp_organic", query: "harbour seals", promptId: null, promptText: null, engine: null, rank, citedUrl: `https://publisher-${rank}.example/seals`, observedAt: "2026-09-10T00:00:00Z", modelServed: null }], extract: { title: `Seals ${rank}`, h1: `Seals ${rank}`, wordCount: words, headings: ["Habitat"], mainText: "Harbour seals rest near shore." } });
    const withWinners = (rows: unknown[]) => jobEvidenceHash({ ...base, research: { ...base.research, winningPages: rows } } as never, [url], "harbour seals"), five = [1, 2, 3, 4, 5].map((rank) => winner(rank));
    expect(withWinners([...five, winner(6)]), "a sixth publisher the writer never receives cannot reopen finished work").toBe(withWinners(five));
  ...
  it("the targeted reader hands the writer the stale body with its dates, and a stale body can never prove a current absence", async () => {
    ...
    expect([flag.version, flag.completeness, pageContains(flag, "Takbir"), pageContains(flag, "Pahlavi")]).toEqual(["current", "complete", "yes", "no"]); const rug = "https://iranopedia.com/persian-rugs", words = (label: string, count: number) => Array.from({ length: count }, (_, i) => `${label}${i}`).join(" "); db.rows = [row(rug, "2026-09-21T12:00:00Z", words("thin", 66), 66, "confirmed"), row(rug, "2026-09-20T12:00:00Z", words("rich", 124), 124, "confirmed")]; const collapsed = (await loadOwnedPageBodies("t", [rug])).get("iranopedia.com/persian-rugs")!;
    expect([collapsed.version, collapsed.fetchedAt?.slice(0, 10), collapsed.newestAt?.slice(0, 10), pageContains(collapsed, "rich80"), pageContains(collapsed, "thin20"), collapsed.heldNote.includes("sharply less content")], "one implausible content collapse preserves the preceding body for presence and proves no current absence").toEqual(["stale_known_good", "2026-09-20", "2026-09-21", "yes", "unknown", true]); db.rows.unshift({ ...row(rug, "2026-09-22T12:00:00Z", words("thin", 65), 65, "confirmed"), content_hash: "h" }); const confirmedThin = (await loadOwnedPageBodies("t", [rug])).get("iranopedia.com/persian-rugs")!; expect([confirmedThin.version, confirmedThin.fetchedAt?.slice(0, 10), pageContains(confirmedThin, "rich80"), pageContains(confirmedThin, "thin20")], "a second capture with the same content identity confirms a real deletion").toEqual(["current", "2026-09-22", "no", "yes"]); expect(confirmedThin.captureStates?.map((r) => [r.fetched_at, r.body_text])).toEqual(db.rows.map((r) => [r.fetched_at, r.body_text])); db.rows[0]!.content_hash = "unrelated"; expect((await loadOwnedPageBodies("t", [rug])).get("iranopedia.com/persian-rugs")!.version, "similar length with different content does not confirm deletion").toBe("stale_known_good"); db.rows = [{ ...row(rug, "2026-09-21T12:00:00Z", words("thin", 66), 66, "confirmed"), id: "new", content_hash: "a" }, { ...row(rug, "2026-09-21T11:59:59Z", words("thin", 67), 67, "confirmed"), id: "recent-b", content_hash: "b" }, { ...row(rug, "2026-09-21T11:59:58Z", words("thin", 68), 68, "confirmed"), id: "recent-c", content_hash: "c" }, ...Array.from({ length: 497 }, (_, i) => ({ ...row(rug, new Date(Date.parse("2026-09-21T11:59:58Z") - (i + 1) * 1000).toISOString(), "", 0, "uncertain"), id: `empty-${i}` })), { ...row(rug, "2026-09-20T12:00:00Z", words("rich", 124), 124, "confirmed"), id: "old" }]; expect((await loadOwnedPageBodies("t", [rug])).get("iranopedia.com/persian-rugs")!.version, "differently hashed thin captures cannot hide the trusted rich predecessor beyond the 500-row boundary").toBe("stale_known_good");
    const hidden = Array.from({ length: 20 }, () => ({ question: "Markup-only question?", answer_excerpt: "Markup-only assertion", source: "jsonld" })); db.rows = [{ ...row(flag.url, "2026-08-29T20:51:00Z", "The flag adopted in 1980 carries the Takbir twenty-two times along the edges of the green and red bands.", 129, "confirmed"), schema_entity_names: ["Markup-only entity"], faqs: [...hidden, { question: "Unanswered heading?", answer_excerpt: "", source: "html_section" }, { question: "Unknown origin?", answer_excerpt: "Legacy assertion" }, { question: "Visible question?", answer_excerpt: "Visible excerpt", source: "html_details" }] }];
