...
describe("a missing proposition is researched, never graded", () => { beforeEach(reset);
  ...
  it("gives the judge a complete answer at the source-window edge, but excludes a sentence too long to fit", async () => { const question = "Why was Allahu Akbar written 22 times on the Iran flag? ", preface = `${question}${"A separate historical note. ".repeat(211)}`, answer = "Allahu Akbar appears 22 times to mark the revolution’s victory on the 22nd day of the 11th Iranian month.", long = `Unrelated observations ${"x".repeat(500)}.`;
    for (const [tail, present] of [[answer, true], [long, false]] as const) { reset(); let grounded = ""; const out = await unit({ held: [row({ statementKey: "why#edge", subject: question, current: "", pageLocator: "missing", note: "Owed again: source support disputed; source changed", sources: [{ url: "https://ajammc.com/flag", kind: "publisher", says: "" }] })], statementKey: "why#edge", scanSourceChanges: false, fetchSource: async () => ({ text: `${preface}${tail}` }), read: async (input: { grounded: string }) => { grounded = input.grounded; return { hold: "capped" as const }; } }); expect([out.failure, db.rows.length, grounded.length <= 6000, grounded.includes("Allahu Akbar appears 22 times"), grounded.includes("Unrelated observations")]).toEqual(["judge_capped", 0, true, present, false]); }
    let giantWindow = ""; await unit({ held: [row({ statementKey: "why#edge", subject: question, current: "", sources: [{ url: "https://ajammc.com/flag", kind: "publisher", says: "" }] })], statementKey: "why#edge", fetchSource: async () => ({ text: `Allahu Akbar 22 times on Iran flag ${"x".repeat(7000)}.` }), read: async (input: { grounded: string }) => { giantWindow = input.grounded; return { hold: "capped" as const }; } }); expect([giantWindow.length, giantWindow.startsWith("Allahu Akbar 22 times")]).toEqual([6000, true]); }); });
