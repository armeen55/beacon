...
describe("backfilling support onto already-banked facts", () => {
  ...
  it("reopens a headword-judged proposition row once, in the account's own words, and passes over one already judged as a proposition", async () => {
    ...
    const back: { why?: string }[] = []; const deps = (rows: FactCheck[]) => ({ rows: async () => rows, rulesVersionFor, rebank: async (_p: string, r: FactCheck[]) => r.length, reopen: async (_p: string, _r: FactCheck[], why?: string) => (back.push({ why }), 1) });
    ...
    const judged = await backfillClaimSupport("t", [{ ...target[0]!, rulesStale: true }], deps([{ ...gapRow(deriveSupport(ctx)!, deriveSupport(other)!), rulesVersion: 5 }])); // the same flag, on a row the paid unit has since re-judged and banked at 5
    expect([moved.map((o) => o.action), back.at(-1)!.why, back.length, rulesVersionFor({ subject: "iran flag before 1979", current: "" }), judged[0]!.action], "THE RULES THAT JUDGE A MISSING ANSWER MOVED: the question is owed again once and says so, even though every artifact under it is current and nothing else would ever re-judge it; a headword row with no current wording and a correction flagged the same way are judged by their SHAPE, not by the flag, and neither is handed back; and the flag is never the last word: the caller computed it from a reading taken before the paid unit ran, so the row is asked its own version here, and one already re-judged at 5 keeps the answer it was just bought").toEqual([["reopened", "banked_unsupported", "banked_unsupported"], "Reopened: the rules that judge a missing answer changed.", 2, 5, "already_current"]); });});
