...
describe("what may authorize replacing published words", () => { beforeEach(reset);
  ...
  it.each(["tenant-one", "tenant-two"])("admits an addition on one publisher that was read and carries it, and keeps a correction of the page's own words at what two agreeing sources earn [%s]", async (tenant) => {
    ...
    expect([authorizedCorrections([add({ confidence: "disputed" })], undefined, tenant).length, authorizedCorrections([add({ confidence: "unsupported" })], undefined, tenant).length], "disputed sources still authorize nothing; an unsupported reading whose source WAS read and proposed an answer authorizes an addition under ship mode (operator, 2026-09-10), with the card saying what backs it").toEqual([0, 1]);
    expect(authorizedCorrections([add({ confidence: "likely", sources: carried("publisher", null) })], undefined, tenant), "under ship mode (operator, 2026-09-10) a read publisher whose passage was not verbatim-verified still admits an ADDITION, with the card naming the single source; a correction still demands the carried quote").toHaveLength(1);
