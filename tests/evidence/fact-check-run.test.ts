...
describe("what may authorize replacing published words", () => { beforeEach(reset);
  ...
  /** EVIDENCE IS PROPORTIONAL TO WHAT THE TREATMENT RISKS (operator, 2026-09-01; the consumer split, 2026-09-05). Held to one bar, the whole missing-answer lane on the acceptance account was dead: the reading landed with its sources against the right page version, it stood at `likely`, and the row owed the identical purchase again every drive for ever. A correction replaces published words and keeps `confirmed`; an addition a reader undoes by deleting it stands on one publisher that was read and whose own passage carries it, which is the section rule the operator already stated. */
  it.each(["tenant-one", "tenant-two"])("admits an addition on one publisher that was read and carries it, and keeps a correction of the page's own words at what two agreeing sources earn [%s]", async (tenant) => {
    ...
    expect(art?.supported, "the publisher's own passage carries the answer, which is what earns it").toBe(true);
    ...
    expect(authorizedCorrections([add({ confidence: "confirmed" })], undefined, tenant), "and confirmed is admitted exactly as before").toHaveLength(1);
    expect([authorizedCorrections([add({ confidence: "disputed" })], undefined, tenant).length, authorizedCorrections([add({ confidence: "unsupported" })], undefined, tenant).length], "disputed and unsupported are what an addition may never stand on").toEqual([0, 0]);
    expect(authorizedCorrections([add({ confidence: "likely", sources: carried("publisher", null) })], undefined, tenant), "an ordinary publisher with no passage shown to carry the answer admits nothing: it is the reading, never the host, that earns this").toHaveLength(0);
