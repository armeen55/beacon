-- publish_customer_release: THE atomic customer release (Codex, 2026-08-23).
-- Ranking stamp and surface blob commit together or not at all; an expected-prior
-- mismatch aborts before either. Additive: no table changed, no data dropped.
create or replace function public.publish_customer_release(
  p_tenant_id text, p_expected_prior text, p_release text,
  p_ids text[], p_lanes text[],
  p_scope_key text, p_store_name text, p_content jsonb
...
begin
  ...
  if p_expected_prior is not null and v_prior is distinct from p_expected_prior then
