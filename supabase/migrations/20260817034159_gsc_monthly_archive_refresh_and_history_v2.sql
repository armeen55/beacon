-- One month's rollup from gsc_daily_rows into gsc_monthly_archive: small enough to always finish inside
-- the statement timeout, idempotent, and the ONLY writer the archive needs. Called per affected month by
-- the sync and backfill paths, so the archive stays current from now on.
...
up as (
  insert into public.gsc_monthly_archive (tenant_id, query, month, impressions, clicks, top_page, updated_at)
  select p_tenant_id, a.query, date_trunc('month', p_month)::date, a.impressions, a.clicks, tp.page, now()
  ...
  do update set impressions = excluded.impressions, clicks = excluded.clicks,
                top_page = excluded.top_page, updated_at = now()
  returning 1
)
select coalesce(count(*), 0)::integer from up;
$$;

-- The unit history, off the ARCHIVE: months instead of 1.2M daily rows, so it always answers inside the
-- timeout. Windows are month-grained; the caller derives day counts from the months actually covered.
...
with rows as (
  select a.query, a.month, a.clicks, a.impressions, a.top_page
  from public.gsc_monthly_archive a where a.tenant_id = p_tenant_id
),
early as (
  select rows.query, sum(rows.clicks) c, sum(rows.impressions) i from rows
  where rows.month >= date_trunc('month', p_early_from)::date and rows.month < date_trunc('month', p_early_to)::date
  group by rows.query
),
recent as (
  select rows.query, sum(rows.clicks) c, sum(rows.impressions) i from rows
  where rows.month >= date_trunc('month', p_recent_from)::date group by rows.query
),
early_pages as (
...
select coalesce(e.query, r.query) as query,
       coalesce(e.c, 0)::bigint, coalesce(e.i, 0)::bigint, null::numeric,
       coalesce(r.c, 0)::bigint, coalesce(r.i, 0)::bigint, null::numeric,
