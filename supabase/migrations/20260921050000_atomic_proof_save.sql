-- The one-candidate proof may write only the exact reviewed row it read, while the account is still paused.
...
begin
  if p_tenant_id is null or p_tenant_id = '' or nullif(p_row->>'id', '') is null
     ...
     or p_expected_status <> 'needs_review' then return 'failed'; end if;
