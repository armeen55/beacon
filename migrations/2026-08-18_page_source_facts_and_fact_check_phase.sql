...
-- ── the claim's own lifecycle: the cursor IS the table ───────────────────────
-- A jsonb cursor held in a lease is not a resumable run: a lost lease lost the page's whole claim inventory
-- and the next pass re-extracted it from scratch (Codex, 2026-08-18). The inventory is persisted instead, one
-- row per claim, the moment the page version is read and BEFORE any claim is researched:
