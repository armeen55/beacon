...
The biggest file-size discipline violation in the repo; touched by everything on /today. **Honest take (UPDATED 2026-06-11 day shift, structural finding):** it is not a file of sections — it is ONE ~2,500-line `loadTodayPageData` function sharing dozens of locals; a split means designing staged sub-loaders (each taking/returning typed slices), not moving code between files. Needs characterization coverage of the composed payload first. Both shifts correctly refused it as a side-quest; the helpers around the monster total only ~350 lines (extracting them is cosmetic). Status: schedule a dedicated multi-hour session with the sub-loader design as step 1.
