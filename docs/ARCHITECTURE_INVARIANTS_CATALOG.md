...
|---|---|---|---|---|---|---|
| thresholds-provenance | `tests/architecture/thresholds-provenance.test.ts` | Section 2.8 / D8 | Pins the BORROWED Profound 6/18/37 defaults in `src/domains/citation-lifecycle/thresholds.ts` and the Phase A.2 replacement promise. | active | Evolves when Phase A.2 (Section 3) swaps the borrowed defaults for Beacon-owned per-tenant medians; rewrite to pin the new contract rather than delete. | 2026-05-14 |
...
| threshold-decision-loader-wiring | `tests/architecture/threshold-decision-loader-wiring.test.ts` | Section 3.7 / Step 3b (2026-05-14) | Pins the loader's threshold-decision wiring: cross-tenant-brain imports, `resolveTenantThresholdsCached` export + cache-key prefix, `threshold_decision: ThresholdDecision` field on `LifecycleForEdit` + `LifecycleSummary`, AND the customer-invisibility contract that `load-lifecycle.ts` does NOT pass custom thresholds into `deriveLifecycleStage` or a `threshold_decision` field into `renderLifecycleCopy` in A.2.3b. | active | Refines when A.2.3c wires the decision into `renderLifecycleCopy` and `deriveLifecycleStage` call sites within `load-lifecycle.ts`. The "do NOT pass" pins flip to "MUST pass" then. | 2026-05-14 |
