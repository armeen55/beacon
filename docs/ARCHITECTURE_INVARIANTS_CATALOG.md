...
| indexability-loader-tenant-isolation | `tests/architecture/indexability-loader-tenant-isolation.test.ts` | Section 4 / Step 3b (2026-05-14) | Pins the read-only indexability loader's tenant-isolation contract: imports + calls `currentTenantId`, references `opts.tenantId`, throws fail-loud on context mismatch with the literal phrase "tenant context mismatch", filters `reconciliation.canonical_pages` by `tenantDomain` before the URL-membership check (defense against the GLOBAL `sitemap-reconciliation` store), and validates `state.siteDomain` against `tenantDomain` before consuming robots data via `normalizeHost(state.siteDomain) !== tenantDomain` (defense against the FLAT-PATH `readRobotsState()` reader). At least 4 `nullRobotsFlags()` exits cover the robots-defense ladder. | active | Refines when `readRobotsState()` / `writeRobotsState()` are retrofitted through tenant-routed dotdata (`readDotDataJson("robots-state")` / `writeDotDataJson(...)`); the loader's siteDomain match becomes redundant defense-in-depth at that point. Until that retrofit lands, this loader is the only structural defense against multi-tenant robots-state overwrite churn. | 2026-05-14 |
...
| indexability-loader-import-allowlist | `tests/architecture/indexability-loader-import-allowlist.test.ts` | Section 4 / Step 3b + post-A.3.5 production-data fix (2026-05-14) | Pins the read-only indexability loader's import surface to an explicit allowlist (`server-only`, `next/cache`, `./types`, `./compute-indexability`, `@/domains/pages/types`, `@/domains/pages/sitemap-reconciliation-store`, `@/domains/pages/robots-parser`, `@/domains/citation-lifecycle/canonicalize-url`, `@/lib/business-config`, `@/lib/tenant-context`, `@/lib/persistence/repositories`). Page-snapshots flow through the tenant-scoped repository (`getRepository().forTenant(tenantId).getPageSnapshots()`) — replaces the prior `@/domains/pages/snapshot-store` direct file read which returned `[]` on Vercel (lambda FS doesn't carry `.data/tenants/{slug}/page-snapshots.json`). Sitemap reconciliation + robots state remain on the file-only read paths and are tracked as separate follow-up debt: sitemap-reconciliation needs a Supabase mirror; robots-state needs the flat-path retrofit through tenant-routed `readDotDataJson("robots-state")` (debt referenced in `indexability-loader-tenant-isolation` retirement-condition). New imports must be added here AND in the loader together. | active | Refines when A.3.b1 lands the GSC connector (the allowlist gains the GSC client path) AND when the sitemap-reconciliation / robots-state Supabase mirrors land (the file-only read modules move out of the allowlist or gain repository wrappers). Permanent otherwise — the indexability domain's read-only loader contract is structural. | 2026-05-14 |
