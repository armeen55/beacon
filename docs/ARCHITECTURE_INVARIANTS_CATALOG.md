...
| citation-lifecycle-stuck-bridge-phrase | `tests/architecture/citation-lifecycle-stuck-bridge-phrase.test.ts` | Section 2.16 | Pins the literal "next bundle will add automated sitemap + robots checks" stuck-stage copy + Phase A.3 REPLACE note in `render-copy.ts`. | active | Retire when Phase A.3 / Section 4.8 indexability ships and replaces the bridge phrase with per-verdict copy. | 2026-05-14 |
