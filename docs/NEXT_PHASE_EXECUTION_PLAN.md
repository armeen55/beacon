...
> 🟢 **TEST-ISOLATION FIX LANDED (2026-05-05).** Real LLM budget ledger now protected from `npm run test` clobbers. NEW architecture invariant `tests/architecture/llm-budget-test-isolation.test.ts` (7/7 PASS) walks every test file and asserts hermetic isolation for any test that writes to one of the four at-risk global stores (llm-budget, adjudicator-cache, adjudicator-history, llm-history-specific-edits). Ledger restored to operator-authorized LR-1 + LR-2 baseline ($0.065741 / 6 calls). Verified ledger byte-identical before/after a full `npm run test`.
>
> **Top of stack — operator decision pending:**
> 1. **LLM-LiveRegen-3 (optional, not urgent).** Current queue exhausted by LR-1 + LR-2 (no medium-conf observation-tier candidates remain that haven't been touched). New candidates will populate as the daily 07:00 UTC cron runs and entity registry / search signals refresh. No-op until the queue regrows organically.
> 2. **Optional broader May restoration.** LLM history shows 21 May 2026 live_call entries totaling $0.268 (15 pre-LR-1 dogfood runs were also lost to clobbers). The current ledger reflects only LR-1 + LR-2 ($0.065741 / 6 calls) per the operator brief's explicit numbers. If the operator wants the broader May reconstruction, that's a one-line follow-up.
> 3. **Optional live-regen-harness allowlist invariant** `tests/architecture/llm-live-regen-allowlist.test.ts` pinning the LR-N harness's safety properties (tenant-locked, candidate-count cap, $1 cap, persistence ONLY through `runProviderAndPersist`, pre-flight assertion no low-conf/inventory/weak candidate enters the loop). Defer until LR-3 is being planned.
> 4. Independent of decisions: all 66 architecture invariants (59 LLM-DryRun-2/3/3.5 + 7 LLM-budget-test-isolation) run on every PR. The `bundleError` gate in `runProviderAndPersist` is proven correct in production (LR-2 surfaced 2 bundleErrors and persistence aborted cleanly).
