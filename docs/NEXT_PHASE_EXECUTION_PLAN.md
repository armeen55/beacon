...
> 🟢 **LR-N HARNESS ALLOWLIST INVARIANT LANDED (2026-05-05).** 13 operator-locked safety properties pinned for any future live-regen harness. NEW `tests/architecture/llm-live-regen-allowlist.test.ts` (28/28 PASS) discovers every `scripts/llm-live-regen-<digit>.ts` file and asserts: tenant-locked, PINNED_CANDIDATES slate, $1 cap, runProviderAndPersist-only, no direct Supabase writes, no broad regen, etc. Both LR-1 + LR-2 verified to carry all 13 properties. Total architecture invariants: **94**. Ledger byte-identical after another full `npm run test`.
>
> **Top of stack — operator decision pending:**
> 1. **Wait for queue regrowth before LR-3.** Current queue exhausted by LR-1 + LR-2's combined runs. New medium-conf observation-tier candidates emerge organically as the daily 07:00 UTC cron runs and entity registry / search signals refresh. The architecture invariants will block any drift when LR-3 is built.
> 2. **Optional: broader May ledger restoration.** Current ledger shows $0.065741 / 6 calls (LR-1 + LR-2 only). LLM history file shows 21 May 2026 live_call entries totaling $0.268 (15 pre-LR-1 dogfood runs were also lost to test clobbers). One-line follow-up if accurate aggregate tracking matters.
> 3. **Optional: pause LR work entirely.** Engine verified across DryRun-1/2/3/3.5 + LR-1/LR-2 cycles ($0.196 total spend). Validator + persistence + budget + harness allowlist all proven. Move to other priorities (UX polish, Profound May 10 expiry, etc.).
> 4. Independent of decisions: all 94 architecture invariants run on every PR. UUID leaks, removed abstention rule wording, removed packet `resolution` block, harness drift to a persistence API, regression of the cutover ceiling at 9, regression of the single-prompt-only-as-hard-trigger rule, test writes to real `.data/global/llm-budget.json`, OR a future LR-N harness drift away from any of the 13 safety properties — all fail the build before reaching production.
