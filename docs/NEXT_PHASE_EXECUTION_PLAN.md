...
> 🟡 **SLICE 9.A1α — GA4 CONNECTOR SUBSTRATE (READY_TO_COMMIT, 2026-05-18).** Split slice. The first 9.A1 bundle exceeded its explicit +500 src/-line stop condition (actual +985), so the operator split it into **9.A1α — substrate only** (this slice) and **9.A1β — Settings UI + property picker** (next slice). 9.A1α ships the backend that makes GA4 a valid Google connector kind WITHOUT exposing any UI or callers. **Substrate scope**: `GoogleConnectorKind = "gsc" | "gbp" | "ga4"`; `GA4_SCOPE = "analytics.readonly"`; `ConnectorProvider` widened with `"google_ga4"`; payload extended with optional `ga4_property_id` + `ga4_property_display_name` + `ga4_account_display_name` via JSONB extension (no schema migration). 3 new server-only modules: `src/lib/connectors/ga4/{types.ts,client.ts,property-selection.ts}`. OAuth callback extended with single discriminator branch for `kind: "ga4" → provider: "google_ga4"`. **Settings UI INTENTIONALLY UNCHANGED in 9.A1α** — `page.tsx` + `connectors-client.tsx` + `actions.ts` byte-identical to HEAD; no Google Analytics card; no GA4 server actions. 4 new architecture invariants cataloged: `ga4-connector-server-only` · `ga4-connector-tenant-isolation` · `ga4-no-page-load-call` (substrate posture — no caller exists yet; tightens in 9.A1β with a positive sanity check on the actions seam) · `ga4-scope-split`. **Quality gates**: typecheck CLEAN · targeted GA4 suite 40/40 · existing connector suite 93/93 still green · catalog-sync green with 4 new rows. **Hard contracts honored**: no LLM · no paid APIs · no Supabase migrations · no GA4 Data API call · no sessions/events/conversions · no Today tile · no Changes detail outcome copy · no `/diagnostics/outcome-attribution` · no CallRail (deferred per K2) · no GBP insights · no customer outcome claims · no revenue/causal copy · no page-load connector calls (no caller exists yet) · no `business-config` GA4 property field · no GSC/GBP/Yelp regression · no Settings UI surfaces the new connector. **STOPPED AT READY_TO_COMMIT.** Net src/ delta: **+490** (under the +500 ceiling). **Recommended next**: operator review of the 9.A1α READY_TO_COMMIT report; on approval, commit + push + Vercel deploy (no hosted smoke needed since no UI). Then begin **Slice 9.A1β** (Settings UI + 4-state machine + property picker server actions + `tests/app/settings/{actions-ga4,connectors-ga4-card}.test.*` + extension of `tests/architecture/ga4-no-page-load-call.test.ts` to add the positive actions-seam sanity check + Settings smoke test extension). Then Slice 9.A2 (GA4 Data API + Mode A outcome attribution read model + first migration). Full detail: [`docs/VERIFICATION_LOG.md`](VERIFICATION_LOG.md#2026-05-18--slice-9a1%CE%B1-ga4-connector-substrate).
