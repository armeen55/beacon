...
SECOND WAVE (same night, 02:00–05:00 PT): DONE #43 #44 (resurrection detectors, diagnostic-only) · #73 (url-map live probes) · #82 (pre-push snapshots + revert) · #84 (batch accept, accept-only) · #90 (create_page verification) · #94+#52-lite (first-citation receipts naming the engine) · #96-v1 (post-push regression alarm) · #118 (digest deep links via /api/tenant-switch) · #126 (per-tenant console access) · #127 (accepted_at stamp + median time-to-approve).
