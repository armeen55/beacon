...
- [x] R15 (2026-07-03). N4 behavior-verdict lane (GA4+Clarity on every ship) + N17 task-completion. Effort M.
...
- [x] R17 COMPLETE 2026-07-03 (P2 GSC depth pack, all 3 slices). R17a brand-split/ingestion-gap/striking-portfolio/anonymized-share; R17b weekly-dimensions(searchAppearance+device)/fresh-tail/back-of-results + co-located tests added this pass; R17c country grain + budgeted indexation sweep (reuses GSC_INSPECT_PER_RENDER_LIMIT) + footprint registry + Discover probe (12h-cached, self-hides honestly). All deterministic, empty-safe, never mutate is_final. Effort L.
...
- [~] R23 (2026-07-03: 15 of 18 packs top-sliced + SHIPPED to main; each additive, empty/no-op-safe, ratchet held, prod-smoked. Remaining: P22 data-blocked (needs on-site-search/reviews connector); P18 code-health rides inside every slice; the per-pack tails + operator-gated/paid parts are roadmapped in each pack line. The 18-pack backlog is the full remaining product roadmap, executed by highest impact first: the two primary screens (P14 Today, P13 Worklist), then the intelligence packs P9/P11/P15/P8/P24/P10/P5, then P17/P23/P20/P12/P19/P16/P21, then substrate wired onto surfaces). Remaining packs in plan order. HIGHEST-IMPACT SLICES SHIPPED 2026-07-03 (the two
...
- [x] N4 (2026-07-03, R15; corroboration-only by construction). **GA4 + Clarity behavior-verdict lane**: engagement, scroll, frustration, conversion, traffic measured together on every ship. (NEW; absorbs v1 503 GA4 floors, 385 protect-revenue objection)
...
- [x] N17 (2026-07-03, R15). **User-task completion measurement**: did visitors find the answer they searched for? (NEW; rides N4)
...
- [x] N21 (2026-07-03, R19; JS-shell smell + demand-gated technical signals from stored snapshots). **Real JavaScript-rendered technical crawl** (v1 110 On-Page API; capped, gauntleted)
- [x] N22 (2026-07-03, R19). **Verify important content in source AND rendered HTML** (v1 111 dual-fetch)
...
- [x] N24 (2026-07-03, R19). **Evidence-based pruning, merging, retiring** (v1 113+241+100+215 merged: one content-lifecycle engine with the merge-and-redirect executor on the Wix Redirects API)
...
- [~] P4 (VERIFIED COMPLETE 2026-07-03: all 5 items - distinct-query growth, equivalence testing, FDR control, clean-window salvage, novelty-decay - were already shipped in R10a/b, wired to N10, rendered on /results; byte-identical-verdict pin holds, history never mutated). **Measurement rigor pack** (v1 150, 151, 152, 285, 286, 288, 289, 291, 378): fixed query panel, distinct-query growth, day-of-week baselines, adaptive windows, alpha-spending, equivalence testing, FDR control, clean-window salvage, novelty-decay flags. All feed N10's grade.
- [~] P5 (top slice SHIPPED 2026-07-03: agreement score + devil's-advocate voice + falsifier line + quorum 'worth a look' gate on the roundtable, all self-hiding; rest roadmapped: auto-retract, age-decayed confidence, debate multiplier, persisted decisions). **Team deliberation pack** (v1 163, 165, 166, 167, 203, 231+314 merged, 309, 312, 313, 386, 387, 455): quorum rules, control vetting, exploration slot, second round, persisted full decisions, devil's advocate, age-decayed confidence, falsifiers with auto-retract, second pass on big bets, debate-derived multiplier, agreement score, coverage ratchet.
- [x] P6 (2026-07-03, R16; ONE gateway, egress allowlist 10 files to 2, four uncapped/untimed call sites now guarded, 21-schema registry, 26-prompt regression harness, $0 call cache, de-templating + numeric + injection guards). **LLM engine pack** (v1 234+284 merged, 144, 145, 148, 277, 278, 280, 281, 282, 283, 375, 377, 403, 432, 499, 500, 502): one gateway with a zod schema registry, brand-voice card, loss-aware redraft, prompt versioning + regression harness, batch API, labeling pass, self-critique grader, call cache, token-true billing, numeric repair, tool-calling drafter, calibrated self-confidence, de-templating guard, best-of-2, injection sanitization.
- [~] P7 (shipped in R18 2026-07-03: N23 internal PageRank + linking/content-depth levers; tails roadmapped). **Internal linking + content depth levers** (v1 95, 96, 112, 244, 409, 411, 412, 564): entity auto-interlink, missing-H2 drafts, term-coverage grade, hub pages, outbound authority citations, near-duplicate detection, boilerplate guard, FAQ JSON-LD dedupe.
- [~] P8. **AEO defense pack** (v1 116, 117, 190, 191+225 merged, 192, 193, 196, 207, 219, 221, 222, 255, 256, 262+352 merged): defensive moves on competitor ships, defended-queries tracker, stat-injection lever, auto prompt-roster expansion, real-bot crawler preflight, zero-source lane, win-decay watchdog, winning-feature mining, AI accuracy watch, fact fingerprinting, persona variants, brand-description audit, zero-click ledger, UGC counter-play. TOP-3 HIGHEST-IMPACT DETERMINISTIC $0 SUB-ITEMS SHIPPED 2026-07-03 (over already-persisted Profound rows, no new API call, each empty-safe): (1) **zero-source lane** (v1 192) -> predicate `aeo_zero_source_opening` (a tracked question AI cites no one confidently for = a first-mover opening); (2) **defensive move on competitor ships / defended-queries** (v1 116/117) -> `aeo_defend_cited_query` (a NEW competitor citation on a query you used to own, from a real 2-capture citation-row history delta); (3) **brand-description audit** (v1 255) -> `aeo_brand_description_check` (AI describes you with a fact your site contradicts). All in `src/domains/aeo/**` + one pure predicate each in `recommendation-intelligence/triggers/`; PREDICATE_COUNT 26->29; 53 new tests + affected suites green (340 total). See VERIFICATION_LOG 2026-07-03. ROADMAPPED (rest): stat-injection lever (193), auto prompt-roster expansion (196), real-bot crawler preflight (207), win-decay watchdog (219), winning-feature mining (221), AI accuracy watch (222), fact fingerprinting (256), persona variants, zero-click ledger, UGC counter-play (262+352). Needs live Profound data on Iranopedia to prove real cards render.
- [~] P9. **Competitor watch pack** (v1 245+325+368 merged, 250+259 merged as an extension of the shipped SERP history, 251, 363, 420, 482, 483): counter-refresh on competitor updates, broken-competitor opportunities, SERP-feature appear/disappear alerts, volatility pre-build check, true-competitor discovery, backlink watch on teardown targets, teardown prioritization by traffic, seed demand from any competitor URL. TOP-3 HIGHEST-IMPACT SLICES SHIPPED 2026-07-03 (deterministic over already-persisted dataforseo_serp_history, no new paid call, empty-safe): (1) broken-competitor opportunity (a competitor page that dropped off Google -> create_page opening Move), (2) true-competitor discovery (who actually beats you, from persisted Google-results overlap; honest read-side list), (3) Google-results feature appear/disappear alert (answer box / FAQ / image pack appeared for a tracked query -> add_answer_block Move). 2 predicates registered (PREDICATE_COUNT to 26 combined with P11); 39 new tests + affected suites green. See VERIFICATION_LOG 2026-07-03. ROADMAPPED (rest): counter-refresh on competitor updates, volatility pre-build check, backlink watch on teardown targets, teardown-by-traffic prioritization, seed-demand-from-competitor-URL, and surfacing the true-competitor list on a read surface.
- [~] P10 (top slice SHIPPED 2026-07-03: sitewide entity+sameAs schema Move, author/reviewer Person byline Move, brand Knowledge-Graph presence check; 3 predicates PREDICATE_COUNT->33; rest roadmapped). **Entity + author system** (v1 118+218 merged, 243+263 merged, 372, 507): sitewide entity graph with Wikidata (extends the shipped biography QIDs), author/reviewer Person bylines, Knowledge Graph presence tracking, connector-free entity check at cold start.
- [~] P11. **Technical SEO pack** (v1 168, 319+475 merged, 320, 321, 456, 457, 458, 515, 516, 517, 518, 519, 560, 585, 586): CrUX/PSI lane, broken-link fixer + nightly link liveness, redirect chains, dead-URLs-with-demand recovery, lang/dir audit, URL-variant splits, freshness propagation, robots/sitemap drift alarm, X-Robots-Tag capture, soft-404s, schema-validator breadth, TTFB capture, pagination index policy, OG/Twitter capture, source-link rot. TOP-3 HIGHEST-IMPACT SLICES SHIPPED 2026-07-03 (new `src/domains/technical-seo/**` domain + one trigger each, wired LAST in the loader, PREDICATE_COUNT 23->26, cross-source cooldown-deduped, empty-safe): (1) dead-URL-with-demand recovery (GSC demand x snapshot 404/410 x URL-inspection index verdict -> fix_status_code recovery Move), (2) broken-link fixer + link liveness (snapshot internal_links x owned-status liveness + optional polite/capped/mockable HEAD pass -> add_internal_link, one card per source page), (3) redirect-chain + soft-404 hygiene (multi-hop chains from the liveness pass + Google's own Soft-404 coverage verdict -> fix_status_code). typecheck clean; 52 new tests + affected suites green. See VERIFICATION_LOG 2026-07-03. ROADMAPPED (rest of the pack): CrUX/PSI lane, lang/dir audit, URL-variant splits, freshness propagation, robots/sitemap drift alarm, X-Robots-Tag capture, schema-validator breadth, TTFB capture, pagination index policy, OG/Twitter capture, source-link rot.
- [~] P12 (safe top slice SHIPPED 2026-07-03: atomic multi-field bundle (all-or-nothing), post-push read-back verification (never claims live without a match), per-URL cooldown, whole-day rollback PLANNER; Ritz-block + dry-run + caps preserved and pinned; NOTHING published, no new callers yet; rest roadmapped). **Push depth pack** (v1 179, 181, 182, 340, 342, 343, 392, 393, 394, 465, 466, 526, 529, 531, 532, 590, 591): atomic multi-field bundles, ground-truth field mapping, Wix Blog drafts, Ricos serializer, reference fields, whole-day rollback, factory-page render audit, per-URL cooldown, element-scoped probes, publish windows, two-phase page creation, read-back tier, scope probes at connect, external-edit flags, pre-bundle export, bulk endpoint.
- [~] P13 (top slice SHIPPED 2026-07-03, R23: rank explanation + honest minute-math + word-level diff + not-now durations/redraft on the worklist; tails roadmapped). **Worklist UX pack** (v1 184, 185, 186, 188, 189, 205+206 merged, 217, 344, 345, 346, 347, 348, 349, 351, 397, 398, 399, 533, 534, 535, 536, 592): live guardrails on paste, per-pick toggles, batch verify, steering reasons, reversible batches, word-level diffs, add-to-today, set-aside view, same-page bundling, carry-over, focus mode, one-click re-draft, plan diffs, rank explanations, not-now durations, Wix-surface grouping, VA spreadsheet export, unlock dates, cumulative payoff, URL-synced filters, honest minute math.
- [~] P14 (top slice SHIPPED 2026-07-03, R23: lead headline + smoke-alarm-with-page-blame + goal pace + still-arriving shading on Today; ratchet dropped 1342->1298; tails roadmapped). **Today dashboard pack** (v1 169, 171, 172, 323, 324, 327, 328, 329, 330, 331, 332, 388, 389, 459, 461, 520, 522, 588): touched-pages toggle, smoke alarm with page blame, early drift flags, goal pace, lead story, bleeding band, YoY context, start-my-day routine, projection cone, revenue series, threshold crossings, still-arriving shading, verdict takeover, workhorse leaderboard, question-mix trend, waiting-on-operator aging, weekday-adjusted brief, time-of-day ordering.
- [~] P15 (SHIPPED 2026-07-03, R23: learn-from-dismissals + do-not-repeat + intent-dimension prior (reused) + visible 'Beacon learned' line; byte-identical when undecided; tails roadmapped). **Learning depth pack** (v1 139, 272, 373, 429, 430, 431, 497, 498): intent-dimension priors, do-not-repeat registry, learn from dismissals, compound-pair track, prior snapshots + brain changelog, coverage map, inconclusive-rate demotion, effort recalibration.
- [~] P16 (generic site-health slice SHIPPED 2026-07-03: robots AI-crawler-block read layer naming exact blocked bots + JS-shell rendering fact + CMS detection; the actionable robots/js-shell MOVES were already wired by prior slices; the site-health-panel read surface awaits mounting; signup-funnel/day-0 parts roadmapped). **Onboarding intelligence pack** (v1 292, 293, 294, 295, 380, 381, 382, 437, 506): industry classifier fallback, day-0 demand pack, derived-services chips, launch teardown, robots AI-block check, CMS detection, schema starter pack, JS-shell rendering fallback, signup-to-first-win funnel instrumentation. (T0e ships first; this deepens it.)
- [~] P17 (top slice SHIPPED 2026-07-03: request-cached loadShippedChanges 5->1 read/req + loadDetectedChangepoints 3->1, byte-identical output; cross-request demand-graph cache already existed; rest roadmapped). **Performance pack** (v1 159, 160, 201, 299, 300, 301, 302+304 merged, 303, 305, 441, 442, 443, 511, 540, 553, 580, 581, 583, 604, 605): compile/read/cache work, RPC consolidation, SWR everywhere, payload slimming, never-cold-after-invalidation.
- [~] P18 (ONGOING by design: 'work opportunistically inside other slices, never a standalone month' - rides inside every slice; not a blocking checkbox). **Code health pack** (v1 306, 307, 445, 446, 447, 448, 450, 451, 452, 512, 513, 514, 541, 542, 554, 556, 557, 558, 559, 584, 594, 595, 597, 606, 607, 608, 609): consolidation debt. Work opportunistically inside other slices; never a standalone month.
- [~] P19 (FREE top slice SHIPPED 2026-07-03, $0: difficulty-from-proof-history, rank-to-visits conversion, real-cost reconciliation, rank-distribution summary; pure library helpers ready to wire; paid producers stay roadmapped/operator-gated on cap raise). **DataForSEO depth pack** (v1 108, 114, 124, 252, 364, 365, 366, 367, 417, 418, 489): AI search volume column, capped backlink radar, difficulty model from proof history, task-queue routing, rank-to-visits conversion, real-cost reconciliation, depth-30 sweeps, Trends wiring, one gauntlet runner, unlinked mentions, rank-distribution chart.
- [~] P20 (SHIPPED 2026-07-03: generic tenant-configured canonical-spelling demand engine, byte-identical no-op when unconfigured, PREDICATE_COUNT->34; native-script-in-answer follow-up roadmapped). **Transliteration + spelling demand** (v1 129, 374): canonical-spelling demand engine for any tenant whose audience types multiple scripts; optional native-script term in answers. English-first surfaces, tenant-configured.
- [~] P21 (buildable slice SHIPPED 2026-07-03: while-you-were-away catch-up block on Today (counts agree with Results) + honest /settings/limits 'what I cannot do yet'; operator-taste memory covered by P15; weekly email + trust-explainer roadmapped/RESEND-gated). **Digest + memory pack** (v1 92, 230, 238, 240, 356, 358): operator-taste memory, while-you-were-away block, strategist history, weekly email digest, trust explainer page, product-limits copy.
- [B] P22 (DATA-BLOCKED 2026-07-03: both sub-items need data sources Beacon does not have a connector for - v1 427 on-site search analytics and v1 490 customer reviews/support-transcript feeds. The GSC-demand-with-no-owned-page angle is already covered by the shipped keyword-gap + create-page triggers, so building a third would duplicate. UNBLOCK = operator connects an on-site-search export or a reviews feed. Not built to avoid duplication + a data blocker). **Zero-result + review mining** (v1 427, 490): on-site search gaps, customer-language moves from reviews and transcripts.
- [~] P23 (top slice SHIPPED 2026-07-03: export-a-win card + internal operator-gated /reports monthly report, counts routed through the /results canonical resolvers so they can never contradict; public share stays DEFERRED). **Reports pack** (v1 233, 257+575 merged, internal-only): export-a-win card, monthly report page (internal; the public version stays deferred).
- [~] P24 (top slice SHIPPED 2026-07-03: snapshot image extractor + alt-text audit/inventory + active add_image_alt_text lever, PREDICATE_COUNT->30; needs live crawl + hosted images column; rest roadmapped). **Image SEO lane** (v1 410+576+578 merged, 248, 552): image inventory + alt audit + alt-text lever from GSC image data and the cold-start crawl; original diagrams and licensed images only behind N5's gate.
