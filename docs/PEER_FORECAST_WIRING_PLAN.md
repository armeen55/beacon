...
**Required before wiring:** extract a single shared `composeMatchKey(TaxonomyTarget)` (or `(primary_bucket, child_tags)`) helper, use it on BOTH the edit-recording side and `loadOutcomesForTenant`, and pin alignment with a test that asserts a rec's matchKey === the matchKey derived from that rec's eventual proof outcome (the #43 "zero drift" invariant, extended to the brain). Without this test the build is not safe to ship.
