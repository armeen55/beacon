...
- Verified source and production on 2026-09-13: `55ea71d87863c12ebdae95ae23345b6780fa8b59`, deployment `dpl_ExuB7K5X8FYTvfVsiS6owNevGV34` READY. Production /api/version HTTP 200 reports that exact SHA, ref main and deployment ID; login HTTP 200 and anonymous Changes HTTP 307 to /login?next=%2Fchanges. Captured body, paragraphs, cards and HTML FAQ extraction now share one pruned main-content root, retaining all top-level main regions or the full multi-article body fallback; hidden/template and inline-hidden material are excluded, block boundaries normalize to spaces without marker tokens. Complete captured HTML answer text is retained in the existing FAQ JSON only within the existing 100,000-character held-body/combined-answer budget. Legacy excerpts and oversized answers remain explicit samples. The owned reader retains question/answer scope within its unchanged 48,000-character budget. The primary atomic and prospective-page writing handoffs now use one 12,000-character page-evidence selector: an exactly matched complete captured FAQ pair rides whole if it fits; oversized comparisons cannot overrun the budget; unpublished prior draft text stays draft-so-far rather than claim support. The prospective-page packet no longer joins passages with their duplicate vocabulary copy. Capture-to-JSON-to-reader workflows and actual primary writer-input checks reproduce these promises at $0; this is not a live generated-copy or full long-page proof. The legacy page-surgeon packet also retains qualified answer text, but it is not the primary writer. FAQ display advice reflects Google's May 7, 2026 rich-result retirement and promises no ranking/citation gain; other schema eligibility warnings still need qualification. Prior material revision identity across PageSnapshot, repository projection, owned evidence and account/job hashes remains: same-sized body/full-answer/parsed-schema value changes move the affected job, unchanged clocks/object-key order/cached competitor decoding/unrelated page drift do not. JSON-LD array order remains conservative, not full semantic graph canonicalization. Prior canonical applied-unit/current-checker learning qualification, full-ledger overlap honesty, tenant-explicit requalified writer examples, measured-zero retention and remaining-attempt-only legacy verification requalification remain; paid new-page briefs still resume without a second brief. Final frozen-source full gate passed: 2,025 tests passed, 23 existing skipped, 90 files; guard/lint/typecheck/build passed, 21 existing lint warnings. Two passing frozen-source gates because primary handoff work followed the first gate, seven focused sweeps (one fixture expectation corrected to exact full-answer equality), four separate typechecks; no assertion waived. Production/test/combined LOC: 74,338/20,520/94,858, all three ceilings ratcheted down to these counts. This source step: production +60/-73, tests +22/-23; superseded extraction clones, the separate prospective-page selector and redundant helper-level test/comments replaced; no new routes, dependencies, domains, business records or facade API, exports unchanged at 1,383. Existing FAQ/body DTOs gain optional captured-answer/scope metadata. Resources: $0 OpenAI/DataForSEO calls/spend; one bounded production Data API metadata GET HTTP 200 confirmed the existing FAQ column is JSONB, not new stored answer data or hosted SQL/RLS. Zero hosted database writes, two native local production builds, one Git-triggered production deployment and three public smoke requests. No funded drive, authenticated walk, cron/environment change, migration or stored-data deletion. Post-deploy handoff update remains working-tree documentation for the next coherent source commit. This is a captured-answer/primary-handoff replacement, NOT full waterfall readiness or future model-quality certification. Full live JSON-LD graphs are not retained in PageSnapshot/projection, content-linked replacement schema is not generated, AtomicEditDraft has no schema field, new-page assembly has no derived schema component, and duplicate detection compares types rather than graph identities. Rendered provider reads still give empty FAQ/schema hashes for unobserved fields. Static main-content qualification does not prove stylesheet/rendered visibility, historical FAQ provenance, all metadata/entity authority or semantic FAQ association. Correction from the subsequent source trace: stored section proposals normalize to answer_block before primary writing and do receive its intent/shape clauses; the prior contrary finding was wrong. Replacement requests were instead briefed as additions with an empty current body, then converted to replacements afterward. Raw vocabulary fallback, full model/query-sensitive identity and useful long-page query-specific handoffs remain open. Historical configured-tenant alignment and saved revision data remain unverified after the prior tenant-filtered read returned zero rows. Exact placement is not retained on applied component records; historical measurement admission still accepts partial verification and fromRow clears verification_needed for partial receipts. Comparison receipt identity/uniqueness, raw window provenance, historical receipt/admission reconciliation and Today frozen-metric wording remain unqualified. NOT CERTIFIED: complete content-linked schema regeneration/graph-aware validation; sustained live first-party editorial quality and complete operator delivery across every edit family; exact-query recurring SEO/AEO winner-pattern usefulness, adequate owned/competitor long-page reads, acquisition-debt resolution, semantic entailment/injection resistance, historical fact-bank/paid partial/Ready reconciliation, stale promise authority, concurrent write recency, acquisition-mode remediation, cross-process cache writers, full-ledger scaling/retention, atomic metric windows, watermark/outage semantics and complete verification validity. Prior harness-clock/cross-day paid-work continuity qualification remains open despite native-time gates passing. Hosted SQL definitions/applied migrations/RLS/advisors/exact cron remain unverified after management HTTP 401. Last inspected cron was active every ten minutes; billing failure is not a global pause. Last inspected GSC finalized through Sep 9 and GA4 page traffic was current; separate GA4 totals/referrals and legacy technical scans were stale. Publishing remains manual; canonical AI recurrence/five-page SEO comparison remains the research backbone, not editorial-quality certification.
