...
- **P14 (today) — Orchestration extract + queue-driven CLI.** New `src/domains/recommendations/load-queue.ts` exports `loadLiveRecommendationQueue` (the orchestration the page used to inline) + `buildPacketForRec` (pure packet builder for one queue rec). New `scripts/build-edits-for-queue.ts` CLI with `--list / --rec-id=<...> / --all / --write` modes; default DRY-RUN; honest empty-inventory reporting. Page render unchanged. Repository now has `getPageElementInventory()` in both backends. 18 new tests; **2061 passing**.
- **Sprint 6A.1 status:** code-complete (P1–P12), orchestration ready (P14). **Phase 6A.1.13 (hosted UI verification) is unblocked once `page_element_inventory` is populated on production** — that's Phase 6A.1.15 (run a scan to populate; not in this commit).
- **Next:** Phase 6A.1.15 — populate `page_element_inventory` on hosted by running a scan with DUAL_WRITE=true. Then rerun Phase 6A.1.13 against a real live-queue stableKey using the new CLI.
