...
- Hosted read-only audit, 2026-09-13 (no source release): exact Supabase project vlxwevsdvwxvopkjsewo/beacon-main is accessible despite list_projects omitting it. Catalogue confirms no raw JSON-LD/graph payload column in page_snapshots. Applied migration metadata: 101 versions, latest lock_internal_audit_snapshots; not every definition verified. Local configured tenant is tenant-ritz-founder, not tenant-iranopedia; Iranopedia has 478 proposals and 1,438 snapshots across 449 stored page IDs. The canonical newest-500-before-dedup query represents only 209 page IDs (207 trusted) and omits 240; these are not asserted to be canonical unique URLs. The stamped Ready queue still has the six pasted historical outputs, including Parthian anchor/copy punctuation mismatch; authenticated rendering/current servability remains unverified. Three restored new-page rows retain generic/instructional openings and owe all planned sections (4/5/3); two schema rows retain historical prose/link-removal redraft obligations. One active Vault-backed Beacon cron dispatches every ten minutes; twelve recent pg_cron executions succeeded, not proof of downstream HTTP success. Iranopedia research_paused=false; Sep 13 background DataForSEO ledger: 174 calls/$1.13408, not audit spend/invoice truth. Security advisors: 13 no-policy RLS infos, four mutable-search-path functions, pg_net in public, authenticated definer-helper execution, leaked-password protection disabled. Inspected helper checks auth.uid() membership with fixed search_path; audit snapshot tables deny anon/authenticated privileges; mutable-path functions are invoker, and relevant tenant policies were inspected. No blanket privilege/policy change justified; complete security/performance qualification remains open. Sensitive Vercel URL read and eleven public login assets did not establish production database URL alignment. Proper graph persistence/content dependencies/identity-aware replacement remain absent; no migration/cron/env change applied. This audit: $0 paid provider calls, zero DB writes/tests/builds/commits/pushes/deploys, source/test/API/routes/dependencies net zero. Existing handoff updated only in working tree; whole rebuild active, not full-run-ready.
- Branch `main`; MVP rebuild started at `4669fbb5`. Stack: Next.js App Router, strict TypeScript, Supabase, Vercel.
- Verified source and production on 2026-09-13: `9d28e98ced3cbc9bfb3513b54f34f096ebd8e1b3`, pushed origin/main; deployment `dpl_8Ew8vf7XimL6z7W8xbRqCStysKyc` READY. Production /api/version HTTP 200 returned this exact SHA, main and deployment ID; /login HTTP 200; anonymous /changes HTTP 307 to /login?next=%2Fchanges. This bounded step replaces four duplicated/capped snapshot readers with one tenant-explicit per-page identity selector: newest capture plus newest trusted capture under the existing word-count/extraction-certainty rule, payloads fetched only after selection. Held-whole zero-word capture qualification is not added by this metadata projection. Busy-page history cannot crowd out quiet pages; an untrusted boundary page searches its older identities. Errors and incomplete selected payloads fail instead of certifying partial coverage. Unscoped production snapshot/link-graph reads reject before the database call. Encoded request size and row count both bound payload batches; the live 16.7 KB selection URI reproduced UND_ERR_HEADERS_OVERFLOW before byte-aware batching replaced it. No total-feed cap was raised. The actual local source reader, explicitly scoped to tenant-iranopedia against vlxwevsdvwxvopkjsewo, matched an independent SQL oracle exactly: 452 selected rows across all 449 stored page IDs, identity MD5 5176f3ff13d9fd7b32d4bd6c23b9e4ca. These are stored IDs, not certified canonical unique URLs or every page on the site. The matching link feed has 448 observed graph rows across 445 IDs; four selected captures have unobserved/null graphs, not proved zero links. Observed empty arrays survive, and word count/extraction certainty reach link-version selection. All twelve requests in the completed snapshot-plus-link comparison were tenant-filtered, row-bounded and below 8,000 encoded URI bytes; total returned response bodies 6,103,792 bytes. This verifies the reader against this database, NOT authenticated production behavior, production database URL alignment, editorial quality or rankings. Frozen-source gate passed: guard/lint/typecheck/tests/build; 2,025 passed, 23 existing skipped, 90 files, 21 existing lint warnings. Focused behavioral tests exercise 1,200 uncertain busy-page captures, an older trusted capture, 540 quiet pages with long IDs, cross-tenant exclusion, bounded projections, observed empty graphs and read failure. Repeated tenant-isolation cases became one exact-record table without dropping their promises. Production/test/combined LOC 74,306/20,520/94,826, production and combined ceilings ratcheted down; production +70/-101, tests +38/-38. Backend file ceiling ratcheted down to 525. No new routes/dependencies/domains/business records/facade exports; exports 1,383. Existing internal link-graph projection gains optional capture word count/certainty. Resources: $0 OpenAI/DataForSEO task calls/spend; four read-only SQL queries, project/index/docs reads, two failed REST reproductions followed by one completed twelve-request comparison, zero database writes; one local production build, one Git-triggered deployment and three public smoke requests. No provider drive, authenticated walk, migration, cron/environment/billing change or stored-data deletion. Post-deploy handoff update remains working-tree documentation for the next coherent source commit. Prior primary body-copy contract remains: exact replacement receives actual before text/mutation mode and keeps its outer heading; insertion/addition/adaptive delivery are distinct; first-party task-appropriate depth, no sentence quota, commentary in metadata; wrong assigned fields stop before review; only primary body requests use draft.body_edit v1. Configured model, transport budgets and acceptance contract were not changed here. Prior verified captured-answer handoff remains: owned/competitor full captured FAQ answers, labelled capture scope, evidence selected for the reader task within unchanged budgets, and shared role-labelled writer/evaluator packets. Metadata and observed-source candidates are not factual support or absence evidence; captured markup does not prove rendered visibility or semantic FAQ association. Prior material revision identity, canonical applied-unit/current-checker learning qualification, full-ledger overlap honesty, tenant-explicit requalified writer examples, measured-zero retention and paid new-page brief/piece reuse remain; JSON-LD array order is conservative, not full semantic graph canonicalization. OPEN: full live JSON-LD graphs are not retained in PageSnapshot/projection; content-linked replacement schema is not generated; new-page assembly explicitly derives no schema component; duplicate detection compares types, not graph identities. Schema rows already have separate canonical JSON-LD validation and prose-row conversion; absence of a schema field in AtomicEditDraft alone does NOT diagnose their actual generation path. Rendered provider reads still give empty FAQ/schema hashes for unobserved fields. Raw vocabulary fallback, full model/query-sensitive identity, useful complete long-page/query-specific handoffs, all metadata/entity authority and historical FAQ provenance remain unqualified. The AI card explanation currently combines a newest-answer count with historical-window recurrence days, producing incompatible denominator wording such as three answers on twenty-two days; these units must be separated. Historical configured-tenant alignment/saved revisions remain unverified after the prior tenant-filtered read returned zero rows. Exact applied-component placement is not retained; historical measurement admission still accepts partial verification and fromRow clears verification_needed for partial receipts. Comparison receipt identity/uniqueness, raw metric-window provenance, historical receipt/admission reconciliation and Today frozen-metric wording remain unqualified. NOT CERTIFIED: complete content-linked schema regeneration/graph-aware validation; sustained first-party editorial quality and complete operator delivery across every edit family; exact-query recurring SEO/AEO top-five winner-pattern usefulness; adequate owned/competitor long-page reads; acquisition-debt resolution; semantic entailment/injection resistance; historical fact-bank/paid-partial/Ready reconciliation; stale promise authority; concurrent/cross-process write recency; acquisition-mode remediation; full-ledger scaling/retention; atomic metric windows/watermark/outage semantics; and complete verification validity. Prior harness-clock/cross-day paid-work continuity qualification remains open despite native-time gates passing. Prior management HTTP 401 is superseded by exact-project MCP catalogue reads above; complete hosted reconciliation and authenticated UI remain unverified. Last inspected cron was active every ten minutes; billing failure is not a global pause. Last inspected GSC finalized through Sep 9 and GA4 page traffic was current; separate GA4 totals/referrals and legacy technical scans were stale. Publishing remains manual; canonical AI recurrence and five-page SEO comparison remain the research backbone, not editorial-quality certification. The whole waterfall rebuild remains active, NOT full-run-ready.
