...
- **P11 (today)** — Persistence layer + CLI. Files: `src/domains/recommendations/recommended-edits-persistence.ts` (`mapSpecificEditToRow`, `persistRecommendedEditsLocal`, `runProviderAndPersist`), `src/lib/persistence/dual-write.ts` (added `syncRecommendedEdits` with onConflict on `rec_id,action_type,target_element_key`), `scripts/generate-specific-edits.ts` (CLI). Deterministic id (`${recId}__${actionType}__${target_element_key ?? "null"}`) makes re-runs idempotent at file + DB layer. Dry-run is the default; `--write` opts in. CLI smoke-tested clean. 19 new tests; **2015 passing**.
- **Next Sprint 6A.1 step:** Phase 6A.1.12 — UI surfacing. Render validated `SpecificEdit` rows on `/recommendations` (Sprint 6's renderer + per-edit Accept UX). The data layer is closed end-to-end at this point; what's left is operator interaction. Still no LLM (that's Sprint 6A.2).
