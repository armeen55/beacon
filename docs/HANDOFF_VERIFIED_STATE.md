...
- Verified source and production on 2026-09-12: `4aabedd63770a754b28ef41fa9351f186d87c56e`, deployment `dpl_FCX1gxfWQye6yR7gMVs3tZuRpmrb` READY. Production `/api/version` HTTP 200 reports that exact SHA on `main`; login HTTP 200 and anonymous Changes HTTP 307 to login. Prospective new-page factual obligations now carry their own tenant/topic/basis scope, without borrowing an owned page or inventing a live body hash. The existing fact engine researches the exact proposition; canonical source rows are read back, unchanged checked readings buy nothing, and opening shortfalls retain the brief and request research before writing. Sections consume exact scoped facts. Runtime sharing and funding identities distinguish topic and successive evidence obligations. Additive facts use one authorization rule: current checked source/support receipts are required, conflicting/disputed/unsupported findings authorize nothing; supported likely additions remain allowed and corrections retain their stronger bar. Prior canonical 28-day AI recurrence, five-page SEO acquisition, owned passage handoffs, source-bound packets, copy-bound targeted new-page repair/resumption, and source-validity measurement remain. Focused replays: 92 cases across five suites plus two dispatcher topic-isolation cases; injected responses prove behavior, not future editorial quality. Full gate: 2,036 passed, 24 skipped, 89 files; build/typecheck/foundation passed, 21 existing lint warnings. Production/test/combined LOC: 74,571/20,547/95,118; production/combined ceilings ratcheted down, no new routes/dependencies/exports. This slice: $0 provider spend, two read-only schema queries, one Git-triggered deploy; no funded drive, cron/environment change or stored-data deletion. NOT CERTIFIED: authenticated production journey or sustained live waterfall; task-adequate competitor/section information and full edit-family quality; historical owned-scoped fact-bank reconciliation for restored prospective pages, acquisition-mode whole-review objections and canonical-validator rejection remediation; model/schema-aware cache identity, warm/fresh validation parity and cross-process cache durability; atomic metric-window consistency, source-watermark outage semantics and complete verification validity. Last inspected cron remains active every ten minutes; billing failure is not a global pause. GSC was finalized through Sep 9 and GA4 page traffic current; separate GA4 totals/referrals and legacy technical scans were stale.
