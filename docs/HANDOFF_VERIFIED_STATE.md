...
- Deployed 2026-09-05, verified at `/api/version` as `c6d402dc`: the research-to-useful-change loop, rounds one to three. One editorial standard per treatment is chosen once from the persisted assignment and read by every door; a search is diagnosed against the page's own passages, never by token absence, a function word is never a topic, a relational word the searcher used must be matched by the text that claims to answer, and the demand gap is the largest search the page does not answer; page understanding stores the title's promise, the missing capability and what the page sells; the evidence bar is proportional to what the treatment risks (a correction needs a confirmed reading, an addition stands on one publisher whose stored passage entails the claim), a winner with a different intent teaches nothing, and every paid acquisition names the row and step it unlocks and whether the purchase could be used. The runtime prepares an account once per drive, spends money instead of reserving it, resumes and demotes unreached work, keeps a run's receipts as its own, reclaims an abandoned run, and records a stale connector on the pass receipt; funding, ranking and explanation read one scoring context; the $0 replay writes every refusal it decides onto the row. The operator loop keeps both the prepared and the applied wording, never loses a press to a returned failure, answers a bulk press per card, and settles a verification record that names nothing to look for without a fetch. Not yet done: the template rule judged against whichever siblings the sweep happens to hold (a Ready row stands on a refusal that fires with the whole family), the demotion path's refusals outside the gate vocabulary, the opportunity line printed as a writer's brief, Results answering the seven questions, and the stale-connector and waiting-on sentences on Today.
