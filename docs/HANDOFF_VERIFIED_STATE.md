...
- Deployed 2026-09-05, verified at `/api/version` as `779c6aea`: the research-to-useful-change loop, rounds one to five. One editorial standard per treatment is chosen once from the persisted assignment and read by every door; a search is diagnosed against the page's own passages, never by token absence; page understanding stores the title's promise, the missing capability and what the page sells; the evidence bar is proportional to what the treatment risks, authority is decided once at the consumer that admitted a reading, a usable reading is never bought again, and a reader's answer never opens with the search phrase; a page is judged against its whole template family from the inventory or the rule refuses to decide, and a read that did not happen is never a fault. The runtime prepares an account once per drive, spends money instead of reserving it, walks unreached work as a tie-break under worth and never as a ratchet, keeps a run's receipts as its own, reclaims an abandoned run, writes every acquisition with the row and step it unlocks, and records a stale connector on the pass receipt; a verification reading stamps itself after its fetch, stops at its cap and reads each address once per pass; funding, ranking and explanation read one scoring context. The operator loop keeps both the prepared and the applied wording, never loses a press and answers a bulk press per card; Results prints raw movement beside the adjusted comparison and says which of the two happened and what the account's one closed reading taught the queue; Today names the last change's outcome; the new-page pathway is proved end to end on injected evidence. Not yet done: a substantive body answer reaching Ready through the unattended walk (the sourced pre-1979 flag answer is fourth in the funded order and owed), and the evaluative-adjective set as the one remaining closed English list.
