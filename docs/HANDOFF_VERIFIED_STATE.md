...
- Verified source and production on 2026-09-12: `388e62403ab0b43e6e63bf3b80a017516a3d57af`, deployment `dpl_eoUqBMrDsEccxrjyEZmLpHxBUDGb` READY. Production `/api/version` HTTP 200 reports that exact SHA, `ref: main` and deployment ID; login HTTP 200 and anonymous Changes HTTP 307 to login. One canonical answered-HTML FAQ projection now feeds owned bodies, factual packets, lean snapshot counts, fresh research extracts and crawl facts; markup-only, unknown-origin and unanswered entries remain raw history but cannot count as answered visible content there. Packet assembly retains the visible answer excerpt rather than reading a nonexistent answer field. Held-word/passages scope excludes heading/card duplication, and markup entity names cannot independently establish visible page presence. Whole-page absence and commissioning scope require explicit complete/current bodies; missing version is unknown, not current. Positive competitor observations survive partial captures as source-bound research candidates, never whole-page absence or factual authority. An empty semantic comparison from incomplete, unshown or wrong-page owned text remains unread rather than nothing missing. Competitor comparison/atomic preparation versions are v4/v8; predecessor cache contracts are re-earned without deleting history. Prior verified AEO task-focused owned/winner reading, full material body/scope/version identity, timestamp-only reuse, diagnosis/AEO v4/v3, headed operator delivery, explicit replacement/addition scope (writer wc21), acquisition acknowledgement and recorded JSON-LD value verification remain. Frozen-source full gate: 2,040 passed, 23 skipped, 89 files; foundation/lint/typecheck/build passed, 21 existing lint warnings. Production/test/combined LOC: 74,432/20,523/94,955; production and combined ceilings ratcheted down, tests unchanged. This source step: production +65/-83, tests +32/-32; no new public facade, routes, dependencies, domains or records, export count unchanged at 1,383. Resources: $0 OpenAI/DataForSEO calls/spend, zero database requests/writes; focused local sweeps, one completed full gate/local build, one Git-triggered production deploy and three public HTTP smoke requests. No funded drive, authenticated walk, cron/environment change, migration or stored-data deletion. This is an evidence-handoff contract repair, NOT full waterfall readiness or future model-quality certification. NOT CERTIFIED: sustained live editorial quality and complete operator delivery across all edit families; HTML FAQ main-content/visibility qualification, all-reader metadata/entity authority, task-adequate long-page reads and acquisition-debt resolution, semantic entailment and prompt-injection resistance; full durable schema property-value capture/hash, visible-content/schema dependency refresh, graph-aware draft validation and current rich-result guidance. The extractor still hashes schema type names rather than all property values; schema draft validation pools support facts with page text and checks duplicate types rather than graph identities. Historical verification receipts, fact-bank reconciliation and paid partial/Ready rows require review, not deletion. Body/model-sensitive durable identity, stale promise authority, concurrent write recency, acquisition-mode remediation, cross-process cache writers, history-query scaling/retention, atomic metric windows, watermark/outage semantics and complete verification validity remain. Hosted SQL definitions/applied migrations/RLS/advisors/exact cron remain unverified after management HTTP 401. Last inspected cron was active every ten minutes; billing failure is not a global pause. Last inspected GSC finalized through Sep 9 and GA4 page traffic was current; separate GA4 totals/referrals and legacy technical scans were stale. Manual publishing and the canonical AI recurrence/five-page SEO comparison remain.
