...
The do-this rebuild (548da9cf, 2026-08-11) makes every screen speak in edits. Today is five things only: "You have N edits ready" (ready + needs_review flat), the Do-This-First card inline with its number and a Make-this-change link, the clicks scoreboard (legend in sentences), the last proven win, and one Update-data button; the war-room list, answers-collected chip, topics counter and research-pass line are deleted. Changes is ONE flat ranked list (39 open at landing: 1 Proven, 2 Early evidence, 1 new-page brief, the rest My best guess), every card the full before-and-after with Copy button, chips, competitor line off its own receipt, set-aside undo, and mark-implemented; Researching and Watching are one closed drawer of one-line rows (no "holding", no ISO dates, a queued page never contradicts its own card). Supply floor: a declining page with no read results page gets one concrete best-guess title edit off its own GSC numbers (MAX_SUGGESTIONS 150); new-page briefs draft at 2+ read winners with no own-page overlap (DEFAULT_MAX_DRAFTS 5), the validator holds one structural count (the page's own section count, source packs exempt as code-built evidence), and a stored adjudicator capUsd below the code default is floored to it (a stale $10 cap was starving the month at $9.98 despite the $30 default). Measuring rows carry the ledger proof clause and changeSentence renders every action slug as a sentence. Readback runs today-first at 100 percent checked; `gsc_page_signals_v1`/`gsc_decay_v1` carry function-scoped work_mem 64MB (211ms warm).
