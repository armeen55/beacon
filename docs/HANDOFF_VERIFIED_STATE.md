...
- Verified source and production on 2026-09-12: `87e2f60cdf203f979dd04f9b50e003bc50b76120`, deployment `dpl_GYdn1oBSak6FvMkykCuhtnsd2wau` READY. Production `/api/version` HTTP 200 reports that exact SHA, `ref: main` and deployment ID; login HTTP 200 and anonymous Changes HTTP 307 to login. Copy-bearing shipment verification now reads complete recorded wording in main content rather than fuzzy headings, opening prefixes, pooled metadata/raw chrome, or new-page word counts. A heading alone cannot confirm section copy; new-page records without applied copy are unanswerable before a fetch. Opening copy must occur at the opening; section-removal absence stays unknown on unread/partial bodies. Held text needs an explicit current/sample-only version and a dated capture not preceding implementation; superseded/unknown versions cannot certify current delivery. HTML block boundaries/entities/inline formatting and Unicode are retained without discarding factual symbols. Prior verified answered-HTML FAQ projection and schema published-content authority repair remain. Frozen-source gate passed all stages: 2,040 tests passed, 23 existing skipped, 89 files; guard/lint/typecheck/native-time production build passed, 21 existing lint warnings. Vitest alone used a temporary steadily ticking clock shifted two hours earlier to keep same-day scenarios within their reporting day; no source/hosted clock or environment setting changed. The first unshifted gate failed four midnight-continuation scenarios; three reproduced identically with the prior production verifier and passed within one operator day. Cross-day paid-work continuity and the harness's uncontrolled real-clock assumption remain unqualified, not waived. An intermediate all-process audit clock passed the tests but broke Next Date instrumentation at build; the final gate restricted that clock to Vitest and built with native time. Production/test/combined LOC: 74,422/20,523/94,945; production/combined ceilings ratcheted down ten lines, tests unchanged. This source step: production +24/-34, tests +14/-14; no new public API, routes, dependencies, domains or records, exports unchanged at 1,383. Resources: $0 OpenAI/DataForSEO calls/spend, zero hosted database requests/writes; focused local sweeps and in-memory reproductions, three gate attempts (one complete successful gate, one test-stage failure, one audit-clock build failure), two local build attempts, one Git-triggered production deploy and three public smoke HTTP requests. No funded drive, authenticated walk, cron/environment change, migration or stored-data deletion. This is a delivery-proof replacement, NOT full waterfall readiness or future model-quality certification. Reproduced unresolved chain: changing ImageObject caption/contentUrl leaves schema_hash unchanged; changing a visible FAQ answer leaves faq_hash unchanged. Full live JSON-LD graphs are not retained in PageSnapshot or its repository projection, and schema/FAQ hashes have no Decision refresh consumers; jobEvidenceHash carries schema type names, not values. AtomicEditDraft permits title/meta/h1/answer_block/section, not schema, and new-page assembly explicitly returns schemaTypes: [] with no schema component. Existing draft duplicate detection compares types rather than graph identities. Current FAQ-rich-result guidance is stale against Google May 2026 deprecation. New actual pure reproductions: a partially_verified bundle with an unverified section still contributes one positive treatment sample; the kernel used by winner-example selection returns won even when the verification receipt is blocked, given positive stored metric windows. Measurement admission and treatment summaries accept partially_verified without restricting the attributed treatment to the verified subset. ShipmentVerification stores time/kind/state but no checker-contract version or inspected page hash; historical heading-only receipts cannot be equated with new full-copy checks. Exact placement is also not retained on applied component records. Learning eligibility/provenance and historical receipt/measurement reconciliation remain required; nothing historical was rewritten. NOT CERTIFIED: complete content-linked schema regeneration and graph-aware validation; sustained live editorial quality and operator delivery across every edit family; HTML FAQ main-content/visibility qualification, all-reader metadata/entity authority, adequate long-page reads and acquisition-debt resolution, semantic entailment and injection resistance; historical fact-bank and paid partial/Ready reconciliation; body/model-sensitive durable identity, stale promise authority, concurrent write recency, acquisition-mode remediation, cross-process cache writers, history-query scaling/retention, atomic metric windows, watermark/outage semantics and complete verification validity. Hosted SQL definitions/applied migrations/RLS/advisors/exact cron remain unverified after management HTTP 401. Last inspected cron was active every ten minutes; billing failure is not a global pause. Last inspected GSC finalized through Sep 9 and GA4 page traffic was current; separate GA4 totals/referrals and legacy technical scans were stale. Manual publishing and canonical AI recurrence/five-page SEO comparison remain.
