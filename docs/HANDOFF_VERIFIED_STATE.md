...
- The rendered-page transport now saves the paid Instant Pages task and uses its free task-ID Raw HTML read to recover one bounded, corroborated page. Cache replay and mismatch/charge holds passed local tests. Its hosted readback and post-JavaScript fidelity are not yet established; the current paid ceiling remains $0.
- Manual implementation intent names the selected version and components; a concurrent redraft cannot mark the newer proposal done. Verification requires co-captured, matching structure and JSON-LD. External title, meta, and H1 edits now use the canonical Shipment writer, stable event IDs, and timezone-qualified implementation instants; local tests passed, while deployment and authenticated behavior are pending. A Shipment written just before a concurrent redraft can still be orphaned; atomic recording remains open.
