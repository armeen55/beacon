...
- Verified source and production on 2026-09-12: `35dcf814b82e9b426a0f2f4b9579f9ad641d8889`, deployment `dpl_DE5yhmf49Z7ZAdmuaWqdQh6d6tk4` READY. Production `/api/version` HTTP 200 reports that exact SHA on `main`; login HTTP 200 and anonymous Changes HTTP 307 to login. Model-call reuse now fingerprints tenant, prompt, requested model, server schema and grounded inputs; cached and fresh values share schema/content validation, current source-authority stamping and the existing answer sanity check. Valid banked work can be reused with OpenAI credentials off. Invalid/foreign records or unavailable cache/history return zero-call failures, never paid cache misses. Independent primary-key Supabase upserts replace the whole-blob writer, process queue and cache mirror; writes require acknowledgment and missing acknowledgments are logged, without claiming the reuse cache landed. Reads never write or prune stored entries. Historical tenant blobs remain read-only; unknown model provenance is not blindly promoted. Focused replay: 65 cases across five suites, including independent concurrent entries, owner stamping, failed acknowledgments, outage refusal, historical bank preservation and zero-call invalid reuse. Hosted read-only Data API queries succeed; existing primary-key and tenant-history indexes serve the query shapes, RLS is enabled with public deny, and service SELECT/INSERT/UPDATE privileges are present. Full gate: 2,045 passed, 24 skipped, 89 files; build/typecheck/foundation passed, 21 existing lint warnings. Production/test/combined LOC: 74,506/20,534/95,040; ceilings ratcheted down, no new routes/dependencies/exports. Cache slice: $0 provider spend, eight read-only SQL requests and two Data API reads, one Git-triggered deploy; no funded drive, cron/environment change, migration or stored-data deletion. Prior prospective facts retain explicit tenant/topic/basis scopes and exact proposition research; paid briefs/pieces survive source shortages. Canonical 28-day AI recurrence, five-page SEO acquisition, owned passage handoffs, source-bound packets, copy-bound targeted page repair and source-validity measurement remain. NOT CERTIFIED: authenticated production journey, future model editorial quality or sustained live waterfall; task-adequate competitor/section information and full edit-family quality; historical owned-scoped fact-bank reconciliation, acquisition-mode whole-review objections and canonical-validator remediation; exact requested competitor-page acquisition, newest-SERP winner-debt reconciliation, live cross-process cache-writer exercise or history-query scaling/retention; atomic metric-window consistency, source-watermark outage semantics and complete verification validity. Last inspected cron remains active every ten minutes; billing failure is not a global pause. GSC was finalized through Sep 9 and GA4 page traffic current; separate GA4 totals/referrals and legacy technical scans were stale.
