...
- Verified source and production on 2026-09-13: `500c0e637a50d105fd8ec1ccfb3c6deff3555359`, pushed origin/main; deployment `dpl_DCLEeZWSbNB71e6WofDHo4F2c4UK` READY. Production /api/version HTTP 200 returned this exact SHA, main and deployment ID; /login HTTP 200; anonymous /changes HTTP 307 to /login?next=%2Fchanges. Owned coverage no longer calls a title, word count or winning-page opening a held owned body: it requires the canonical targeted reader's current, URL-matched, hash-bound capture with actual vocabulary and explicit complete/partial scope. Freshness uses the shared owned-page matrix; samples, stale-known-good/blank/unconfirmed versions, missing hashes/content, wrong addresses and expired captures remain owned_content debt. A confirmed empty held body is complete and can prove absence; an unqualified blank stays unknown. Capture dates prefer the actual held body rather than projected metadata. The first release `2fb6df23636b825ce96cb2becb0b7f2f74623f95` deployed READY (`dpl_HvvMoBGs823Q4LJePcDcuhNdgU8Q`) and its exact version SHA was verified, but a new four-contender falsifier returned owned_content with no ownedUrl after three reads. Restarting would repeat those same three and never qualify the fourth. The ONE repair above replaces page-row accounting with three comparison-contender-complete stored-read frontiers per pass, each bounded by the existing five-candidate mapper and canonical paged reader; URL reads are reused across topics. Every unresolved strong candidate is named even when no read slot remains. The actual four-page seam advances all four from banked bodies, including on a fresh repeat pass; it is not a live customer reconciliation. Title-only/800-word metadata falsifiers through the actual mapper retain the exact page but correctly report bodyHeld=false. Full repair gate: 2,041 passed, 15 skipped, 90 files, successful production build, 21 existing lint warnings and the existing future Vitest hoist warning. First release gate: 2,040 passed, 15 skipped, successful build; focused checks exposed and corrected fixture-clock and newly-held-opening assumptions. Production +20/-20 and tests +33/-33 across both source commits: 73,833/20,502/94,335 counted LOC, ceilings unchanged; exports 1,383, routes/domains/dependencies/public facades unchanged. Disabled retired packet-format assertions and redundant commentary were removed; net zero is not an architecture/performance claim. Task resources: $0 task-initiated OpenAI/DataForSEO calls/spend; zero hosted DB reads/writes/DDL/cron/environment/billing changes; two full test sweeps/local builds and two Git-triggered Vercel deployments (initial plus one falsified-defect repair), zero CI dispatches, five deployment-metadata reads and four public smoke requests. This post-deploy handoff update stays working-tree documentation for the next coherent source release, not a documentation-only redeploy. Prior verified contracts remain: pure atomic FAQ metadata derives from the typed graph instead of inherited prose instructions/eligibility justification; common Ready-save defects survive across every family; wrapper-only conversion retains honest no-model/paid-attempt history; matching associated complete HTML question/answer pairs are required for FAQ approval, and missing/stale/ambiguous capture stays page_source debt through replay/promotion. Qualified pure-FAQ Answer.text mismatches reconcile without a model while retaining identities/unrelated properties/independent objections; unchanged replay saves nothing. Shared Evidence-owned Schema.org interpretation handles standard contexts, nested/unnamed @graph, cross-script @id, linked Question/Answer values, IRIs/multiplicity/@list and fails uncertified for malformed/custom/expansion-dependent/conflicting graphs. Document-order h1-h6 questions/details through builder wrappers retain parent/nested associations and content boundaries; full held answers preserve incomplete labels within unchanged budgets. Native HTML/owned-reader seams are not rendered CSS/JavaScript certification or a live backfill. Targeted snapshot reads use explicit tenant/identities, newest-plus-trusted selection, bounded bytes/rows and typed failed-chunk unknowns; lean newest-500 inventory completeness remains open as audited above. Exact before/mutation/outer-heading, distinct replacement/addition/adaptive tasks and first-party depth instructions remain, but the body_edit prompt id still requests kind atomic_edit under its existing 2,000-character before/after schema. Actual safeParse accepted short copy and refused a 2,340-character after only at that limit: naming the prompt did not create a body-specific output contract or prove quality. Role-labelled owned/competitor packets, material revision identity, current-checker applied-unit learning, full-ledger overlap honesty, explicit-tenant examples, measured-zero retention and paid new-page brief/piece reuse remain implemented. OPEN / NOT CERTIFIED: live raw graph persistence (schema_capture nullable-column approval pending; no DDL), content-linked schema regeneration/identity-managed replacement/full live before-block qualification/full graph semantics/new-page schema; sustained first-party copy, complete question/heading/context and operator-ready delivery across EVERY family and restored paid pages; exact-query SEO and recurring-cited AEO top-five usefulness/evidence adequacy/query-sensitive prompt/cache identity; weak-only owned shortlist rows are still excluded from acquisition/comparison contenders, and their absence/duplicate implications remain unqualified; owned/competitor long-page handoffs/metadata authority/reading modes; schema presence currently inferred from entityNames in winning-pattern (actual valid unnamed FAQ retains FAQPage/Question/Answer types but reports hasSchema=false; malformed/unobserved absence is also flattened); semantic entailment/injection resistance/raw vocabulary fallback/fact-bank history and assignment half-token matching (actual positive assignment matched a negated checked sentence, not asserted as final Ready acceptance); acquisition debt ownership/actual settlement/read-mode remediation/cross-process recency/full-ledger scale and retention/historical FAQ/fact-bank/Ready/paid-partial reconciliation/stale promise authority. Nearly-empty rendered reads still fail to persist their one-look settlement and provider-only captures retain unobserved empty FAQ/schema hashes. Seven-day refusal expiry still resets attempts without requiring changed evidence here; downstream repeat-spend implications remain unqualified. Exact applied placement is not retained; partial historical receipt decoding still clears verification_needed; historical admission/reconciliation/comparison uniqueness/atomic raw 7/14/28 windows/source watermarks/outage missing-versus-zero/Today frozen metrics/full verification validity remain open. AI wording still mixes newest-answer counts with recurrence days. Complete hosted SQL/migrations/RLS/security/performance/database-URL reconciliation/authenticated production UI/harness cross-day continuity/configured-tenant and saved-revision alignment remain unverified. Last inspected cron was active every ten minutes; OpenAI billing failure is NOT a global DataForSEO pause. GSC finalized through Sep 9; GA4 page traffic was current while separate totals/referrals and legacy technical scans were stale. No customer copy was generated or provider drive initiated here. Publishing stays manual. The whole waterfall rebuild is ACTIVE, NOT full-run-ready.
