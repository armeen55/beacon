...
- Deployed 2026-09-05, verified at `/api/version` as `4a7fc9cd`: the research-to-useful-change loop, rounds one to two and a half. One editorial standard per treatment (summary, missing answer, restructuring, correction, internal link, structured data, repositioning) is chosen once from the persisted assignment and read by every door; a search is diagnosed against the page's own passages (answered, scattered, false promise, missing, unknown capture), a function word is never a topic, a relational word the searcher used must be matched by the text that claims to answer, and the demand gap is the largest search the page does not answer; page understanding stores the title's promise, the missing capability and what the page sells behind the content fingerprint; the canon reads the packet before asking a person; provenance is typed; the validator and the technical findings speak in Beacon voice. The runtime prepares an account once per drive, spends money instead of reserving it, resumes work it funded and never reached, demotes work unreached three drives running, writes every acquisition on the run row with the row and step it unlocks and whether the purchase could be used, keeps a run's receipts as its own, reclaims an abandoned run, and records a connector that would not sync on the pass receipt; funding, ranking and explanation read one scoring context that carries the account's stated goal; the $0 replay writes every refusal it decides onto the row. Not yet done: the missing-answer consumer still requires a confirmed reading where an additive answer should stand on one authoritative publisher, so the largest stalled need now settles terminal instead of being written; the assignment that files an all-unsupported answer as research; intent comparison before learning from ranked pages; the stale-connector and waiting-on sentences on a surface; the operator loop's recording, retry and verification rules; Results answering the seven questions.
