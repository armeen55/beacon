...
- Verified source and production on 2026-09-12: `2202700dcd27815b80f6936dbc9140bc5a698506`, deployment `dpl_E4YVs9MxBPfwSkK6rJ2fW1eVMmJG` READY. Production `/api/version` HTTP 200 reports that exact SHA, `ref: main` and deployment ID; login HTTP 200 and anonymous Changes HTTP 307 to login. Schema validation now distinguishes the target page text from researched support facts: a supported but unpublished FAQ answer is rejected, while the same genuinely published answer passes; researched JSON-LD cannot invent a live duplicate. Counterexamples run through the canonical validator; historical stored Ready rows have NOT been reconciled by this task. Prior verified evidence-handoff repair remains: one answered-HTML FAQ projection feeds owned bodies, factual packets, lean counts, fresh research extracts and crawl facts; raw markup-only, unknown-origin and unanswered entries remain history but cannot count as answered visible content there. Packet assembly retains the visible answer excerpt. Held-word/passages scope excludes heading/card duplication; markup entity names cannot independently establish visible presence. Whole-page absence and commissioning scope require complete/current bodies; missing version remains unknown. Positive competitor observations survive partial captures as source-bound research candidates, never whole-page absence or factual authority; incomplete, unshown or wrong-page empty comparisons remain unread. Competitor/atomic preparation versions v4/v8, diagnosis/AEO v4/v3, headed delivery, explicit replacement/addition scope (writer wc21), full material AEO body/scope/version identity, timestamp-only reuse, acquisition acknowledgement and recorded JSON-LD property verification remain. Frozen-source full gate: 2,040 passed, 23 skipped, 89 files; foundation/lint/typecheck/build passed, 21 existing lint warnings. Production/test/combined LOC unchanged: 74,432/20,523/94,955, exports 1,383. This source step: production +3/-3, tests +1/-1; no new public API, routes, dependencies, domains or records, ceilings unchanged. Resources: $0 OpenAI/DataForSEO calls/spend, zero hosted database requests/writes; 144 focused tests, one full gate/local build, one Git-triggered production deploy and three public smoke HTTP requests. Free primary-source documentation reads and in-memory TypeScript reproductions added no files; fetch/write seams in reproductions never reached external services. No funded drive, authenticated walk, cron/environment change, migration or stored-data deletion. This is a schema evidence-authority repair, NOT full waterfall readiness or future model-quality certification. Reproduced unresolved chain: changing ImageObject caption/contentUrl leaves schema_hash unchanged; changing a visible FAQ answer leaves faq_hash unchanged. Full live JSON-LD graphs are not retained in PageSnapshot or its repository projection, and schema/FAQ hashes have no Decision refresh consumers; jobEvidenceHash carries schema type names, not values. AtomicEditDraft permits title/meta/h1/answer_block/section, not schema, and new-page assembly explicitly returns schemaTypes: [] with no schema component. Existing draft duplicate detection compares types rather than graph identities. Current FAQ-rich-result guidance is stale against Google May 2026 deprecation. A separate actual verifyShipment reproduction returns verified for section_add when only its heading is present and all promised substantive copy is absent: copy-aware live verification and historical receipt/measurement reconciliation are required before claiming learning validity. NOT CERTIFIED: complete content-linked schema regeneration and graph-aware validation; sustained live editorial quality and operator delivery across every edit family; HTML FAQ main-content/visibility qualification, all-reader metadata/entity authority, adequate long-page reads and acquisition-debt resolution, semantic entailment and injection resistance; historical fact-bank and paid partial/Ready reconciliation; body/model-sensitive durable identity, stale promise authority, concurrent write recency, acquisition-mode remediation, cross-process cache writers, history-query scaling/retention, atomic metric windows, watermark/outage semantics and complete verification validity. Hosted SQL definitions/applied migrations/RLS/advisors/exact cron remain unverified after management HTTP 401. Last inspected cron was active every ten minutes; billing failure is not a global pause. Last inspected GSC finalized through Sep 9 and GA4 page traffic was current; separate GA4 totals/referrals and legacy technical scans were stale. Manual publishing and canonical AI recurrence/five-page SEO comparison remain.
