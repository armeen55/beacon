...
- **P12 (today) — Sprint 6A.1 COMPLETE.** /recommendations reads `recommended_edits` fresh per request (new `getRecommendedEdits` on the repository, wired in both backends). New `Specific edits (N)` collapsible section in `recommendations-client.tsx` shows action-type badge / display label / current → proposed text / why / evidence / confidence / difficulty per edit. Accept button copy switches to "Accept — track N edits" when edits exist. `acceptRecommendation` action fans out to N changelog entries (one per edit) stamped with `action_type` + `target_element_key` + `source_rec_id`; legacy single-entry path preserved when no edits exist; graceful degrade on read failure. ChangelogEntry type extended with `action_type` + `target_element_key` (matching the P1 migration). 28 new tests (22 wiring + 6 behavioral); **2043 passing**.
- **Sprint 6A.1: COMPLETE end-to-end.** Migrations → registries → extractors → packet → provider interface → deterministic generators → validator → persistence → CLI → UI + Accept fan-out. No LLM, no paid API calls. The full data layer is locked.
- **Next sprint:** Sprint 6A.2 (LLM activation) OR pivot to Sprint 7 (multi-tenant hardening for beta testers). Operator picks.
