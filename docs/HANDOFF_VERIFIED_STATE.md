...
- Production 73,954 lines, tests 12,328, exports 1,320, all at exact ceilings. Causal repair run deployed and verified at `a2edb7f8` (2026-08-17, authenticated prod): the keyword-stuffed funny-farsi title was withdrawn by the boundary itself once the per-query decomposition showed three stories the merged unit hid, a held-position click-rate fall now buys the unit's results page before it may name ctr_snippet (the recovery producer's acquisitions feed researchNeeds with one guaranteed plan slot), a deterministic gate refuses any title that says a topic word twice, the drafting packet hands the editor the six stored passages most about the card's question at 700 characters, cache hits refund attempts, measuring pages are skipped before drafting, fed-back retries accumulate every refusal across three rounds, and carrier grammar (include, means, also) is excluded from a coverage gate that says it is not a vocabulary test. Three gate-passed ready rows render on authenticated /changes (the basic-persian-phrases AI citation section at rank 1 with complete exact copy, the persian-rugs section, the shirt description) and the girl-names ranking_loss section sits ready in the store but surface-deferred behind the account's own unsettled two-page split. An adversarial four-lens workflow panel returned 26 findings (5 blocking) that now form the next program: unit history positions can average a second own page into a false slide, placement anchors carry crawler-glued text, the section judge passed a verbless list fragment, recovery headlines sold lost as win-back (fixed: LOST labeled, only the supported number offered), and Iranopedia's own name glosses are wrong Persian, an operator-owned content defect Beacon amplifies faithfully. Research is ON with the daily $1 cap mechanical.
