...
> 🟢 **Sprint 7 active (2026-04-26):** 7.0 → 7.1 → 7.1a → 7.2 → 7.3 → 7.4 → 7.5a-d → 7.7a → 7.7b → 7.7c → 7.7d → 7.7e → 7.8a → 7.8a.1 → 7.8b plan → 7.8b-0 (store classification) → 7.8b-1 (`dotdata-json` tenant routing) → 7.8b-2-a (shared `resolveDataPath`) → 7.8b-2-b (`json-store.readStore`/`writeStore` async + tenant-aware) → 7.8b-2-c (low-risk caller cascade) → **7.8b-2-d** (deferred sync→async helper cascade: adjudicator-cache/budget/history, entity helpers, milestones, answer-snapshots, co-mention, source-trust, global-patterns, prompt-library + their callers and tests). **Phase 7.8b-2-d COMPLETE.** Typecheck clean. Full vitest **2316 / 2317** (single pre-existing `finding-actions.test.ts:213` baseline failure, unrelated to this work). All `.data` reads/writes now route through `.data/tenants/<slug>/` (per-tenant + singleton) or `.data/global/` (cross-tenant aggregates) with read-only flat-fallback during migration. Active plan: `/Users/armeen/.claude/plans/starting-the-safe-save-sequence-sprightly-treasure.md`. Latest verification: `docs/VERIFICATION_LOG.md` 2026-04-26 entry. Next: **7.8b-2-e** (architectural invariants + any remaining shell-page cascade) when operator approves; then 7.8c (--commit migration run), 7.8d (move flat → `_legacy`, fail-loud on unknown stores), 7.8e (lift seed-data.server top-level await to request-scope).
