...
- **P10 (today)** — Output validation layer at `src/domains/recommendations/specific-edit-validator.ts`. Exports `validateSpecificEdit(edit, packet)` + `validateSpecificEditBundle(bundle, packet)` plus helpers `parseElementTypeFromKey`, `isAdditiveElementKey`, `validateSerializable`. Discriminated `{ ok: true } | { ok: false; field: string; reason: string }` result. 10 check categories: actionType valid + allowed, targetUrl in allowedTargetUrls, targetElement page-level vs element gate, elementKey parseable + in domain + inventory match (or `[new]` for additive), required current/proposed text per registry, evidence refs known, difficulty/confidence enums, risks shape, source/providerName/model/costUsd coherence, JSON-serializability walker. Bundle validator also enforces tenantId/recId/evidenceHash mirror packet + non-negative totalCostUsd. Pure; never mutates inputs. 47 new tests; **all deterministic provider outputs validate clean**. **1996 passing**.
- **Next Sprint 6A.1 step:** Phase 6A.1.11 — persistence layer. Map validated `SpecificEdit[]` → `recommended_edits` rows + dual-write. Map rejected `{ edit, result }` pairs → `llm_rejections` rows (rare today since deterministic output is always valid; matters for Sprint 6A.2 LLM). Persistence is invoked AFTER validation, never on render.
