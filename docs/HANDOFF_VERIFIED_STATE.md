...
- Verified source and production on 2026-09-13: `691311892843720830fd445db5029360e4ab6386`, deployment `dpl_CyDw5zRimb1JVcR3TpVC9piTanQV` READY. Production /api/version HTTP 200 returned this exact SHA, main and deployment ID; /login HTTP 200; anonymous /changes HTTP 307 to /login?next=%2Fchanges. This step replaces the conflicting primary body-copy prompt contract: an exact replacement receives its actual current passage and replacement mode before generation, retains its existing outer heading, and is never briefed as a headed addition. Contextual insertion, headed addition and adaptive delivery have explicit mutation instructions; copy is first-party publishable content, depth follows the diagnosed task rather than a sentence quota, and assignment commentary belongs in metadata. Internal links use contextual insertion, not a new-section instruction. Schema-valid responses for a different assigned field stop before editorial review and the outer corrective writing loop; section remains a valid alias for assigned body copy. Only primary body requests use draft.body_edit v1; unchanged title/meta/H1 prompt identities, configured model, transport, evidence budgets, acceptance standards and review contract remain unchanged. Correction: stored section proposals normalize to answer_block before primary writing and already received its intent/shape clauses; the previous contrary finding was wrong. Actual writer-to-stored-delivery regressions assert Ready status, exact before/complete after/findable replacement placement and no invented outer heading; wrong-field delivery is refused with one atomic writing attempt and no editorial call. These are injected contract proofs, NOT live model-quality or ranking/citation certification. Final frozen-source gate passed: 2,025 tests passed, 23 existing skipped, 90 files; guard/lint/typecheck/build passed, 21 existing lint warnings. Six focused sweeps and one separate typecheck; one earlier full gate was stopped after six failures exposed a shared fixture returning title for body tasks. The fixture now answers the assigned field; its behavioral expectations were not weakened. The final gate retains the native slow-provider/late-work continuation simulation unchanged. Production/test/combined LOC 74,337/20,520/94,857; production and combined ceilings ratcheted down, tests unchanged. This step production +20/-21, tests +8/-8; superseded body clauses and obsolete historical comments replaced, redundant selector-level assertions removed while actual stored-standard/re-read/serving coverage remains. No new routes, dependencies, domains, business records or facade exports; exports remain 1,383. Existing internal structured-request metadata gains an optional registered prompt identity. Resources: $0 OpenAI/DataForSEO calls/spend; zero task database queries/writes, one native local production build, one Git-triggered production deployment and three public smoke requests. One read-only local test-worker inspection confirmed deliberate provider latency, not a deadlock; no timing bypass. No funded drive, authenticated walk, cron/environment change, migration, subscription change or stored-data deletion. Post-deploy handoff update remains working-tree documentation for the next coherent source commit. Prior verified captured-answer handoff remains: owned/competitor full captured FAQ answers, labelled capture scope, evidence selected for the reader task within unchanged budgets, and shared role-labelled writer/evaluator packets. Metadata and observed-source candidates are not factual support or absence evidence; captured markup does not prove rendered visibility or semantic FAQ association. Prior material revision identity, canonical applied-unit/current-checker learning qualification, full-ledger overlap honesty, tenant-explicit requalified writer examples, measured-zero retention and paid new-page brief/piece reuse remain; JSON-LD array order is conservative, not full semantic graph canonicalization. OPEN: full live JSON-LD graphs are not retained in PageSnapshot/projection; content-linked replacement schema is not generated; new-page assembly explicitly derives no schema component; duplicate detection compares types, not graph identities. Schema rows already have separate canonical JSON-LD validation and prose-row conversion; absence of a schema field in AtomicEditDraft alone does NOT diagnose their actual generation path. Rendered provider reads still give empty FAQ/schema hashes for unobserved fields. Raw vocabulary fallback, full model/query-sensitive identity, useful complete long-page/query-specific handoffs, all metadata/entity authority and historical FAQ provenance remain unqualified. The AI card explanation currently combines a newest-answer count with historical-window recurrence days, producing incompatible denominator wording such as three answers on twenty-two days; these units must be separated. Historical configured-tenant alignment/saved revisions remain unverified after the prior tenant-filtered read returned zero rows. Exact applied-component placement is not retained; historical measurement admission still accepts partial verification and fromRow clears verification_needed for partial receipts. Comparison receipt identity/uniqueness, raw metric-window provenance, historical receipt/admission reconciliation and Today frozen-metric wording remain unqualified. NOT CERTIFIED: complete content-linked schema regeneration/graph-aware validation; sustained first-party editorial quality and complete operator delivery across every edit family; exact-query recurring SEO/AEO top-five winner-pattern usefulness; adequate owned/competitor long-page reads; acquisition-debt resolution; semantic entailment/injection resistance; historical fact-bank/paid-partial/Ready reconciliation; stale promise authority; concurrent/cross-process write recency; acquisition-mode remediation; full-ledger scaling/retention; atomic metric windows/watermark/outage semantics; and complete verification validity. Prior harness-clock/cross-day paid-work continuity qualification remains open despite native-time gates passing. Prior management HTTP 401 is superseded by exact-project MCP catalogue reads above; complete hosted reconciliation and authenticated UI remain unverified. Last inspected cron was active every ten minutes; billing failure is not a global pause. Last inspected GSC finalized through Sep 9 and GA4 page traffic was current; separate GA4 totals/referrals and legacy technical scans were stale. Publishing remains manual; canonical AI recurrence and five-page SEO comparison remain the research backbone, not editorial-quality certification. The whole waterfall rebuild remains active, NOT full-run-ready.
