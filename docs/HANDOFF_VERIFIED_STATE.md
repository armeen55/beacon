...
- Verified source and production on 2026-09-13: `7aeebbfaccdaea1ca57f23c6b87b066186792573`, deployment `dpl_73ToHKpXpbsGPEqkC9rwVFCUwRyZ` READY. Production `/api/version` HTTP 200 reports that exact SHA, `ref: main` and deployment ID; login HTTP 200 and anonymous Changes HTTP 307 to login. Learning now requires the canonical applied-unit identity and current checker contract: verification binds page, implementation time, normalized applied copy/components, inspected-evidence hash and every applied component state. Blocked, partially verified, pre-implementation, missing-component, changed-copy and old checker receipts retain history but cannot teach ranking, Results/Today confirmation counts or writer examples. Actual applied subsets teach only their recorded copy, not unapplied proposal text; missing comparison receipts and insufficient receipt counts cannot be bypassed by frozen pins. Whole-ledger overlap invalidates later-confounded cached examples while a completed clean 14-day reading remains valid; measured zeros remain samples. Cached writer examples are tenant-explicit and requalified against the full current ledger on read, fail closed on old/missing identity and cache-read failure, and unchanged harvests make zero writes. Legacy verified receipts may requalify only within the original remaining three checks and buy no SERP call for that requalification; exhausted records never reset. The unused ranking helper and redundant helper-level tests were replaced with actual verification-to-learning-to-writer/customer workflows. Prior full recorded-copy main-content verification, current/dated held-body authority, answered-HTML FAQ projection and schema published-content authority remain. Frozen-source native-time full gate passed: 2,031 tests passed, 23 existing skipped, 90 files; guard/lint/typecheck/build passed, 21 existing lint warnings. Four gate attempts: one test-stage failure on three obsolete positive fixtures, one test-typing failure, one known smoke-fixture failure cancelled early, then one complete passing gate; no assertions waived. Production/test/combined LOC: 74,353/20,521/94,874; ceilings ratcheted down 69/2/71 lines. This source step: production +116/-185, tests +109/-111; no new routes, dependencies, domains, business records or facade API, exports unchanged at 1,383. Resources: $0 OpenAI/DataForSEO calls/spend, zero hosted database requests/writes; focused local sweeps and in-memory reproductions, one native local production build, one Git-triggered production deployment and three public smoke requests. No funded drive, authenticated walk, cron/environment change, migration or stored-data deletion. Post-deploy handoff update remains working-tree documentation for the next coherent source commit. This is a delivery-qualified learning replacement, NOT full waterfall readiness or future model-quality certification. Reproduced unresolved schema/revision chain: changing ImageObject caption/contentUrl leaves schema_hash unchanged; changing visible FAQ answers leaves faq_hash unchanged. Full live JSON-LD graphs are not retained in PageSnapshot or its repository projection; Decision owned-content DTO drops body revision identity, and jobEvidenceHash compares title/word count/schema type names plus winner word/heading counts rather than material copy and graph values. AtomicEditDraft has no schema field; new-page assembly has no derived schema component; duplicate detection compares types rather than graph identities. FAQ-rich-result advice remains stale against Google May 2026 deprecation. Exact placement is not retained on applied component records. Historical measurement admission still accepts partial verification and fromRow still clears verification_needed for partial receipts; this task restricts teaching, not those historical observations. Comparison receipt identity/uniqueness, raw window provenance, historical receipt/admission reconciliation and Today frozen-metric wording remain unqualified. NOT CERTIFIED: complete content-linked schema regeneration and graph-aware validation; sustained live editorial quality and complete operator delivery across every edit family; HTML FAQ main-content/visibility qualification, all-reader metadata/entity authority, adequate long-page reads and acquisition-debt resolution, semantic entailment and injection resistance; historical fact-bank and paid partial/Ready reconciliation; body/model-sensitive durable identity, stale promise authority, concurrent write recency, acquisition-mode remediation, cross-process cache writers, full-ledger scaling/retention, atomic metric windows, watermark/outage semantics and complete verification validity. Prior same-day harness-clock/cross-day paid-work continuity qualification remains open despite this native-time gate passing. Hosted SQL definitions/applied migrations/RLS/advisors/exact cron remain unverified after management HTTP 401. Last inspected cron was active every ten minutes; billing failure is not a global pause. Last inspected GSC finalized through Sep 9 and GA4 page traffic was current; separate GA4 totals/referrals and legacy technical scans were stale. Publishing remains manual; canonical AI recurrence/five-page SEO comparison remains the research backbone, not editorial-quality certification.
