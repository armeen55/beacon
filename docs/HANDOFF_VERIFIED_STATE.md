...
- Verified source and production on 2026-09-12: `a21dbbcb5def8f132b11a29b48eafb9a611fc9ff`, deployment `dpl_H4EYfUnTCX7NhwwQbH1aEwQm8tJc` READY. Production `/api/version` HTTP 200 reports that exact SHA on `main`; login HTTP 200 and anonymous Changes HTTP 307 to login. Query-backed winner identity, acquisition readback, banked-content preservation and task-focused section/table comparison retain their prior verified contracts. Existing/new-page writers and reviewers share captured competitor material, provenance and completeness limits, explicitly research rather than claim authority. Comparison reader v2 binds quotations exclusively to the exact sanitized source supplied, not candidate quotations; retains source headings/entities as typed topics, including comma-containing entity names; supplies task focus and capture-selection limits; and reports rejected-only readings as unread rather than a valid empty comparison. Existing sanitation and job receipt/cache-refund accounting remain; semantic entailment, whole-page absence proof and full prompt-injection resistance are not claimed. Frozen-source full gate: 2,049 passed, 24 skipped, 89 files; foundation/lint/typecheck/build passed, 21 existing lint warnings. Two redundant comparison test cases were merged without deleting their assertions; adversarial cases cover suggestion self-authentication, stripped instruction quotations, foreign pages, topic loss and rejected-versus-empty readings. Production/test/combined LOC: 74,485/20,532/95,017; production/combined ceilings ratcheted down, no new routes/dependencies/domains/exports. This source slice: $0 provider spend, no database requests, two gate attempts (first stopped during tests after review found comma-identity loss; second completed), one local production build and one Git-triggered production deploy; no funded drive, cron/environment change, migration or stored-data deletion. Prior narrow audit used one management HTTP 401 and three Data API HTTP 200: scoped saved rugs/Takbir/slang drafts were held for review, while a Parthian link remained stored ready with differing anchor/copy spelling; dated Farsi-numbers/Safavid captures showed no schema/ImageObject only. NOT CERTIFIED: authenticated production journey, future model editorial quality or sustained live waterfall; task-adequate owned-page reading and whole-page absence proof; page-job missing-capability inference from metadata/600-character opening, body/prompt-sensitive durable fingerprint and save-acknowledged read state; reader-task shape/capacity, complete operator heading/copy/mutation contract and all edit-family quality; visible-content/schema dependency refresh and exact schema-value verification (current schema_hash fingerprints types only), current rich-result guidance; historical owned-scoped fact-bank reconciliation, acquisition-mode whole-review objections and canonical-validator remediation; live cross-process cache-writer exercise or history-query scaling/retention; atomic metric-window consistency, source-watermark outage semantics and complete verification validity. Prior cache reuse, tenant/topic/basis-scoped factual work, paid-piece preservation, canonical AI recurrence, five-page SEO comparison and manual publishing remain. Last inspected cron remains active every ten minutes; billing failure is not a global pause. GSC was finalized through Sep 9 and GA4 page traffic current at last inspection; separate GA4 totals/referrals and legacy technical scans were stale.
