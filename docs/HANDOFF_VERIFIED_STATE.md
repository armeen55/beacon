...
- Production 74,861 lines, tests 12,547, exports 1,343, all at exact ceilings. Source-research repair deployed at `97200f70` (2026-08-18): THE SCHEMA IS A CHECKED-IN MIGRATION (2026-08-18_page_source_facts_and_fact_check_phase.sql), idempotent against the table that was created by hand, preserving all 192 rows, and it widens research_runs_current_phase_check which would have rejected every fact_check write. Production no longer depends on an unrecorded dashboard mutation. ONE CLAIM PER UNIT: extract claims for one page version, take the next claim that is not current, search with a query derived from the CLAIM TYPE (word meaning, date, quantity, definition, specification, entity, geography), FETCH the candidate source's own text, judge only against those passages, bank one row, advance a cursor. `confirmed` requires a quote the judge can point at inside a fetched authoritative passage; a source found but unread banks `unsupported` and proposes nothing. Statement identity is the normalized claim plus its locator, so two claims about one subject coexist. A check is current only when page, content hash, exact wording and basis all hold, so a changed page goes STALE rather than being called correct. One absolute deadline governs the unit and no call starts that cannot be finished and persisted. Page selection rotates on coverage age, so page two is reachable. Nine behavioral tests cover the unit, including the downgrade of an unquotable verdict and the refusal to invent a replacement after a failed fetch. NOT YET TRUE, do not claim otherwise: the phase has never executed live, retireCorrectedFacts still has no runtime caller so retirement is unproven, the correction detail still shows 40 of 102 rather than all of them grouped, and AEO drafting still does not consume source facts. Zero Ready remains the boundary's defended count. Research is ON with the daily $1 cap mechanical.
