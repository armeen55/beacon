...
- Production 73,881 lines, tests 12,328, exports 1,319, all at exact ceilings. Causal boundary run deployed and verified at `fc4f74d4` (2026-08-17, authenticated prod): a card exists only when its diagnosed cause is treatable and its treatment addresses that cause (CAUSE_LEVERS gates minting, ranking claims proven recoverable only for treatable causes, an untreatable cause or a lever mismatch argues from the research lane), the monthly archive carries impressions-weighted position so the recovery producer decomposes each collapse off its own two windows (a slipped position is a ranking loss owing content, a held position with a collapsed click rate is a ctr_snippet owing the title, an unseparable decline claims no cause), historical loss is labeled LOST with only the current window's shortfall claimed recoverable, AI absence cards carry ai_citation_gap with a falsifier and land on the canonical unit's own earning pages, the zero-dollar sweep is no longer shielded by the paid-draft cap (35 boundary-orphaned title cards withdrawn, the cheetah contradiction dead), the editor drafts under fed-back retries told each refusal verbatim on gpt-5.4-mini, and ONE Ready Google Change is live end to end: /funny-farsi-phrases ctr_snippet title (115 to 53 clicks a month measured, position 2.9 to 1.8, click rate down over 40 percent, 39 a month claimed recoverable), rendered rank 1 on authenticated /changes as the one finished change. The AI citation cards carry real diagnoses and exact briefs but no gate-passing section copy yet: the claim-coverage gate refuses words the 400-character evidence slices do not carry, so richer body packets are the named next step, not a softer gate. Research is ON with the daily $1 cap mechanical.
