...
- Verified source and production on 2026-09-13: `8a3cc9ad38a155738801bc887b013e067d63e9ce`, deployment `dpl_H8G8ubbV72Zw6o3SFtMPoi9ha8XR` READY. Production `/api/version` HTTP 200 reports that exact SHA, `ref: main` and deployment ID; login HTTP 200 and anonymous Changes HTTP 307 to login. Learning now requires the canonical applied-unit identity and current checker contract: verification binds page, implementation time, normalized applied copy/components, inspected-evidence hash and every applied component state. Blocked, partially verified, pre-implementation, missing-component, changed-copy and old checker receipts retain history but cannot teach ranking, Results/Today confirmation counts or writer examples. Actual applied subsets teach only their recorded copy, not unapplied proposal text; missing comparison receipts and insufficient receipt counts cannot be bypassed by frozen pins. Whole-ledger overlap invalidates later-confounded cached examples while a completed clean 14-day reading remains valid; measured zeros remain samples. Cached writer examples are tenant-explicit and requalified against the full current ledger on read, fail closed on old/missing identity and cache-read failure, and unchanged harvests make zero writes. Legacy verified receipts may requalify only within the original remaining three checks and buy no SERP call for that requalification; exhausted records never reset. The unused ranking helper and redundant helper-level tests were replaced with actual verification-to-learning-to-writer/customer workflows. Prior full recorded-copy main-content verification, current/dated held-body authority, answered-HTML FAQ projection and schema published-content authority remain. Material revision identity now survives the existing PageSnapshot repository projection into owned-page evidence and both account/job hashes. Same-sized body changes, complete FAQ-answer changes beyond the retained 200-character excerpts, and full parsed JSON-LD value changes invalidate the affected job; unchanged capture clocks, reordered object keys/schema blocks, fresh-versus-cached competitor decoding and unrelated page drift do not. Competitor cached sections survive decoding, and unreported h3/schema fields remain absent rather than fabricated empty observations. Actual capture-to-store-to-loader-to-job and existing proposal-save workflows reproduce these promises; paid new-page briefs still resume without a second brief call. JSON-LD array order remains conservative, not full semantic graph canonicalization. Frozen-source native-time full gate passed: 2,026 tests passed, 23 existing skipped, 90 files; guard/lint/typecheck/build passed, 21 existing lint warnings. One complete passing gate, four focused sweeps and three separate typechecks; no assertions waived. Production/test/combined LOC: 74,351/20,521/94,872; production/combined ceilings ratcheted down two lines, tests unchanged. This source step: production +39/-41, tests +42/-42; no new routes, dependencies, domains, business records or facade API, exports unchanged at 1,383; existing OwnedPageContent gains optional canonical revision fields. Resources: $0 OpenAI/DataForSEO calls/spend; one bounded tenant-filtered Data API read against the configured production project accepted the four existing hash columns (HTTP 200) but returned zero rows, so saved revision data and configured tenant alignment remain unverified. Zero hosted database writes, one native local production build, one Git-triggered production deployment and three public smoke requests. No funded drive, authenticated walk, cron/environment change, migration or stored-data deletion. Post-deploy handoff update remains working-tree documentation for the next coherent source commit. This is a material-revision and banked-evidence replacement, NOT full waterfall readiness or future model-quality certification. Full live JSON-LD graphs are still not retained in PageSnapshot or its repository projection, and content-linked replacement schema is not generated by this identity repair. Rendered provider reads still carry empty FAQ/schema hashes when those fields were not observed; long-page capture completeness and metadata/entity authority need qualification. AtomicEditDraft has no schema field; new-page assembly has no derived schema component; duplicate detection compares types rather than graph identities. FAQ-rich-result advice remains stale against Google May 2026 deprecation. Exact placement is not retained on applied component records. Historical measurement admission still accepts partial verification and fromRow still clears verification_needed for partial receipts; this task restricts teaching, not those historical observations. Comparison receipt identity/uniqueness, raw window provenance, historical receipt/admission reconciliation and Today frozen-metric wording remain unqualified. NOT CERTIFIED: complete content-linked schema regeneration and graph-aware validation; sustained live editorial quality and complete operator delivery across every edit family; HTML FAQ main-content/visibility qualification, all-reader metadata/entity authority, adequate long-page reads and acquisition-debt resolution, semantic entailment and injection resistance; historical fact-bank and paid partial/Ready reconciliation; full model/query-sensitive durable identity, stale promise authority, concurrent write recency, acquisition-mode remediation, cross-process cache writers, full-ledger scaling/retention, atomic metric windows, watermark/outage semantics and complete verification validity. Prior same-day harness-clock/cross-day paid-work continuity qualification remains open despite this native-time gate passing. Hosted SQL definitions/applied migrations/RLS/advisors/exact cron remain unverified after management HTTP 401. Last inspected cron was active every ten minutes; billing failure is not a global pause. Last inspected GSC finalized through Sep 9 and GA4 page traffic was current; separate GA4 totals/referrals and legacy technical scans were stale. Publishing remains manual; canonical AI recurrence/five-page SEO comparison remains the research backbone, not editorial-quality certification.
