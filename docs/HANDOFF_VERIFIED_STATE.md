...
- Deployed 2026-09-05, verified at `/api/version` as `32bf6daf`: the research-to-useful-change loop, rounds one to seven. One editorial standard per treatment is chosen once from the assignment, which survives a zero-cost re-mint, and read by every door; a search is diagnosed against the page's own passages, never by token absence; page understanding stores the title's promise, the missing capability and what the page sells; the evidence bar is proportional to what the treatment risks, each claim is authorized by its own cited support, a usable reading is never bought again, a checked fact reaches the writer as its own sentence with the search behind it, and a reader's answer never opens with the search phrase; a page is judged against its whole template family from the inventory or the rule refuses to decide, and a read that did not happen is never a fault. The runtime prepares an account once per drive, spends money instead of reserving it, reserves the walk's box before any reading is bought and buys a reading ahead of the walk only for the ranking it serves, funds only what its box can begin and says why on the run row, walks unreached work as a tie-break under worth, keeps a run's receipts as its own, reclaims an abandoned run, writes every acquisition and every deferral with the row and step it unlocks, and records a stale connector on the pass receipt; a verification reading stamps itself after its fetch, stops at its cap, reads each address once per pass, and a record closed by a rule that no longer holds reopens once; funding, ranking and explanation read one scoring context. The operator loop keeps both the prepared and the applied wording, never loses a press and answers a bulk press per card; Results prints raw movement beside the adjusted comparison and says which of the two happened and what the account's one closed reading taught the queue; Today names the last change's outcome; the new-page pathway is proved end to end on injected evidence. Not yet done: a substantive body answer reaching Ready through the unattended walk, which the next unattended ticks decide now that the walk keeps its box; and the day's run reaching its surface publication, which the AI answers phase has held since 09:03Z.
