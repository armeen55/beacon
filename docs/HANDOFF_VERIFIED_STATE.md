...
- Deployed 2026-09-04, verified at `/api/version` as `03bdfdaf`: the incident recovery. On 2026-09-04 16:30Z a cause named for the first time was read as a cause that changed and 31 drafted rows lost their words to their own briefs; the words survived in previousCopy under one receipt. The store now un-does exactly that transition on a row that is still unwritten (the retired words return as the current recommendation, the attempt the incident charged is given back, nothing is stamped Ready, the recovery is idempotent), and the cause comparison is two-way. Producers, the ownership cards included, no longer write an assignment into recommendedChange.after: the copy field holds paste-ready words or one sentence saying they are not written, the assignment lives in the research field the writer reads, and research-only work cannot be stored Ready. Every funded job ends with one typed outcome (a reason is required wherever a receipt is filed; the pass's fault sentence is gone; work never begun keeps its owed ending; already complete, superseded, provider blocked and cost blocked are named), and one row answers to one identity (the collapse producer's audiences fold to the largest measured loss, deterministically). The drafter's owed note is display only: never identity, never a hold, never part of an operator's confirmation. Proven: two zero-cost passes over the 66-row cohort with database hashes, pass one recovering 28 rows and pass two identical before and after (a20db929bf174fe5), zero provider calls; Shipments 149 and the implemented 117 byte-identical throughout. Unattended: the one Supabase job was paused for the recovery and resumed as it was, and seven ticks from 19:30Z to 22:30Z on 2026-09-04 each claimed and released the lease, walked with every funded key ending in a typed reason (not reached 25, evidence required 3, provider blocked 1, deterministic refusal 1, none without a reason), and left the recovered words, the 149 Shipments and the 117 implemented rows untouched. Not yet achieved, stated plainly: the walk reaches one funded card per drive and 25 recovered descriptions wait unreached behind the owed readings inside the same box, Ready stayed at 1, and the second pass of the day ran the paid discovery funnel the first run never reached (keywords, engine pairs, results pages), which is where that day's provider spend went. Earlier deploy 2026-09-04 (ec26130a) verified the Ready-River Closure: the ranked walk gets its turn in every drive, the fact units run behind it, owed readings are bought in rank order, an unchanged row is byte-stable, Ready is a finished state, and Changes says what a card waits on. Earlier deploy 2026-09-04 (58078512) verified the Trusted-Learning Loop: Results receives the canonical Shipment whole and places every row on one evidence rung, the verifier lands every unconfirmed Shipment on a typed cause, the readiness verdict refuses tautology, off-intent answers, superseded taxa, unsupported reconstructions and templated filler, and ranking is one global expected-value order; then f013e9a8 added template-aware page truth (a page's own words are not page-specific when its siblings carry them) and the pass ceiling counting money spent. Earlier deploy 2026-09-04 (d841711f) verified the measurement backbone: every shipment gets a comparison, first readings at 14 days with blind readings barred from teaching, revival reached from the ordinary pass, the recrawl clock, the detectable-lift gate, the Google-is-showing check, one pooled estimate per kind; the Supabase scheduler proven live then with five clean ticks. Earlier deploy 2026-08-25 verified the draft-to-evidence loop chain 5957ed17..3fad056a. Work identity is JOB-SCOPED (`jobEvidenceHash`; the account-wide hash re-minted every row on any page's Google drift, verified live as 627 versions on one row and byte-identical Ready copy across five dispatches after), bundle status is reconciled by the sweep (the flag bundle's stored-ready/rendered-review split closed in production at 03:16Z), refusals carry ONE typed resolution from both producers and mint evidence requirements the runtime executes for ALL FOUR declared kinds (a robots-blocked winner is skipped, verified live: the requirement moved from reddit to web.mit.edu), gates are symmetric (the claim-drift check runs at draft time), and an evidence-waiting candidate ranks behind untried work. Earlier chain (2026-08-21/23): the correction chain 702c112d.., THE CANDIDATE-COMPLETION SPINE. Four live dispatches refused every single draft, and the receipts named why: the workflow was deadlocked, not the writer. **The cursor is the ranking**: sorting a funded-but-unreached candidate to the BACK put /persian-female-first-names, worth 560 recoverable clicks, behind a product page worth 0.22 and a category page worth 0.13, unattempted for three dispatches. Deferral is DELETED; a candidate that was selected and not started is owed FIRST, because only settled work leaves the day's memory. **Grounding is about meaning, not spelling**: two gates demanded every content word of the copy appear literally in the cited evidence, so finished work was refused over "reader", "start", "here", "say", "reason", "looking" and "accessory"; no sentence a person writes can pass one. They are replaced by the check that targets what actually reaches a reader as a false fact: a NAME nothing on file mentions (the card that shipped "Cyrus the Great" to a paying customer is still refused, now by that gate), grounded in the page's stored words and handed-over evidence and NEVER in the writer's own claims, so a fabrication cannot ground itself by being asserted twice. Meaning is judged by the evaluator that reads each claim against the evidence it names. **No universal word count**: the 80-word floor refused a 72-word rewrite, a 69-word flag answer, a 68-word wolf answer and a 44-word phrase answer, and told the writer to pad rather than answer. The band is a sanity bound (30 to 180) and the prompt asks for a complete answer that stops. **A thin page is a reason to get facts, not to abandon the change**: the material floor that refused /funny-farsi-phrases at $0 over "44 words of material", on a page of 1,222 words with real assistant evidence, is deleted. **The surface leads with work**: Today leads with Ready when any exists (it led with research while finished work sat below), and the secondary section is "Future opportunities", not "Beacon is preparing". Ceilings green by compression, none raised: production 75,493/75,605, exports 1,387/1,387, markdown 1,000/1,000; 1,170 tests pass. Research stays ENABLED and the cap is unchanged.
