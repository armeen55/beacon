...
- Deployed 2026-09-05, verified at `/api/version` as `7cc2930f`: the research-to-useful-change loop, rounds one and two. One editorial standard per treatment (summary, missing answer, restructuring, correction, internal link, structured data, repositioning) is chosen once from the persisted assignment and read by every door; a search is diagnosed against the page's own passages (answered, scattered, false promise, missing, or unknown when the capture is incomplete) and never by token absence, a relational word the searcher used must be matched by the text that claims to answer, the canon reads the packet before asking a person to confirm a figure, provenance is typed so a draft never authorizes its own claims, and the validator speaks in Beacon voice. The runtime prepares an account once per drive, spends money instead of reserving it, resumes work it funded and never reached, demotes work unreached three drives running, and records a connector that would not sync on the pass receipt; funding, ranking and explanation read one scoring context, and the $0 replay writes every refusal it decides onto the row as a typed fault, so no unfinished row owes nothing. Not yet done: page understanding's stored promise and missing-capability fields, three of research targeting's four clauses, the missing-answer assignment that files unsupported propositions as research, the stale-connector sentence and the waiting-on sentence on a surface, and the operator loop's recording, retry and verification rules.
