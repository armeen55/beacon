...
- **P7 (today)** — `buildSpecificEditEvidencePacket` pure builder ships at `src/domains/recommendations/specific-edit-evidence.ts`. Composes recommendation + cluster + prompt opportunities + competitor-primary + owned page inventory + page_element_inventory rows into the contract every Specific Edit Generator (deterministic + LLM) will consume. Top-level `clusterId | clusterLabel | clusterKind` (flat). 16-char sha256 evidenceHash; deterministic; changes when meaningful evidence changes (8 distinct change-detection scenarios). allowedTargetUrls owned-only + `needs_new_page` sentinel. Strict serializability (no Date/Map/Set/class instances/functions) + no-route-render-generation guard locked by tests. 54 tests, **1893 passing**.
- **Next Sprint 6A.1 step:** Phase 6A.1.8 — provider-adapter contract `SpecificEditProvider` interface + stub implementations. Still no generators / LLM (those are P9 / 6A.2).
