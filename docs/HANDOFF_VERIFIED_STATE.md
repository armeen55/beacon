...
> 🟢 **Sprint 7 active (2026-04-26):** 7.0 → 7.1 → 7.1a → 7.2 → 7.3 → 7.4 → 7.5a-d → 7.7a-e → 7.8a → 7.8a.1 → 7.8b plan → 7.8b-0 → 7.8b-1 → 7.8b-2-a → 7.8b-2-b → 7.8b-2-c → 7.8b-2-d → 7.8b-2-e → 7.8c (`--commit` migration: 70 files written to `.data/tenants/ritz-builders/` + `.data/global/`) → 7.8c.1 (test-pollution + fresh-data hygiene) → 7.8d-1 (flat fallback removed; unknown stores throw fail-loud; +7 production stores classified that the invariant exposed) → 7.8d-2 (72 flat `.data/*.json` files moved to `.data/_legacy/`) → **7.8d-3** (docs sync). **Phase 7.8d COMPLETE.** Routed dirs are sole runtime truth. `.data/*.json` root is empty locally; `.data/_legacy/` holds the 72 originals (gitignored — local-only state). `.data/_pre-migration-hygiene/co-mention-matrix.flat.apr14.json` and the freshly-rebuilt routed `.data/global/co-mention-matrix.json` (Apr 26 / 13,946 answers) both preserved. Typecheck clean. Full vitest **2330 / 2330** (zero failures). Active plan: `/Users/armeen/.claude/plans/starting-the-safe-save-sequence-sprightly-treasure.md`. Latest verification: `docs/VERIFICATION_LOG.md` 2026-04-26 entries. Next: **7.8e** (lift `seed-data.server.ts` top-level await to request-scope) when operator approves. Rollback for 7.8d-2 file move: `mv .data/_legacy/*.json .data/ && rmdir .data/_legacy`.
