...
- Deployed and exact-SHA verified 2026-09-12: `8c2bd0d6f789c8a1a121ea3f1418e8d71207e25e`, Vercel `dpl_9JfSBycUyBCoo42kBuJEuucBLZmm` READY; production `/api/version` returned HTTP 200, that SHA and `main`. This replaces assigned writer/editor packet drift, separates diagnostic numbers from publishable claim grounding, binds essential editor acceptance to exact assigned/additive work and stored pieces, preserves source/anchor/schema identity, and compares the exact five ranked/cited leaders including uncaptured unknowns. Prior `0f9df347` preserves checked-source handoff and lossless resumed page bundles. Full gate: 2,032 passed, 24 skipped, 89 files, build/typecheck/foundation passed; 21 existing lint warnings. Counted production/test/combined LOC: 74,593/20,546/95,139; ceilings unchanged. Provider spend for these replacements $0; no paid proof, cron/environment change or stored-data deletion. NOT CERTIFIED: authenticated production journey, future model quality, a live waterfall, whole-new-page title/meta/plan acceptance, historical canonical competitor/fanout projection, consistent additive claim-support authorization, model-aware cross-process cache identity, and canonical metric-read/verification validity. Preserving privilege repair applied live as migration `20260912220209_lock_internal_audit_snapshots`: both internal snapshots now have RLS enabled and no PUBLIC/anon/authenticated table privileges (including TRUNCATE), service/admin access retained, all 66/203 rows and aggregate hashes unchanged; security advisor cleared their disabled-RLS errors. Current cron is active every ten minutes; recent HTTP 200 dispatch is not evidence of customer output. GSC finalized through Sep 9 and GA4 page traffic is current, while separate GA4 totals/referral tables and legacy technical scan remain stale.
