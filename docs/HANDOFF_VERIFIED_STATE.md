...
- **P15 (today, 2026-04-25) — `page_element_inventory` populated on production.** New `scripts/run-orchestrated-scan.ts` wrapper called the same scan path the operator's "Scan now" button + GitHub Actions cron use. Scanned 35 owned URLs, wrote 4312 inventory rows to production Supabase (4315 raw → 4312 after defensive dedup of 3 duplicate `(source_snapshot_id, element_key)` pairs). All 13 active extractor types fired (title / meta / canonical / h1 / h2 / h3 / faq_question / faq_answer / schema_type / schema_property / internal_link / city_mention / service_mention). **Zero writes to `recommended_edits` or `changelog_entries`** — both verified at 0 via SQL probe.
- **Phase 6A.1.13 is now unblocked.** Real live-queue stableKey verification can run: `--list` to pick a stableKey, then `--rec-id=<...> --write` via Phase 14's CLI to persist `recommended_edits`, then refresh `https://beacon-bice.vercel.app/recommendations` to confirm the **Specific edits (N)** section appears.
- **Out-of-scope notes from P15:** `page_snapshots` dual-write failed with a `body_paragraph_sample`-column-missing schema-drift error (pre-existing; not Phase 6A.1.x scope). `observation_runs` had 46 of 50 in-file duplicate run_ids (separate file-append bug; not Phase 6A.1.x scope).
