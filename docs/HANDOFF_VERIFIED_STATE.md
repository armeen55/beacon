...
- Verified source and production on 2026-09-12: `474a7f3ceaff74680556725c22f0a9dcf4fa854f`, deployment `dpl_DhyTbcAJ9irncAsp83WqvUiTumUk` READY. Production `/api/version` HTTP 200 reports that exact SHA, `ref: main` and deployment ID; login HTTP 200 and anonymous Changes HTTP 307 to login. The targeted owned-page projection now preserves visible FAQ origin (`html_details`/`html_section`), excludes markup-only/unknown-origin FAQs from its visible-content projection, and filters before its existing item bound. A nonempty held body establishes capture scope; metadata, excerpt counts and blank bodies cannot establish whole-page completeness. The AEO gap reader spends its unchanged aggregate body-plus-FAQ allowance on intact main-content passages, separates owned from credited evidence IDs, and permits absence only when every supplied body passage is shown from a complete current capture. Its identity covers full supplied body text, scope and stale/current state, including unshown changes; a new capture timestamp alone reuses the same material reading. Diagnosis/cache contracts are v4/v3, so predecessors are re-earned without deleting history. Bundle assembly derives its private input from the canonical owned-page type and retains capture version/newest timestamp: stale bodies cannot commission missing-coverage sections while equivalent current bodies can. These are source/behavioral evidence-contract proofs, not global whole-page absence or future model-quality certification. Prior verified reader-task copy preparation, headed operator delivery, explicit replacement/addition scope (writer wc21/v7), task-focused owned/winner comparison, acquisition acknowledgement and recorded JSON-LD value verification remain. Frozen-source full gate: 2,040 passed, 23 skipped, 89 files; foundation/lint/typecheck/build passed, 21 existing lint warnings. Production/test/combined LOC: 74,450/20,523/94,973; production and combined ceilings ratcheted down. This source step: production +31/-47, tests +45/-45; existing FAQ projection gains origin, no new exports/routes/dependencies/domains/records. Resources: $0 OpenAI/DataForSEO calls/spend, zero database requests/writes; two completed full gates/local builds, one failed sweep cancelled before completion, one Git-triggered production deploy and three public HTTP smoke requests. The smoke wrapper initially asserted a nonexistent `branch` property; correct `ref`/deployment assertions were checked against the captured responses without more HTTP requests. No funded drive, authenticated walk, cron/environment change, migration or stored-data deletion. NOT CERTIFIED: future editorial quality, sustained live waterfall and complete operator delivery across all edit families; all-reader FAQ/markup/entity authority, task-adequate long-page reads, semantic entailment and prompt-injection resistance; full durable schema-value capture, visible-content/schema dependency refresh, graph-aware draft validation, current rich-result guidance and historical verification-receipt reconciliation; body/model-sensitive durable identity, stale promise authority and concurrent write recency; historical fact-bank reconciliation, acquisition-mode review/remediation, cross-process cache writers, history-query scaling/retention, atomic metric windows, watermark/outage semantics and complete verification validity. Hosted SQL definitions/applied migrations/RLS/advisors/exact cron remain unverified after management HTTP 401. Last inspected cron was active every ten minutes; billing failure is not a global pause. Last inspected GSC finalized through Sep 9 and GA4 page traffic was current; separate GA4 totals/referrals and legacy technical scans were stale. Manual publishing and the canonical AI recurrence/five-page SEO comparison remain.
