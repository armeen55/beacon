...
- **P8 (today)** — `SpecificEditProvider` interface + 3 implementations at `src/domains/recommendations/specific-edit-provider.ts` + `providers/{deterministic,openai,anthropic,index}.ts`. `SpecificEdit` row matches `recommended_edits` shape (actionType / targetUrl / targetElement / why / evidence / expectedImpact / difficulty / confidence / measurementPlan / risks / source / providerName / model / costUsd). `SpecificEditBundle` mirrors packet's evidenceHash for cache-key continuity. Deterministic provider shell returns `recommendations: []` (Phase 9 fills generators). OpenAI + Anthropic stubs throw `not_implemented (Sprint 6A.2)`. **No SDK deps added** (asserted by source-scan + package.json test). 22 new tests, **1915 passing**.
- **Next Sprint 6A.1 step:** Phase 6A.1.9 — deterministic generators for the v1 active action types (edit_title, add_h2_section, add_faq). Still no LLM.
