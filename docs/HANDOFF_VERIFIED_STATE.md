...
- Production 74,321 lines (553 under the frozen 74,861 after deleting seven dead modules across two passes), tests 12,723, exports 1,343. Tenth-contract closure deployed 2026-08-18: FACT CHECKING GETS ITS TURN. `fact_check` now sits AHEAD OF EVERY PAID PHASE in the run order and behind only the free ones that feed it, because a money reserve alone could not make it reachable: bulk work spent the dollar before its turn arrived. BOTH budget doors hold the reserve and count the call about to be made: `dailyCapReason` compares today plus the PROJECTED amount (at $0.769 spent a $0.21 bulk buy is now refused, where the spent-only check admitted it), bulk search stops at 0.77 of the cap, non-fact model work at 0.92, and fact checking may draw the whole cap. VERIFICATION RULES ARE VERSIONED: every row carries the rules that produced it, a verdict from older rules is not current evidence, and the one live checked row (Ahvaz, researched by the subject-only query) is archived under its own key and re-opened as owed so the repaired engine researches it instead of skipping it for ever. Decision refuses a correction whose rules were replaced. PROVIDER OUTCOMES ARE CARRIED END TO END: capped, waiting, refused and unavailable each keep their name against the stage that took them (extraction, search, fetch, judge), so repeated paid attempts expose which debt they are hitting. The token fingerprint is named for what it is, content-token equality in any order, never semantic. An account with no stored page words answers done rather than pausing a fresh account at a phase that now precedes the crawl. Migration history stays immutable: 18b lifecycle, 18c ledger reporting day, 18d rules version, with the applied 2026-08-18 file byte-identical and sha-pinned by test. NOT YET TRUE: no repaired acquisition has completed live, no page_wrong is confirmed, no correction card exists, AEO does not consume source facts; research is PAUSED at $1 pending one fresh-Pacific-day observation.
