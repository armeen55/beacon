...
- Deployed 2026-09-05, verified at `/api/version` as `7849aab9`: the research-to-useful-change loop, round one. One editorial standard per treatment (summary, missing answer, restructuring, correction, internal link, structured data) is chosen once from the persisted assignment and read by the writer, the paid reading, the stored review, the re-read and the serving door; the evaluator's universal add-information and no-verbless-list rules and the task table inferred from evidence ids are deleted, every edit owes six mechanical things and exactly one editorial standard, and seven refusal classes each lead somewhere (a weakness buys a revision, missing evidence walks the acquisition ladder, a factual concern goes to the claim's authority, a wrong reader task files typed debt, a transport failure is never a verdict). No stored row moved by the ladder; the store's record sentence retires with the reading that answers it. The runtime prepares an account once per drive (one working context keyed by part, dropped where the write happens), spends money instead of reserving it (a job the money never reaches files cost_blocked and is owed again at its own rank; work funded and never begun resumes first on the next drive), boxes every walk inside the lease, crawls only while the walk after it can still run, and a connector that would not sync records its debt on the run instead of pausing the drive; that sentence reaches no operator surface yet.
