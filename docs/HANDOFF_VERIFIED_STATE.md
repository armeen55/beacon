...
- Deployed 2026-09-05, verified at `/api/version` as `603b226e`: the research-to-useful-change loop, rounds one to four. One editorial standard per treatment is chosen once from the persisted assignment and read by every door; a search is diagnosed against the page's own passages with the whole account's function words and relational words handled by the tokenizer and the verdict, never by token absence; page understanding stores the title's promise, the missing capability and what the page sells; the evidence bar is proportional to what the treatment risks and a usable reading is never bought again; a page is judged against its whole template family from the inventory or the rule refuses to decide, and the vocabulary that list held for one account is derived from the family; the opportunity line is a name and a zero suppresses its superlative. The runtime prepares an account once per drive, spends money instead of reserving it, resumes and demotes unreached work, keeps a run's receipts as its own, reclaims an abandoned run, writes every acquisition with the row and step it unlocks, and records a stale connector on the pass receipt; funding, ranking and explanation read one scoring context; every refusal the $0 replay or the demotion path decides is written on the row through one composer. The operator loop keeps both the prepared and the applied wording, never loses a press, answers a bulk press per card, and settles a record that names nothing to look for without a fetch; Results prints raw movement beside the adjusted comparison and says which of the two happened, what the account's one closed reading taught the queue, and what is done next; Today names the last change's outcome. Not yet done: the canon counts a fact id on the row's own support as a cited source, a reader's answer never opens with the search phrase, the new-page injected end-to-end contract, and the evaluative-adjective set as the one remaining closed English list.
