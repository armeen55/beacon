...
- Production 74,458 lines, tests 12,393, exports 1,329, all at exact ceilings. Sixth-contract run deployed and verified at `de2978d0` plus the receipt-provenance follow-up (2026-08-17, authenticated prod fetched with a live session): TWO SILENT BLINDNESSES CLOSED. The answer reader filtered on a status this table never writes, so no stored answer body had ever reached a diagnosis; with it fixed, /basic-persian-phrases reads 40 answers where ChatGPT retrieved the page 8 times and cited it once, and the cause is retrieved_not_cited (the page is read and passed over) rather than an absence gap. The unit-history read that narrows candidates before doing per-page work runs in 2.2s where the old shape took 8.5s and died on the API statement timeout. CORRECTIONS ARE PRODUCT OUTPUT, NOT A SCRIPT: a durable page-fact-checks store holds each checked statement with its sources, agreement and confidence, and a producer mints one correction bundle per page from those rows alone, so the same checks mint the same card on every pass and a corrected page retires its own card. All 192 name meanings on /persian-female-first-names were graded against Steingass, Encyclopaedia Iranica, Wiktionary and Behind the Name, then re-checked by a hostile pass that downgraded 8: 114 confirmed, 41 likely, 28 disputed, 9 unsupported, and only confirmed contradictions carrying a scholarly, dictionary or encyclopedia source become components. ACCURACY AND RANKING ARE SEPARATE TRUTHS: a card is judged on its own named cause, accuracy is its own hypothesis family so both cards stand on one page, and a cause that claims no clicks never inherits the page's fall. Bundle families now join the stale sweep on pages the door re-walked. Authenticated /changes serves 4 drafts (rank 1 the 40-correction bundle, rank 2 the retrieved-not-cited AI card) and 7 researched opportunities; zero Ready is the boundary's defended count, with the hostile reviewer refusing every section that restates a page. Research is ON with the daily $1 cap mechanical.
