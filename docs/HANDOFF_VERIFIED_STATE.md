...
- Verified source and production on 2026-09-12: `291dbe3427617ae2e6e84c7a709d93a5e9e50a9a`, deployment `dpl_3K9e32HqUJEQogHsd4RBk4KwTLz9` READY. Production `/api/version` HTTP 200 reports that exact SHA on `main`; login HTTP 200 and anonymous Changes HTTP 307 to login. Winner debt, acquisition and SERP projection now share one newest completed identity-valid result per canonical query; older rankings remain stored, never unioned into current comparisons. Exact competitor obligations prioritize their query-backed URL (including resolved citation wrappers) while retaining the bounded comparison batch. Unbacked/foreign-query targets refuse; acquisition requires nonblank page words read back under the requested tenant/basis, not word-count metadata or unit status. Replays cover chronological offset dates, reversed histories, pending refreshes, empty completed results, target isolation and banked-content preservation. The first full gate caught a real target-only batch regression; corrected selection passed the unchanged research-to-delivery harness. Full gate: 2,049 passed, 24 skipped, 89 files; build/typecheck/foundation passed, 21 existing lint warnings. Production/test/combined LOC: 74,501/20,533/95,034; ceilings ratcheted down, no new routes/dependencies/domains/exports. Competitor slice: $0 provider spend, two read-only SQL requests during acquisition investigation, one Git-triggered deploy; no funded drive, cron/environment change, migration or stored-data deletion. Prior model-call reuse fingerprints tenant/prompt/model/schema/grounded inputs, validates cached and fresh values identically, uses acknowledged independent Supabase upserts, fails closed on invalid/foreign/outage reads and preserves legacy history read-only. Prospective factual work retains tenant/topic/basis scope, exact proposition research and paid briefs/pieces through source shortages. Canonical 28-day AI recurrence, five-page SEO comparison, source-bound packets, targeted page repair and source-validity measurement remain. NOT CERTIFIED: authenticated production journey, future model editorial quality or sustained live waterfall; task-adequate competitor/section information and full edit-family quality; historical owned-scoped fact-bank reconciliation, acquisition-mode whole-review objections and canonical-validator remediation; live cross-process cache-writer exercise or history-query scaling/retention; atomic metric-window consistency, source-watermark outage semantics and complete verification validity. Source trace: the comparison reader receives selected competitor prose, while writer briefing lines retain only observations/short quotes; this handoff still needs task-adequacy proof. Last inspected cron remains active every ten minutes; billing failure is not a global pause. GSC was finalized through Sep 9 and GA4 page traffic current; separate GA4 totals/referrals and legacy technical scans were stale.
