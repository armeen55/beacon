...
- Production 74,729 lines, tests 12,414, exports 1,336, all at exact ceilings. Source-fact loop deployed and verified (2026-08-18): SOURCE-BACKED FACTS ARE ROWS IN SUPABASE, not an array on a developer's disk. The first fact-check store was registered for ownership but never for mirroring, so production held zero rows while 192 checks sat in a local file: a seeded demonstration, not a loop. `page_source_facts` is row-wise keyed on tenant, page and statement, carries the page-content hash it was checked against, upserts idempotently, and THROWS on a failed read instead of degrading to an empty list that would erase another page's facts. 192 rows are in production; a cold process with the local file deleted reads them and two ordinary passes regenerate the identical correction bundle. ACQUISITION HAS A PRODUCTION CALLER: `fact_check` is a real research phase (nine steps now, fail-soft) that reads the account's most-shown page, extracts its checkable statements, buys one results page per statement through the same budgeted capability every other bought read uses, grades each domain's authority, compares, and banks one row per statement. Its confidence ceiling is honest: it identifies and grades sources without fetching their text, so it may never mint `confirmed` and never authorizes a live edit on its own. NOTHING DISAPPEARS: 102 corrections are authorized, the card shows 40 worst-first and says 'batch 1 of 3, 62 more confirmed after this' in its headline, steps and limitations. A simulated correction retires exactly its own statement (102 to 101) and no other. EVIDENCE VOLUME IS CONFIDENCE, NOT IMPACT: the evidence factor is logarithmic and a cause claiming no traffic or citation gain cannot ride its page's impressions up the queue. AI WINDOWS, STATED SEPARATELY AND NEVER MIXED: the complete stored history for the basic-phrases question is 54 answers with 10 Iranopedia retrievals and 2 citations; the bounded reader that feeds diagnosis inspects the newest 40, which carry 8 retrievals and 1 citation. Zero Ready remains the boundary's defended count. Research is ON with the daily $1 cap mechanical.
